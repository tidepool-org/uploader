...
appActions._handleUploadSuccess = function(uploadIndex, records) {
  ...
  this._updateUpload(uploadIndex, function(upload) {
    //log metric details
    self._logMetric(
      self.trackedState.UPLOAD_SUCCESS,
...
appActions._handleUploadError = function(uploadIndex, error) {
  ...
  this._updateUpload(uploadIndex, function(upload) {
    //log the errors
    self._logMetric(self.trackedState.UPLOAD_FAILED, {type: upload.source.type ,source:upload.source.driverId, error: error });
    self._logError(self.trackedState.UPLOAD_FAILED, error, {type: upload.source.type ,source:upload.source.driverId});
    var instance = _.assign(upload.progress, {
      finish: self._now(),
      error: error
    });

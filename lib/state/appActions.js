...
appActions.upload = function(uploadIndex, options, cb) {
  ...
  var onUploadFinish = function(err, records) {
    if (err) {
      self._logMetric('Upload failed ',{type: upload.source.type ,source:upload.source.driverId, error: err });
      self._logError('Upload failed ',err,{type: upload.source.type ,source:upload.source.driverId});
      self._handleUploadError(uploadIndex, err);
      return cb(err);
    }
    self._logMetric('Upload success',{type: upload.source.type ,source:upload.source.driverId});
  ...
  self._logMetric('Upload starting ',{type: upload.source.type ,source:upload.source.driverId});

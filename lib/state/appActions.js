...
appActions.detectDevices = function(cb) {
  ...
  device.detectAll(function(err, devices) {
    ...
    self._logMetric(self.trackedState.FOUND_DEVICES,devices);
...
appActions.upload = function(uploadIndex, options, cb) {
  ...
  var onUploadFinish = function(err, records) {
    if (err) {
      self._logMetric(self.trackedState.UPLOAD_FAILED,{type: upload.source.type ,source:upload.source.driverId, error: err });
      self._logError(self.trackedState.UPLOAD_FAILED,err,{type: upload.source.type ,source:upload.source.driverId});
      self._handleUploadError(uploadIndex, err);
      return cb(err);
    }
    self._logMetric(self.trackedState.UPLOAD_SUCCESS,{type: upload.source.type ,source:upload.source.driverId});
  ...
  self._logMetric(self.trackedState.UPLOAD_STARTED,{type: upload.source.type ,source:upload.source.driverId});
...
appActions._uploadCarelink = function(credentials, options, cb) {
  ...
  api.upload.fetchCarelinkData(payload, function(err, data) {
    ...
    if (err) {
      ...
      self._logError(self.trackedState.CARELINK_FETCH_FAILED,err);
      return cb(err);
    }
    ...
    carelink.upload(data, options, function(err, records) {
      if (err) {
        self._logMetric(self.trackedState.CARELINK_LOAD_FAILED);
        self._logError(self.trackedState.CARELINK_LOAD_FAILED,err);
        return cb(err);
      }
      self._logMetric(self.trackedState.CARELINK_LOAD_SUCCESS);

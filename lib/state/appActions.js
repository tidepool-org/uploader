...
appActions.login = function(credentials, options, cb) {
  var self = this;
  var loadAcknowledgments = function(userId) {
    return function(cb) {
      localStore.get('acknowledgments', function(stored) {
        var initial = appState.getInitial();
        self.app.setState({
          acknowledgments: (stored.acknowledgments) ? stored.acknowledgments[userId] || initial.acknowledgments : initial.acknowledgments
        });
        cb();
      });
    };
  ...
  async.series([
    ...
    async.series([
      loadAcknowledgments(results[0].userid)
    ]);
    // once we've logged in, we don't want the right-click menus around, so
    // delete them.
    // because our test env isn't chrome, we have to test for it
...
appActions.acknowledge = function (key) {
  ...
  if (!update[this.app.state.targetId]) {
    update[this.app.state.targetId] = {};
  }

  update[this.app.state.targetId][key] = Date.now();
  localStore.setItem('acknowledgments', update);

  this.app.setState({
    acknowledgments: update[this.app.state.targetId]
  });

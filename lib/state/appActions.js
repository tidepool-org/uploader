...
appActions._uploadDevice = function(driverId, options, cb) {
  ...
  device.detect(driverId, options, function(err, d) {
    ...
    if (err) {
      if (driverId === 'OneTouchVerioIQ'){
        var currentOS = this.app.device._os; // "mac", "win", "android", "cros", "linux", or "openbsd"
        if(currentOS === 'mac') {
          appActions.addMoreInfoToError(err, appActions.errorStages['STAGE_VERIOIQ_MAC_DRIVER']);
        }else if(currentOS === 'win') {
          appActions.addMoreInfoToError(err, appActions.errorStages['STAGE_VERIOIQ_WIN_DRIVER']);
        }else {
          appActions.addMoreInfoToError(err, appActions.errorStages['STAGE_VERIOIQ_NO_DRIVER']);
        }

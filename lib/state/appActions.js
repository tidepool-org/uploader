...
appActions._afterLoginPage = function(user, cb) {
  ...
  var initial = appState.getInitial();
  ...
  var acks = localStore.getItem('acknowledgments');
  ...
  if (!_.isEmpty(targetDevices) && !_.isEmpty(targetTimezone)) {
    self.app.setState({
      ...
      targetTimezone: targetTimezone,
      page: 'main',
      acknowledgments: acks || initial.acknowledgments
    });
...
appActions.acknowledge = function (key) {
  var update = localStore.getItem('acknowledgments');
  ...
  update[key][this.app.state.targetId] = Date.now();
  localStore.setItem('acknowledgments', update);
 ...
 * @return {boolean}
 */
appActions.shouldDisplayDaylightSavingsMessage =  function(isoString) {
  ...
  var localTime = sundial.formatInTimezone(isoString, state.targetTimezone, 'YYYY-MM-DD[T]HH:mm:ss');
  var timeChangeTimestamp = sundial.getUTCFromLocalTimeAndTimezone('2015-11-01T01:59:59.999', state.targetTimezone);
  var localTimeTimestamp = sundial.getUTCFromLocalTimeAndTimezone(localTime, state.targetTimezone);
  
  return ((localTimeTimestamp > timeChangeTimestamp) && !state.acknowledgments.daylightSavings[state.targetId]);
};

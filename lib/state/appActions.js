...
appActions.errorStages = {
  ...
  STAGE_VERIOIQ_DRIVER : { code: 'E_VERIOIQ_DRIVER' , friendlyMessage: 'Windows driver for VerioIQ not installed, please download it from www.lifescan.es/ourproducts/accessories/usb-cable-driver' },
  ...
  device.detect(driverId, options, function(err, d) {
    ...
    if (err) {
      if (driverId === 'OneTouchVerioIQ'){
        appActions.addMoreInfoToError(err, appActions.errorStages['STAGE_VERIOIQ_DRIVER']);
        return cb(err);
      }

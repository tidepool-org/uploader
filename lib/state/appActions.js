...
appActions.addMoreInfoToError = function(err, stage) {
  ...
  err.code = err.code || appActions.errorStage[stage].code;
  err.friendlyMessage = err.friendlyMessage || appActions.errorStage[stage].friendlyMessage;
  err.stringifiedStack = _.pluck(_.filter(stacktrace.parse(err), function(cs) { return cs.functionName !== null; }), 'functionName').join(', ');
  err.debug = 'Detail: ' + err.message + ' | ' +  'Error UTC Time: ' + sundial.utcDateString() + 
    ' | ' + 'Code: ' + err.code + ' | ' + 'Error Type: ' + err.name + ' | ' + 'Stack Trace: ' + 
    err.stringifiedStack + ' | ' + 'Version: ' + err.version;
};
...
appActions.load = function(cb) {
  ...
  async.series([
    ...
    if (err) {
      appActions.addMoreInfoToError(err, 'STAGE_SETUP');
      return cb(err);
    }
...
appActions.login = function(credentials, options, cb) {
  ...
  async.series([
    ...
    if (err) {
      appActions.addMoreInfoToError(err, 'STAGE_LOGIN');
      return cb(err);
    }
...
appActions.logout = function(cb) {
  ...
  api.user.logout(function(err) {
    if (err) {
      appActions.addMoreInfoToError(err, 'STAGE_LOGOUT');
      return cb(err);
    }
...
appActions.detectDevices = function(cb) {
  ...
  device.detectAll(function(err, devices) {
    if (err) {
      appActions.addMoreInfoToError(err, 'STAGE_DETECT_DEVICES');
      return cb(err);
    }
...
appActions.readFile = function(uploadIndex, targetId, file, extension) {
  ...
  if (file.name.slice(-extension.length) === extension) {
    ...
    reader.onerror = function() {
      ...
      appActions.addMoreInfoToError(error, 'E_READING_FILE');
      inputError(error);
    ...
    appActions.addMoreInfoToError(error, 'STAGE_PICK_FILE');
    inputError(error);
    // the return is just for ease of testing
    return error;
  }
...
appActions.upload = function(uploadIndex, options, cb) {
  ...
  if (appState.hasUploadInProgress()) {
    ...
    appActions.addMoreInfoToError(err, 'STAGE_UPLOAD');
    throw err;
  }
  ...
  if (upload.source.type === 'device' || upload.source.type === 'block') {
    ...
    appActions.addMoreInfoToError(error, 'STAGE_UPLOAD');
    throw error;
  }
};
...
appActions._uploadDevice = function(driverId, options, cb) {
  ...
  device.detect(driverId, options, function(err, d) {
    if (err) {
      appActions.addMoreInfoToError(err, 'STAGE_DEVICE_DETECT');
      return cb(err);
    }
    ...
    device.upload(driverId, options, function(err, records) {
      if (err) {
        appActions.addMoreInfoToError(err, 'STAGE_DEVICE_UPLOAD');
        return cb(err);
      }
...
appActions._uploadCarelink = function(credentials, options, cb) {
  ...
  api.upload.fetchCarelinkData(payload, function(err, data) {
    ...
    if (err) {
      appActions.addMoreInfoToError(err, 'STAGE_CARELINK_FETCH');
    ...
    carelink.upload(data, options, function(err, records) {
      if (err) {
        appActions.addMoreInfoToError(err, 'STAGE_CARELINK_UPLOAD');
        return cb(err);
      }

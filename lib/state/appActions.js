...
appActions.errorStages = {
  ...
  STAGE_VERIOIQ_WIN_DRIVER : { code: 'E_VERIOIQ_DRIVER' , friendlyMessage: 'Please make sure the correct device driver is installed - ', driverLink:'http://tidepool.org/downloads/', driverName: 'TidepoolUSBDriverSetup.exe' },
  STAGE_VERIOIQ_MAC_DRIVER : { code: 'E_VERIOIQ_DRIVER' , friendlyMessage: 'Please make sure the correct device driver is installed - ', driverLink:'https://www.silabs.com/Support%20Documents/Software/Mac_OSX_VCP_Driver.zip', driverName: 'Mac_OSX_VCP_Driver.zip' },
  STAGE_VERIOIQ_NO_DRIVER : { code: 'E_VERIOIQ_DRIVER' , friendlyMessage: 'VerioIQ appears to be connected, please make sure the correct device driver is installed.' },
  STAGE_DRIVER : { code: 'E_DRIVER' , friendlyMessage: 'Please make sure the correct device driver is installed - ' },
...
appActions._setDriverLinks = function() {

    var currentOS = this.app.device._os; // "mac", "win", "android", "cros", "linux", or "openbsd"

    for(var uploadIndex in this.app.state.uploads) {

      if(currentOS === 'mac') {
        this._updateUpload(uploadIndex, function(upload) {

          if(upload.mac) {
            upload.driverLink = upload.mac.driverLink;
            upload.driverName = upload.mac.driverName;
          }
          return upload;
        });
        ...
        this._updateUpload(uploadIndex, function(upload) {
          if(upload.win) {
            upload.driverLink = upload.win.driverLink;
            upload.driverName = upload.win.driverName;
          }
          return upload;
        });
      }
...
appActions._uploadDevice = function(driverId, options, cb) {
  ...
  device.detect(driverId, options, function(err, d) {
    ...
    if (err) {
      ...
      if(upload = _.findWhere(this.app.state.uploads, {source : {driverId: driverId}})) {
        if((currentOS === 'mac') && upload.mac) {
          appActions.errorStages['STAGE_DRIVER'].driverLink = upload.mac.driverLink;
          appActions.errorStages['STAGE_DRIVER'].driverName = upload.mac.driverName;
          ...
          appActions.errorStages['STAGE_DRIVER'].driverName = upload.win.driverName;
          appActions.addMoreInfoToError(err, appActions.errorStages['STAGE_DRIVER']);
          return cb(err);
        }
      }

...
appActions.errorText = {
  ...
  E_UNSUPPORTED_VERSION : 'Please update your version of the uploader from the chrome store'
};

appActions.errorStages = {
  STAGE_SETUP : { code: 'E_SETUP' , friendlyMessage: 'Error during setup' },
  STAGE_SETUP_VERSION : { code: 'E_SETUP_VERSION' , friendlyMessage: 'You need to update your uploader version so that we can upload your device data' },
  ...
  STAGE_UPLOAD_VERSION : { code: 'STAGE_UPLOAD_VERSION' , friendlyMessage: 'You need to update your uploader version so that we can upload your data' },
...
appActions.login = function(credentials, options, cb) {
  ...
  var err = appActions._assertUploaderVersion();
  if (err) {
    return cb(err);
  }
...
appActions.upload = function(uploadIndex, options, cb) {
  ...
  if (appState.hasUploadInProgress()) {
    err = new Error(self.errorText.E_UPLOAD_IN_PROGRESS);
...
appActions._assertUploaderVersion = function() {
  if (!api.upload.isAllowed(config.namedVersion)){
    var err = new Error(this.errorText.E_UNSUPPORTED_VERSION);
    appActions.addMoreInfoToError(err, appActions.errorStages['STAGE_SETUP_VERSION']);
    return err;
  }
  return null;
};

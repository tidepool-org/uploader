...
appActions.changeTimezone = function(timezone) {
  // to clear some really annoying React warnings, we have to append the array
  // index plus underscore to the timezone names in the selection component
  // so we clean that back up here
  var cleanTimezone = timezone.replace(/^\d+_/, '');
  this.app.setState({targetTimezone: cleanTimezone});
};
...
appActions._afterLoginPage = function(user, cb) {
  ...
  var targetDevicesWithTimezones = devices[user.userid];

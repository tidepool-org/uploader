...
appActions.changeGroup = function() {
  this.app.setState({
    uploads: null, // clear upload state
    targetId: groups[0].userid // todo: put something ref .value here.
  });
};
...
appActions.load = function(cb) {
  ...
  async.series([
    ...
    async.series([
      ...
      var user = _.assign({}, account, {profile: profile, uploadGroups: groups});

      // todo add state for when the user cannot upload to any account
      self.app.setState({
        user: user,
        targetId: groups[0].userid,
        page: 'main'
      });

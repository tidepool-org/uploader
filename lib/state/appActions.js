...
appActions.errorStages = {
  ...
  STAGE_DRIVER : { code: 'E_DRIVER' , friendlyMessage: 'You may need to install the device driver for ' },
...
appActions.addMoreInfoToError = function(err, stage) {
  ...
  err.driverLabel = stage.driverLabel;
...
appActions._uploadDevice = function(driverId, options, cb) {
  ...
  device.detect(driverId, options, function(err, d) {
    ...
    if (err) {
      ...
      var currentOS = this.app._os; // "mac", "win", "android", "cros", "linux", or "openbsd"
      ...
      if(upload !== undefined) {
        if((currentOS === 'mac') && upload.mac) {
          ...
          appActions.errorStages['STAGE_DRIVER'].driverLabel = upload.mac.driverLabel;
          ...
          appActions.errorStages['STAGE_DRIVER'].driverLabel = upload.win.driverLabel;
          appActions.addMoreInfoToError(err, appActions.errorStages['STAGE_DRIVER']);
          return cb(err);
        }
      }

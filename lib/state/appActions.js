...
appActions.upload = function(uploadIndex, options, cb) {
  ...
  // Update app state with an upload in progress
  // and only start upload after re-render is complete
  // (makes sure the Scan component has unmounted)
  this._setUploadStart(uploadIndex, options, function() {
    self._logMetric(
    ...
    if (upload.source.type === 'device') {
      var driverId = self.app.state.uploads[uploadIndex].source.driverId;
      return self._uploadDevice(driverId, options, onUploadFinish);
    }
      ...
      return self._uploadCarelink(credentials, options, onUploadFinish);
    }
    else {
...
appActions._setUploadStart = function(uploadIndex, options, cb) {
  var self = this;
...
appActions._updateUpload = function(uploadIndex, updateFn, cb) {

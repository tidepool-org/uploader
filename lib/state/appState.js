...
appState.getInitial = function() {
  ...
  var uploads = [
    ...
    },
    {
      ...
      win: {driverLink:'http://tidepool.org/downloads/', driverName: 'TidepoolUSBDriverSetup.exe' },
      mac: {driverLink:'https://www.silabs.com/Support%20Documents/Software/Mac_OSX_VCP_Driver.zip', driverName: 'Mac_OSX_VCP_Driver.zip'}
    },
    // {
...
appState.hideUnavailableDevices = function() {
  if(this.app._os === 'mac') { // this.app._os can be "mac", "win", "android", "cros", "linux", or "openbsd"
    this.app.state.uploads = _.reject(this.app.state.uploads, function(d){
      return d.key === 'precisionxtra'; // TODO: || d.key === 'abbottfreestylelite' || d.key === 'abbottfreestylefreedomlite'
    });
  }
};
...
appState.uploadsWithFlags = function() {
  ...
  return _.map(targetedUploads, function(upload, index) {
    ...
    if (source.driverId === 'OneTouchVerioIQ') {
      var currentOS = this.app.device._os;
      if (currentOS !== 'win'){
        upload.onetouchverioiqOnWindows = false;
      }else{
        upload.onetouchverioiqOnWindows = true;
      }
      upload.onetouchverioiq = true;
    }

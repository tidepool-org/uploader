...
module.exports = function(timezone, mostRecent, changes) {
  ...
  if (!_.isEmpty(changes)) {
    ...
    // squish consecutive intervals with the same offset
    for (var j = 0; j < offsetIntervals.length; ++j) {
      var intvl = offsetIntervals[j], nextIntvl;
      if (j + 1 < offsetIntervals.length) {
        nextIntvl = offsetIntervals[j + 1];
        if (intvl.timezoneOffset === nextIntvl.timezoneOffset) {
          intvl.start = nextIntvl.start;
          intvl.startIndex = nextIntvl.startIndex;
          nextIntvl.toDelete = true;
        }
      }
    }
    offsetIntervals = _.reject(offsetIntervals, 'toDelete');
  }

...
module.exports = function(timezone, mostRecent, changes) {
  ...
  if (!_.isEmpty(changes)) {
    ...
    for (var i = 0; i < sorted.length; ++i) {
      ...
      else if ((parseInt(change.change.from.slice(0,4)) < (new Date().getUTCFullYear() - 1)) ||
          (parseInt(change.change.to.slice(0,4)) < (new Date().getUTCFullYear() - 1))) {

        // TODO: this part is not working
        debug('Pump shutdown, restarted on', change.jsDate);
        // timezoneOffset = 0;
        clockDriftOffset = 0;
        conversionOffset = 0;
        debug('Removed', sorted.splice(i, 1)); // remove shutdown time change
      }
      else {

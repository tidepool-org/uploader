...
class Medtronic600Simulator {
  ...
  filterOutInvalidData() {
    return _.filter(this.events, (event) => {
      ...
      } else if (event.type === 'basal') {
        // Filter out any basals with no duration. This can happen inside
        // applySuspendedBasals(), when a basal is suppressed completely by a pump suspend
        if (event.duration === 0) {
          return false;
        }
      }
    ...
    for (const entry of orderedBasals.entries()) {
      ...
      if (suppressedBasal != null) {
        if (Date.parse(suppressedBasal.basalEndTime) > Date.parse(activeSuspendedBasal.time)) {
          ...
          // Zero-duration suppressedBasals are removed in filterOutInvalidData()

    ...
    if (this.currBasal.deliveryType !== 'scheduled' && this.currPumpSettings) {
      ...
      const checkSchedules = (startTime, endTime, durationBeforeMidnight) => {
        for (const i of currSchedule) {
          debug(this.currBasal.deviceTime, 'startTime:', startTime, 'endTime', endTime, 'currSchedule.start', currSchedule[i].start, currSchedule[i].rate);
          ...
          if ((startTime <= currSchedule[i].start) && (endTime > currSchedule[i].start) &&
            ((this.currBasal.suppressed.deliveryType === 'scheduled' && currSchedule[i].rate !== this.currBasal.suppressed.rate) ||
              (this.currBasal.suppressed.suppressed && // there's a nested suppresed
                this.currBasal.suppressed.suppressed.deliveryType === 'scheduled' && currSchedule[i].rate !== this.currBasal.suppressed.suppressed.rate))) {
  ...
  alarm(event) {
    ...
    if ((type === 'no_delivery' || type === 'auto_off' || type === 'no_power') &&
      ...
      if (type === 'other' && event.payload.alarm_id === 3 && this.currBasal) {
        debug('Battery out too long at', event.time, ', annotating previous basal at', this.currBasal.time);
        ...
        if (this.currBasal.deliveryType === 'scheduled' && this.currPumpSettings) {
          ...
          for (let i = 0; i < currSchedule.length; i++) {
            debug(this.currBasal.deviceTime, 'startTime:', startTime, 'currSchedule:', currSchedule[i].start, currSchedule[i].rate);
  ...
  finalBasal(finalRecordTime) {
    if (this.currBasal) {
      if (this.currBasal.deliveryType === 'scheduled' && this.currPumpSettings) {
        ...
        if (!this.currBasal.isAssigned('duration')) {
          this.currBasal = common.finalScheduledBasal(this.currBasal, this.currPumpSettings, 'medtronic');
        }
      } else {
        if (!this.currBasal.isAssigned('duration')) {

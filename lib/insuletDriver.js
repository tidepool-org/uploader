...
module.exports = function (config) {
  ...
  var addTimestamp = function (o) {
    var ts = cfg.timeutils.buildTimestamp(o, cfg.timezone);
    if (ts) {
      o.timestamp = ts;
      ts = cfg.timeutils.buildTimestamp(o, null);
      o.deviceTime = ts;
      o.timezoneOffset = cfg.timeutils.computeTimezoneOffset(o.deviceTime, o.timestamp);
    }
  };
  ...
  var fixedRecords = {
    ...
    eeprom_settings: {
      ...
      postprocess: function (rec) {
        ...
        rec.insulin_duration_msec = rec.INSULIN_DURATION * cfg.timeutils.MIN30_TO_MSEC;
        return null;
      }
    },
  ...
  var logRecords = {
    ...
    0x0004: { value: 0x0004, name: 'Bolus', format: 'isss', fields: [
      ...
      rec.extended_duration_msec = rec.extended_duration_minutes * cfg.timeutils.MIN_TO_MSEC;
      rec.immediate_duration_msec = rec.immediate_duration_seconds * cfg.timeutils.SEC_TO_MSEC;
    ...
    0x0008: { value: 0x0008, name: 'Basal_Rate', format: 'iss', fields: [
      ...
      rec.duration_msec = rec.duration * cfg.timeutils.MIN_TO_MSEC;
    }
    },
    ...
    0x2000: { value: 0x2000, name: 'Terminate_Bolus', format: 'is', fields: [
      ...
      rec.time_left_msec = rec.time_left_seconds * cfg.timeutils.SEC_TO_MSEC;
    }
    },
    0x4000: { value: 0x4000, name: 'Terminate_Basal', format: 's', fields: [
      'time_left_minutes'
    ], postprocess: function (rec) {
      rec.time_left_msec = rec.time_left_minutes * cfg.timeutils.MIN_TO_MSEC;
    }
    },
    ...
    for (var i = 0; i < 11; ++i) {
      ...
      for (var j = 0; j < 48; ++j) {
        profile.steps.push({
                             ...
                             starttime: j * cfg.timeutils.MIN30_TO_MSEC,
  ...
  // attempts to post a block of records to jellyfish; if it gets a 'duplicate'
  // response, tries again in the opposite order
  var postToJellyfish = function (records, progress, cb) {
    cfg.jellyfish.post(records, progress, function (err, result) {
      if (err) {
        console.log(err);
        console.log(result);
        if (err === 'duplicate') {
          console.log('got dup, trying again, omitting dup and earlier records');
          cfg.jellyfish.post(records.slice(result + 1), progress, cb);
          return;
        }
      }
  ...
  var buildBasalRecords = function (data, records) {
    ...
    for (var b = 0; b < basalrecs.length; ++b) {
      ...
      if (basal.detail.duration === 0) {
        postbasal = cfg.builder.makeScheduledBasal()
          ...
          .with_duration(24 * 60 * cfg.timeutils.MIN_TO_MSEC)
    ...
    postrecords[postrecords.length - 1].duration = 30 * cfg.timeutils.MIN_TO_MSEC;
    return records.concat(postrecords);
  };
  ...
  return {
    ...
    uploadData: function (progress, data, cb) {
      ...
      cfg.builder.setDefaults({
                                deviceId: data.ibf_version.vendorid + ' ' +
                                          data.ibf_version.productid + ' ' + data.eeprom_settings.REMOTE_ID.toString(),
                                source: 'device',
      ...
      postToJellyfish(postrecords, progress, function (err, results) {
        if (err) {
          console.log(err);
          console.log(results);
          progress(100);

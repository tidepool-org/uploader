...
module.exports = function(config) {
  ...
  function send(bytes, callback) {
    ...
    var abortTimer = setTimeout(function () {
      debug('Timeout error during HID write');
      return callback(
        new Error('Timed out while sending a message. Please retry.'), null);
    }, 500); // commmand sent are typically very short, so timeout can be small
  ...
  function sendPromisified(bytes) {
    return new Promise((resolve, reject) => {
      send(bytes, function(err) {
        if (err) {
          // exceptions inside promise aren't automatically thrown,
          // so we have to reject them here
          reject(err);
        }
        resolve();
      });
  ...
  return {
    connect: connect,
    connectPromisified: promisify(connect),
    disconnect: disconnect,
    removeListeners: removeListeners,
    receive: receive,
    receiveTimeout: receiveTimeout,
    sendPromisified: sendPromisified,
    send: send
  };

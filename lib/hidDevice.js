  ...
  function receive(cb){
    chrome.hid.receive(connectionId, function(reportId, rawdata) {
      if(chrome.runtime.lastError) {
        cb(new Error('Could not connect to device: '+ chrome.runtime.lastError.message));
      } else {
        var packet;
  ...
  function send(bytes, callback) {
    ...
    if (bytes == null) {
      ...
      chrome.hid.send(connectionId, reportId, bytes, function(err){
        if(chrome.runtime.lastError) {
            callback(new Error('Could not connect to device: '+ chrome.runtime.lastError.message));
          } else {
            callback();
        }
      });

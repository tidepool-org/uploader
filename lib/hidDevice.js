...
module.exports = function(config) {
  ...
  //PV comentando para cuando necesitamos iterar recursivamente sobre esta funcion
  //function receive(bytes, timeout, cb){
  function receive(cb){
    chrome.hid.receive(connectionId, function(reportId, data) {
      cb(buildInput(new Uint8Array(data)));
    });
  ...
  function send(bytes, callback) {
    ...
    if (bytes == null) {
      ...
      var reportId = 0;
      chrome.hid.send(connectionId, reportId, bufView, function(){
          callback();
      });
    }
  }

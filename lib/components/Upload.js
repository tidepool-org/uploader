...
var Upload = React.createClass({
  ...
  renderBlockModeInput: function() {
    ...
    return (
      <div className="Upload-inputWrapper">
        <input className="Upload-fileinput" ref="file" type="file" onChange={this.onBlockModeInputChange} disabled={this.isDisabled()}/>
      </div>
    );
  },
  onBlockModeInputChange: function(e) {
    var file = e.target.files[0];
    this.props.readFile(file, this.props.upload.source.extension);
  },
  ...
  renderStatus: function() {
    ...
    if (this.isUploadSuccessful()) {
      var dataDownloadLink = null;
      if (__DEBUG__ && (!_.isEmpty(this.props.upload.data) && Array.isArray(this.props.upload.data))) {
        var filename = 'uploader-processed-records.json';
        var jsonData = JSON.stringify(this.props.upload.data, undefined, 4);
        var blob = new Blob([jsonData], {type: 'text/json'});
        var dataHref = URL.createObjectURL(blob);
        dataDownloadLink = (
          <a href={dataHref}
            download={filename}
  ...
  renderLastUpload: function() {
    ...
    var lastUpload = _.find(history, function(upload) {

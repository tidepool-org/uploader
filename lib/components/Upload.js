...
export default class Upload extends Component {
    ...
    return (
      <div className={styles['Upload']}>
        ...
        <div className={styles['Upload-right']}>
          <div className={styles['Upload-statusSection']}>
            {this.renderStatus()}
          </div>
          {this.renderProgress()}
          {this.renderActions()}
  ...
  renderBlockModeInput() {
    ...
    return (
      <div className={styles['Upload-inputWrapper']}>
        <input
  ...
  renderButton() {
    ...
    return (
      <div className={styles['Upload-button']}>
        <button
          className={styles['Upload-button--button']}
          disabled={disabled}
  ...
  renderInstructions() {
    ...
    if (Array.isArray(details)) {
      return (
        <div className={styles['Upload-detail']}>
          {_.get(details, 0, '')}
          {_.get(details, 1, '')}
        </div>
      );
    ...
    return (
      <div className={styles['Upload-detail']}>{details}</div>
    );
  }
  ...
  renderLastUpload() {
    ...
    return (
      <div className={styles['Upload-detail']}>{this.props.text.LAST_UPLOAD + time}</div>
    );
  }
  ...
  renderName() {
    ...
    return (
      <div className={styles['Upload-name']}>{upload.name || text.DEVICE_UNKOWN}</div>
    );
  }
  ...
  renderProgress() {
    const { upload } = this.props;
    if (upload.failed) {
      return <div className={styles['Upload-progress']}></div>;
    }

    if (this.isFetchingCareLinkData()) {
      return <div className={styles['Upload-progress']}><LoadingBar/></div>;
    }
    ...
    return <div className={styles['Upload-progress']}><ProgressBar percentage={percentage}/></div>;
  }
  ...
  renderStatus() {
    const { upload } = this.props;
    if (this.isFetchingCareLinkData()) {
      return <div className={styles['Upload-status']}>{this.props.text.CARELINK_DOWNLOADING}</div>;
    }

    if (upload.uploading) {
      return <div className={styles['Upload-status']}>{this.props.text.UPLOAD_PROGRESS + this.props.upload.progress.percentage + '%'}</div>;
    }
    ...
    if (upload.successful) {
      ...
      return <div className={styles['Upload-status']}>{this.props.text.UPLOAD_COMPLETE}&nbsp;{dataDownloadLink}</div>;
    }

    if (this.isBlockModeFileChosen()) {
      return (
          <div className={styles['Upload-blockMode']}>
            <div className={styles['Upload-blockMode--preparing']}>Preparing file &hellip;</div>
            <div className={styles['Upload-blockMode']}>{this.props.upload.file.name}</div>
          </div>
      );
    }
    return null;

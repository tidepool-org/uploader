...
export default class Upload extends Component {
  ...
  onMedtronicSerialNumberInputChange() {
    const { refs } = this;
    let serial_number = refs.medtronic_serial_number;

    if (serial_number && serial_number.value) {
      if (serial_number.value.length === 6) {
        this.setState({
          medtronicFormIncomplete: false,
          medtronicSerialNumberValue: serial_number.value
        });
      }
      else if (serial_number.value.length < 6) {
        this.setState({
          medtronicSerialNumberValue: serial_number.value,
          medtronicFormIncomplete: true
        });
  ...
  renderButton() {
    ...
    if (_.get(upload, 'source.type', null) === 'carelink') {
      ...
      disabled = disabled || this.state.carelinkFormIncomplete || this.state.medtronicFormIncomplete;
    }

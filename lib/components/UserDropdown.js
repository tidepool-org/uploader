...
var UserDropdown = React.createClass({
  ...
  groupSelector: function(){
    var users = this.props.users.targetsForUpload;

    // and now return them sorted them by name
    var sorted = _.sortBy(users, function(targetId) {
    ...
    var selectorOpts = _.map(sorted, function(targetId) {
      var targetInfo = users[targetId];
      if (targetId.patient.isOtherPerson) {
    ...
    return (
      <div className="UserDropdown">
        <div className="UserDropdown-uploadGroup">
          ...
          <div className={'UserDropdown-uploadGroup--list UserDropdown--' + this.props.page}>
            {this.groupSelector()}
          </div>
        </div>
      </div>

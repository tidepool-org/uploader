...
var DeviceSelection = React.createClass({
  ...
  render: function() {
    ...
    var targetUser = this.props.targetId;
    ...
    var items = _.map(devices, function(device) {
      ...
      if (isChecked && !_.isEmpty(driverLink)) {
        driverLinkDisplay = (
          <div className="DeviceSelection-detail">
        ...
        <div key={device.key}>
          <div className="Device-checkbox">
            <input type="checkbox"
              ...
              <label htmlFor={device.key}>{device.name}</label>
          </div>
          {driverLinkDisplay}
        </div>
      );
    ...
    var formClasses = cx({
      ...
      'DeviceSelection-form--onlyme': !this.props.userDropdownShowing,
      'DeviceSelection-form--groups': this.props.userDropdownShowing,

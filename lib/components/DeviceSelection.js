  ...
  render: function() {
    ...
    var items = _.map(devices, function(device) {
      ...
      if (isChecked && !_.isEmpty(driverLink)) {
        driverLinkDisplay = (
          <div className={styles['DeviceSelection-detail']}>
          <a href={driverLink} target="_blank">Download driver</a></div>
        );
      }
      return (
        <div key={device.key}>
          <div className={styles['Device-checkbox']}>
            <input type="checkbox"
              value={device.key}
              ref={device.key}
              id={device.key}
    ...
    formClassesObject[styles['DeviceSelection-form']] = true;
    formClassesObject[styles['DeviceSelection-form--onlyme']] = !this.props.userDropdownShowing;
    formClassesObject[styles['DeviceSelection-form--groups']] = this.props.userDropdownShowing;
    //formClassesObject[styles['DeviceSelection-form--timezone']] = true;
    ...
    return (
      <div className={styles['DeviceSelection']}>
        <h3 className={styles['DeviceSelection-headline']}>Choose devices</h3>
        <form className={formClasses}>{items}</form>
        <button type="submit"
          className={styles['DeviceSelection-button']}

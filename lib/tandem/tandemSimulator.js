  ...
  return {
    ...
    basal: function(event){
      ...
      if(event.deliveryType === 'suspend') {
        setCurrStatus('suspended');
      }else if(event.deliveryType === 'resume') {
        setCurrStatus('resumed');
      }else{
        setCurrStatus(null);
      }
      setCurrBasal(event);
    ...
    finalBasal: function() {
      if (currBasal != null) {
        if (currBasal.deliveryType !== 'scheduled') {
          ...
          currBasal.with_scheduleName(_.find(config.profiles,{idp:currBasal.idp}).name);
    ...
    newDay: function(event) {
      ensureTimestamp(event);
      if((currBasal !== null) && (currStatus !== 'suspended')) {

  ...
  return {
    ...
    tempBasal: function(event) {
      if(event.subType === 'start') {
        ...
        if (currBasal !== null) {
          if (currBasal.deliveryType === 'temp' && currTempBasal && currTempBasal.time_left) {
          // check if temp basal was cancelled
            if (currTempBasal.time_left !== 0 ) {
              var duration;
              if (currBasal.isAssigned('duration')) {
                // a new-day event during a cancelled temp basal
                duration = currBasal.duration - currTempBasal.time_left;
              }
              else{
                duration = currTempBasal.duration - currTempBasal.time_left;
              }
              if (duration >= 0) {
                currBasal.duration = duration;
                annotate.annotateEvent(currBasal, 'tandem/basal/fabricated-from-time-left');
              }
              else{
                currBasal.duration = 0;
                annotate.annotateEvent(currBasal, 'basal/unknown-duration');
        ...
        if (currBasal.time !== event.time) {
          ...
          if(currBasal.deliveryType === 'temp') {
            if (currTempBasal != null) {
              ...
              event.duration = currTempBasal.duration - currBasal.duration;
            }

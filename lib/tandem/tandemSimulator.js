  ...
  return {
    ...
    tempBasal: function(event) {
      if(event.subType === 'start') {
        ...
        if(currBasal !== null) {
          if(currBasal.deliveryType === 'temp' && _.find(currBasal.annotations,{code:'tandem/basal/fabricated-from-new-day'})) {
          // if it's a temp basal fabricated from new-day event, check if temp basal was cancelled
            if(currTempBasal.time_left !== 0) {
              var duration = currBasal.duration - currTempBasal.time_left;
              if(duration > 0) {

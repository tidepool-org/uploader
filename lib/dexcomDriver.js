...
module.exports = function (config) {
  ...
  var BASE_DATE_DEVICE = moment.utc('2009-01-01').valueOf();
  var BASE_DATE_UTC = moment.tz('2009-01-01', cfg.timezone).valueOf();
  debug('timezone=' + cfg.timezone + ' Device=' + BASE_DATE_DEVICE + ' UTC=' + BASE_DATE_UTC);
  debug(new Date(BASE_DATE_DEVICE));
  debug(new Date(BASE_DATE_UTC));
  ...
  return {
    ...
    connect: function (progress, data, cb) {
      debug('STEP: connect');
      cfg.deviceComms.connect(dexcomPacketHandler, function(err) {
        if (err) {
          return cb(err);
        }
    ...
    uploadData: function (progress, data, cb) {
      ...
      cfg.api.upload.toPlatform(
        data.post_records,
        {deviceId:data.post_records[0].deviceId, start: sundial.utcDateString(), tzName : cfg.timezone, version: cfg.version},
        function (err, result) {
        if (err) {
          debug(err);
          debug(result);

    ...
    function openPort(cb) {
      if (deviceInfo.port) {
        ...
        switch(deviceInfo.userSpaceDriver) {
          case 'pl2303':
            connection = new PL2303(deviceInfo.usbDevice, connectopts);
          ...
          case 'cp2102':
            ...
            break;
          case 'ftdi':
            connection = new FTDI(deviceInfo.usbDevice, connectopts);
            connection.addEventListener('error', function (event) {
              return cb(event.detail);
            });
            connection.addEventListener('ready', function () {
  ...
  function writeSerial(bytes, callback) {
    ...
    if (bytes == null) {
      ...
      if(connection == null) {
        ...
        if (connection.writable && !connection.userSpaceDriver) {

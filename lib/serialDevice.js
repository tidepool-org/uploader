...
if (!(process.env.NODE_ENV === 'test')) {
  ...
  (async () => {
    ...
    PL2303 = await import('pl2303');
    CP2102 = await import('cp2102');
    FTDI = await import('ftdi-js');
    ...
    function openPort(cb) {
      if (deviceInfo.port) {
        ...
        switch(deviceInfo.userSpaceDriver) {
          case 'pl2303':
            connection = new PL2303.default(connectopts);
          ...
          case 'cdc-acm':
            ...
            break;
          case 'cp2102':
            connection = new CP2102.default(deviceInfo.vendorId, deviceInfo.productId, connectopts);
            connection.on('ready', function () {
              return cb();
            });
            break;
            case 'ftdi':
              connection = new FTDI.default(deviceInfo.vendorId, deviceInfo.productId, connectopts);

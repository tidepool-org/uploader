...
if (!(process.env.NODE_ENV === 'test')) {
  // usb-cdc-acm tries to initialize libusb when testing, which is not available
  // on CI environments like Appveyor
  ...
  function connect(deviceInfo, handler, cb) {
    ...
    function openPort(cb) {
      if (deviceInfo.path) {
        ...
        debug('Connecting via userspace driver');
        /*
        deviceInfo.path = 'cdc-acm';

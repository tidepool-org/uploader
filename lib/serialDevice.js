...
module.exports = function(config) {
  ...
  function connect(deviceInfo, handler, cb) {
    ...
    connection = new SerialPort(deviceInfo.path, { baudRate: bitrate} , function (err) {
      if (err) {
      ...
      debug('connected to ' + deviceInfo.name);
      if(connectopts.ctsFlowControl) {
        connection.set({dtr:false,rts:true}, function(){

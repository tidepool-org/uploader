...
module.exports = function(config) {
  ...
  function connect(deviceInfo, handler, cb) {
    ...
    function canOpen(cb) {
      debug('Checking if we can open port', deviceInfo.path);
      var port = new SerialPort(deviceInfo.path, connectopts, function (err) {
        if (err) {
          return cb(err, null);
        } else {
          port.close();
    ...
    */
    async.retry({times: RETRIES, interval: TIME_TO_WAIT}, canOpen,
      function(err, result) {
        if (err) {
          return cb(new Error('Could not open the serial port: ' + err));
        }

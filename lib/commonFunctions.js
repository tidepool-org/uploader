...
exports.updatePreviousBasalDuration = function(datum, cfg, cb) {

  var withDurationAnnotation = function(basal) {
    if(_.find(basal.annotations || [], function(ann) {
  ...
  api.getLatestRecord(cfg.groupId, cfg.deviceInfo.deviceId, 'basal', function(err, previous) {
    var lastBasal = _.omit(previous,
      'createdTime', 'id', 'modifiedTime', 'revision', 'uploadId',
      '_active', '_deduplicator', '_id', '_schemaVersion', '_userId',
    );
    debug('Last basal from previous upload was: ', JSON.stringify(lastBasal, null, 4));

    if (datum == null) {
      return cb(null, null);
    }

    // TODO: fix for suspended basals

    var updatedDuration = Date.parse(datum.time) - Date.parse(lastBasal.time);

    if((lastBasal.duration > updatedDuration) || (lastBasal.duration === 0)) {
      ...
      debug('Updating duration of last basal from previous upload');
      if(lastBasal.duration > 0) {
        lastBasal.expectedDuration = lastBasal.duration;
      }
      lastBasal.duration = updatedDuration;

      if (withDurationAnnotation(lastBasal)) {
        // if the duration was previously fabricated from the schedule,
        // or of unknown duration, we can now remove that annotation
        return cb(null, _.omit(lastBasal, 'annotations'));
      } else {
        if (lastBasal.deliveryType === 'scheduled'){
          annotate.annotateEvent(lastBasal, 'basal/mismatched-series');
        }
        return cb(null, lastBasal);
      }
    } else {
      return cb(null, null);
    }
  ...
  api.getTime(function (err, result) {
    ...
    if (isBrowser && currentDeviceTime != null) {
      ...
      if ( Math.abs(serverTime.valueOf()-deviceTime.valueOf()) > FIFTEEN_MINUTES ) {
        ...
        displayTimeModal(cb, cfg, {serverTime, deviceTime});
        return;
      }
    } else {

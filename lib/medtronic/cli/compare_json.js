...
program
  ...
  .option('-f, --filter <filter>', 'filter by type')
  ...
  if (fs.existsSync(file1) && fs.existsSync(file2)){
    ...
    loadFile(file1, function(json1) {
      loadFile(file2, function(json2) {
        if(program.filter) {
          console.log(intro,'Filtering by', program.filter);
          json1 = _.filter(json1,{'type' : program.filter});
          json2 = _.filter(json2,{'type' : program.filter});
        }

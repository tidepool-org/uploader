...
function cleanup(record) {
  ...
  if (record.bolus) {
    record.bolus = _.omit(record.bolus, ['index', 'previous', 'payload', 'deviceId', 'jaebPayload', 'jsDate']);
  }
  ...
  for (var i = 0; i < data1.length; i++) {
    ...
    if(records2.length > 0) {
      ...
      if(!matched) {
        ...
        if(data1[i+1] && (data1[i+1].subType === 'reservoirChange' || data1[i+1].subType === 'alarm')) {

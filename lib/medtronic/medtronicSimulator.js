      ...
      if (currBasal != null) {
          ...
          var checkSchedules = function(startTime, endTime, durationBeforeMidnight) {
            for(var i in currSchedule) {
              ...
              if((startTime <= currSchedule[i].start) && (endTime > currSchedule[i].start) &&
                ...
                newBasal.with_time(new Date(Date.parse(currBasal.time) + adjustedDuration).toISOString())
                  .with_duration(oldDuration - adjustedDuration)
                  .with_deviceTime(sundial.formatDeviceTime(newJsDate))
                  .with_timezoneOffset(currBasal.timezoneOffset)
                  .with_conversionOffset(currBasal.conversionOffset)
        ...
        if(event.deliveryType === 'suspend') {
          ...
          if(currBasal.deliveryType === 'temp') {
            ...
            // also, restart temp basal on resume
            if(currStatus && currStatus.index === event.index) {
              // if the indexes are the same, the suspended basal was
              ...
              // finish up the suspended basal itself
              common.truncateDuration(event, 'medtronic');

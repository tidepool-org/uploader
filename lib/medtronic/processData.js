...
var buildWizardRecords = function (records) {
  ...
  var postrecords = [];
  console.log("WIZARD RECORDS");
  wizardRecords.forEach(function(record) {
    console.log(record.jsDate, common.bytes2hex(record.date), common.bytes2hex(record.body));
    ...
    var carbRatio = struct.extractByte(record.body,14) / 10.0; //TODO: double-check
    var bgTarget = {};
    bgTarget.low = struct.extractByte(record.body,3);
    var isf = struct.extractByte(record.body,4);
    bgTarget.high = struct.extractByte(record.body,5);
    var suggested = struct.extractByte(record.body,7) / 10.0;
    var recommended = {};
    recommended.carb = struct.extractByte(record.body,8) / 40.0;
    recommended.correction = struct.extractByte(record.body,6) / 40.0;
    recommended.net = struct.extractBEShort(record.body,12) / 40.0;
    var iob =  struct.extractShort(record.body,10);
...
var buildBolusRecords = function (records) {
  ...
  bolusRecords.forEach(function(bolus) {
    console.log(bolus.jsDate, common.bytes2hex(bolus.head), common.bytes2hex(bolus.date));
  });
  ...
  for(var i=0; i < bolusRecords.length; i++) {
    ...
    if (larger) {
      console.log("HEAD:", common.bytes2hex(record.head));
      console.log("DATE:", common.bytes2hex(record.date));

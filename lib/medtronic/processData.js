...
function buildCGMRecords(events) {
  ...
  for( var i = 0; i < events.length; i++ ) {
    ...
    console.log('cgm:',event.jsDate, event.type.name, common.bytes2hex([event.head]),common.bytes2hex(event.date),common.bytes2hex(event.body),common.bytes2hex(event.descriptor) );

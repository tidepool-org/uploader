...
var CBG_RECORD_TYPES = {
  ...
  SENSOR_CAL: { value:0x03, name: 'SENSOR_CAL'},
  ...
  CAL_BG_FOR_PH: { value: 0x0E, name:'CAL_BG_FOR_PH'},
  SENSOR_CAL_FACTOR: { value: 0x0F, name:'SENSOR_CAL_FACTOR'},
  UNKNOWN_10: { value: 0x10, name:'UNKNOWN_10'}
};
...
var processCBGPages = function(data, callback) {
  ...
  var cbgBytes = new Uint8Array(1024 * pages.length);
  var index = 0;
  ...
  for(var i = 0; i < pages.length; i++) {
    ...
    cbgBytes.set(struct.extractBytes(pages[i],0,MAX_PAGE_SIZE-2),offset);
    offset += MAX_PAGE_SIZE-2;
  };
  ...
  while (index < cbgBytes.length) {
    ...
    if(record.type.value === CBG_RECORD_TYPES.DATA_END.value) {
    ...
    console.log(record.jsDate, record.type.name, common.bytes2hex([record.head]),common.bytes2hex(record.date),common.bytes2hex(record.body),common.bytes2hex(record.descriptor) );

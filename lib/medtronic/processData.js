...
var RECORD_TYPES = {
  ...
  UNKNOWN_B3: { value:0xB3, body_length:1, name:'UNKNOWN_B3'}, // TODO: remove this
...
function buildBGRecords(records) {
  ...
  for( var i = 0; i < bgRecords.length; i++ ) {
    ...
    bgRecord.with_units('mg/dL')  // TODO: check if values from meters are always in mg/dL
  ...
  for(var i = pages.length - 1; i >= 0; i--) {
    ...
    while(index < MAX_PAGE_SIZE-2) { // last two bytes are check bytes
      ...
      if(record.type.value === RECORD_TYPES.UNKNOWN_55.value) {
        ...
        return callback(Error('Not supporting type 0x55 yet:', common.bytes2hex(record.head)));
      }
      ...
      if(record.type === 'unknown') {
        return callback(new Error('Unknown type: ' + common.bytes2hex([record.head[0]])),null);
      }

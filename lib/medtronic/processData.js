...
function buildSettings(records) {
  ...
  var settingsChange = {
    basalSchedules : cfg.settings.basalSchedules,
    bgTarget : cfg.settings.bgTarget,
    insulinSensitivity : cfg.settings.insulinSensitivity,
    carbRatio : cfg.settings.carbRatio,
    units : cfg.settings.units
  };
  settingsChanges.push(_.clone(settingsChange));

  changes.forEach(function (settingsEntry) {
    console.log('settings change:',settingsEntry.jsDate, common.bytes2hex(settingsEntry.head), common.bytes2hex(settingsEntry.date), common.bytes2hex(settingsEntry.body));
  });
  ...
  for(var i=0; i < changes.length; i++) {
    ...
    if(prevRecord && (record.jsDate.valueOf() !== prevRecord.jsDate.valueOf())) {
      ...
      var settings = settingsChanges[settingsChanges.length-2];
    ...
    if(record.type.value === RECORD_TYPES.CHANGE_BASAL_PROFILE_NEW.value) {
      ...
      if(_.isEqual(settingsChange.basalSchedules[activeSchedule],newSchedule)) {
        settingsChange.basalSchedules[activeSchedule] = oldSchedule;
      } else {

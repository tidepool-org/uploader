...
function buildBasalProfiles(records) {
  ...
  var postrecords = [];
  var activeSchedule, currSchedule;
  ...
  for(var i=0; i < basalProfiles.length; i++) {
    ...
    console.log("ACTIVATED:", activeSchedule, record.jsDate, record.type.name);

    if(postsettings && activeSchedule !== currSchedule) {
      postrecords.push(postsettings);
      console.log("SAVING:", postsettings);
    }
    currSchedule = activeSchedule;

    if(record.type.value === RECORD_TYPES.CHANGE_BASAL_PROFILE_OLD) {

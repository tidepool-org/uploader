...
var CBG_RECORD_TYPES = {
  ...
  DATE_TIME_CHANGE: { value:0x0C, length:4, date_type:'sec', name:'DATE_TIME_CHANGE'},
  ...
  UNKNOWN_10: { value: 0x10, length:4, date_type:'min', name:'UNKNOWN_10'},
  UNKNOWN_13: { value: 0x13, length:0, date_type:'prev', name:'UNKNOWN_13'},
  UNKNOWN_14: { value: 0x14, length:5, date_type:'min', name:'UNKNOWN_14'}
};
...
var processCBGPages = function(data, callback) {
  ...
  for(var i = pages.length - 1; i >= 0; i--) {
    ...
    while(index < MAX_PAGE_SIZE-2) { // last two bytes are check bytes
      ...
      record.body = struct.extractBytes(page, index,record.type.length);
      index += record.type.length;

      //record.jsDate = decodeDate(record.date);
      console.log(/*record.jsDate,*/ record.type.name, common.bytes2hex([record.head]),common.bytes2hex(record.body));

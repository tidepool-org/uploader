...
var CBG_RECORD_TYPES = {
  DATA_END: { value: 0x01, length:0, date_type:'none', name:'DATA_END'},
  SENSOR_WEAK_SIGNAL: { value:0x02, length:0, date_type:'prev', name: 'SENSOR_WEAK_SIGNAL'},
  SENSOR_CAL: { value:0x03, length:1, date_type:'prev', name: 'SENSOR_CAL'},
  UNKNOWN_07: { value:0x07, length:1, date_type:'prev', name: 'UNKNOWN_07'},
  SENSOR_TIMESTAMP: { value:0x08, length:4, date_type:'min', name: 'SENSOR_TIMESTAMP'},
  BATTERY_CHANGE: { value:0x0A, length:4, date_type:'min', name:'BATTERY_CHANGE'},
  SENSOR_STATUS: { value:0x0B, length:4, date_type:'min', name:'SENSOR_STATUS'},
  DATE_TIME_CHANGE: { value:0x0C, length:0, date_type:'min', name:'DATE_TIME_CHANGE'},
  SENSOR_SYNC: { value: 0x0D, length:4, date_type:'min', name:'SENSOR_SYNC'},
  CAL_BG_FOR_PH: { value: 0x0E, length:5, date_type:'min', name:'CAL_BG_FOR_PH'},
  SENSOR_CAL_FACTOR: { value: 0x0F, length:6, date_type:'min', name:'SENSOR_CAL_FACTOR'},
  UNKNOWN_10: { value: 0x10, length:4, date_type:'sec', name:'UNKNOWN_10'}
};
...
function buildTimeChangeRecords(records, mostRecent) {
  ...
  return { postrecords : cfg.tzoUtil.records, tzoUtil : cfg.tzoUtil };
}
...
var processCBGPages = function(data, callback) {
  ...
  var DATE_LENGTH = 5;
  var isigBytes = [];
  var isigBytes = new Uint8Array(1024 * isigPages.length);
  var isigIndex = 0;

  var offset = 0;
  for(var i = 0; i < isigPages.length; i++) {
    console.log('ISIG page',i,common.bytes2hex(isigPages[i]));
    isigBytes.set(struct.extractBytes(isigPages[i],0,MAX_PAGE_SIZE-2),offset);
    offset += MAX_PAGE_SIZE-2;
  };

  for(var i = pages.length - 1; i >= 0; i--) {
    // pages are reverse chronological, but records in each page are chronological
    var page = pages[i];
    console.log('CBG page',i,common.bytes2hex(page));
    var index = 0;
    var prevIndex = 0;
    ...
    while(index < MAX_PAGE_SIZE-2) { // last two bytes are check bytes
      ...
      var bodyPart = struct.extractByte(page, index);
      while((bodyPart < 0x10) ||
            (bodyPart > 0x20) ||
            (bodyPart === 0x13)) {
        record.body.push(bodyPart);
        index += 1;
        bodyPart = struct.extractByte(page, index);
      }

      record.date = struct.extractBytes(page, index, 4);
      index += 4;
      record.jsDate = decodeDate(record.date);

      record.head = struct.extractByte(page,index);
      index += 1;

      if(record.head == null) {
        console.log('End of CBG page.');
        break;
      }
      record.type = getType(record.head,CBG_RECORD_TYPES);

      if(record.head > 20) {
        record.type = { value: null, length:0, date_type:'prev', name: 'GLUCOSE_VALUE'};
        record.raw = struct.extractBytes(page, prevIndex, index - prevIndex);
      }
      ...
      if(record.type.value === CBG_RECORD_TYPES.CAL_BG_FOR_PH.value) {

        // next byte could be SENSOR_CAL_FACTOR with fixed body length,
        // so we push CAL_BG_FOR_PH on stack on read next record
        ...
        record = { body : []};
        while(count < 5) {
          var byte = struct.extractByte(page, index);

          if(byte > 1) {
            count++;
          }

          record.body.push(byte);
          index += 1;
        }
        ...
        if(record.type.value !== CBG_RECORD_TYPES.SENSOR_CAL_FACTOR.value) {
          console.log('CAL_BG_FOR_PH not followed by SENSOR_CAL_FACTOR');
          index = prevIndex; // go back so that we can parse record again
          continue;
        }
      }


      console.log('ISIG:', common.bytes2hex(struct.extractBytes(isigBytes,isigIndex,(index - prevIndex)*2)));
      isigIndex += (index - prevIndex)*2;

      if(record.type.value === null) {
        console.log(record.type.name, common.bytes2hex(record.raw));
      } else {
        console.log(record.jsDate, record.type.name, common.bytes2hex([record.head]),common.bytes2hex(record.date),common.bytes2hex(record.body));
      }
      record.index = numRecords;

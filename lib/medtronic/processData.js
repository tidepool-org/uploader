...
var RECORD_TYPES = {
  ...
  IOB: { value:0x5C, date_length:0, name:'IOB'},
...
function buildBasalProfiles(records) {
  ...
  var basalProfiles = filterHistory([RECORD_TYPES.CHANGE_BASAL_PROFILE_OLD,
                                 RECORD_TYPES.CHANGE_BASAL_PROFILE_NEW,
                                 RECORD_TYPES.SELECT_BASAL_PROFILE], records);
  var postrecords = [];
  var activeSchedule, currSchedule, postsettings;

  basalProfiles.forEach(function (settingsEntry) {
    console.log('basal profile:',settingsEntry.jsDate, common.bytes2hex(settingsEntry.head), common.bytes2hex(settingsEntry.date), common.bytes2hex(settingsEntry.body));
  });

  for(var i=0; i < basalProfiles.length; i++) {
    var record = basalProfiles[i];

    var profileNum = struct.extractByte(record.date,2) >> 5;
    activeSchedule = PROFILES[profileNum];
    console.log("ACTIVE:", activeSchedule, record.jsDate, record.type.name);

    if(postsettings && activeSchedule !== currSchedule) {
      postrecords.push(postsettings);
    }
    currSchedule = activeSchedule;

    if(record.type.value === RECORD_TYPES.CHANGE_BASAL_PROFILE_OLD.value) {
      // basal profile modified and automatically started

      var oldSchedule = getSchedule(record);
      i += 1; //advance to next record for new schedules
      record = basalProfiles[i];
      if(record && record.type.value === RECORD_TYPES.CHANGE_BASAL_PROFILE_NEW.value) {
        var newSchedule = getSchedule(record);
      } else {
        throw new Error('New basal schedule is missing');
      }

      console.log("OLD", oldSchedule, "NEW",newSchedule);
    }


/*
    var found = false;
    for(var i in basalSchedules) {
      var scheduleEntry = basalSchedules[i];
      if(_.isEqual(scheduleEntry,oldSchedule)) {
        // existing schedule is being replaced
        found = true;
        basalSchedules[i] = newSchedule;
      }
    }
    ...
    */
    ...
    postsettings = cfg.builder.makePumpSettings()
      .with_activeSchedule(activeSchedule)
      .with_units({ carb: 'grams', bg: 'mg/dL' }) // TODO: get these units
      .with_basalSchedules(basalSchedules)
      .with_carbRatio(0) // TODO
      .with_insulinSensitivity(0) // TODO
      .with_bgTarget({}) // TODO
      .with_deviceTime(sundial.formatDeviceTime(record.jsDate))
      .set('index', record.index);
...
var buildTempBasalRecords = function (records) {
  ...
  for(var i=0; i < basalRecords.length; i++) {
    ...
    if(type === 'absolute') {
      rate = struct.extractByte(record.head,1) / strokesPerUnit;
    } else {
      percentage = struct.extractByte(record.head,1);
    }

    // TODO: figure out suppressed info
    // TODO: calculate rate for percentage temp basal using suppressed info
...
var buildBasalRecords = function (records) {
  ...
  for(var i=0; i < basalRecords.length; i++) {
    ...
    var rate = struct.extractByte(record.body,1) / strokesPerUnit;
...
var processPages = function(data, callback) {
  ...
  strokesPerUnit = data.settings.strokesPerUnit;
  larger = data.settings.larger;
  ...
  for(var i = pages.length - 1; i >= 0; i--) {
    ...
    while(index < MAX_PAGE_SIZE) {
      ...
      if(record.type.value === RECORD_TYPES.IOB.value) {
...
module.exports.buildBasalProfiles = buildBasalProfiles;

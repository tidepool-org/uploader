...
function buildCGMRecords(events) {
  ...
  for( var i = 0; i < events.length; i++ ) {
    ...
    if(event.jsDate != false) {
      console.log('cgm:',sundial.formatDeviceTime(event.jsDate), event.type.name, common.bytes2hex([event.head]),common.bytes2hex(event.date),common.bytes2hex(event.body),common.bytes2hex(event.descriptor) );
          ...
          if(isig !== 0x22 && isig > 100) {
            ...
            if(start instanceof Date) {
              ...
              if(cbg >= 40) { // ignore magic numbers
                ...
                cfg.tzoUtil.fillInUTCInfo(record, date);
...
var processCBGPages = function(data, callback) {
  ...
  for(var i = 0; i < isigPages.length; i++) {
    ...
    isigBytes.set(struct.extractBytes(isigPages[i],0,(DEFAULT_PAGE_SIZE * 2)-2),offset);
  ...
  for(var i = 0; i < pages.length; i++) {
    ...
    cbgBytes.set(struct.extractBytes(pages[i],0,DEFAULT_PAGE_SIZE-1),offset);
  ...
  while (index < cbgBytes.length) {
    ...
    if(record.head > 20) {
      record.type = { name: 'GLUCOSE_VALUES'};
      record.body.push(record.head);
      record.head = '';
    } else {
    ...
    var date = null;
    if(record.jsDate instanceof Date) {
      date = sundial.formatDeviceTime(record.jsDate);
    }
    console.log('Record:',date, record.type.name, common.bytes2hex(record.body),',', common.bytes2hex(record.descriptor));

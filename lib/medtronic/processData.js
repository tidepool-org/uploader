...
function buildBasalProfiles(records) {
  ...
  var getWizardSettings = function(record) {
    ...
    // TODO: get bg units from head

    console.log('Wizard changes:', common.bytes2hex(record.body));
    var carbRatios = [];
    for(var j = 0; j < 8; j++ ) {
      var carbRatio = struct.unpack(record.body,(j*3)+2,'bS',['offset', 'ratio']);
      if (carbRatio.offset === 0 && carbRatio.ratio === 0) {
        break;
      }
      var startTime = carbRatio.offset * 30 * sundial.MIN_TO_MSEC;
  ...
  for(var i=0; i < basalProfiles.length; i++) {
    ...
    if(prevRecord && (record.jsDate.valueOf() !== prevRecord.jsDate.valueOf())) {
      ...
      postsettings.with_units({ carb: 'grams', bg: 'mg/dL' }) // TODO: get these units
                  .with_carbRatio(cfg.settings.carbRatio) // TODO
                  .with_insulinSensitivity(cfg.settings.insulinSensitivity) // TODO
                  .with_bgTarget(cfg.settings.bgTarget) // TODO read history of changes

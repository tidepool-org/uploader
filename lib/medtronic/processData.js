...
var RECORD_TYPES = {
  ...
  LOW_RESERVOIR_508: { value: 0x10, date_length: 2, name: 'LOW_RESERVOIR_508' },
  ...
  BOLUS_WIZARD_CHANGE_512: { value:0x2E, body_length:100, name: 'BOLUS_WIZARD_CHANGE_512'},
  ...
  SENSOR_SETTINGS: { value:0x50, body_length:30, larger:{ body_length:34 }, name:'SENSOR_SETTINGS'},
  ...
  TOTAL_DETAILS: { value:0x6C, head_length:1, date_length:2, body_length:40, name:'TOTAL_DETAILS'}, //TODO: body_length model522:41, model508:34
  MODEL_522_RESULTS_TOTALS: { value:0x6D, head_length:1, date_length:2, body_length:40, name:'MODEL_522_RESULTS_TOTALS'},
  SENSOR_TOTAL_DETAILS: { value:0x6E, head_length:1, date_length:2, body_length:49, name:'SENSOR_TOTAL_DETAILS'}, //TODO: make sure about date_length, body_length
...
var LGS_TYPES = {
  ...
  SUSPEND_USER: { value: 0x05, name: 'Suspend user'},
...
var buildWizardRecords = function (records) {
  ...
  wizardRecords.forEach(function(record) {
    ...
    if (settings.larger) {
      ...
      recommended.carb = struct.extractBEShort(record.body,6) / settings.strokesPerUnit;
      ...
      recommended.correction = struct.extractBEShort(correctionRaw,0) / settings.strokesPerUnit;

      recommended.net = struct.extractBEShort(record.body,11) / settings.strokesPerUnit;
      iob =  struct.extractBEShort(record.body,9) / settings.strokesPerUnit;
...
var buildBolusRecords = function (records) {
  ...
  for(var i=0; i < bolusRecords.length; i++) {
    ...
    if(settings.larger) {
      ...
      amount = struct.extractByte(record.head,2)/settings.strokesPerUnit;
      programmed = struct.extractByte(record.head,1)/settings.strokesPerUnit;
      iob = null;
    }
      ...
      if(event.body) {
        ...
        while(offset >= 0) {
          ...
          if(descriptor > 255) {
            ...
            console.log('CBG:', sundial.formatDeviceTime(date), common.bytes2hex(struct.extractBytes(event.descriptor, offset * 2, 2)),',',common.bytes2hex([struct.extractByte(event.body,offset)]),',',cbg,',',(descriptor  & 0xFFFE) / 100.0);
            if(cbg >= 40) { // ignore magic numbers
              var record = cfg.builder.makeCBG()
                    ...
                    .with_payload({ interstitialSignal : (descriptor  & 0xFFFE) / 100.0 }) // mask out lsb
...
function buildSuspendResumeRecords(records) {
  ...
  for(var i=0; i < suspendResumeRecords.length; i++) {
    ...
    if(suspendEntry.type.value === RECORD_TYPES.PUMP_SUSPEND.value) {
      ...
      if((suspendEntry.head[1] & BITMASKS.RIGHT_FIVE) === LGS_TYPES.SUSPEND_ALARM.value) {
        // we are dealing with a Low Glucose Suspend (LGS) alarm
        var nextEntry = suspendResumeRecords[i+1];
        suspendResume.reason.suspended = 'automatic';
        var lgsTypes = [];
        console.log('LGS at', suspendEntry.jsDate);

        if(nextEntry.type.value === RECORD_TYPES.PUMP_SUSPEND.value) {
          if((nextEntry.head[1]  & BITMASKS.RIGHT_FIVE) === LGS_TYPES.SUSPEND_LGS.value) {
            i += 1; // ready to read next event
            while(suspendResumeRecords[i+1].type.value === RECORD_TYPES.PUMP_SUSPEND.value &&
              (suspendResumeRecords[i+1].head[1] & BITMASKS.RIGHT_FIVE) === LGS_TYPES.SUSPEND_USER.value) {
              // we use a while loop here, as the user can select suspend in the UI multiple times
              i += 1; //LGS suspended by user, ready to read next event
              console.log('LGS suspended by user');
              lgsTypes.push(LGS_TYPES.SUSPEND_USER.name);
            }

          } else if ((nextEntry.head[1]  & BITMASKS.RIGHT_FIVE) === LGS_TYPES.SUSPEND_NO_RESPONSE.value) {
            i += 1;
            console.log('LGS no response');
            lgsTypes.push(LGS_TYPES.SUSPEND_NO_RESPONSE.name);
          }
        }
      }
      ...
      if(resumeEntry && resumeEntry.type.value === RECORD_TYPES.PUMP_RESUME.value) {
        ...
        if(headCheck === LGS_TYPES.RESUME_AUTO_NO_RESPONSE.value) {
          suspendResume.reason.resumed = 'automatic';
          lgsTypes.push(LGS_TYPES.RESUME_AUTO_NO_RESPONSE.name);
          console.log('LGS resumed automatically after no response at', resumeEntry.jsDate);
          i += 2; // LGS auto has three resume records, so increment counter twice
        }

        if(headCheck === LGS_TYPES.RESUME_AUTO_USER_SUSPEND.value) {
          suspendResume.reason.resumed = 'automatic';
          lgsTypes.push(LGS_TYPES.RESUME_AUTO_USER_SUSPEND.name);
          console.log('LGS resumed automatically after user suspend at', resumeEntry.jsDate);
        }

        if(headCheck === LGS_TYPES.RESUME_USER.value) {
          lgsTypes.push(LGS_TYPES.RESUME_USER.name);
          console.log('LGS resumed by user at', resumeEntry.jsDate);

          if(suspendResumeRecords[i+1].type.value === RECORD_TYPES.PUMP_RESUME.value) {
      ...
      suspendResume.with_payload({lgs_types : lgsTypes});
      ...
      console.log('Suspend event out of order:',suspendEntry);
...
function getCarbRatios(encoded, units) {
  ...
  for(var j = 0; j < 8; j++ ) {
    ...
    if((carbRatios.length > 0) && (startTime < carbRatios[carbRatios.length-1].start)) {
      debug('Invalid start time for carb ratio:');
      break;
    }
    if(units === 'exchanges') {

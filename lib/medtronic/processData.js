...
var decodeDate = function (payload) {
 ...
 if(payload.length > 5) {
   encoded.year = struct.extractByte(payload,8);
 }
...
function buildBasalProfiles(records) {
  ...
  var basalProfiles = filterHistory([RECORD_TYPES.CHANGE_BASAL_PROFILE_OLD,
                                 ...
                                 RECORD_TYPES.SELECT_BASAL_PROFILE], records).reverse();
  var postrecords = [];
  var scheduleChanges = [];
  var activeSchedule, prevRecord;

  // when we record schedule changes, we need to pre-fill with the
  // most recent schedules, and remember that they only change for the
  // *previous* schedules
  var scheduleChange = _.clone(cfg.settings.basalSchedules);
  scheduleChanges.push(scheduleChange,scheduleChange);
  ...
  for(var i=0; i < basalProfiles.length; i++) {
    ...
    if(prevRecord && (record.jsDate.valueOf() !== prevRecord.jsDate.valueOf())) {
      ...
      var basalSchedules = scheduleChanges[scheduleChanges.length-2];
      var postsettings = cfg.builder.makePumpSettings();
      postsettings.with_units({ carb: 'grams', bg: 'mg/dL' }) // TODO: get these units
                  .with_carbRatio([]) // TODO
                  .with_insulinSensitivity([]) // TODO
                  .with_bgTarget([]) // TODO
                  .set('index', prevRecord.index)
                  .with_basalSchedules(basalSchedules)
                  .with_activeSchedule(prevRecord.activeSchedule)
    ...
    if(record.type.value === RECORD_TYPES.CHANGE_BASAL_PROFILE_NEW.value) {
      ...
      if(_.isEqual(scheduleChange[activeSchedule],newSchedule)) {
        scheduleChange[activeSchedule] = oldSchedule;
      } else {
        throw new Error('Could not find basal schedule');
      }

      scheduleChanges.push(_.clone(scheduleChange));
    }

    prevRecord = record;
    prevRecord.activeSchedule = activeSchedule;
  }
...
function buildPrimeRecords(records) {
  ...
  primeRecords.forEach(function (primedatum) {
    ...
    var type = fixed === 0 ? 'manual' : 'fixed';

    console.log('Prime values:', amount, fixed, type);
...
var processPages = function(data, callback) {
  ...
  for(var i = pages.length - 1; i >= 0; i--) {
    ...
    while(index < MAX_PAGE_SIZE-2) { // last two bytes are check bytes
      ...
      if(record.type.value === RECORD_TYPES.TEMP_BASAL_DURATION.value) {
        // temp basal record can have variable date length
        var yearByte = struct.extractByte(page,index+6);
        var yearBit = yearByte >> 7;
        if(yearBit) {
          console.log('Found variable length temp basal duration');
          record.type.date_length = 9;
        } else {
          record.type.date_length = 5;
        }
      }

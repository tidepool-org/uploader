	...
	for (var i = 0; i < settings.length; ++i) {
		...
		if (rec.internalTime.slice(0,4) !== '2009') {
			if (rec.displayOffset !== lastDisplayOffset) {
				// timeChanges.push(rec);
				var change = opts.builder.makeDeviceMetaTimeChange()
					.with_change({
						from: sundial.formatDeviceTime(new Date(rec.systemTimeMsec + 1000 * lastDisplayOffset)),
						to: sundial.formatDeviceTime(new Date(rec.systemTimeMsec + 1000 * rec.displayOffset)),
						agent: 'manual'
					})

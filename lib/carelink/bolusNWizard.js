...
module.exports = function(timezone, model) {
   ...
   *
   * The general strategy here is to build hash of all bolus (component) and wizard events
   * keyed by `uploadSeqNum` (within an `uploadId`), then as we're traversing the data
   * we try to handle each event as it comes, removing it from the cache *as well as any
    ...
    if (common.autoGenModels[model]) {
      return function(parsed, i) {
        ...
        if (parsed.type === 'wizard') {
          ...
          if (obj != null && obj.type === 'bolusNormal') {
            // only allow three minutes between wizard and bolus
            if (Math.abs(Date.parse(parsed.time) - Date.parse(obj.time)) > 1000 * 60 * 3) {
              return null;
            }
            if (i !== 0) {
    ...
    if (common.autoGenModels[model]) {
      return function(bolus) {
        ...
        if (bolus.type === 'bolusNormal') {
          ...
          if (obj != null && obj.type === 'wizard') {
            // only allow three minutes between wizard and bolus
            if (Math.abs(Date.parse(bolus.time) - Date.parse(obj.time)) > 1000 * 60 * 3) {
              return null;
            }
            return obj;
          }
      ...
      switch(parsed.type) {
        ...
        case 'wizard':
          if (isInCache(parsed)) {
            ...
            // because other events (like alarms) can't sometimes intervene between

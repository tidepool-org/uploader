...
module.exports = function (timezone) {
  ...
  /*
   * In some instance there aren't both parts to a dual bolus
   * to test that we want to treat this as a dual bolus then make
   * sure that the whole amount hasn't already been delivered
   */
  function isValidDualFromWizard(theBolus, theWizard){
    if(theBolus.dualComponent && !_.isEmpty(theWizard)){

      var bolusAmount = 0;

      if(theBolus.type === 'bolusNormal'){
         bolusAmount = theBolus.normal;
      }else{
         bolusAmount = theBolus.extended;
      }
      var wizAmount = theWizard.recommended.carb + theWizard.recommended.correction;
      return wizAmount !== bolusAmount;
    }
    return theBolus.dualComponent;
  }
  ...
  return function (simulator, datum, i, data) {
    ...
    if (parsed != null) {
      ...
      if( isValidDualFromWizard(bolus,wizard) ){

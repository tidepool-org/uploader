...
var entryParser = common.makeParser(
  {
    /** basal **/
    ChangeBasalProfile: {
...
module.exports = function (timezone) {
  ...
  function buildCache(data) {
    ...
    // dedupe the settings by timestamp because it seems jellyfish is rejecting the second of two
    // with exact same timestamp, but second is the more up-to-date one!
    var unique = {};
    _.each(cachedObjects, function(obj) {
      unique[obj.time] = obj;
    });

    cachedObjects = _.map(Object.keys(unique), function(key) { return unique[key]; });
  }

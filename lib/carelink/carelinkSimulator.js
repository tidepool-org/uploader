...
exports.make = function(config){
  ...
  var events = [], hasSeenScheduled = false;
      ...
      if (doneTime < timeMillis) {
          ...
          case 'scheduled':
            ...
            if (currSettings == null) {
              ...
              if (currSchedule == null || currSchedule.length === 0) {
                ...
                var millisInDay = common.computeMillisInCurrentDay({
                  time: doneTime,
                ...
                var basalBuilder = bob.makeScheduledBasal()
                  ...
                  .with_clockDriftOffset(sched.clockDriftOffset)
                  .with_conversionOffset(sched.conversionOffset)
                  .with_deviceTime(sundial.formatDeviceTime(new Date(newTsMillis + sched.timezoneOffset*60*1000 + sched.conversionOffset)))
        ...
        _.assign(
          parts[0],
          {
            ...
            deviceTime: sundial.formatDeviceTime(new Date(doneTime + parts[0].timezoneOffset*60*1000 + parts[0].conversionOffset)),
  ...
  return {
      ...
      if (currBasal != null && currBasal.time >= event.time &&
        ...
        if (currStatus != null) {
          ...
          // so we fabricate a resume and we'll catch it and use when the actual resume fires
            ...
            setCurrStatus(deviceEventResume);
      ...
      if (currSchedule == null || currSchedule.length === 0) {
        if (event.duration == null) {
          ...
          var nullDurationBasal = bob.makeScheduledBasal()
            ...
            .with_clockDriftOffset(event.clockDriftOffset)
        ...
        var millisInDay = common.computeMillisInCurrentDay(event);
     ...
     *
     * @param argument... Variable number of arguments, each should be an object.  The field:value pairs passed in
     *                    are smooshed together into a deviceEvent event recorded by the simulator
     */
     ...
     *
     * @param argument... Variable number of arguments, each should be an object.  The field:value pairs passed in
     *                    are smooshed together into a suspend deviceEvent event recorded by the simulator
     */
    lgsAutoResume: function(){
      ...
      var deviceEventResumeBuilder = bob.makeDeviceEventResume()
        .with_time(event.time)
        .with_timezoneOffset(event.timezoneOffset)
        .with_clockDriftOffset(event.clockDriftOffset)
        .with_conversionOffset(event.conversionOffset)
      ...
      if (event.deviceId != null) {
        deviceEventResumeBuilder.set('deviceId', event.deviceId);
      }

      if (currStatus != null) {
        deviceEventResumeBuilder.with_previous(_.omit(currStatus, 'previous'));
      }

      var deviceEventResume = deviceEventResumeBuilder.done();
      setCurrStatus(deviceEventResume);
    ...
    resume: function(){
      ...
      if (currBasal.deliveryType !== 'suspend') {
        if (currStatus != null && currStatus.status === 'resumed' &&
          ...
          currStatus = addEvent(bob.makeDeviceEventResume(), currStatus);
      ...
      if (event.payload != null) {
        deviceEventResumeBuilder.with_payload(event.payload);
      }
    ...
    pumpSettings: function(){
      ...
      if (currBasal == null && config.autoGenScheduleds) {
        ...
        var millisInDay = common.computeMillisInCurrentDay(currSettings);
        for (var i = currSchedule.length - 1; i > 0; --i) {
          if (currSchedule[i].start <= millisInDay) {
            break;
          }
        ...
        var basalBuilder = bob.makeScheduledBasal()
          .with_scheduleName(currSettings.activeSchedule)
          .with_deviceTime(currSettings.deviceTime)
          .with_time(currSettings.time)
          .with_timezoneOffset(currSettings.timezoneOffset)
          .with_clockDriftOffset(currSettings.clockDriftOffset)
          .with_conversionOffset(currSettings.conversionOffset)
          .with_rate(currSchedule[i].rate)
          .with_duration((i+1 >= currSchedule.length ? twentyFourHours : currSchedule[i+1].start) - millisInDay)
          .set('annotations', [{ code: 'carelink/basal/fabricated-from-schedule' }]);
    ...
    suspend: function(){
       ...
       * but the user cancels the LGS, then the LGS fires again within the duration of the original
      ...
      var deviceEventSuspendBuilder = bob.makeDeviceEventSuspend()
        .with_time(event.time)
        .with_timezoneOffset(event.timezoneOffset)
        .with_clockDriftOffset(event.clockDriftOffset)
        .with_conversionOffset(event.conversionOffset)
      ...
      if (event.payload != null) {
        deviceEventSuspendBuilder.with_payload(event.payload);
      }

      if (event.deviceId != null) {
        deviceEventSuspendBuilder.set('deviceId', event.deviceId);
      }

      var deviceEventSuspend = deviceEventSuspendBuilder.done();
    ...
    /**
     * Return a boolean reflecting whether or not this simulator instance has seen
     * a scheduled basal event yet. In very rare instances, we've seen data that starts
     * with a suspend event, which is rather nonsensical and not what the simulator
     * was designed to handle, so now we skip everything until we've seen a scheduled
     * basal.
     *
     * @returns {Boolean} whether simulator has seen a scheduled basal
     */
    hasSeenScheduled: function() {
      return hasSeenScheduled;
    }
  };
};

...
module.exports = function(simulatorMaker, jellyfish, api){
  return function (config) {
    ...
    return {
      ...
      fetchData: function (progress, payload, cb) {
        ...
        var formData = {j_username: cfg.username, j_password: cfg.password, j_character_encoding: 'UTF-8'};

        tidepoolApi.upload.careLink(formData, function(err, results){
           if (err) {
              console.log('error getting carelink data ['+JSON.stringify(err)+']');
              console.log('error results ['+JSON.stringify(results)+']');
              console.log(results);
              return cb(err);
            }
        ...
        for (var i = 0; i < payload.theData.length; ++i) {
          ...
          if (device == null) {
            ...
            device.simulator = simulatorMaker.make(
              {
                autoGenScheduleds: key === 'Paradigm 722',
      ...
      uploadData: function (progress, payload, cb) {
        ...
        Object.keys(payload.devices).map(function(key){
          jellyfish.post(payload.devices[key].simulator.getEvents(), progress, function (err, result) {
            if (err) {
              console.log(err);
              console.log(result);
              progress(100);

...
//>>>>>>> mvp
      ...
      uploadData: function (progress, payload, cb) {
        ...
        payload.post_records = Object.keys(payload.devices).map(function(key){
          return payload.devices[key].simulator.getEvents();
        });

        jellyfish.post(payload.post_records, progress, function (err, result) {
          if (err) {
            debug(err);
            debug(result);
            progress(100);

...
/*********************************************************************
 * Function:    calcCRC16Modbus()
 * Description: Compute CRC-16/MODBUS of a given array of bytes.
 *              Poly 0x8005 (reflected as 0xA001), init 0xFFFF,
 *              reflect input & output, no final XOR.
 *              Used by VivaChek / Keto-Mojo BLE glucose meters.
 * Inputs:      bytes - array-like of bytes
 *              size  - number of bytes (optional, defaults to bytes.length)
 * Returns:     The 16-bit CRC.
 *********************************************************************/
module.exports.calcCRC16Modbus = function (bytes, size) {
  var len = size !== undefined ? size : bytes.length;
  var crc = 0xFFFF;
  for (var i = 0; i < len; i++) {
    crc ^= bytes[i] & 0xFF;
    for (var j = 0; j < 8; j++) {

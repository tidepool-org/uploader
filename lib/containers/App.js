...
export default class App extends Component {
  ...
  renderHeader() {
    ...
    return (
      <LoggedInAs
        dropMenu={dropdown}
        isUploadInProgress={Boolean(this.props.uploads.uploadInProgress)}
...
export default connect(
  (state) => {
    function getActiveUploads(state) {
      const { devices, uploads, uploadTargetUser } = state;
      if (uploadTargetUser === null) {
        return [];
      }
      let activeUploads = [];
      const targetUsersUploads = _.get(uploads, uploadTargetUser, []);
      _.map(Object.keys(targetUsersUploads), (deviceKey) => {
        const upload = uploads[uploadTargetUser][deviceKey];
    ...
    function getPotentialUploadsForUploadTargetUser(state) {
      return Object.keys(
        _.get(state, ['uploads', state.uploadTargetUser], {})
      );
    }

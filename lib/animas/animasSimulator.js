...
exports.make = function(config){
  ...
  return {
    ...
    basal: function(event){
      ...
      if(currStatus !== null) {
        if(currStatus.status === 'suspended' && event.rate === 0) {
          ...
          event.suppressed = _.omit(currBasal, 'previous');
        }
      }
    ...
    wizard: function(event) {
      if (event.syncCounter !== currBolus.syncCounter) {
        console.log('Wizard bolus mismatch!', event.syncCounter, currBolus.syncCounter);
      }
      else {
        event = event.with_recommended({
          // TODO: handle this correctly!
            carb: 0,
            correction: 0,
            net: 0
          })
          .with_bolus(currBolus)
          .with_units('mg/dL') //TODO: get correct unit from config byte
          .with_deviceTime(currBolus.deviceTime)
          .with_time(currBolus.time)
          .with_timezoneOffset(currBolus.timezoneOffset)
          .with_conversionOffset(currBolus.conversionOffset)
          .with_clockDriftOffset(currBolus.clockDriftOffset);

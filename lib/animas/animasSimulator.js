  ...
  return {
    ...
    resume: function(event) {
      if(currStatus != null && currStatus.status === 'suspended') {
        ...
        var duration = Date.parse(event.time) - Date.parse(currStatus.time);

        if(duration >= 0) {
          currStatus.duration = duration;
        }
        else {
          currStatus.duration = 0;
          annotate.annotateEvent(currStatus,'basal/unknown-duration');
        }
        currStatus.reason.resumed = event.reason.resumed;

        dataservicesEvents.push(currStatus.done());
        setCurrStatus(event);
      }
    },
    ...
    finalize: function() {
      if(currStatus != null && currStatus.status === 'suspended') {
        // add the final suspend event
        currStatus = currStatus.done();

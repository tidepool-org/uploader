...
module.exports = function (config) {
  ...
  var extractPacket = function (bytes) {
    ...
    // look for another ETX
    var startIndex = 1; // start past the opening one
  ...
  var freeStyleErrorHandler = function(info) {
    if (info.connectionId && info.error) {
      if (info.error == 'timeout') {
        // whenever we get a timeout, we insert an ETX into the stream to differentiate packets
  ...
  var freeStyleMessageHandler = function (buffer) {
    ...
    // (we must see an ETX to call it a packet)
    var discardCount = 0;
    while (buffer.len() > discardCount && buffer.get(0) != ETX) {
      ++discardCount;
    }
    // discard excess ETXs
    while (buffer.len() > discardCount + 1 && buffer.get(1) == ETX) {
      ++discardCount;
    }
    if (discardCount) {
...
NEED TO WRITE PACKET SEND CODE, remove rest of onetouch code

...
export default class App extends Component {
  ...
  handleClickEditUser() {
    const { setPage } = this.props.sync;
    setPage(pages.CLINIC_USER_EDIT, true);
  }
  ...
  renderPage() {
    ...
    let changePersonLink = this.props.isClinicAccount ?
      this.renderChangePersonLink() : null;
    ...
    if (page === pages.LOADING) {
      ...
      let viewDataLinkButton = null;
      let clinicUserBlock = null;
      if(this.props.isClinicAccount){
        clinicUserBlock = this.renderClinicUserBlock(this.renderTimezoneDropdown());
        viewDataLinkButton = <ClinicUploadDone
          onClicked= {this.handleClickChangePerson.bind(this)} />;
      } else {
        timezoneDropdown = this.renderTimezoneDropdown();
        viewDataLinkButton = <ViewDataLink
          href={viewDataLink}
      ...
      return (
        <div className={styles.mainWrap}>
          ...
          <UploadList
            ...
            onChooseDevices={this.handleClickChooseDevices}
            onClinicDone={this.handleClickChangePerson} />
          {viewDataLinkButton}
        </div>
      );
    } else if (page === pages.SETTINGS) {
      let clinicUserBlock = this.props.isClinicAccount ?
        this.renderClinicUserBlock() : null;
      return (
        <div>
          {userDropdown}
          {changePersonLink}
      ...
      return (
        <div>
          <ClinicUserSelect
            allUsers={allUsers}
            onUserChange={this.props.async.setUploadTargetUserAndMaybeRedirect}
            page={page}
            targetId={uploadTargetUser}
      ...
      return (
        <div>
          <ClinicUserEdit
            ...
            cancelEdit={this.handleClickChangePerson.bind(this)} />
        </div>
      );
    } else {
  ...
  renderChangePersonLink() {
    return (
      <div className={styles.changePerson}
        onClick={this.handleClickChangePerson.bind(this)}>Change Person</div>
    );
  }

  renderClinicUserBlock(timezoneDropdown) {
    return (
      <ClinicUserBlock
        allUsers={this.props.allUsers}
        targetId={this.props.uploadTargetUser}
        timezoneDropdown={timezoneDropdown}
        onEditUser={this.handleClickEditUser.bind(this)} />
    );
  }
}

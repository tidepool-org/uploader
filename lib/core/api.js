...
api.setHosts = function(hosts) {
  ...
  if (hosts.DATASERVICES_URL) {
    // TODO: enable when connecting to new platform-client
...
function getUploadFunction(uploadType) {
  if(uploadType === 'dataservices') {
    return tidepool.uploadDataServicesDataForUser;
  }
  else if (uploadType === 'uploads') {
   return tidepool.uploadDeviceDataForUser;
  }
  return null;
}

function postBlockToPlatform(data, groupId, blockIndex, uploadType, callback) {
  ...
  uploadForUser(groupId, data, function(err, result) {
    if (err) {
      return sad(err, err);
    }
    return happy(result);
...
api.upload.toPlatform = function(data, sessionInfo, progress, groupId, cb, uploadType) {
  uploadType = uploadType || 'uploads'; // can either be 'uploads' or 'dataservices'
  ...
  var post_and_progress = function (data, callback) {
    ...
    return postBlockToPlatform(data, groupId, nblocks, uploadType, callback);
  };

  var post_upload_meta = function (uploadMeta, callback) {
    return postBlockToPlatform(uploadMeta, groupId, 0, uploadType, callback);
  };
  ...
  async.waterfall([
    function(callback) {
      ...
      if(uploadType === 'uploads') {
        //TODO: data services api should support upload metadata too

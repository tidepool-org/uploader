...
api.errors.log = (error, message, properties) => {
  ...
  if (rollbar) {
    ...
    rollbar.error(error, extra, (err, data) => {
      if (err) {
        api.log('Error while reporting error to Rollbar:', err);
      } else {
        api.log(`Rollbar UUID: ${data.result.uuid}`);
        properties.uuid = data.result.uuid;
        console.log('Properties:', JSON.stringify(properties, null, 4)); // TODO: remove this
      }
    });
  }

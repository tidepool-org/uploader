...
function buildError(dataset, datasetId) {
  ...
  err.name = 'API Error';
  var status = null;
  if(dataset.error && dataset.error.text) {
    api.log('Error status:', dataset.error.text);
    status = JSON.parse(dataset.error.text);
  }
  err.status = status;
...
function createDatasetForUser(userId, info, callback) {
  ...
  var sad = function (jqxhr, err) {
    api.log('platform create dataset failed:', err);
    var error = buildError(jqxhr);
    callback(error);
  };
...
function finalizeDataset(datasetId, callback) {
  ...
  var sad = function (err, datasetId) {
    api.log('platform finalize dataset failed:', err);
    var error = buildError(null, datasetId);
    callback(error);
  };
...
function addDataToDataset(data, datasetId, blockIndex, uploadType, callback) {
  ...
  var sad = function (jqxhr, err) {
    ...
    if (jqxhr.responseJSON && jqxhr.responseJSON.errorCode && jqxhr.responseJSON.errorCode == 'duplicate') {
      ...
      var err = buildError(jqxhr, datasetId);
      callback(err);
    }
  };

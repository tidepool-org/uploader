...
api.create = function(options) {
  ...
  tidepool = createTidepoolClient({
    ...
    dataservicesApi: options.dataservicesUrl,
    log: {
      warn: tidepoolLog,
      info: tidepoolLog,
      debug: tidepoolLog
...
api.setHosts = function(hosts) {
  ...
  if (hosts.DATASERVICES_URL) {
    tidepool.setDataServicesHost(hosts.DATASERVICES_URL);
  }
  if (hosts.BLIP_URL) {
...
function postBlockToPlatform(data, groupId, blockIndex, dataservices, callback) {
  ...
  if(dataservices) {
    tidepool.uploadDataServicesDataForUser(groupId, data, function(err, result) {
      if (err) {
        return sad(err, err);
      }
      return happy(result);
...
api.upload.toPlatform = function(data, sessionInfo, progress, groupId, cb) {
  ...
  var post_and_progress = function (data, callback) {
    ...
    return postBlockToPlatform(data, groupId, nblocks, false, callback);
  };

  var post_and_progress_dataservices = function (data, callback) {
    progress(nblocks++ * 100.0 / blocks.length);
    //off to the dataservices platfrom we go
    return postBlockToPlatform(data, groupId, nblocks, true, callback);
  };

  var post_upload_meta = function (uploadMeta, callback) {
    return postBlockToPlatform(uploadMeta, groupId, 0, false, callback);
  };
...
/*
 * process the data sending it to the dataservices platform in blocks and feed back progress to the calling function
 */
api.upload.toDataServices = function(data, sessionInfo, progress, groupId, cb) {

  api.log('attempting to upload', data.length, 'records to dataservices api');
  var grouped = _.groupBy(data, 'type');
  for (var type in grouped) {
    api.log(grouped[type].length, 'records of type', type);
  }
  ...
  var decorate = function (data, sessionInfo) {
    var deviceRecords = _.map(data, function(item) {
      return _.extend({}, item, {
        uploadId: 'upid_' + md5(sessionInfo.deviceId + '_' + sessionInfo.start).slice(0, 12),
        guid: uuid.v4()
      });
    });
    return deviceRecords;
  ...
  data =  decorate(data, sessionInfo);

  for (var i = 0; i < data.length; i += BLOCKSIZE) {
    blocks.push(data.slice(i, i + BLOCKSIZE));
  }
  api.log('start uploading dataservices data');
  async.mapSeries(blocks, post_and_progress, function(err, result) {
    if(_.isEmpty(err)){
      api.log('upload.toDataServices: all good');
      return cb(null, result);
    }
    api.log('upload.toDataServices: failed ',err);
    return cb(err);
  });
};

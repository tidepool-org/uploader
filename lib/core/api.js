...
api.upload.careLink = function(form, cb) {
  //off to jellyfish we go
  api.log('POST /careLink');

  tidepool.uploadCarelinkDataForUser(form, function(err, result) {
    if(err){
...
api.fetch.careLink = function(dataId, cb) {
  //get our carelink data to process
  api.log('GET /careLink');
  return tidepool.getCarelinkData(dataId, cb);
};

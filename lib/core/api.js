...
function addDataToDataset(data, datasetId, blockIndex, callback) {
  ...
  tidepool.addDataToDataset(datasetId, data, (err, result) => {
    if (err) {
      return sad(err);
    }
    return happy(result);
 ...
 * `devices` is the final argument (instead of the callback) so that existing calls to
 * api.upload.toPlatform don't have to be modified in every driver, and is used to handle
 * the progress bar for uploading multiple devices
 */
api.upload.toPlatform = (data, sessionInfo, progress, groupId, cb, devices) => {

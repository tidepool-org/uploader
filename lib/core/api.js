  ...
  async.waterfall([
    function(callback) {
      ...
      if(sessionInfo.delta != null) {
        uploadItem.client.private.delta = sessionInfo.delta;
      }

      if(sessionInfo.blobId != null) {
        uploadItem.client.private.blobId = sessionInfo.blobId;
      }
...
api.upload.blob = function(blob, contentType, cb) {
  ...
  var digest = btoa(md5(blob, null, true));
...
api.errors.log = function(error, message, properties) {
  ...
  if (rollbar) {
    if (error.data && error.data.blobId) {
      rollbar.error(error, { blobId: error.blobId });
    } else {
      rollbar.error(error);
    }
  }

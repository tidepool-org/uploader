...
function buildError(error, datasetId) {
  ...
  var errText = JSON.parse(_.get(error, 'response.error.text', null));
  if (_.has(errText,'meta')) {
    err.requestTrace = errText.meta.trace.request;
    err.sessionTrace = errText.meta.trace.session;
    err.sessionToken = md5(errText.meta.trace.token);
  }
  return err;
}

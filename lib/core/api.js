...
function buildError(dataset, datasetId) {
  var err = new Error('Uploading data to platform failed.');
  err.name = 'API Error';
  var status = JSON.parse(_.get(dataset, 'error.text', null));
  api.log('Error status:', status);
  err.status = status;
  err.datasetId = datasetId;
  err.sessionToken = dataset.sessionToken; //TODO: calculate MD5 hash
  if (_.has(status,'meta')) {
    err.requestTrace = status.meta.trace.request;
    err.sessionTrace = status.meta.trace.session;
  }
  return err;
}
...
function finalizeDataset(datasetId, callback) {
  ...
  var sad = function (err, dataset) {
    ...
    var error = buildError(dataset, datasetId);
    callback(error);
  };
...
function addDataToDataset(data, datasetId, blockIndex, uploadType, callback) {
  ...
  var sad = function (error, dataset) {
    ...
    if (error.responseJSON && error.responseJSON.errorCode && error.responseJSON.errorCode == 'duplicate') {
      ...
      var err = buildError(dataset, datasetId);
      callback(err);
    }
  };

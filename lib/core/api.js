...
api.user.getUploadGroups = function(cb) {
  ...
  tidepool.getViewableUsers(userId, function(err, groups) {
    if (err) {
      return cb(err);
    }
    ...
    common.step([
      function(next) {
        ...
        for(var id in groups) {
          ...
          if(group.upload || group.root) {
            uploadGroups.push(id);
            tidepool.findProfile(id, next.parallel());
          }
        }

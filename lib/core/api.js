...
function initializeUploadSession(details, cb) {
  //initialise our upload
  api.log('GET /data/session/'+details.deviceId);
  return tidepool.startUploadSession(details.deviceId, details.startTime, details.tzName, cb);
}

/*api.upload.toPlatform = function(data, cb) {
  //off to jellyfish we go
  api.log('POST /data');
  return tidepool.uploadDeviceDataForUser(data, cb);
};*/

function postBlockToPlatform(data, callback) {
 ...
 */
api.upload.toPlatform = function(data, sessionInfo, progress, cb) {
  ...
  initializeUploadSession(sessionInfo,function(err, uploadSessionMeta){
    if(err){
      return cb(err);
    }
...
api.errors.log = function(message, error, properties) {
  api.log('POST /errors');

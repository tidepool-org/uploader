...
api.upload.isAllowed = function(v){
  tidepool.checkUploadVersions(function(err, versions){
    if(!_.isEmpty(err)){
      return false;
    }
    //comes in as `tidepool-uploader x.xxx.x`
    v = v.split(' ')[1];

    return semver.gte(v,versions.uploaderMinimum);
  });
};

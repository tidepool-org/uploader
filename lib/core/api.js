...
api.upload.toPlatform = function(data, sessionInfo, progress, groupId, cb) {
  ...
  async.waterfall([
    function(callback) {
      ...
      post_upload_meta(uploadItem, function(err, count){
        if(err){
          api.log('error saving upload metadata so bailing ', err);
          callback(err);
        }
        api.log('upload metadata saved now moving onto processing the  ');
        callback(null, uploadItem);
      });
    },

...
api.user.updateProfile = function(userId, updates, cb){
  api.user.profile(userId, function(err, profile){
    ...
    if (_.isArray(emails) && emails.length === 1 && emails[0] !== currentEmail){
      return async.series([
        function(callback){return tidepool.updateCustodialUser({username: emails[0], emails: emails}, userId, callback);},
        function(callback){return tidepool.addOrUpdateProfile(userId, newProfile, callback);},
        function(callback){return tidepool.signupStart(userId, callback);}

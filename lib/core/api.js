...
api.upload.blob = async (blob, contentType, cb) => {
  ...
  const digest = btoa(hashArray.map(b => String.fromCharCode(b)).join(''));

  const blobObject = new Blob([blob], { type: contentType });

  tidepool.uploadBlobForUser(tidepool.getUserId(), blobObject, contentType, `sha-256=${digest}`, (err, result) => {
    if (err) {
      return cb(err, null);
    }
    return cb(null, result);

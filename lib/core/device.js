...
var PrecisionNeoDriver = require('../drivers/abbott/abbottPrecisionNeo');
...
device._deviceDrivers = {
  ...
  'AbbottPrecisionNeo': PrecisionNeoDriver,
};
...
device.detect = function(driverId, options, cb) {
  ...
  if(driverManifest.mode === 'HID'){
    ...
    self._deviceInfoCache[driverId] = _.cloneDeep(devdata);
    if (!device._deviceComms[driverId].ble) {
      device._deviceComms[driverId] = new device._deviceComms[driverId](options);
    }

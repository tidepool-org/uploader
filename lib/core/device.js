...
// this is a cache for driver managers -- we need it so that what we learn in detect()
// can be used by process().
device._driverManagers = {};
...
device.detectHelper = function(driverId, devdata, options, cb) {
  // Detect can run on a loop, so don't pollute the console with logging
  console.log('dethelp options: ', options);
  options.silent = true;
  var dm = this._createDriverManager(driverId, devdata, options);
  dm.detect(driverId, cb);
};

device._createDriverManager = function(driverId, devdata, options) {
  // we maintain a cache of driver managers per device
  // if it's not already in the cache, set it up.
  if (!this._driverManagers[driverId]) {
    var drivers = {};
    drivers[driverId] = this._deviceDrivers[driverId];
    var configs = {};
    if (devdata == null) {
      throw(new Error('If deviceId is not in the cache, you must supply devdata'));
    }
    configs[driverId] = this._createDriverConfig(devdata, options);
    this._driverManagers[driverId] = driverManager(drivers, configs);
  }

  return this._driverManagers[driverId];
};

device._createDriverConfig = function(deviceInfo, options) {
  options = options || {};
  var timezone = options.timezone || this._defaultTimezone;
  var comms = options.silent ? this._silentComms : this._deviceComms;
  return {
    deviceInfo: _.clone(deviceInfo),
    deviceComms: comms[deviceInfo.driverId],
    timeutils: timeutils,
    timezone: timezone,
    api: this._api,
...
device.detect = function(driverId, options, cb) {
  ...
  this.detectUsb(driverId, function(err, devdata) {
    ...
    // self.detectHelper(driverId, function(err, ftdiDevice) {
    console.log('device.detect options: ', options);
    self.detectHelper(driverId, devdata, options, function(err, ftdiDevice) {
      if (err) {
        return cb(err);
      }
...
device.upload = function(driverId, options, cb) {
  var dm = this._createDriverManager(driverId, null, options);

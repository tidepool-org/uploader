...
device.init = (options, cb) => {
  device.log('Uploader destination:', options.uploaderDestination);
  try {
    if (options.uploaderDestination === 'platform') {
      device.log('Now using platform versions of Tandem and Omnipod drivers');
      ...
      device.log('Using Jellyfish versions of Tandem and Omnipod drivers');
      device.deviceDrivers.Tandem = require('../drivers/tandem/jellyfish/tandemDriver');
      device.deviceDrivers.InsuletOmniPod = require('../drivers/insulet/jellyfish/insuletDriver');
    }
  } catch (e) {

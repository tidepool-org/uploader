...
import driverManifests from './driverManifests.js';
import { ipcRenderer } from '../../app/utils/ipc.cjs';
import env from '../../app/utils/env.js';

import dexcomDriver from '../drivers/dexcom/dexcomDriver.js';
import oneTouchUltraMini from '../drivers/onetouch/oneTouchUltraMini.js';
import abbottPrecisionXtra from '../drivers/abbott/abbottPrecisionXtra.js';
import oneTouchUltra2 from '../drivers/onetouch/oneTouchUltra2.js';
import oneTouchVerio from '../drivers/onetouch/oneTouchVerio.js';
import oneTouchVerioIQ from '../drivers/onetouch/oneTouchVerioIQ.js';
import oneTouchVerioBLE from '../drivers/onetouch/oneTouchVerioBLE.js';
import abbottFreeStyleLite from '../drivers/abbott/abbottFreeStyleLite.js';
import abbottFreeStyleLibre from '../drivers/abbott/abbottFreeStyleLibre.js';
import abbottFreeStyleNeo from '../drivers/abbott/abbottFreeStyleNeo.js';
import bayerContourNext from '../drivers/bayer/bayerContourNext.js';
import bayerContour from '../drivers/bayer/bayerContour.js';
import medtronicDriver from '../drivers/medtronic/medtronicDriver.js';
import medtronic600Driver from '../drivers/medtronic600/medtronic600Driver.js';
import TrueMetrixDriver from '../drivers/trividia/trueMetrix.js';
import accuChekUSBDriver from '../drivers/roche/accuChekUSB.js';
import bluetoothLEDriver from '../drivers/bluetoothLE/bluetoothLEDriver.js';
import careSensDriver from '../drivers/i-sens/careSens.js';
import WeitaiUSB from '../drivers/weitai/weiTaiUSB.js';
import glucocardExpression from '../drivers/i-sens/glucocardExpression.js';
import libreViewDriver from '../drivers/abbott/libreViewDriver.js';
import reliOnPrime from '../drivers/i-sens/reliOnPrime.js';
import glucoRxDriver from '../drivers/glucorx/glucoRxDriver.js';
import agm4000Driver from '../drivers/allmedicus/agm4000.js';
import ketoMojo from '../drivers/vivachek/ketomojo.js';
import autocodeDriver from '../drivers/prodigy/autocode.js';
...
device.deviceDrivers = {
  ...
  InsuletOmniPod: null, // gets defined later
  Tandem: null, // gets defined later
...
_.forEach(_.keys(device.deviceComms), (driverId) => {
  ...
  // usbDevice and BLEDevice are ES6 classes not handled here; compare by
  // identity rather than by name, as terser drops the UsbDevice class name
  // in production builds (keep_classnames defaults to false)
  if (comm !== usbDevice && comm !== bleDevice) {
...
device.init = (options, cb) => {
  ...
  device.deviceDrivers.InsuletOmniPod = require('../drivers/insulet/insuletDriver');
  device.deviceDrivers.InsuletOmniPodJellyfish = require('../drivers/insulet/jellyfish/insuletDriver');
...
device.detectHelper = (driverId, options, cb) => {
  ...
  device.api.upload.getInfoForUser(options.targetId, (err, info) => {
    if (err) {
      return cb(err);
    }
    device.uploaderDestination = info.uploaderDestination;
    device.log('Uploader destination:', device.uploaderDestination);

    const dm = device.createDriverManager(driverId, options);
    if (dm == null) {
      cb(new Error('Driver not available:', driverId));
    } else {
...
device.createDriverManager = (driverId, options) => {
  ...
  if (device.uploaderDestination == null) {
    throw new Error('Could not determine upload destination');
  }

  drivers[driverId] = device.deviceDrivers[driverId];

  if (device.uploaderDestination === 'jellyfish') {
    if (driverId === 'Tandem') {
      // eslint-disable-next-line no-param-reassign
      drivers[driverId] = device.deviceDrivers.TandemJellyfish;
      device.log('Using Jellyfish version of Tandem driver');
    }
    if (driverId === 'InsuletOmniPod') {
      // eslint-disable-next-line no-param-reassign
      drivers[driverId] = device.deviceDrivers.InsuletOmniPodJellyfish;
      device.log('Using Jellyfish version of Omnipod driver');
    }
  }
...
device.detect = (driverId, options, cb) => {
  ...
  if (options.hidDevice) {
    ...
    (async () => {
      ...
      if (driverId === 'InsuletOmniPod' && webUSBDevice.vendorId === 3725 && webUSBDevice.productId === 8200) {
        // This is an attempt to upload an Omnipod 5, which is not yet supported
        return cb('E_OP5_UNSUPPORTED');
      }

...
device.driverManifests = {
  ...
  Dexcom: {
    mode: "serial",
    usb: [{ vendorId: 8867, productId: 71, driver: "cdc-acm" }],
  },
  AbbottPrecisionXtra: {
    mode: "serial",
    bitrate: 19200,
    usb: [{ vendorId: 6753, productId: 13344, driver: "tusb3410" }],
  },
  Tandem: {
    mode: "serial",
    bitrate: 921600,
    sendTimeout: 50,
    receiveTimeout: 50,
    usb: [{ vendorId: 1155, productId: 22336, driver: "cdc-acm" }],
  },
  AbbottFreeStyleLite: {
    mode: "serial",
    bitrate: 19200,
  ...
  Animas: {
    ...
    sendTimeout: 500,
    receiveTimeout: 500,
    usb: [{ vendorId: 1659, productId: 8963, driver: "pl2303" }],
  },
  OneTouchVerio: {
    mode: "usb",
    usb: [
  ...
  OneTouchVerioIQ: {
    mode: "serial",
    bitrate: 38400,
    usb: [{ vendorId: 4292, productId: 34215, driver: "cp2102" }],
  },
  OneTouchUltraMini: {
    mode: "serial",
    usb: [
  ...
  CareSens: {
    mode: "HID",
    usb: [{ vendorId: 4292, productId: 35378 }],
  },
  Weitai: {
    mode: "usb",
    bitrate: 19200,
    usb: [
      { vendorId: 1478, productId: 37152 }, // weitai limit
      { vendorId: 6353, productId: 11521 }, // weitai max Connect
    ],
  },
};
...
device.findUsbDevice = (driverId, usbDevices) => {
  ...
  const combos = _.map(usbDevices, (i) =>
    _.pick(i, "product", "vendorId", "productId")
  );

  device.log(
    "Looking for USB PID/VID(s): ",
    JSON.stringify(driverManifest.usb)
  );
  ...
  for (let i = 0; i < driverManifest.usb.length; i++) {
    device.log(
      "USB details for ",
      JSON.stringify(driverManifest.usb[i]),
      ":",
      util.inspect(
        usb.findByIds(
  ...
  const matchingUsbDevices = _.filter(usbDevices, (matching) => {
    ...
    for (let i = 0; i < driverManifest.usb.length; i++) {
      if (
        driverManifest.usb[i].vendorId === matching.vendorId &&
...
device.detectHid = (driverId, cb) =>
  cb(null, device.findUsbDevice(driverId, hid.devices()));
  ...
  (async () => {
    try {
      ...
      serialDevices = _.filter(serialDevices, (matching) => {
        ...
        for (let i = 0; i < driverManifest.usb.length; i++) {
          if (
            driverManifest.usb[i].vendorId === vendorId &&
      ...
      if (!devdata) {
        ...
        return device.detectUsb(driverId, (error, devdata2) => {
          ...
          if (devdata2.userSpaceDriver) {
            return device.detectHelper(
              driverId,
              options,
              (error2, userspaceDevice) => {
                if (error2) {
                  return cb(err);
                }

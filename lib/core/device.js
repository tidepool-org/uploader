...
device.detectHelper = function(driverId, devdata, cb) {
  // Detect can run on a loop, so don't pollute the console with logging
  var dm = this._createDriverManager(driverId, devdata, {silent: true});
...
device.detect = function(driverId, cb) {
  ...
  this.detectUsb(driverId, function(err, devdata) {
    ...
    self.detectHelper(driverId, devdata, function(err, ftdiDevice) {
      if (err) {
        return cb(err);
      }

    ...
    process: function (driver, cb) {
      ...
      function fetchData(data, next){
        ...
        api.getTime(function (err, result) {
          ...
          if (data.currentDeviceTime != null) {
            debug('Current device time:', data.currentDeviceTime);
            var deviceTime = sundial.applyTimezone(data.currentDeviceTime, configs[driver].timezone);

            debug('UTC device time:', deviceTime);

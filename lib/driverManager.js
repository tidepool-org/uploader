  ...
  return {
    ...
    process: function (driver, cb) {
      ...
      function fetchData(data, next){
        try{
          drvr.fetchData(stat.progressForStep(3), data, function(err, recs) {
            console.log('Records:', recs); // TODO: remove this
            console.log('Pages:', recs.pages.length); // TODO: remove this
              ...
              */
              ...
              configs[driver].api.upload.blob(jsonDataBinary, CONTENT_TYPE, function (err, result) {
                ...
                if (result && result.id) {
                  ...
                  recs.blobId = result.id;
                }

                console.timeEnd('upload blob elapsed');
                return next(err, recs);
              });
            } else {
              return next(err, recs);
            }

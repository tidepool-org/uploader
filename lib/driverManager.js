...
module.exports = function (driverObjects, configs) {
  ...
  return {
    ...
    process: function (driver, cb) {
      ...
      // records the data flowing into a step in lastData without adding a
      // try/catch, so debug mode keeps its good stack traces
      function trackData(fn) {
      ...
      if (configs.debug === true) {
        ...
        async.waterfall([
          ...
          trackData(drvr.connect.bind(drvr, stat.progressForStep(1))),
          trackData(drvr.getConfigInfo.bind(drvr, stat.progressForStep(2))),
          trackData(drvr.fetchData.bind(drvr, stat.progressForStep(3))),
          uploadBlob,
          trackData(drvr.processData.bind(drvr, stat.progressForStep(4))),
          trackData(drvr.uploadData.bind(drvr, stat.progressForStep(5))),
          trackData(drvr.disconnect.bind(drvr, stat.progressForStep(6)))
        ], function(err, result) {
          result = result || {};

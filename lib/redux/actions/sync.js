...
export function uploadRequest(device) {
  return {
    ...
    meta: {
      ...
      metric: {
        ...
        properties: {
          type: _.get(device, 'source.type', 'Could not find upload type'),
          source: _.get(device, 'source.driverId', 'Could not find upload driverId')
        }
      }
    }
  };
...
export function uploadStart(userId, deviceKey, upload) {
  return {

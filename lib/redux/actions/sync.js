...
import { addInfoToError, errorText, getLoginErrorMessage, getLogoutErrorMessage, UnsupportedError } from '../utils/errors';
...
export function fetchCareLinkSuccess() {
  return {
...
export function uploadSuccess(userId, device, upload, data, utc) {
  ...
  return {
    ...
    meta: {
      ...
      metric: {
        eventName: `${metrics.UPLOAD_SUCCESS} ${getUploadTrackingId(device)}`,
        properties: {
          type: _.get(device, 'source.type', undefined),
          source: _.get(device, 'source.driverId', undefined),

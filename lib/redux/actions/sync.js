...
export function uploadFailure(err, device, utc) {
  // allow to pass in for testing
  const time = `UTC Time: ${_.isEmpty(utc) ? sundial.utcDateString() : utc}`;
  if (!_.isEmpty(err.debug)) {
    err.debug = [time, err.debug].join(' | ');
  }
  else {
    err.debug = time;
  }

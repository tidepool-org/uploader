...
export function doLogin(creds, opts) {
  return (dispatch, getState) => {
    ...
    async.series([
      ...
      if(actionUtils.userHasRole(results[0].user, 'clinic')){
        return dispatch(syncActions.setPage(pages.CLINIC_USER_SELECT, actionSources.USER));
      }
      ...
      api.user.updateProfile(uploadTargetUser, profile, (err, profile) => {
        if (err) {
          ...
          async.series([
            ...
            dispatch(setUploadTargetUserAndMaybeRedirect(uploadTargetUser));
          });
        }
      });
    }
...
export function createCustodialAccount(profile) {
  return (dispatch, getState) => {
    ...
    api.user.createCustodialAccount(profile, (err, account) => {
      if (err) {
        dispatch(syncActions.createCustodialAccountFailure());
      }
      else {
        ...
        async.series([
          ...
          dispatch(syncActions.setPage(pages.SETTINGS, actionSources.USER));
        });
      }
    });
  };
...
export function setUploadTargetUserAndMaybeRedirect(targetId) {
  return (dispatch, getState) => {
    ...
    if (_.isEmpty(targetedDevices) || !atLeastOneDeviceSupportedOnSystem) {
      ...
      if (actionUtils.userHasRole(_.get(allUsers, loggedInUser), 'clinic')) {
        return dispatch(syncActions.setPage(pages.MAIN));
      }
    }
  };
...
export function clickAddNewUser(){
  return (dispatch, getState) =>{
    ...
    dispatch(syncActions.setPage(pages.CLINIC_USER_EDIT));
  };
}

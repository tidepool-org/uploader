      ...
      async.series([
        ...
        if (uploadTargetUser !== null) {
          dispatch(syncActions.setBlipViewDataUrl(
            api.makeBlipUrl(viewDataPathForUser(uploadTargetUser))
          ));
        }
...
function makeUploadCb(dispatch, getState, errCode, utc) {
  return (err, recs) => {
    const { devices, uploads, uploadTargetUser, version } = getState();
    ...
    const currentUpload = _.get(uploads, [uploadTargetUser, targetDevice.key], {});
    ...
    api.upload.getVersions((err, versions) => {
      ...
      try {
        ...
        if (!upToDate) {
          ...
          const { devices, uploads, uploadTargetUser } = getState();
...
export function putTargetsInStorage() {
  return (dispatch, getState) => {
    ...
    const uploadsByUser = getUploadsByUser(usersWithTargets);

...
export function putTargetsInStorage() {
  return (dispatch, getState) => {
    ...
    //);
    /*
    //TODO: only check against target devices here
    ...
    console.log('uploadsByUser: ',uploadsByUser);
...
export function retrieveTargetsFromStorage() {
  return (dispatch, getState) => {
    const { devices, uploadTargetUser, targetDevices, targetTimezones } = getState();
    ...
    if (targets[uploadTargetUser] != null) {
      ...
      const targetDeviceKeys = userTargets;//_.pluck(userTargets, 'key');
      ...
      */
      ...
      dispatch(syncActions.setUploads({[uploadTargetUser]: userTargets}));

      //TODO edit timezone check here
    ...
    dispatch(syncActions.retrieveUsersTargetsFromStorage());
    targets = localStore.getItem('devices');
    if (targets === null && _.isEmpty(targetDevices)) {
      return dispatch(syncActions.setPage(pages.SETTINGS));
    }
    else {
      if(targets === null) {

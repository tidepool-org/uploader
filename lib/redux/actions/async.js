...
export function doVersionCheck() {
  return (dispatch, getState) => {
    ...
    api.upload.getVersions((err, versions) => {
      ...
      if (!semver.gte(version, uploaderMinimum)) {
...
export function retrieveTargetsFromStorage() {
  return (dispatch, getState) => {
    ...
    if (targets[users.uploadTargetUser] != null) {
      const viewDataUrl = api.makeBlipUrl(`/patients/${users.uploadTargetUser}/data`);
      dispatch(syncActions.setBlipViewDataUrl(viewDataUrl));
...
export function setUploadTargetUserAndMaybeRedirect(targetId) {
  return (dispatch, getState) => {
    ...
    const { api } = services;
    const viewDataUrl = api.makeBlipUrl(`/patients/${targetId}/data`);
    dispatch(syncActions.setBlipViewDataUrl(viewDataUrl));

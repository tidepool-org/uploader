...
export function doUpload(deviceKey, opts) {
  return function (dispatch, getState) {
    ...
    dispatch(syncActions.uploadRequest(devices[deviceKey]));

    const uploadTargetUser = users.uploadTargetUser;
    let upload = uploads[uploadTargetUser][deviceKey];
    upload = {
      progress: {
        // allow to pass in start time for testing
        start: opts.start || sundial.utcDateString(),
        step: steps.START,
        percentage: 0
      }
    };
    dispatch(syncActions.uploadStart(uploadTargetUser, deviceKey, upload));
  };
}

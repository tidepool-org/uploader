...
export function targetDevices(state = {}, action) {
  switch (action.type) {
    ...
    case actionTypes.SET_USER_INFO_FROM_TOKEN: {
      ...
      _.each(memberships, (membership) => {
        if (isPwd(membership)) {
          const membershipTargetDevices = _.get(membership, ['profile', 'patient', 'targetDevices'], []);
          if(_.isEmpty(state[membership.userid])){
            newState[membership.userid] = membershipTargetDevices;
          } else {
            if(_.isEmpty(membershipTargetDevices)){
              newState[membership.userid] = state[membership.userid];
            } else {
              newState[membership.userid] = membershipTargetDevices;
            }
          }
...
export function targetTimezones(state = {}, action) {
  switch (action.type) {
    ...
    case actionTypes.SET_USER_INFO_FROM_TOKEN:
      ...
      _.each(memberships, (membership) => {
        if (isPwd(membership)) {
          const membershipTargetTimezone = _.get(membership, ['profile', 'patient', 'targetTimezone'], null);
          if(_.isEmpty(state[membership.userid])){
            newState[membership.userid] = membershipTargetTimezone;
          } else {
            if(membershipTargetTimezone === null){
              newState[membership.userid] = state[membership.userid];
            } else {
              newState[membership.userid] = membershipTargetTimezone;
            }

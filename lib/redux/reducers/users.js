...
export function targetDevices(state = {}, action) {
  console.log('targetDevices reducer',action);
  switch (action.type) {
    case REHYDRATE:
      console.log('REHYDRATE', action);
      var incoming = action.payload.targetDevices;
    ...
    case actionTypes.SET_USER_INFO_FROM_TOKEN: {
      ...
      let newState = {};
      _.each(memberships, (membership) => {
        console.log(state[membership.userid]);
        if (isPwd(membership)) {
...
export function targetTimezones(state = {}, action) {
  switch (action.type) {
    ...
    case REHYDRATE:
      ...
      var incoming = action.payload.targetTimezones;
      if (incoming) return {...state, ...incoming};
      return state;
    default:
      return state;

...
exports.make = function(config){
  ...
  return {
    ...
    basalTemp: function(){
      ...
      if (currBasal != null) {
        if (currBasal.annotations &&
          currBasal.annotations[0].code === 'basal/fabricated-from-suppressed') {
    ...
    /**
     * Gets ready to resume basal and bolus delivery from an LGS suspend.
     * (Mainly just pushes basal clock forward and creates a `nextBasal` in case no other resume event is found)
     *
     * @param argument... Variable number of arguments, each should be an object.  The field:value pairs passed in
     *                    are smooshed together into a suspend deviceMeta event recorded by the simulator
     */
    ...
    suspend: function(){
      ...
      // a user has a long temp basal scheduled, and the treshold suspend interrupts it
      // but the user cancels the threshold suspend
      // then the threshold suspend fires again within the duration of the original temp
      // in this case, there are no scheduled basal events to force emission of the nextBasal event
      // fabricated in simulator.resume, so we have to catch this event on the next threshold suspend
      // and emit it then
      if (currBasal != null && currBasal.previous && currBasal.previous.deliveryType === 'suspend') {
        ...
        else if (currBasal.deliveryType === 'scheduled') {
          addEvent(bob.makeScheduledBasal(), currBasal);
        }
      }

...
function computeMillisInCurrentDay(e){
  var dateTime = sundial.parse(e.time).zone(-e.timezoneOffset);
          ...
          case 'scheduled':
            ...
            if (currSettings == null) {
              ...
              if (currSchedule == null || currSchedule.length === 0) {
                ...
                var newTs = sundial.parse(doneTime).subtract(millisInDay, 'ms');
                if (i === currSchedule.length) {
                  i = 0;
                  newTs.add(twentyFourHours, 'ms');
                } else {
                  newTs.add(currSchedule[i].start, 'ms');
                }
  ...
  return {
    ...
    settings: function(){
      ...
      if (currBasal == null && config.autoGenScheduleds) {
        ...
        var basal = bob.makeScheduledBasal()

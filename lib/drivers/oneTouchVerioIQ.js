...
module.exports = function (config) {
  ...
  var readRecordNumber = function(n) {
    return {
      ...
      parser: function (result) {
        return struct.unpack(result.payload, 0, '..is', ['timestamp', 'glucose']);
      }
    };
  };
  ...
  var processReadings = function(readings) {
    _.each(readings, function(reading, index) {
      var dateTime = (reading.timestamp * 1000) + EPOCH; // date/time is a Unix timestamp
      var parsed = sundial.applyTimezone(dateTime); // for other devices, sundial.parseFromFormat applies timezone
      readings[index].displayTime = sundial.formatDeviceTime(new Date(parsed).toISOString());

...
module.exports = function (config) {
  ...
  var buildReadRecordNumber = function(recnum) {
    var cmd = [0x04, 0x21];  // the two 0s are filled with the rec #
    struct.pack(cmd, 2, 's', recnum);
    return buildPacket(LINK_CTRL_MASK.NONE, cmd.length, cmd);
  };

  var buildGetRecordCount = function() {
    var cmd = [0x04, 0x27, 0x00];  // the two 0s are filled with the rec #
    return buildPacket(LINK_CTRL_MASK.NONE, cmd.length, cmd);  // magic number that means 'tell me how many records you have'
  };
  ...
  var readHeader = function() {
    return {
      ...
      parser: function (result) {
        ...
        var model = String.fromCharCode.apply(null, result.payload.subarray(2));
        return { model: model};
      }
    };
  };
  ...
  var processReadings = function(readings) {
    _.each(readings, function(reading, index) {
      readings[index].displayTime = sundial.formatDeviceTime(new Date(reading.timestamp * 1000).toISOString());
  ...
  var probe = function (cb) {
    debug('attempting probe of oneTouch Mini');
      var cmd = readHeader();
      oneTouchCommandResponse(cmd, function (err, result) {
        if (err) {
          debug('Failure trying to talk to device.');

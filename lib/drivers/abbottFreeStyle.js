...
module.exports = function (config) {
  ...
  function parseDateTime(match, offset) {
    ...
    var utc = sundial.parseAndApplyTimezone(match[0], fmt, cfg.timezone).toISOString();
    var dev = sundial.formatDeviceTime(utc);
    return { dev: dev, utc: utc };
  }
  ...
  var extractPacket = function (bytes) {
    ...
    while (startLine < endIndex) {
      ...
      // var lineArray = bytes.subarray(startLine, endLine);
  ...
  return {
    ...
    uploadData: function (progress, data, cb) {
      ...
      cfg.api.upload.toPlatform(data.post_records, sessionInfo, progress, function (err, result) {
        if (err) {
          console.log(err);
          console.log(result);
          progress(100);

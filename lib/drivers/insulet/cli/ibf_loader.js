 ...
 */
function processInsulet(driverMgr, saveFile) {
  driverMgr.process('Insulet', (err, result) => {
    ...
    if (saveFile) {
      const buf = new Uint8Array(
        Buffer.from(JSON.stringify(result.post_records, null, 4)),
      );
      console.log('Saving POST records to disk');
      fs.writeFile(saveFile, buf, error => {
        if (error) {
 ...
 */
function loadFile(filePath, tz, userid, saveFile) {
  ...
  fs.readFile(filePath, (error, compressed) => {
    ...
    processInsulet(driverManager(drivers, cfg), saveFile);
  });
}
...
program
  ...
  .option('-s, --save [filename]', 'save POST records to disk')
  .parse(process.argv);
...
if (program.file && program.username && program.password) {
  if (fs.existsSync(program.file)) {
    login(program.username, program.password, config, (err, data) => {
      ...
      loadFile(program.file, program.timezone, data.userid, program.save);
    });
  } else {

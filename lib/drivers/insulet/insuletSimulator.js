...
import simulations from '../../commonFunctions.js';
    ...
    getEvents: () => {
      function filterOutZeroBoluses() {
        return _.filter(events, (event) => {
          ...
          delete event.index;
          if (event.type === 'bolus') {
            // there are instances where we can't combine a bolus that was split over
            // midnight, for example where an extended bolus ran over midnight and just
            // before midnight a normal bolus was also delivered, so make sure all
            // carryover tags are removed before upload. Split boluses will still be
            // annotated correctly with insulet/bolus/split-extended
            delete event.carryover;
            delete event.aftercarryover;
      ...
      orderedEvents.forEach((item) => {
        // there are edge cases where the PDM reports a basal or a bolus with
        // an unusually long duration, so we set the duration to zero and
        // annotate as unknown duration
        if (item.type === 'basal' && item.duration > 604800000) {
          item.duration = 0;
          annotate.annotateEvent(item, 'basal/unknown-duration');
        }

        if (item.type === 'bolus' && item.duration > 86400000) {
          item.duration = 0;
          annotate.annotateEvent(item, 'bolus/unknown-duration');

          if (item.expectedDuration > 86400000) {
            _.set(item, 'payload.expectedDuration', item.expectedDuration);
            delete item.expectedDuration;
          }
        }

        if (item.type === 'wizard' && item.bolus.duration > 86400000) {
          item.bolus.duration = 0;
          annotate.annotateEvent(item, 'bolus/unknown-duration');

          if (item.bolus.expectedDuration > 86400000) {
            _.set(item, 'payload.bolus.expectedDuration', item.bolus.expectedDuration);

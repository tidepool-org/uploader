...
exports.make = function(config){
  ...
  function fillInSuppressed(e) {
    ...
    if(e.isAssigned('scheduleName')) {
      schedName = e.scheduleName;
    } else if (currPumpSettings != null) {
      schedName = currPumpSettings.activeSchedule;
    }
  ...
  return {
      ...
      if (currBasal != null) {
        ...
        if (currStatus != null && currStatus.status === 'suspended') {
          if (activationStatus != null && activationStatus.status === 'resumed') {
            var resume = activationStatus.with_reason({resumed: 'manual'})
              .done();
    ...
    changeReservoir: function(event) {
      // TODO: re-enable when status events are linked again
      /*

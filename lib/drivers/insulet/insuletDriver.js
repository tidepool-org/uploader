  ...
  const linkWizardRecords = (data, bolusrecs) => {
    ...
    for (let b = 0; b < bolusrecs.length; ++b) {
      ...
      if (bolus.detail.extended_duration_msec !== null ||
          (bolus.hours === 0 && bolus.detail.immediate_duration_msec === 0)) {
        // the extended portion of a dual-wave bolus is split into two records
        // if it crosses local (deviceTime) midnight, and for DASH PDMs
        // the duration of the extended portion after midnight can be null, so
        // we also check the immediate portion's duration at midnight
        if (r.extended != null) {

...
import annotate from '../../eventAnnotations.js';
import common from './common.js';
import commonFunctions from '../../commonFunctions.js';
import logic from './objectBuildingLogic.js';
import insuletSimulatorMaker from './insuletSimulator.js';
import TZOUtil from '../../TimezoneOffsetUtil.js';
import is from '../../../app/utils/env.js';
...
module.exports = (config) => {
  ...
  const logRecords = {
    ...
    0x0004: {
      ...
      postprocess: (rec) => {
        if (rec.volume > 0xFFFF0000) {
          // this is not documented, but apparently there can be special values for the volume,
          // similar to special values for the extended duration below
          rec.volume_units = 0;
        } else {
          rec.volume_units = toUnits(rec.volume);
        }
        if (rec.extended_duration_minutes > 65530) {
          ...
          // We initially thought it would only be 65535 (0xFFFF), but sometimes
          // 65533 is used as well, so we use above 65530
    ...
    0x0040: {
      ...
      postprocess: (rec) => {
        ...
        if (rec.carb_grams > 65520) {
          // if carb count was not given to the wizard, don't report it.
          // We initially thought it would only be 65535 (0xFFFF), but sometimes
          // 65526 is used as well, so we use above 65520
          rec.carb_grams = null;
        }
  ...
  const buildAlarmRecords = (data, records) => {
    ...
    function makeSuspended(anAlarm) {
      ...
      if (anAlarm.detail && anAlarm.log_index) {
        const suspend = cfg.builder.makeDeviceEventSuspendResume()
        ...
        if (wizRecords[wiz_idx].isDual) {
          ...
          if (square2 != null) {
            ...
            if (millisInDay <= 5000 && !data.log_records[wiz_idx]) {
              postbolus.aftercarryover = true; // this part happened after midnight
      ...
      if (postbolus) {
        ...
        if (wizRecords[wiz_idx]) {
          ...
          if (!_.isEmpty(wiz) && !_.isEmpty(wiz.detail)) {
            ...
            postwiz = cfg.builder.makeWizard()
              .with_recommended({
        ...
        if (postwiz == null) {
          if (
            hasFlag(LOG_FLAGS.CARRY_OVER_FLAG, bolus.flags)
            && !hasFlag(LOG_FLAGS.NEW_DAY_FLAG, bolus.flags) // Eros also has carry over flag on second part
          ) {
            postbolus.carryover = true;
          }
  ...
  const buildSuspendRecords = (data, records) => {
    ...
    for (let s = 0; s < suspendrecs.length; ++s) {
      ...
      postsuspend = cfg.builder.makeDeviceEventSuspendResume()
      ...
      // we don't call .done() on the suspend or basal b/c it still need
      // duration added which happens in the simulator
  ...
  const buildBGRecords = (data, records) => {
    ...
    for (let i = 0; i < bgrecs.length; ++i) {
      ...
      if (value > 65530) {
        debug('Ignoring BG record with special value', value);
        continue;
      }
  ...
  const buildSettingsRecord = (data, records) => {
    ...
    const starts = _.sortBy(Object.keys(bgSettingsByStart), (start) => parseInt(start, 10));
    ...
    setup(deviceInfo, progress, cb) {
      ...
      (async () => {
        try {
          ...
          if (err.message.includes('EPERM: operation not permitted, scandir') && os.platform() === 'darwin') {
            err.code = 'E_OMNIPOD_WRITE';
          }
          return cb(err);
        }
      })();

      ...
      if (wizRecords[wiz_idx]) {
        ...
        if (wizRecords[wiz_idx].isDual) {
          ...
          if (ext2 != null) {
            ...
            if (data.log_records[ext].detail.extended_duration_msec === null && data.log_records[ext].detail.volume_units === 0) {
              // extended bolus was cancelled before anything was delivered
              postbolus = postbolus.with_extended(0)
                .with_duration(0);
            } else {
      ...
      (async () => {
        try {
          ...
          if (process.platform !== 'win32') {
            ...
            const filtered = _.filter(list, (obj) => _.endsWith(obj.filename, '.ibf'));
            const fileObj = _.maxBy(filtered, 'id');
            debug('Retrieving', fileObj);
            mtp.getFile(fileObj.id, tempFile);

  ...
  const linkWizardRecords = (data, bolusrecs) => {
    ...
    for (let b = 0; b < bolusrecs.length; ++b) {
      ...
      if (isDash && hasFlag(LOG_FLAGS.CARRY_OVER_FLAG, bolus.flags)) {
        ...
        if (sugg_calc && sugg_calc.hours > 0) {
          // wizard record should be at midnight, but isn't, so we have
          // to assume that the offset was not calculated correctly by the PDM
  ...
  const buildBolusRecords = (data, records) => {
    ...
    for (let b = 0; b < bolusrecs.length; ++b) {
      ...
      if (wizRecords[wiz_idx]) {
        if (wizRecords[wiz_idx].immediate !== bolusrecs[b] &&
          wizRecords[wiz_idx].extended !== bolusrecs[b] &&
          wizRecords[wiz_idx].extended2 !== bolusrecs[b]) {
          debug(`Wizard record offset provided by PDM is incorrect: Bolus ${bolusrecs[b]} at ${bolus.deviceTime} not found in wizard record ${wiz_idx} linked to ${JSON.stringify(wizRecords[wiz_idx])}`);
          if (rollbar) {
            rollbar.info('Wizard record offset provided by PDM is incorrect');
          }
          continue;
        }
    ...
    setup(deviceInfo, progress, cb) {
      ...
      (async () => {
        try {
          ...
          const WebMTP = require('webmtp');
          const mtp = new WebMTP(deviceInfo.vendorId, deviceInfo.productId);

      ...
      drivelist.list(function(err, drives) {
        ...
        if (filePath) {
          ...
          if (process.platform != 'win32') {
            ...
            var appFolder = path.dirname(app.getAppPath());
            var tempPath = path.join(appFolder, 'temp/');

            if (isDev) {
              var rootDir = path.resolve(appFolder, '../../../../../../');
            ...
            // TODO: only get newest IBF file
            var loopStr = 'foreach($Item in $sourceFolder.Items() | ?{$_.Type -match "(IBF File)$"}) { \
                $destFolder.CopyHere($Item); \
                Write-Host "Copied $($item.Name)" \
            }';

            ps.addCommand(loopStr);

...
module.exports = function (config) {
  ...
  var logRecords = {
    ...
    0x0004: { value: 0x0004, name: 'Bolus', format: 'ishs', fields: [
      'volume',
  ...
  var decodeSerial = function(encoded) {
    var phoneFamily = (encoded & 0x7E000000) >> 25;
    var year = (encoded & 0x01E00000) >> 21;
    var reset = (encoded & 0x001E0000) >> 17;
    var sequence = (encoded & 0x1FFFC) >> 2;
    var podBits = (encoded & 0x03);

    var serialNumber = _.padStart(phoneFamily.toString(), 2, '0') +
  ...
  var getProfiles = function (offset, basalProgramNames) {
    ...
    for (var i = 0; i < 11; ++i) {
      ...
      for (var j = 0; j < 48; ++j) {
        var val = struct.extractInt(profile.rawdata, step_offset + j * 4);
        if (isDash) {
  ...
  var buildAlarmRecords = function (data, records) {
    ...
    function makeSuspended(anAlarm) {
      ...
      if (anAlarm.detail && anAlarm.log_index) {
        var suspend = cfg.builder.makeDeviceEventSuspendResume()
      ...
      if (basal.detail.duration === 0) {
        ...
        if (basal.detail.temp_basal_percent != null) {
          var suppressed = {
            ...
            rate: common.fixFloatingPoint(basal.detail.basal_rate_units_per_hour/basal.detail.temp_basal_percent, 2)
          };
  ...
  var buildSuspendRecords = function (data, records) {
    ...
    for (var s = 0; s < suspendrecs.length; ++s) {
      ...
      // we don't call .done() on the suspend or basal b/c they still need
  ...
  var buildTimeChangeRecords = function (data, records, settings) {
    ...
    var tzoUtil = new TZOUtil(cfg.timezone, settings.time, _.filter(postrecords, function(rec) {
      ...
      return rec.change.from.slice(0,4) > '2015';
    }));
    cfg.tzoUtil = tzoUtil;
  ...
  return {
    setup: function (deviceInfo, progress, cb) {
      ...
      drivelist.list(function(err, drives) {
        ...
        drives.forEach((drive) => {
          ...
          if (drive.description && drive.description.toLowerCase().includes('omnipod pdm') && !drive.system) {
            if (drive.mountpoints.length > 0) {
              var devicePath = drive.mountpoints[0].path;
              var files = _.filter(fs.readdirSync(devicePath), function (file) {
                var regex = /(.+\.ibf)/g;
        ...
        if (filePath) {
          ...
          fs.readFile(filePath, function(err2, res) {
            if (err2) {
              return cb(err2, null);
            } else {
              buf = res.buffer;
          ...
          if (process.platform != 'win32') {
            ...
            import('node-mtp').then(function (mtp) {
              try {
                mtp.attach();
                list = mtp.getFileListing();
                console.log('Files:', list);
                tempFile = path.join(require('electron').remote.app.getPath('temp'), 'temp.ibf');
            ...
            return cb(new Error('No filedata available!'), null);
          }
        }
      });
    },
    ...
    uploadData: function (progress, data, cb) {
      ...
      var insuletDeviceId = data.ibf_version.vendorid.slice(0,3) + data.ibf_version.productid.slice(0,3) + '-' + data.eeprom_settings.serial_number;
      ...
      var sessionInfo = {
        ...
        deviceSerialNumber: data.eeprom_settings.serial_number,
      ...
      if(!isDash) {
        ...
        sessionInfo.deviceManufacturers.push('Abbott');
      }

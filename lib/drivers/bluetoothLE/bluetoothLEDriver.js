...
import uploadDataPeriod from '../../../app/utils/uploadDataPeriod.js';
import api from '../../core/api.js';
import getModelName from '../roche/models.js';
       ...
       if (cfg.deviceInfo.name.startsWith('CareSens') ||
           ...
           cfg.deviceInfo.name.startsWith('ReliOn 0015')) {
            ...
            (async () => {
              ...
              handleTimeSync = (event) => {
                ...
                common.checkDeviceTime(cfg, async (timeErr, serverTime) => {
                  if (timeErr) {
                    ...
                    struct.pack(bytes, 0, 'bbbbsbbbbb',
                      0xC0, 0x03, 0x01, 0x00,
              ...
              const buf = new ArrayBuffer(11);
              const bytes = new Uint8Array(buf);
              struct.pack(bytes, 0, 'bbbbsbbbbb', 0xC0, 0x03, 0x01, 0x00, 0, 0, 0, 0, 0, 0); // get time

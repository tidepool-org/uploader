       ...
       if (cfg.deviceInfo.name.startsWith('CareSens') || 
           cfg.deviceInfo.name.startsWith('ReliOn 2395')) {
            ...
            (async () => {
              ...
              handleTimeSync = (result) => {
                debug('Time sync result:', result);
              };
              cfg.deviceComms.ble.addEventListener('characteristicvaluechanged', handleTimeSync);  // FIXME

              const buf = new ArrayBuffer(11);
              const bytes = new Uint8Array(buf);
              let ctr = struct.pack(bytes, 0, 'bbbbsbbbbb', 0xC0, 0x03, 0x01, 0x00, 1982, 9, 29, 17, 15, 15);

              cfg.deviceComms.ble.customCharacteristic.writeValue(bytes);

            })().catch((error) => {

              // TODO: implement v1.5 time sync if v1.4 sync fails

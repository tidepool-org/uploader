...
class AccuChekSmartPix {
  ...
  static async privateListReports() {
    return new Promise((resolve) => {
      setTimeout(async () => {
        resolve(await listDirFiles(/\.XML$/i));
      }, 5000); // wait a bit in case there are changes still bein written to disk
    });
  }
  ...
  static async read(progress) {
    ...
    if (_.includes(s.flags, 'IPREPORT') || _.includes(s.flags, 'BGREPORT')) {
      progress(99);
      const reports = await AccuChekSmartPix.privateListReports();

...
class AccuChekSmartPix {
   ...
   * @throws Error if no device could be found, no report was created, multiple new reports were found, or other error.
   */
  async read(progress) {
    ...
    if (_.includes(s.flags, 'IPREPORT')) {
      ...
      if (newReports.length !== 1) {
        ...
        return path.join(this.reportPath, newReports[0]);
      }
    } else {
  ...
  async process(fullPath, progress) {
    ...
    let data = null;
    if (result.IMPORT.IP) {
      await this.privateCheckTime(result.IMPORT.IP[0].$);
      data = parsePumpData(result);
    } else if (result.IMPORT.DEVICE) {
      await this.privateCheckTime(result.IMPORT.DEVICE[0].$);
      data = parseMeterData(result);
    } else {
      ...
      (async () => {
        try {
          if (QUICK_SINGLE_REPORT === true) {
            ...
            if (files.length === 1) {
              data.dataFile = path.join(driver.reportPath, files[0].name);
              cb(null, data);
              return;
            }

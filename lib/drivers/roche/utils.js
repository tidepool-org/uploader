...
export function getObject(bytes, type) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (type === objClass) {
      obj.bytes = new DataView(bytes.buffer, offset - length, length);
      return obj;
    }
  }
...
export function getAttributeList(bytes) {
  ...
  offset += 4;
  obj.bytes = new DataView(bytes.buffer, offset, length);
...
export function getProductionSpecEntry(bytes, entry) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (entry === type) {
      return new DataView(bytes.buffer, offset - length, length);
    }
  }
...
export function getAttribute(obj, type) {
  ...
  for (let i = 0; i < obj.attributeCount; i++) {
    ...
    if (type === attributeId) {
      return new DataView(obj.bytes.buffer, offset - length, length);
    }
  }

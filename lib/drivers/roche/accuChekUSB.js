...
class AccuChekUSB {
  ...
  async openDevice(deviceInfo, cb) {
    this.usbDevice = new UsbDevice(deviceInfo).device;
    ...
    try {
      ...
      const incoming = await this.usbDevice.transferIn(this.usbDevice.usbconfig.inEPnum, 128);
      ...
      if (error.message === 'LIBUSB_TRANSFER_TIMED_OUT') { // FIXME
  ...
  async getData(invokeId, pmStoreHandle, cb) {
    ...
    try {
      ...
      if (incoming.length === 22 && incoming.getUint16(20)) {
        const dataResponse = incoming.getUint16(20);
        if (dataResponse === 3) {
          ...
          throw new Error(`Could not retrieve data: ${DATA_RESPONSE[dataResponse]}`);
            ...
            incoming.getUint16(14) !== ACTION_TYPE.MDC_ACT_SEG_TRIG_XFER) {
  ...
  return {
    ...
    getConfigInfo(progress, data, cb) {
      ...
      (async () => {
        ...
        const modelId = getArray(getAttribute(deviceDetails, MDC_PART_OBJ.MDC_ATTR_ID_MODEL));

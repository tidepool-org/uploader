...
class AccuChekUSB {
  ...
  async getConfig(data, cb) {
    try {
      ...
      await this.transferOut(this.ep, Buffer.from(AccuChekUSB.buildActionRequest(invokeId)));
      incoming = await this.transferIn(this.ep, 1024);
  ...
  async getData(invokeId, cb) {
    let data;

    try {
      await this.transferOut(this.ep, AccuChekUSB.buildDataTransferRequest(invokeId));
      ...
      if (incoming.length === 22 && incoming.readUInt16BE(20)) {
        throw new Error(`Could not retrieve data: ${DATA_RESPONSE[incoming.readUInt16BE(20)]}`);
      }
...
module.exports = (config) => {
  ...
  return {
    ...
    async fetchData(progress, data, cb) {
      ...
      driver.getData(data.lastInvokeId, (err, result) => {
        if (err) {
          debug('Error:', err);
          return cb(err, null);
        }

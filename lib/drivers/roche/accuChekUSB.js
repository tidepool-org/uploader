...
class AccuChekUSB {
  ...
  async setTime(invokeId, pmStoreHandle, timestamp, cb) {
    await this.usbDevice.transferOut(
      this.ep,
      Buffer.from(AccuChekUSB.buildSetTimeRequest(invokeId, pmStoreHandle, timestamp)),
    );
    const incoming = await this.usbDevice.transferIn(this.ep, 1024);
    const lastInvokeId = incoming.readUInt16BE(6);
    debug('Received set time response:', _.toUpper(incoming.toString('hex')));
  ...
  static buildSetTimeRequest(invokeId, pmStoreHandle, timestamp) {
    ...
    buffer.writeUInt16BE(24, 4); // octet string length
    buffer.writeUInt16BE(invokeId + 1, 6); // invoke-id
    buffer.writeUInt16BE(DATA_ADPU.INVOKE_CONFIRMED_ACTION, 8);
    buffer.writeUInt16BE(18, 10); // length
    buffer.writeUInt16BE(0, 12);
    buffer.writeUInt16BE(ACTION_TYPE.MDC_ACT_SEG_SET_TIME, 14);
    buffer.writeUInt16BE(12, 16); // length
    buffer.write(timestamp, 18, 8, 'hex'); // AbsoluteTime
    buffer.writeUIntBE(0, 26, 4); // accuracy (FLOAT-type, set to zero)

    debug('Set time request:', _.toUpper(buffer.toString('hex')));
  ...
  return {
    ...
    getConfigInfo(progress, data, cb) {
      ...
      (async () => {
        ...
        [cfg.deviceInfo.serialNumber] = parsed;
        cfg.deviceInfo.deviceId = `Roche-${cfg.deviceInfo.model}-${cfg.deviceInfo.serial}`;

        const timestamp = getAttribute(deviceDetails, MDC_PART_OBJ.MDC_ATTR_TIME_ABS);
        cfg.deviceInfo.meterTime = sundial.parseFromFormat(timestamp.toString('hex'), 'YYYYMMDDHHmm');

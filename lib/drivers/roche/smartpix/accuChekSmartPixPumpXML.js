...
function parseHeader(pumpInfo, context) {
  const profiles = {};
...
function parseIPData(ipdata, header, builder) {
  const state = {
  ...
  _.forEach(_.keys(ipdata), (indexStr) => {
    ...
    if (result !== null) {
      ...
      result = result
        ...
        .with_timezoneOffset(sundial.getOffsetFromTime(time, header.context.timezone))
 ...
 */
export default function parsePumpData(document, context, builder) {
  const theBuilder = builder || objectBuilder();

  // log(util.inspect(root.IPDATA, false, null));
  const root = document.IMPORT;
  const records = [];
  const header = parseHeader(root.IP[0], context);
  ...
  const stream = parseIPData(root.IPDATA[0].$$, header, theBuilder);

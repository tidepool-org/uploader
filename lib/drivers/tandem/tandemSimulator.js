  ...
  return {
    ...
    cbg: function(event) {
      if (currCBG && (event.value === currCBG.value)) {
        // backfill values can cause duplicates, so we have remove any dupes
        // that happen between the Five Minute Reading (FMR) intervals
        var duration = Date.parse(event.time) - Date.parse(currCBG.time);
        // FMR is approximately 5 minutes, so leave 2-minute buffer
        if (duration < (3 * sundial.MIN_TO_MSEC)) {
          return;
        }
      }

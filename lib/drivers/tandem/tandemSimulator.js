...
exports.make = function(config){
  ...
  var currBasal = null;
  var currBasalStatus = null
  var currStatus = null;
  var currTimestamp = null;
  var currTempBasal = null;
  var currCBG = null;
  var currClosedLoop = null;

  function setCurrBasal(basal) {
    currBasal = basal;
  }

  function setCurrBasalStatus(status) {
    currBasalStatus = status;
  }
  ...
  function setCurrClosedLoop(status) {
    currClosedLoop = status;
  }
  ...
  return {
    ...
    basalStatus: function(event) {
      ensureTimestamp(event);
      setCurrBasalStatus(event);
    },
    closedLoop: function(event) {
      ensureTimestamp(event);
      setCurrClosedLoop(event);
    },
    basal: function(event){
      ensureTimestamp(event);
      if (currBasal != null) {

        if (currClosedLoop && currClosedLoop.subType === 'Closed loop') {
          event.deliveryType = 'automated';
        }

        if (currBasalStatus.subType === "Temp Rate") {
          debugger;
        }
        ...
        if (currBasal.time !== event.time) {
          ...
          if (currBasalStatus && currBasal.rate === currBasalStatus.rate && currBasal.time === currBasalStatus.time) {
            currBasal.with_payload({ rateSource: currBasalStatus.subType });
          }
        ...
        if(currTempBasal && currTempBasal.subType === 'stop') {
          ...
          if (currBasalStatus && tempBasal.rate === currBasalStatus.rate && tempBasal.time === currBasalStatus.time) {
            tempBasal.payload.rateSource = currBasalStatus.subType;
          }

...
module.exports = function (config) {
  ...
  var RESPONSES = {
    ...
    AA_INFO_TE: {
      value: 231,
      name: 'Control-IQ Information',
      version: 105900,
      format: 'bbbsb32Z',
      fields: ['closed_loop_enabled', 'hypo_light_alert_enabled', 'total_daily_insulin', 'weight', 'weight_units', 'sleep_schedule']
    },
    MAX_BOLUS_TE: {
      value: 246,
      name: 'Max Bolus Message',
      format: 's',
      fields: ['max_bolus_setting_milliunits'],
      postprocess: function (rec) {
        rec.max_bolus_setting_units = rec.max_bolus_setting_milliunits * 0.001;
      }
    },
  };
  ...
  var COMMANDS = {
    ...
    AA_INFO_REQ: {
      value: 230,
      name: 'Control-IQ Information Request',
      version: 105900,
      response: RESPONSES.AA_INFO_TE
    },
    MAX_BOLUS_REQ: {
      value: 245,
      name: 'Max Bolus Request',
      response: RESPONSES.MAX_BOLUS_TE,
    },
  };
  ...
  var PUMP_LOG_RECORDS = {
    ...
    LID_BG_READING_TAKEN: {
      ...
      name: 'BG Taken Event',
      format: 'h.bfhh....',
      fields: ['bg', 'bg_source', 'iob', 'target_bg', 'isf']
    },
    LID_BOLEX_ACTIVATED: {
      value: 0x3B,
    ...
    LID_BOLUS_REQUESTED_MSG2: {
      ...
      postprocess: function (rec) {
        rec.bolus_option = BOLUS_OPTIONS[rec.options];
    ...
    LID_BASAL_DELIVERY: {
      value: 0x117,
      name: 'Basal Delivery Event',
      format: 's..ssss....',
      fields: ['commanded_rate_source', 'commanded_rate', 'profile_basal_rate', 'algorithm_rate', 'temp_rate']
    },
    LID_AA_PCM_CHANGE: {
      value: 0xE6,
      name: 'Control-IQ Pump Control Mode (PCM) Change',
      format: 'bb..............',
      fields: ['current_PCM', 'previous_PCM']
    },
    LID_AA_USER_MODE_CHANGE: {
      value: 0xE5,
      name: 'Control-IQ User Mode Change',
      format: 'bb..............',
      fields: ['current_user_mode', 'previous_user_mode']
    },
    LID_BOLUS_DELIVERY: {
      value: 0x118,
      name: 'Bolus Delivery Event',
      fields: 'sbbb.sssss',
      fields: ['bolus_id', 'bolus_delivery_status', 'bolus_type', 'bolus_source', 'requested_now', 'requested_later', 'correction', 'extended_duration_requested', 'delivered_total']
    }
  };
  ...
  var BG_READING_SOURCE = {
    MANUAL_ENTRY : { value: 0, name: 'Manual entry by the user'},
    AUTO_POPULATED : { value: 1, name: 'Auto populated BG using CGM'}
  };

  var COMMANDED_RATE_SOURCE = {
    SUSPENDED : { value: 0, name: 'Suspended' },
    PROFILE : { value: 1, name: 'Profile' },
    TEMP : { value: 2, name: 'Temp Rate' },
    ALGORITHM : { value: 3, name: 'Algorithm'}
  };

  var PUMP_CONTROL_MODE = {
    NO_CONTROL : { value: 0, name: 'No control' },
    OPEN_LOOP: { value: 1, name: 'Open loop' },
    PINING : { value: 2, name: 'Pining' },
    CLOSED_LOOP : { value: 3, name: 'Closed loop'}
  };

  var USER_MODE = {
    NORMAL : { value: 0, name: 'normal' },
    SLEEPING: { value: 1, name: 'sleep' },
    EXERCISING : { value: 2, name: 'physicalActivity' }
  };
  ...
  var INTERVAL_FREQ = 10;  // time between sending serial packets
  ...
  var RECONNECT_DELAY = 5000; // time to wait before reconnecting to pump
  var MAX_TRIES = 5;
  ...
  var getCommandRateSource = function (idx) {
    return _getName(COMMANDED_RATE_SOURCE, idx);
  };

  var getPumpControlMode = function (idx) {
    return _getName(PUMP_CONTROL_MODE, idx);
  };

  var getUserMode = function (idx) {
    return _getName(USER_MODE, idx);
  };
  ...
  var listenForPacket = function (command,args,callback) {
    ...
    }, INTERVAL_FREQ * 2); // spin on this one quickly
  };
        ...
        var processPacket = function (pkt) {
          ...
          pkt.payload['success'] === 0) {
            ...
            if (receiveSeq <= end) {
              ...
              // failed to recover remaining packets, return what we have
  ...
  var multiLogRequester = function (start, end, progress, callback) {
    ...
    var retryTimeout = function() {
        ...
        if (_.has(lastPacketRead, 'payload.deviceTime')) {
          debug('Last record read successfully has device time', lastPacketRead.payload.deviceTime);
        }

        // first we reconnect and flush to clear any connection issues
        cfg.deviceComms.clearPacketHandler();
        cfg.deviceComms.disconnect(function () {
          debug(`Waiting ${RECONNECT_DELAY/1000} seconds before reconnecting to pump...`);
          setTimeout(() => {
            cfg.deviceComms.connect(cfg.deviceInfo, tandemPacketHandler, function (err) {
              if (err) {
                callback(err,null);
              } else {
              ...
              // verify we can get a response from the pump
              tandemCommandResponse(COMMANDS.VERSION_REQ, null, function (err, result) {
                if (err) {
                  if (rollbar) {
                    rollbar.info('Unable to get response from Tandem pump on retry');
                  }
                  debug(err);
                  callback(err, null);
                }
                else {
                  debug('Tandem found: ', result);

                  // cancel outstanding multi log request just in case
                  tandemCommand(COMMANDS.LOG_ENTRY_SEQ_MULTI_STOP_DUMP, null, function (err) {
                    if(err) {
                      ...
                      if (end-receiveSeq < 128) {
                        debug('Fewer than 128 records remaining');
                        // download remaining records one at a time
                        err.name = 'multi-fail';
                      } else {
                        err.name = 'multi-retry';
                      }

                      err.startSeq = receiveSeq;
                      err.endSeq = end;
          ...
          }, RECONNECT_DELAY); // wait before re-connect
        });
    };
            ...
            var processPacket = function (pkt) {
              if (pkt.valid &&
                ...
                if (receiveSeq != pkt.payload['header_log_seq_no']) {
                  ...
                  // reset timeouts
                  clearTimeout(retryTimer);
                  clearTimeout(abortTimer); // abortTimer is set by retryTimeout
  ...
  var tandemDownloadRecords = function (progress, data, callback) {
    ...
    function iterate(err, result) {
      if (err) {
        if (err.startSeq && err.endSeq) {
          start_seq = err.startSeq;
          end_seq = err.endSeq;
        }
        ...
        if (result.payload && !result.payload.tdeps) {
          retval.push(result.payload);
        }
        if (result.payload && result.payload.header_log_seq_no === end_seq) {
          ...
          debug(`finished fetching records, missing ${end_seq - retval[retval.length-1].header_log_seq_no} records at end`);
          if (rollbar) {
            rollbar.info('Unable to retrieve last records on Tandem pump');
          }
          data.log_records = retval;
          callback(null, data);
        }
  ...
  var tandemFetchSettings = function (progress, data, callback) {
    ...
    tandemCommandResponse(COMMANDS.GLOBALS_REQ, null, function (err, pkt) {
      if (err) {
        ...
        tandemCommandResponse(COMMANDS.MAX_BOLUS_REQ, null, function (err, pkt) {
          if (err) {
            debug('Pump does not have a global max bolus setting');
          }

          if (pkt && pkt.valid && pkt.descriptor === RESPONSES.MAX_BOLUS_TE.value) {
            data.max_bolus_units = pkt.payload.max_bolus_setting_units;
          }
  ...
  var buildSettingsRecords = function buildSettingsRecord(data, postrecords) {
    ...
    data.profiles.forEach(function (profile) {
      ...
      if (data.max_bolus_units) {
        // software versions starting with 6.4 and 7.4 have a global max bolus setting,
        // per-schedule max bolus setting is invalid and should be replaced with global one
        bolusSettings[scheduleName].amountMaximum.value = data.max_bolus_units;
      }
    });
    ...
    var postsettings = cfg.builder.makeTandemPumpSettings()
      ...
      .with_firmwareVersion(cfg.deviceInfo.firmwareVersion.toString())
      ...
      if (data.controlIQ) {
        postsettings.with_automatedDelivery(data.closed_loop_enabled === 1 ? true : false);
      }
  ...
  var buildBolusRecords = function (data, records) {
    ...
    for (var key in boluses) {
      ...
      else if (bolus.bolus_option === 'automated') {
        record = cfg.builder.makeAutomatedBolus();
      }
      else {
      ...
      if((record.subType === 'normal' && bolus.insulin_delivered === undefined) ||
         ...
         (record.subType === 'square' && bolus.duration === undefined) ||
         (record.subType === 'dual/square' && bolus.duration === undefined)) {
  ...
  var buildControlModeRecords = function(data, records) {
    var controlModeRecords = filterLogEntries([PUMP_LOG_RECORDS.LID_AA_PCM_CHANGE], data.log_records);

    for(var i = 0; i < controlModeRecords.length; ++i) {
      var event = controlModeRecords[i];
      var record = {
        type: 'control-mode',
        subType: getPumpControlMode(event.current_PCM),
        previous: getPumpControlMode(event.previous_PCM),
      };
  ...
  var buildPumpSettingsOverrideRecords = function(data, records) {
    var activityRecords = filterLogEntries([PUMP_LOG_RECORDS.LID_AA_USER_MODE_CHANGE], data.log_records);

    for(var i = 0; i < activityRecords.length; ++i) {
      var event = activityRecords[i];

      var mode = getUserMode(event.current_user_mode);
      var prevMode = getUserMode(event.previous_user_mode);

      var modeChange = cfg.builder.makeDeviceEventPumpSettingsOverride()
        .with_overrideType(mode)
        .with_deviceTime(event.deviceTime)
        .set('index', event.index);

      if(prevMode !== USER_MODE.NORMAL.name) {
        modeChange.set('previousOverride', prevMode);
      }

      cfg.tzoUtil.fillInUTCInfo(modeChange, event.jsDate);
      records.push(modeChange);
    }
    return records;
  };
  ...
  var buildBGRecords = function (data, records) {
    ...
    bgRecords.forEach(function (bgEntry) {
      if (bgEntry.bg_source !== BG_READING_SOURCE.AUTO_POPULATED.value) {
  ...
  var tandemGetConfigInfo = function (cb, data) {
    tandemCommandResponse(COMMANDS.VERSION_REQ, null, function (err, result) {
      if (err) {
        ...
        data.deviceModel = result.payload.model_no.toString();
        ...
        tandemCommandResponse(COMMANDS.AA_INFO_REQ, null, function (err, pkt) {
          if (err) {
            debug('Not a Control-IQ pump');
            data.controlIQ = false;
          } else {
            debug('This is a Control-IQ pump');
            data.controlIQ = true;
            data.closed_loop_enabled = pkt.payload.closed_loop_enabled;
          }
          ...
          cfg.deviceInfo.deviceId = prefix + data.deviceModel + data.pump_sn;
          ...
          _.assign(cfg.deviceInfo, {
            ...
            serialNumber: data.pump_sn.toString(),
    ...
    uploadData: function (progress, data, cb) {
      ...
      */
      if (!debugMode.isDebug) {
        ...
        debug('Log records:', data.log_records);
      }
      ...
      if (!_.isEmpty(data.log_records)) {
        ...
        postrecords = buildControlModeRecords(data, postrecords);
        postrecords = buildPumpSettingsOverrideRecords(data, postrecords);
        ...
        return cb(new Error('No records since last upload or most recent pump shut down; nothing to upload.'));
      }

      var simulator = tandemSimulatorMaker.make({
        settings,
      ...
      if (data.controlIQ) {
        let firstControlMode = _.find(postrecords, o => o.type === 'control-mode');
        if (firstControlMode === undefined) {
          // no control mode changes found
          return cb(new Error('Not enough Control-IQ data for upload.'));
        } else {
          simulator.initControlMode(firstControlMode);
        }
      }
      ...
      for (var n = 0; n < postrecords.length; ++n) {
        ...
        switch (datum.type) {
          ...
          case 'deviceEvent':
            ...
            else if (datum.subType === 'pumpSettingsOverride') {
              simulator.pumpSettingsOverride(datum);
            }
            else {

...
module.exports = function (config) {
  ...
  var buildClosedLoopRecords = function(data, records) {
    var closedLoopRecords = filterLogEntries([PUMP_LOG_RECORDS.LID_AA_PCM_CHANGE], data.log_records);

    for(var i = 0; i < closedLoopRecords.length; ++i) {
      var event = closedLoopRecords[i];
      var record = {
        type: 'closed-loop',
  ...
  var buildBasalStatusRecords = function(data, records) {
    var basalStatusRecords = filterLogEntries([PUMP_LOG_RECORDS.LID_BASAL_DELIVERY], data.log_records);

    for(var i = 0; i < basalStatusRecords.length; ++i) {
      var event = basalStatusRecords[i];
      var record = {
        type: 'basal-status',
        subType: getCommandRateSource(event.commanded_rate_source),
      };

      console.log(record);

      switch(event.commanded_rate_source) {
        case COMMANDED_RATE_SOURCE.SUSPENDED.value:
          record.rate = 0;
          break;
        case COMMANDED_RATE_SOURCE.PROFILE.value:
          record.rate = event.profile_basal_rate;
          break;
        case COMMANDED_RATE_SOURCE.ALGORITHM.value:
          record.rate = event.algorithm_rate;
          break;
        case COMMANDED_RATE_SOURCE.TEMP.value:
          record.rate = event.temp_rate;
          break;
      }

      if (event.commanded_rate !== record.rate) {
        // TODO: figure out what exactly to do here
        console.log('Mismatch between current rate and commanded rate');
      }
      record.rate /= 1000.0; // convert from milliunits/hr to U/hr
    ...
    uploadData: function (progress, data, cb) {
      ...
      */
      ...
      if (!_.isEmpty(data.log_records)) {
        ...
        postrecords = buildBasalStatusRecords(data, postrecords);
        postrecords = buildClosedLoopRecords(data, postrecords);
      ...
      for (var n = 0; n < postrecords.length; ++n) {
        ...
        switch (datum.type) {
          case 'basal':
            ...
            break;
          case 'basal-status':
            simulator.basalStatus(datum);
            break;
          case 'closed-loop':
            simulator.closedLoop(datum);
            break;
          case 'bolus':

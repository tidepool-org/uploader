...
module.exports = function (config) {
  ...
  var USER_MODE = {
    ...
    SLEEPING: { value: 1, name: 'sleeping' },
    EXERCISING : { value: 2, name: 'exercising' }
  };
  ...
  var buildActivityRecords = function(data, records) {
    ...
    for(var i = 0; i < activityRecords.length; ++i) {
      ...
      var modeChange = cfg.builder.makeDeviceEventMode()
        .with_status(mode)
        .with_deviceTime(event.deviceTime)
        .set('index', event.index);
      cfg.tzoUtil.fillInUTCInfo(modeChange, event.jsDate);
      records.push(modeChange.done());
    }
    return records;
  };
    ...
    uploadData: function (progress, data, cb) {
      ...
      */
      ...
      if (!_.isEmpty(data.log_records)) {
        ...
        postrecords = buildActivityRecords(data, postrecords);
      ...
      for (var n = 0; n < postrecords.length; ++n) {
        ...
        switch (datum.type) {
          ...
          case 'deviceEvent':
            ...
            else if (datum.subType === 'mode') {
              simulator.mode(datum);
            }
            else {

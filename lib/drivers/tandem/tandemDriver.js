      ...
      if (err) {
        ...
        if (result.valid && (result.descriptor === RESPONSES.LOG_SIZE_TE.value)) {
          ...
          api.getMostRecentUploadRecord(cfg.groupId, cfg.deviceInfo.deviceId, function(err, lastUpload) {
            ...
            // TODO: put this back: var lastEndPosition = _.get(lastUpload, 'client.private.delta.lastEndPosition', null);
  ...
  return {
    ...
    uploadData: function (progress, data, cb) {
      ...
      */
      ...
      api.getLatestRecord(cfg.groupId, cfg.deviceInfo.deviceId, 'basal', function(err, latestRecord) {
        if (err) {
          return callback(err);
        }

        console.log("BASAL:", latestRecord);

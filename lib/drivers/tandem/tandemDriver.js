...
module.exports = function (config) {
  ...
  var getCGMTypeName = function (idx) {
    return _getName(CGM_GX_TYPE, idx);
  };
  ...
  var newestEventRequester = function (start, end, progress, callback) {
    ...
    var headerIdFilter = [
      ...
      PUMP_LOG_RECORDS.LID_CGM_DATA.value,
      PUMP_LOG_RECORDS.LID_CGM_CALIBRATION.value,
      PUMP_LOG_RECORDS.LID_CGM_DATA_GX.value,
      PUMP_LOG_RECORDS.LID_CGM_CALIBRATION_GX.value
    ];
  ...
  var buildCGMRecords = function(data, records) {
    var cgmRecords = filterLogEntries([PUMP_LOG_RECORDS.LID_CGM_DATA,PUMP_LOG_RECORDS.LID_CGM_CALIBRATION,PUMP_LOG_RECORDS.LID_CGM_DATA_GX,PUMP_LOG_RECORDS.LID_CGM_CALIBRATION_GX], data.log_records);
    for (var b = 0; b < cgmRecords.length; ++b) {
      ...
      if(event.header_id === PUMP_LOG_RECORDS.LID_CGM_CALIBRATION.value
        && value <=401
        && value >=39) { // don't include magic numbers
      ...
      } else if (event.header_id === PUMP_LOG_RECORDS.LID_CGM_CALIBRATION_GX.value) {
        ...
        if (event.header_id === PUMP_LOG_RECORDS.LID_CGM_DATA_GX.value) {
          record.with_payload({
            type: getCGMTypeName(event.type),
            rate: event.rate,
            rssi: event.rssi,
            timestamp: event.timestamp,

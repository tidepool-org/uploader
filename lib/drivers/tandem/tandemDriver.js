      ...
      postprocess: function (rec) {
        ...
        if (recordType != null) {
          ...
          if (debugMode.isDebug) {
              debug('Unknown log record: id=0x'+ rec.header_id.toString(16)+ ' record=', rec)
          }
        }
      }
    },
    ...
    tandemCommandResponse(COMMANDS.LOG_SIZE_REQ, null, function (err, result) {
      if (err) {
        ...
        if (result.valid && (result.descriptor === RESPONSES.LOG_SIZE_TE.value)) {
          ...
          var fetch = function() {
            debug('Reading from record', data.start_seq);
          ...
          // if we have api access, only read records we haven't seen yet
          if (cfg.api.tidepool) {

...
module.exports = function (config) {
  ...
  var parse_egv_g5_records = function (header, data) {
    var all = [];
    var ctr = 0;
    var format = 'iisiiybbs';
    var flen = struct.structlen(format);

    for (var i = 0; i < header.nrecs; ++i) {
      var rec = struct.unpack(data, ctr, format, [
        'systemSeconds', 'displaySeconds', 'glucose', 'systemTimeSeconds', 'transmitterTimeSeconds', 'filteredRateByte', 'trendArrowAndNoiseMode', 'algorithmStateByte', 'crc'
      ]);
      // TODO: add new G5 properties?
      // TODO: double-check that trendArrowAndNoiseMode still provides trend arrow in same format

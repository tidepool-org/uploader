...
module.exports = function (config) {
  ...
  var parse_egv_g5_records = function (header, data) {
    var all = [];
    var ctr = 0;
    var format = 'iisiiiybbss';
    var flen = struct.structlen(format);

    for (var i = 0; i < header.nrecs; ++i) {
      var rec = struct.unpack(data, ctr, format, [
        'systemSeconds', 'displaySeconds', 'glucose', 'systemTimeSeconds', 'timeStampSeconds', 'transmitterTimeSeconds', 'filteredRateByte', 'trendArrowAndNoiseMode', 'algorithmStateByte', 'trendArrow', 'crc'
      ]);
      // TODO: add new G5 properties?
      // TODO: double-check that second-last property still provides trend arrow
  ...
  var parse_meter_g5_records = function (header, data) {
    var all = [];
    var ctr = 0;
    var format = 'iisbiis';
    var flen = struct.structlen(format);
    ...
    for (var i = 0; i < header.nrecs; ++i) {
      var rec = struct.unpack(data, ctr, format, [
        'systemSeconds', 'displaySeconds', 'meterValue', 'entryType', 'meterSystemTimeSeconds', 'meterTimeSeconds', 'crc'
      ]);
  ...
  var parse_setting_g5_records = function (header, data) {
    var all = [];
    var ctr = 0;
    var format = 'iinn6Zissssssssbbis';
    var flen = struct.structlen(format);
  ...
  var readDataPages = function (rectype, startPage, numPages) {
    ...
    if (dexcomModel === 'G5MobileReceiver') {
      if (rectype == RECORD_TYPES.EGV_DATA) {
        parser = makeHeaderParser(parse_egv_g5_records);
      } else if (rectype == RECORD_TYPES.METER_DATA) {
        parser = makeHeaderParser(parse_meter_g5_records);
      } else if (rectype == RECORD_TYPES.USER_SETTING_DATA) {
        parser = makeHeaderParser(parse_setting_g5_records);
      }
    }
    else {
      // it's a G4

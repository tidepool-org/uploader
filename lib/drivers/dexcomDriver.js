...
module.exports = function (config) {
  ...
  var parse_egv_records = function (header, data) {
    // TODO: add new G5 properties? (systemTimeSeconds, transmitterTimeSeconds, filteredRateByte, NoiseMode, algorithmStateByte)
  ...
  var parse_meter_records = function (header, data) {
    ...
    if (header.revision === 4) {
      //it's a G5
      format = 'iisbiis';
      recordformat = ['systemSeconds', 'displaySeconds', 'meterValue', 'entryType', 'meterSystemTimeSeconds', 'meterTimeSeconds', 'crc'];
    }

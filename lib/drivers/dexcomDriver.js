...
module.exports = function (config) {
  ...
  var parse_setting_records = function (header, data) {
    ...
    for (var i = 0; i < header.nrecs; ++i) {
      var rec = struct.unpack(data, ctr, format, ['systemSeconds', 'displaySeconds',
        ...
        'alarmProfile', 'setUpState',
        // TODO: find out what the 4 bytes in whoKnows are for
        'whoKnows', 'crc'
      ]);
  ...
  var getPartitionInfo = function() {
    debug('getting partition info');
    var cmd = readPartitionInfo();
    dexcomCommandResponse(cmd, function(err, result) {
      if (err) {
        debug('Failure trying to get partition info.');
        debug(err);
        debug(result);
      }
    });
  ...
  return {
    ...
    processData: function (progress, data, cb) {
      ...
      // TODO: figure out what to do about true last datum instead of giving new Date().toISOString
      var tzoUtil = new TZOUtil(cfg.timezone, new Date().toISOString(), data.setting_data.timeChanges);
      cfg.tzoUtil = tzoUtil;
      data.post_records = cfg.tzoUtil.records;
      console.log(data.post_records);

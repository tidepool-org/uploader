...
import crcCalculator from '@tidepool/uploader-common/crc';
import env from '../../../app/utils/env';
import structJs from '@tidepool/uploader-common/struct';
...
const COMMAND_HISTORY_DONE = 0xd1;
...
export class KetoMojo extends EventTarget {
  ...
  static timeout(delay) {
    return new Promise((_, reject) => setTimeout(reject, delay, new Error('Timeout error')));
  }
  ...
  static parsePacket(bytes) {
    ...
    const fields = struct.unpack(body, 1, 'b.bbbS', ['src', 'tgt', 'cmd', 'ext', 'size']);
    fields.data = body.slice(8);
    return fields;
  }

  static parseHistoryRecord(data) {
    if (data.length < 9) {
    ...
    const fields = struct.unpack(data, 0, '9b', [
      'yearOffset', 'month', 'day', 'hours', 'minutes', 'rawHi', 'rawLo', 'meal', 'sampleType',
    ]);
    ...
    const unitsNibble = fields.meal & 0x0F;
    /* eslint-enable no-bitwise */

    const jsDate = sundial.buildTimestamp(fields);
    const isKetone = fields.sampleType === 0x55 || fields.sampleType === 0x66;
    const isMgdl = !isKetone && unitsNibble === 0x0A;
    const units = isKetone || !isMgdl ? 'mmol/L' : 'mg/dL';
    const divisor = isKetone ? 100.0 : (isMgdl ? 1.0 : 10.0);
    const value = raw === 0 ? 0 : raw / divisor;
  ...
  async scan() {
    ...
    if (typeof navigator !== 'undefined') {
      ...
      this.deviceName = this.device.name.replace(/ Meter$/, '');
      debug(`Name: ${this.deviceName}`);
  ...
  getDeviceInfo() {
    return {
      ...
      manufacturers: ['Vivachek', 'Keto-Mojo'],
    };
  }
  ...
  async getNumberOfRecords() {
    ...
    self.abortTimer = setTimeout(async () => {
      if (self.retries < MAX_RETRIES) {
        self.retries += 1;
        debug(`Retrying getNumberOfRecords (${self.retries}/${MAX_RETRIES})..`);
        await self.getNumberOfRecords();
      } else {
        self.retries = 0;
        self.dispatchEvent(new ErrorEvent('KetoMojoError', {
  ...
  async setDateTime(serverTime) {
    const { timezone } = this.cfg;
    await this.writeCharacteristic.writeValue(
      KetoMojo.buildPacket(COMMAND_SET_DATETIME, EXTENDED_CODE_WRITE, [
        _.toInteger(sundial.formatInTimezone(serverTime, timezone, 'YY')),
        _.toInteger(sundial.formatInTimezone(serverTime, timezone, 'M')),
        _.toInteger(sundial.formatInTimezone(serverTime, timezone, 'D')),
        _.toInteger(sundial.formatInTimezone(serverTime, timezone, 'H')),
        _.toInteger(sundial.formatInTimezone(serverTime, timezone, 'm')),
        _.toInteger(sundial.formatInTimezone(serverTime, timezone, 's')),
      ]),
    );
  }
  ...
  async handleNotifications(event) {
    ...
    // Packet layout: BEGIN(1) + header(8) + data(size) + CRC-nibbles(4) + END(1) = 14 + size bytes.
    // The size field is a big-endian short at bytes [7..8] of the raw packet.
    while (self.buffer.length >= 9) {
      if (self.buffer[0] !== BEGIN) {
        self.buffer.shift(); // discard leading garbage
        // eslint-disable-next-line no-continue
        continue;
      }

      /* eslint-disable-next-line no-bitwise */
      const dataSize = struct.extractBEShort(self.buffer, 7);
      const packetLen = 14 + dataSize;

      if (self.buffer.length < packetLen) break; // incomplete — wait for more data

      const packet = self.buffer.slice(0, packetLen);
      self.buffer = self.buffer.slice(packetLen);

      if (packet[packetLen - 1] !== END) {
        debug('Packet framing error: missing END marker, discarding');
        continue;
      }
      ...
      try {
        ...
        switch (parsed.cmd) {
          ...
          case COMMAND_GET_RECORDS: {
            ...
            const record = KetoMojo.parseHistoryRecord(parsed.data.slice(0, 9));
            if (record) {
              debug('Record:', record);
          ...
          case COMMAND_HISTORY_DONE:
            debug('History done');
            self.dispatchEvent(new CustomEvent('historyDone'));
            break;
...
export default (config) => {
  ...
  return {
    ...
    getConfigInfo(progress, data, cb) {
      ...
      (async () => {
        _.assign(cfg.deviceInfo, cfg.deviceComms.ble.getDeviceInfo());
        ...
        cfg.deviceTags = ['bgm'];
        cfg.deviceInfo.serial = _.trimStart(serial, '0');
        cfg.deviceInfo.deviceId = `${cfg.deviceInfo.driverId}-${cfg.deviceInfo.serial}`;
        ...
        cfg.api.getTime(function (err, result) {
          ...
          cfg.displayTimeModal(function (error) {
            ...
            const setTimeAndReadPromise = new Promise((resolve) => {
              cfg.deviceComms.ble.addEventListener('dateTimeSet', resolve, { once: true });
            });

            cfg.deviceComms.ble.setDateTime(serverTime)
              .then(() => setTimeAndReadPromise)
              .then(() => {
                cfg.deviceInfo.deviceTime = sundial.formatInTimezone(serverTime, cfg.timezone, 'YYYY-MM-DDTHH:mm:ss');
                data.connect = true;
                progress(100);
                cb(null, data);
              }).catch((err) => cb(err));
    ...
    fetchData(progress, data, cb) {
      ...
      cfg.deviceComms.ble.addEventListener('KetoMojoError', handleError, { once: true });
    ...
    processData(progress, data, cb) {
      ...
      _.forEach(data.records, (record, index) => {
        ...
        if (record.sampleType === 'ketone') {
          ...
          if (data.deviceModel.includes('GKI') && !isMMOL) {
            return cb(new Error('Unsupported model: GKI set to mg/dL'));
          } else if (data.deviceModel.includes('GK+') && isMMOL) {
            return cb(new Error('Unsupported model: GK+ set to mmol/L'));
          }

          if (isMMOL) {
            if (record.value > 33.3) {
              record.value = 33.4;
              annotation = {
                code: 'bg/out-of-range',
                threshold: 600, // thresholds are always in mg/dL
                value: 'high',
              };
            } else if (record.value < 0.6) {
              record.value = 0.5;
              annotation = {
                code: 'bg/out-of-range',
                threshold: 10, // thresholds are always in mg/dL
                value: 'low',
              };
            }
          } else {
    ...
    uploadData(progress, data, cb) {
      ...
      const sessionInfo = {
        ...
        deviceModel: cfg.deviceInfo.name.replace(/^Keto-Mojo\s*/, ''),

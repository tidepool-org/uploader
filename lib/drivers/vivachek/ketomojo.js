...
const COMMAND_READ_ALL_RECORDS = 0x13;
...
export class KetoMojo extends EventTarget {
  ...
  static buildPacket(payload) {
    const packetSize = 6 + payload.length;
    ...
    ctr += struct.storeByte(END, bytes, ctr);
  ...
  async getSerial() {
    ...
    await this.writeCharacteristic.writeValue(KetoMojo.buildPacket([
      COMMAND_READ_SERIAL, EXTENDED_CODE_READ,
      0x00, 0x00, 0x01, 0x0b, 0x0b, 0x04
    ]));
  }
  ...
  async getNumberOfRecords() {
    ...
    await this.writeCharacteristic.writeValue(KetoMojo.buildPacket([
      COMMAND_READ_NROFRECORDS, EXTENDED_CODE_READ,
      0x00, 0x00, 0x03, 0x0a, 0x0e, 0x04
    ]));
  }

  async getRecords(nrOfRecords) {
    debug('Getting records..');
    await this.writeCharacteristic.writeValue(KetoMojo.buildPacket([
      COMMAND_GET_RECORDS, EXTENDED_CODE_READ,
      0x00, 0x02, ...nrOfRecords,
      0x02, 0x02, 0x0f, 0x07 // changes with nrOfRecords
    ]));
  }
/*
  async getAllRecords() {
    ...
    await this.writeCharacteristic.writeValue(KetoMojo.buildPacket([
      COMMAND_READ_ALL_RECORDS, EXTENDED_CODE_READ,
      0x00, 0x01, 0x01, 0x04, 0x03, 0x07, 0x07
    ]))
  }
*/
  ...
  async handleNotifications(event) {
    ...
    if (done) {
      ...
      if (self.command === COMMAND_READ_NROFRECORDS) {
        self.dispatchEvent(new CustomEvent('nrOfRecords', {
          detail: self.buffer.slice(1),
        }));
      }
      
      if (self.command === COMMAND_GET_RECORDS) {
        console.log("REC:", self.buffer.slice(1));
  ...
  return {
    ...
    fetchData(progress, data, cb) {
      ...
      const recordsPromise = new Promise((resolve) => {
          const handleNrOfRecords = async (event) => {
            clearTimeout(self.abortTimer);

            cfg.deviceComms.ble.getRecords(event.detail);
            //cfg.deviceComms.ble.getAllRecords();
            resolve(); //FIXME
          };
          cfg.deviceComms.ble.addEventListener('nrOfRecords', handleNrOfRecords, { once: true });
        });

      cfg.deviceComms.ble.getNumberOfRecords().then(async () => {
        const nrOfRecords = await recordsPromise;

...
export class KetoMojo extends EventTarget {
  ...
  static buildPacket(payload) {
    const packetSize = 12 + payload.length;
  ...
  async getSerial() {
    ...
    await this.writeCharacteristic.writeValue(KetoMojo.buildPacket([COMMAND_READ_SERIAL, EXTENDED_CODE_READ, 0x00, 0x00, 0x01, 0x0b, 0x0b, 0x04]));
  }

  async getNumberOfRecords() {
    debug('Getting number of records..');
    await this.writeCharacteristic.writeValue(KetoMojo.buildPacket([0xdb, 0x55, 0x00, 0x00, 0x03, 0x0a, 0x0e, 0x04]));
  }

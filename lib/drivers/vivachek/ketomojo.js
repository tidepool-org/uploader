...
export class KetoMojo extends EventTarget {
  ...
  async setDateTimeAndRead(serverTime) {
  ...
  async handleNotifications(event) {
    ...
    while (self.buffer.length >= 9) {
      ...
      if (packet[packetLen - 1] !== END) {
        ...
        throw new Error('Packet framing error');
      }
      ...
      try {
        ...
        switch (parsed.cmd) {
          ...
          case COMMAND_SET_DATETIME:
            ...
            self.dispatchEvent(new CustomEvent('setTimeAndRead'));
            break;
  ...
  return {
        ...
        cfg.api.getTime(function (err, result) {
          ...
          cfg.displayTimeModal(function (error) {
            ...
            const setTimeAndReadPromise = new Promise((resolve) => {
              cfg.deviceComms.ble.addEventListener('setTimeAndRead', resolve, { once: true });
            });

            cfg.deviceComms.ble.setDateTimeAndRead(serverTime)
    ...
    uploadData(progress, data, cb) {
      ...
      const sessionInfo = {
        ...
        deviceModel: cfg.deviceInfo.name,

...
module.exports = function (config) {
  ...
  var newestEventRequester = function (start, end, progress, callback) {
    ...
    var headerIdFilter = [
      ...
      PUMP_LOG_RECORDS.LID_IDP_TD_SEG.value
    ];
  ...
  var buildSettingsRecords = function buildSettingsRecord(data, postrecords) {
    ...
    data.profiles.forEach(function (profile) {
      ...
      profile.tdeps.forEach(function (tdep) {
        ...
        targetSchedule.push({ target: tdep['TargetBG'], high: tdep['TargetBG'], start: tdep['startTime'] });
        //TODO: remove high value!
      });
    ...
    var records = filterLogEntries([PUMP_LOG_RECORDS.LID_IDP_TD_SEG], data.log_records);
    var last = null;
    records = _.sortByOrder(records, ['idp','segment_index','index'], [true, true, false]);

    records.forEach(function (record) {
      console.log('profile', record.idp, ':',record.segment_index, ', deviceTime: ',record.deviceTime, ', index: ', record.index);

      if (last && ((record.segment_index !== last.segment_index) || (record.idp !== last.idp))) {
        last = null; // different profile or segment; so start with new last value
      }

      if(last) {
        if ((record.segment_index !== last.segment_index) || (record.idp !== last.idp)) {
          last = null; // different profile or segment; so start with new last value
        }

        if(record.basal_rate !== last.basal_rate) {
          console.log('basal rate change from ',record.basal_rate,' to ',last.basal_rate);
        }
        if (record.carb_ratio !== last.carb_ratio) {
          console.log('carb ratio change from ',record.carb_ratio,' to ',last.carb_ratio);
        }
        if (record.isf !== last.isf) {
          console.log('isf change from ',record.isf,' to ',last.isf);
        }
        if (record.target_bg !== last.target_bg) {
          console.log('target bg change from ',record.target_bg,' to ',last.target_bg);
        }
        if (record.start_time !== last.start_time) {
          console.log('start time change from ',record.start_time,' to ',last.start_time);
        }
        if (record.modification_type === 0) {
          console.log('creating new segment');
        }else if (record.modification_type === 1) {

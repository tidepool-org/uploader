...
module.exports = function (config) {
    ...
    var extractPacket = function (bytes) {
        ...
        packet.timestamp = humanReadableTime(struct.extractBEInt(bytes, packet.packet_len - 6));
        packet.valid = true;
        //console.log(packet);
        return packet;
    };
    ...
    var tandemDownloadRecords = function (progress, callback) {
        ...
        function iterate(err, result) {
            if (err) {
                // TODO errors need to be retried sanely
                console.log ('error retrieving record', seq);

            } else {
                if (result.valid && seq < end_seq && result.descriptor == RESPONSES.LOG_ENTRY_TE.value) {
                    var recordType = getLogRecordById(result.payload['header_id']);
                    if (recordType != null) {
                        var record = {};
                        var tdeps = result.payload['tdeps'];
                        delete result.payload['tdeps'];
                        _.assign(record, result.payload);
                        _.assign(record, struct.unpack(tdeps, 0, recordType.format, recordType.fields));
                        record.name =getLogRecordName(result.payload['header_id']);
                        retval.push(record);
                        //console.log(record);
    ...
    var tandemFetch = function (progress, data, callback) {
         ...
         */
        tandemDownloadRecords(progress, function(err, retval) {
            if (err) {
                console.log('fetch failed');
            } else {
                console.log(retval);
                tandemFetchSettings(progress, function() {
                    callback(null, data);
                });
            }
        });
    ...
    var tandemFetchSettings = function (progress, callback) {
        /// TODO flesh this out
        console.log('tandemFetchSettings');
        //globals, IDP list, IDPs
        var profile_ids = [];
        var parsed_profiles = [];

        function iterate(err, result) {
            console.log('iterate', err, result);
            if (err) {
                console.log('error reading profile', profile_id);
            }
            if (result.valid && result.descriptor == RESPONSES.IDP_TE.value) {
                var profile = {};
                _.assign(profile, result.payload);
                var tdeps = [];
                var tdep_size = struct.structlen(IDP_TDEP.format);
                console.log('about to decode tdeps');
                for (var i = 0; i < result.payload['tdep_num']; i++) {
                    console.log('tdep ', i, tdep_size, tdeps);
                    tdeps.push(struct.unpack(result.payload['tdeps'], i * tdep_size, IDP_TDEP.format, IDP_TDEP.fields));
                }
                profile.tdeps = tdeps;
                parsed_profiles.push(profile);
                console.log('Got profile', profile);
                var profile_id = profile_ids.shift();
                console.log('Getting profile,', profile_id, profile_ids);
                if (profile_id === undefined) {
                    console.log('parsed profiles', parsed_profiles);
                    callback(null, parsed_profiles);
                } else {
            ...
            if (err) {
                ...
                tandemCommandResponse(COMMANDS.IDP_LIST_REQ, null, function (err, pkt) {
                    if (err) {
                        ...
                        console.log('profiles:', profile_ids);

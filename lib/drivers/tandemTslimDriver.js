    ...
    var RESPONSES = {
        ...
        LOG_ENTRY_TE: {
            ...
            postprocess: function (rec) {
                rec.header_ts_raw = rec.header_ts;
                rec.base_time = BASE_TIME;
                rec.header_ts = sundial.applyTimezone(BASE_TIME + rec.header_ts*sundial.SEC_TO_MSEC, cfg.timezone).valueOf();
                rec.headerUtc = new Date(rec.header_ts);
                rec.headerDeviceTime = sundial.formatDeviceTime(rec.header_ts, cfg.timezone);
/*
  ...
  */              var recordType = getLogRecordById(rec.header_id);
    ...
    var extractPacket = function (bytes) {
        ...
        packet.displayUtc = new Date(packet.timestamp);
        packet.displayTime = sundial.formatDeviceTime(sundial.applyTimezone(packet.displayUtc, cfg.timezone).toISOString());
    ...
    var buildBolusRecords = function (data, records) {
        ...
        bolusLogs.forEach( function(event) {
            ...
            if (event.header_id == PUMP_LOG_RECORDS.LID_BOLUS_REQUESTED_MSG1.value) {
                ...
                bolus.wizardUtc = new Date(event.headerDeviceTime);
            }
    ...
    var buildBasalRecords = function (data, records) {
        ...
        var makeScheduledBasal = function(event) {
        ...
        var finalizeBasal = function(event, basal, currentTemp) {
            ...
            var duration = event.header_ts - basal.time.getTime(); // TODO revisit this for temp basal events
            if (duration != basal.duration) {
                //basal.with_expectedDuration(currentTemp.duration);
                if (currentTemp)
                    basal.expectedDuration = currentTemp.duration;
                ...
                switch(event.change_type) {
                    ...
                    case 'new_profile':
                        if (currentTemp != undefined) {
                            console.log('time change within temp basal', event);
                            ...
                            currentScheduled = makeScheduledBasal(event);
   ...
   /*     cartridgeRecords.forEach( function(record) {
    ...
    return {
        ...
        uploadData: function (progress, data, cb) {
            ...
            var deviceId = 'tandemTslim'+data.model_no+data.pump_sn; // TODO pull this from version

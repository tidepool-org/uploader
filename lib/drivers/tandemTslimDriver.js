...
module.exports = function (config) {
    ...
    var buildSettingsRecords = function buildSettingsRecord(data, postrecords) {
        ...
        var postsettings = cfg.builder.makeSettings()
            ...
            .with_deviceTime(data.profiles[0].timestamp)
    ...
    var buildTimeChangeRecords = function (data, postrecords) {
        var changerecs = filterLogEntries([PUMP_LOG_RECORDS.LID_TIME_CHANGED, PUMP_LOG_RECORDS.LID_DATE_CHANGED],
            data.log_records);
        changerecs.forEach( function(change) {
            ...
            postrecords.push(cfg.builder.makeDeviceMetaTimeChange()
                .with_time(getUTCTime(change.header_ts))
                .with_deviceTime(getDeviceTime(change.header_ts))
                .with_timezoneOffset(cfg.timezone)
                .with_change({
    ...
    var buildBolusRecords = function (data, records) {
        var bolusRecords = filterLogEntries([PUMP_LOG_RECORDS.LID_BOLUS_ACTIVATED, PUMP_LOG_RECORDS.LID_BOLUS_COMPLETED,
        ...
        data.log_records);
        var boluses = {};
        bolusRecords.forEach( function(event) {
        ...
        for (var key in boluses) {
            ...
            record = record.with_time(bolus.start_time)         // TODO fix this mess around times
                        .with_deviceTime(bolus.start_time)   // TODO fix this mess around times
                        .with_timezoneOffset(cfg.timezone)   // TODO fix this mess around times
                        ;
            ...
            if (bolus.type == "standard" && bolus.bolus_size == undefined) { // cancelled before any insulin was given
                record.with_time(bolus.header_ts)
                    .with_deviceTime(bolus.header_ts)
                    .with_normal(0);
            ...
            if (bolus.type == "extended" && bolus.bolex_size == undefined) { // cancelled before any insulin was given
                record.with_time(bolus.wizard_ts)
                    .with_deviceTime(bolus.wizard_ts)
                    .with_duration(0)
                    .with_extended(0);
                record.expectedDuration = bolus.duration;

...
module.exports = function (config) {
    ...
    var RESPONSES = {
        ...
        125: {
            ...
            version: 36102,
            format: 'i2.hiii4i',
            fields: ['index', 'header_id', 'header_ts', 'header_log_seq_no', 'header_spare', 'td_a', 'td_b', 'td_c', 'td_d']
        },
        169: {
            // LOG_SIZE_TE
            value: 169,
        ...
        176: {
            // IDP_TE
            value: 176, name: 'Personal Profile Message', version: 36102, format: 'b17zb208Zhhbb',
    ...
    var PUMP_LOG_RECORDS = {
        ...
        0x4A: { // LID_PARAM_GLOBAL_SETTINGS
            value: 0x4A,
        ...
        0x49: { // LID_PARAM_PUMP_SETTINGS
            value: 0x49,
            name: 'Pump Parameter Change Event',
            format: 'b.hbbbb.bh....',
            fields: ['modification', 'status', 'low_insulin_threshold', 'cannula_prime_size', 'is_feature_locked', 'auto_shutdown_enabled', 'oled_timeout']
        },
        0x61: { // LID_PARAM_REM_SETTINGS
            value: 0x61,
            name: 'Reminder Parameter Change Event',
            format: 'bb..hhb.......',
            fields: ['modification', 'status', 'low_bg_threshold', 'high_bg_threshold', 'site_change_days']
        },
        0x96: { // LID_PARAM_REMINDER
            value: 0x96,
    ...
    var _tandemBaseTime = new Date(2008, 0, 1, 0, 0, 0).valueOf();


    var humanReadableTime = function (t) {

        var time = _tandemBaseTime + t * sundial.SEC_TO_MSEC;
    ...
    var getUTCTime = function (t) {
        ...
        var time = _tandemBaseTime + t * sundial.SEC_TO_MSEC;
        return sundial.tz(time, cfg.timezone).toISOString();
    ...
    var extractPacket = function (bytes) {
        ...
        if (packet.payload_len) {
            ...
            var response = RESPONSES[packet.descriptor];
    ...
    var tandemCommandResponse = function (command, args, callback) {
        ...
        cfg.deviceComms.writeSerial(commandPacket, function () {
    ...
    var tandemDownloadRecords = function (progress, callback) {
        ...
        function iterate(err, result) {
            if (err) {
                console.log ('error retrieving record', seq);
            } else {
                //console.log(result);
                // console.log('fetching', seq);
                // cfg.deviceComms.emitLog(true);
                // TODO handle err with retransmissions
                if (result.valid && seq < end_seq) {
                    if (result.descriptor === 125) {
                        var recordType = PUMP_LOG_RECORDS[result.payload['header_id']];
                        if (recordType != null) {
                            var record = {};
                            _.assign(record, result.payload);
                            retval.push(record);
                            console.log(record);

                        }
                        progress(100 * (end_seq - seq) / entries);
                        tandemCommandResponse(COMMANDS.LOG_ENTRY_SEQ_REQ, [seq++], iterate);
                    }
                } else if (seq == end_seq) {
                    console.log('fetched all records');
                    callback(null, retval);
                } else {
                    console.log('error fetching record ' + seq);
                    callback(err, null);
                }
            }
        }
        ...
        tandemCommandResponse(COMMANDS.LOG_SIZE_REQ, null, function (err, result) {
            if (err) {
                // TODO handle this by repeating a limited number of times
                console.log('Error reading log size', err);
                callback(err, null);
            } else if (result.valid && result.value === COMMANDS.LOG_SIZE_REQ.response) {
                entries = result.payload['entries'];
                end_seq = result.payload['end_seq'];
                seq = result.payload['end_seq'] - 1000;
                //end_seq = 500;
                console.log('requesting ' + entries + 'log entries');
                tandemCommandResponse(COMMANDS.LOG_ENTRY_SEQ_REQ, [seq++], iterate);
            }
        });
    };

    var tandemFetch = function (progress, callback) {
         ...
         */
        tandemDownloadRecords(progress, callback);
    };

    var tandemFetchSettings = function(progress, callback) {
        /// TODO flesh this out
        //globals, IDP list, IDPs
        tandemCommandResponse(COMMANDS.GLOBALS_REQ, null, function(err, pkt) {
            if (err) {
                ...
                /// TODO read in inactive profiles too with iterator
                tandemCommandResponse(COMMANDS.IDP_LIST_REQ, null, function(err, pkt) {
                    if (err) {
                        ...
                        tandemCommandResponse(COMMANDS.IDP_REQ, [pkt.payload['slot1']], function(err, pkt) {
    ...
    var tandemBuildBolusRecords = function (data) {
        // TODO unimplemented
        var postrecords = [];
        return postrecords;
    };

    var tandemBuildWizardRecords = function (data) {
        // TODO unimplemented
    ...
    var tandemBuildSettingsRecord = function (data) {
        // TODO unimplemented
    };

    var tandemBuildBasalRecords = function (data) {
        // TODO unimplemented
    };
    ...
    return {
        ...
        connect: function (progress, data, cb) {
            ...
            //cfg.deviceComms.setBitrate(19200);
        ...
        fetchData: function (progress, data, cb) {
            ...
            progress(0);
            tandemFetchSettings(progress, cb);
            tandemFetch(progress, cb);
        },

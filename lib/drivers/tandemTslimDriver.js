...
module.exports = function (config) {
  ...
  var getAlarmName = function (idx) {
    return _getName(ALARM_TYPES, idx);
  };
  ...
  var buildBasalRecords = function (data, records) {
    ...
    for (var b = 0; b < basalRecords.length; ++b) {
      ...
      var changesTypesArray = event.change_types.split('|');

      switch (event.change_types) {
        case 'timed_segment':
        case 'timed_segment|new_profile':
        case 'timed_segment|pump_resumed':
        ...
        case 'pump_resumed':
          ...
          cfg.tzoUtil.fillInUTCInfo(postbasal, event.jsDate);
          break;
        case 'timed_segment|temp_rate_start':
        ...
        case 'temp_rate_end|pump_shut_down':
          ...
          break;
        default:
          if (event.change_types.search('pump_shut_down') === -1) {
            throw new Error('Unhandled combination of basal change types: ' + event.change_types);
          }
      }
      if (postbasal != null) {
  ...
  var buildAlarmRecords = function (data,records) {
    var alarmRecords = filterLogEntries([PUMP_LOG_RECORDS.LID_ALARM_ACTIVATED],data.log_records);
    alarmRecords.forEach(function (alarmEntry) {
      ...
      //TODO: low_insulin, low_power, : alert id

      var alarmValue = alarmEntry.alarm_id;
      var alarmText = getAlarmName(alarmValue);

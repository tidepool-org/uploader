...
module.exports = function (config) {
    ...
    var RESPONSES = {
        ...
        LOG_ENTRY_TE: {
            ...
            postprocess: function (rec) {
                rec.header_ts = getDeviceTime(rec.header_ts);
    ...
    var PUMP_LOG_RECORDS = {
        LID_BASAL_RATE_CHANGE: {
            ...
            postprocess: function(rec) {
                // TODO handle ChangeType mapping to tidepool
            }
        },
    ...
    var tandemLogRequestor = function (start, end, callback) {
        // TODO implement and test multi-record download commands (my pump doesn't support the command)
        console.log( 'tandemLogRequestor', start, end);
        var seq = start;
        var receive_seq = start;
        var alarm_seq = receive_seq;
        var abortFunction = function () {
            ...
            if (alarm_seq == receive_seq) {
                ...
                callback('TIMEOUT', null);
            } else abortTimer = setTimeout(abortFunction, 5000);
        };
        var abortTimer = setTimeout(abortFunction, 5000); // timeout after 10 seconds


        var sendTimer = setInterval(function() {
            console.log('requesting', seq);

            tandemCommand(COMMANDS.LOG_ENTRY_SEQ_REQ, [seq++], function(){});
            if (seq > end) {
                clearInterval(sendTimer);
            }
        }, 1);
        ...
        var listenTimer = setInterval(function() {
            ...
            while (cfg.deviceComms.hasAvailablePacket()) {
                //console.log('packet found');
                var pkt = cfg.deviceComms.nextPacket();
                if (pkt.valid && pkt.descriptor == RESPONSES.LOG_ENTRY_TE.value) {
                    receive_seq = pkt.payload['header_log_seq_no'];
                    console.log('received', receive_seq);
                    callback(null, pkt);
                    if (receive_seq == end) {
                        console.log('end tandemLogRequestor');
                        clearInterval(listenTimer);
                        clearTimeout(abortTimer);
                        callback('END', null);
                    }
        ...
        tandemCommandResponse(COMMANDS.LOG_SIZE_REQ, null, function (err, result) {
            ...
            if (err) {
                ...
                if (result.valid && (result.descriptor == RESPONSES.LOG_SIZE_TE.value)) {
                    ...
                    start_seq = result.payload['end_seq'] - 300;
                    tandemLogRequestor(start_seq, end_seq, iterate);
                }
            }
        });
    };
    ...
    var tandemFetchSettings = function (progress, callback) {
        ...
        function iterate(err, result) {
            ...
            console.log('read profile', result);

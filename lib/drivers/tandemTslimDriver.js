...
module.exports = function (config) {
  ...
  var buildBasalRecords = function (data, records) {
    var basalRecords = filterLogEntries(
      [PUMP_LOG_RECORDS.LID_BASAL_RATE_CHANGE,PUMP_LOG_RECORDS.LID_NEW_DAY],
      data.log_records
    );
    var postbasal = null;
  ...
  var buildNewDayRecords = function(data, records) {
    ...
    for (var b = 0; b < newDayRecords.length; ++b) {
      ...
      var utcTime = sundial.applyTimezone(event.deviceTime, cfg.timezone).toISOString();
      if(event.commanded_basal_rate !== undefined) {
        // new day event; breaks up flat-rate basals
        var postrecord = cfg.builder.makeScheduledBasal()
            .with_deviceTime(event.deviceTime)
            .with_rate(event.commanded_basal_rate)
            .set('index', event.index)
            .with_conversionOffset(0)
            .with_timezoneOffset(0)
            .with_time(utcTime); //new day events should always occur at midnight, so don't shift with time changes

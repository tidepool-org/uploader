...
module.exports = function (config) {
      ...
      function getNewestEvent(err, result) {
        ...
        if (err) {
          ...
          if (result.payload.rawTimeStamp < searchElement) {
            ...
            tandemLogRequester(currentIndex, currentIndex, binarySearch);
          }
        }
  ...
  var buildTimeChangeRecords = function (data, postrecords) {
    ...
    for (var i = 0; i < timeChangeLogs.length; ++i) {
      ...
      var found = false;
      var index = data.log_records.indexOf(tc);
      var start = index < 5 ? 0 : index - 5; // look at 5 entries before
      var end = (index + 5) < (data.log_records.length - 1) ? index + 5 : data.log_records.length - 1; // and 5 entries after
      for(var k = start; k <= end; k++) {
        var event = data.log_records[k];
        if(event.header_id == PUMP_LOG_RECORDS.LID_DATE_CHANGED.value) {
          console.log('found a corresponding date change:', event);
          found = true;
          ...
          console.log('date & time changed: ',datetimechange.change);
  ...
  var buildBasalRecords = function (data, records) {
    ...
    var makeTempBasal = function (event, suppressed, currentTemp) {
      ...
      postbasal = cfg.builder.makeTempBasal() // TODO verify that this handles canceled temps properly
    ...
    var finalizeBasal = function (event, basal, currentTemp) {
      var duration = Date.parse(event.deviceTime) - Date.parse(basal.deviceTime);

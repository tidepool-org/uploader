...
var debug = isBrowser ? require('bows')('LibreViewDriver') : console.log;
    ...
    return {
        ...
        fetchData: function (progress, payload, cb) {
            ...
            parsed.meta.fields.forEach(function(field) {
                Object.keys(types_units).forEach(function(key) {
                    if (field.startsWith(key)) {
                        types_units[key] = field.replace(key + ' ', '');
                    }

                });
            });

            for (record_type of [RECORD_TYPE_HISTORIC, RECORD_TYPE_SCAN, RECORD_TYPE_STRIP]) {
                var units = types_units[TYPES_TO_READ[record_type]];
                if (GLUCOSE_HI[units] === undefined) {
                    const error = `Unexpected units for ${TYPES_TO_READ[record_type]}: ${units}`;
                    debug('Error:', error);
                    return cb(new Error(error));
                }
            }

            var ketone_units = types_units[TYPES_TO_READ[RECORD_TYPE_KETONE]];
            if (ketone_units !== 'mmol/L') {
                const error = `Unexpected units for ${TYPES_TO_READ[RECORD_TYPE_KETONE]}: ${ketone_units}`;
                debug('Error:', error);
                return cb(new Error(error));
            }
            ...
            for (var i = parsed.data.length - 1; i >= 0; --i) {
                ...
                if (!TYPES_TO_READ[record_type]) {
                    continue;
                }
                datum.device = `${datum['Device']} ${datum['Serial Number']}`;
                datum.type = TYPES_TO_READ[record_type];
                datum.units = types_units[datum.type];

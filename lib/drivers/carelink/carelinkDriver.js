  ...
  return function (config) {
    ...
    return {
      ...
      fetchData: function (progress, payload, cb) {
        ...
        cfg.deviceInfo = getDeviceInfo(payload.preamble.data, /Pump|Monitor/);
        ...
        if (_.includes(_.map(payload.cgmData, RAW_TYPE), CGM_TYPE)) {
      ...
      uploadData: function (progress, payload, cb) {
        ...
        async.eachSeries(Object.keys(payload.devices), function(key, done) {
          ...
          var deviceIds = _.uniq(_.map(deviceRecords, 'deviceId'));
          var sessionInfo = {

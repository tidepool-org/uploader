...
class AGM4000 {
  ...
  buildPacket(command, data = []) {
    ...
    const checksumLow = ~(STX ^ ~size ^ data.filter((_, i) => i % 2 === 0).reduce((acc, val) => acc ^ val, 0));
    const checksumHigh = ~(size ^ command ^ data.filter((_, i) => i % 2 !== 0).reduce((acc, val) => acc ^ val, 0));
    ctr += struct.pack(bytes, ctr, 'bb', checksumLow, checksumHigh);
  ...
  async getRecords(nrOfRecords) {
    ...
    for (let startIndex = 0; startIndex < nrOfRecords; startIndex++) {
      const payload = [];
      const record = {};
      struct.storeBEShort(startIndex, payload, 0);
      // requests to devices are sequential
      // eslint-disable-next-line no-await-in-loop
      const result = await this.commandResponse(COMMAND.READ_RECORD, payload);

      debug(`Record:`, result.payload);
      // TODO: parse record
...
module.exports = (config) => {
  ...
  // TODO: add model being uploaded (will have to be based on driverId)

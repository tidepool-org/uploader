  ...
  var setDateTime = function(serverTime, cb) {
    ...
    async.series({
      eot : function(callback){
        bcnCommandResponse(buildPacket([ASCII_CONTROL.EOT],1), true, function (err, result) {
          if(err) {
            return cb(err,null);
          }
      ...
      setDate: function(callback){
        var newDateTime = [];
        struct.storeString(sundial.formatInTimezone(serverTime, cfg.timezone, 'YYMMDD|') + '\r', newDateTime, 0);
        bcnCommandResponse(buildPacket(newDateTime, 8), false, function(err, result) {
          if(err) {
            return cb(err,null);
          }
          if(result.messageType !== ASCII_CONTROL.ACK) {

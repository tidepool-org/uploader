...
module.exports = function (config) {
  ...
  */
  ...
  function makeChecksum (frame) {
    var sum = frame.split('').reduce(function (previousValue, currentValue, currentIndex, _array) {
    ...
    async.series({
      ...
      eot: function(callback) {
        ...
        if ((cfg.deviceInfo.model !== 'Contour Next One') &&
          ...
          bcnCommandResponse(buildPacket([constants.ASCII_CONTROL.EOT], 1), false, function (err, _result) {
            if(err) {
              return cb(err, null);
            }
            callback(null, 'eot');
            ...
            common.checkDeviceTime(cfg, function(err2, serverTime) {
              if (err2) {
                if (err2 === 'updateTime') {
                  ...
                  hidDevice.send(buildAckPacket(), function(_err, _result) {
                    progress(100);
                    return cb(err2, data);
                  });
                }

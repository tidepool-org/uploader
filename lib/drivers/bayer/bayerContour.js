...
module.exports = (config) => {
  ...
  const buildAckPacket = () => buildPacket(ASCII_CONTROL.ACK, 1);

  const buildHeaderCmd = () => ({
  ...
  const listenForPacket = (timeout, parser, callback) => {
  ...
  const bcnCommandResponse = (commandpacket, callback) => {
    serialDevice.writeSerial(commandpacket.packet, () => {
      listenForPacket(5000, commandpacket.parser, (err, result) => {
        if (err === 'TIMEOUT') {
          debug('timeout');
          callback(err, null);
        } else {
          const rawtext = commandpacket.parser(result);
  ...
  const getDeviceInfo = (obj, cb) => {
    debug('DEBUG: on getDeviceInfo');
    const cmd = buildHeaderCmd();
    bcnCommandResponse(cmd, (err, datatxt) => {
      if (err) {

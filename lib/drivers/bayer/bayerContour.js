...
module.exports = (config) => {
  ...
  const COMMANDS = {
    READ_TIME: [0x52, 0x7c, 0x54, 0x7c, 0x0d], // R|T|
    WRITE_TIME: [0x57, 0x7c, 0x54, 0x7c, 0x0d], // W|T|
    READ_DATE: [0x52, 0x7c, 0x44, 0x7c, 0x0d], // R|D|
    WRITE_DATE: [0x57, 0x7c, 0x44, 0x7c, 0x0d], // W|D|
  };
  ...
  const buildStrCmd = (cmd, length) => ({
    packet: buildPacket(cmd, length),
    parser: packetParser,
  });

  const buildRawCmd = (cmd, length) => ({
    packet: buildPacket(cmd, length),
    ...
    listenTimer = setInterval(() => {
      if (serialDevice.hasAvailablePacket()) {
        ...
        if (hasETB !== -1 || hasETX !== -1) {
          // send a new ACK
          const cmd = buildRawCmd(ASCII_CONTROL.ACK, 1);
          serialDevice.writeSerial(cmd, () => {
            debug('New ACK SENT');
          });
        } else if (rec.TYPE === ASCII_CONTROL.EOT || rec.TYPE === ASCII_CONTROL.ACK || rec.TYPE === ASCII_CONTROL.ENQ) {
  ...
  const getDeviceInfo = (cb) => {
    ...
    (async () => {
      const [eot] = await bcnCommandResponse(buildRawCmd(ASCII_CONTROL.NAK, 1));
      if (eot !== ASCII_CONTROL.EOT) {
        throw new Error('Expected EOT, got', eot);
      }

      const [ack] = await bcnCommandResponse(buildRawCmd(ASCII_CONTROL.ENQ, 1));
      if (ack !== ASCII_CONTROL.ACK) {
        throw new Error('Expected ACK, got', ack);
      }

      console.log(await bcnCommandResponse(buildStrCmd('R|T|\r\n', 6)));

      const cmd = buildStrCmd(ASCII_CONTROL.ACK, 1);

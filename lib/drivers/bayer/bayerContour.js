...
import { MODELS, ASCII_CONTROL, COMMANDS, METHODS, MARKS } from './bayerConstants';
...
module.exports = (config) => {
  ...
  const parseDataRecord = (str, callback) => {
    ...
    if (data) {
      debug('Record:', data);
      const result = data.trim().match(/^.*\d+R\|(\d+).*Glucose\|(\d+)\|(\w+\/\w+)\^(\w*)\|\d*\|(>|<|T|>\\T|<\\T|)\|(\w*)\|(\w*)\|{3}(\d{12})$/);
      if (result != null) {
  ...
  const listenForPacket = (timeout, callback) => {
    ...
    listenTimer = setInterval(() => {
      if (serialDevice.hasAvailablePacket()) {
        ...
        const hasEOT = pkt.bytes[pkt.bytes.length - 1] === ASCII_CONTROL.EOT;
        debug(rec);
        _.map(pkt.bytes, (e) => { raw.push(e); });
        const hasCRLF = _.isEqual(_.takeRight(raw, 3), [ASCII_CONTROL.CR, ASCII_CONTROL.LF, ASCII_CONTROL.ACK]);
        ...
        } else if (rec.TYPE === ASCII_CONTROL.EOT || rec.TYPE === ASCII_CONTROL.ACK || rec.TYPE === ASCII_CONTROL.ENQ || hasCRLF || hasEOT) {
  ...
  const getDeviceInfo = (cb) => {
    ...
    (async () => {
      const [response] = await bcnCommandResponse(buildCmd(ASCII_CONTROL.NAK, 1));
      if (response !== ASCII_CONTROL.EOT && response !== ASCII_CONTROL.ENQ) {
        throw new Error('Expected EOT or ENQ, got', response.toString(16));
      }

...
module.exports = (config) => {
    ...
    listenTimer = setInterval(() => {
      if (serialDevice.hasAvailablePacket()) {
        ...
        const hasCRLF = _.isEqual(_.takeRight(raw, 3), [ASCII_CONTROL.CR, ASCII_CONTROL.LF, ASCII_CONTROL.ACK])
                        || (pkt === [ASCII_CONTROL.CR, ASCII_CONTROL.LF]);
  ...
  const getDeviceInfo = (cb) => {
    ...
    (async () => {
      let [response] = await bcnCommandResponse(buildCmd(ASCII_CONTROL.NAK, 1));
  ...
  return {
    ...
    cleanup(progress, data, cb) {
      debug('in cleanup');
      const cmd = buildCmd(ASCII_CONTROL.EOT, 1);
      // don't wait for callback, because serial port may not be working
      serialDevice.writeSerial(cmd.packet, () => {});
      setTimeout(() => {
        // wait a second before we disconnect, so that there's enough
        // time to send the EOT packet if serial port is still open

      ...
      var checkSchedules = function(startTime, endTime, durationBeforeMidnight) {
        for(var i in currSchedule) {
          ...
          if((startTime <= currSchedule[i].start) && (endTime > currSchedule[i].start) &&
            ...
            var newJsDate = sundial.applyOffset(new Date(sundial.applyOffset(currBasal.deviceTime, -currBasal.timezoneOffset).valueOf() + adjustedDuration), currBasal.timezoneOffset);

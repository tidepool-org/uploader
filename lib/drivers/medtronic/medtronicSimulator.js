...
exports.make = function(config){
  ...
  function addDurationToTime(time, timezoneOffset, duration) {
    /*
       Since the Date constructor (and its equivalent Date.parse) for parsing
       date/time strings are implementation dependent, they can introduce
       incorrect timezone info, so we use sundial.applyOffset instead.

       Returns Date() object
    */
    return sundial.applyOffset(new Date(sundial.applyOffset(time, -timezoneOffset).valueOf() + duration), timezoneOffset);
  }
      ...
      var checkSchedules = function(startTime, endTime, durationBeforeMidnight) {
        for(var i in currSchedule) {
          ...
          if((startTime <= currSchedule[i].start) && (endTime > currSchedule[i].start) &&
            ...
            var newJsDate = addDurationToTime(currBasal.deviceTime, currBasal.timezoneOffset, adjustedDuration);
            ...
            if(currStatus.duration > 0) {
              ...
              // temp basal is still active after suspend, so restart temp basal on resume
                ...
                if(currStatus && currStatus.index === event.index) {
                  ...
                  resumeBasal.time = addDurationToTime(currStatus.time,currStatus.timezoneOffset, currStatus.duration).toISOString();
                  resumeBasal.deviceTime = sundial.formatDeviceTime(addDurationToTime(currStatus.deviceTime, currStatus.timezoneOffset, currStatus.duration));
    ...
    bolus: function(event) {
      if(currWizard != null ) {
        ...
        if ( (bolusTime >= wizardTime) && ( bolusTime < (wizardTime + 5000)) ) {
          ...
          console.log('Could not find matching bolus for wizard:' + JSON.stringify(currWizard,null,4));
    ...
    alarm: function(event) {
      ...
      if((type === 'no_delivery' || type === 'auto_off' || type === 'no_power')
        ...
        if(type === 'other' && event.payload.alarm_id === 3 && currBasal) {
          console.log('Battery out too long at',event.time,', annotating previous basal at', currBasal.time);
          ...
          if(currBasal.deliveryType === 'scheduled' && currPumpSettings) {
            ...
            for(var i =0; i < currSchedule.length; i++) {
              console.log(currBasal.deviceTime, 'startTime:', startTime, 'currSchedule:', currSchedule[i].start, currSchedule[i].rate);
              ...
              if((startTime >= currSchedule[i].start) && (currBasal.rate === currSchedule[i].rate)) {
                if(currSchedule[i+1]) {
                  ...
                  console.log('Found scheduled duration:', scheduledDuration);

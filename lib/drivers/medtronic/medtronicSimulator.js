...
exports.make = function(config){
  ...
  return {
    basal: function(event) {
      ...
      if (event.deliveryType === 'temp') {
        if(event.percent && event.rate == null) {
    ...
    pumpSettings: function(event) {
      event.with_manufacturers(config.settings.deviceManufacturers)
           .with_model(config.settings.modelNumber)
           .with_serialNumber(config.settings.serialNumber)
           .with_display({ units: config.settings.units.bg });
      simpleSimulate(event.done());
      // TODO: build up history of bolus and basal settings based on point-in-time values
      // console.log("SETTINGS:", event.deviceTime, JSON.stringify(event.bolus), JSON.stringify(event.basal));

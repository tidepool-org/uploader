...
var buildWizardRecords = function (records) {
  ...
  wizardRecords.forEach(function(record) {
    ...
    var carbInput, bgInput, carbRatio, bgTarget = {}, isf, recommended = {}, units;
    ...
    if (duration > 0) {
      ...
      if (subType === BOLUS_TYPES.DUAL_SQUARE) {
        ...
        if (subType !== BOLUS_TYPES.SQUARE) {
          throw new Error('Expected a square bolus, but found type' + subType);
        }
        bolus = cfg.builder.makeSquareBolus()
      ...
      // normal bolus
      if (subType !== BOLUS_TYPES.NORMAL && subType !== BOLUS_TYPES.NORMAL_VARIABLE) {
        throw new Error('Expected a normal bolus, but found type ' + subType);
      }
...
/*
    oldSettings.bolus = {
      calculator: {
        enabled: record.head[1] >> 2 & 1
      }
    };
    newSettings.bolus = {
      calculator: {
        enabled: record.head[1] >> 3 & 1
      }
    };
*/
  ...
  var changes = filterHistory([RECORD_TYPES.CHANGE_BASAL_PROFILE_OLD,
                                 ...
                                 RECORD_TYPES.SET_BOLUS_WIZARD_ENABLED], records).reverse();
  var postrecords = [];
  var settingsChanges = [];
  var prevDate = null;
  ...
  postsettings.with_units(settings.units)
              ...
              .with_basal(settings.basal)
              .with_bolus(settings.bolus)
              .with_insulin(settings.insulin)
  ...
  for(var i=0; i < changes.length; i++) {
    ...
    /*
    if(record.type.value === RECORD_TYPES.SET_BOLUS_WIZARD_ENABLED.value) {
      settingsChange.bolus.calculator.enabled = record.head[1] ? true : false;
    };*/
    ...
    if(record.type.value === RECORD_TYPES.BOLUS_WIZARD_CHANGE.value ) {
      ...
      var currSettings = _.pick(settingsChange,['bgTarget','carbRatio','insulinSensitivity','units','insulin','bolus']);
    ...
    if(prevDate && prevDate.valueOf() !== record.jsDate.valueOf()) {
      ...
      postsettings.with_units(pumpSettings.units)
                  ...
                  .with_bolus(pumpSettings.bolus)
                  .with_basal(pumpSettings.basal)
                  .with_insulin(pumpSettings.insulin)
  ...
  initialSettings.with_units(first.units)
              ...
              .with_basal(first.basal)
              .with_bolus(first.bolus)
              .with_insulin(first.insulin);
...
function buildTimeChangeRecords(records, mostRecent, type) {
  ...
  for (var i = 0; i < timeChangeRecords.length; i++) {
    ...
    if (type === 'pump' && ((timeChangeRecords[i] == null) || (timeChangeRecords[i].type.value !== RECORD_TYPES.NEW_TIME_SET.value))) {

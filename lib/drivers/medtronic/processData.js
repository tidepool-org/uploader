...
var RECORD_TYPES = {
  ...
  BOLUS_WIZARD_CHANGE_OLD: { value:0x5A, body_length:137, name:'BOLUS_WIZARD_CHANGE_OLD'},
...
var processCBGPages = function(data, callback) {
  ...
  for(var i = 0; i < isigPages.length; i++) {
    if (isigPages[i].valid) {
      console.log('ISIG page',i,common.bytes2hex(isigPages[i].data));
      isigBytes.set(struct.extractBytes(isigPages[i].data,0,DEFAULT_PAGE_SIZE * 2),offset);
  ...
  for(var i = 0; i < pages.length; i++) {
    if (pages[i].valid) {
      console.log('CBG page',i,common.bytes2hex(pages[i].data));
      cbgBytes.set(struct.extractBytes(pages[i].data,0,DEFAULT_PAGE_SIZE),offset);

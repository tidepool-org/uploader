...
function buildSettings(records) {
  ...
  if(changes.length > 0) {
    ...
    // no record of any changed settings, so use current pump date and time
    // current settings do not have an index, so can't use TZOUtil.
    postsettings.with_time(sundial.applyTimezone(settings.currentDeviceTime, cfg.timezone).toISOString())

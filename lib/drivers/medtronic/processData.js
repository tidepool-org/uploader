...
var buildWizardRecords = function (records) {
  ...
  wizardRecords.forEach(function(record) {
    ...
    debug('BUILD WIZARD RECORDS');
    debug(record);
    // record.head[0] = oneRecord[0]
    // record.date[0] = oneRecord[2]
    // record.body[0] = oneRecord[7]
    ...
    recommended.correction = twosComplement(
      ((struct.extractByte(record.body,9) >> 3 & BITMASKS.RIGHT_FIVE) + struct.extractByte(record.body,6)),11
    ...
    units = (struct.extractByte(record.body,1) >> 6 & BITMASKS.RIGHT_TWO) !== 2 ? 'mg/dL' : 'mmol/L';
...
function buildSettings(records) {
  ...
  var getWizardSettings = function(record) {
   ...
   // bolus calculator is enabled
    ...
    if (oldSettings.bolus.calculator.enabled) {
    ...
    if (newSettings.bolus.calculator.enabled) {

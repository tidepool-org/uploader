...
function buildTimeChangeRecords(records, mostRecent, type) {
  ...
  function getCGMTimeChangeType(obj) {
    var type = obj.date[1] >> 5 & 7;
    return CGM_TIME_CHANGE_TYPES[type];
  }
  ...
  for (var i = 0; i < timeChangeRecords.length; i++) {
    ...
    if ((type === 'pump' && timeChangeRecords[i].type.value !== RECORD_TYPES.CHANGE_TIME.value) ||
        (type === 'cgm' && getCGMTimeChangeType(timeChangeRecords[i]) !== 'from')) {
    ...
    if ((timeChangeRecords[i] == null) ||
        ...
        (type === 'cgm' &&
         getCGMTimeChangeType(timeChangeRecords[i]) !== 'manual' &&
         getCGMTimeChangeType(timeChangeRecords[i]) !== 'alarm')) {
    ...
    if(toDatum) {
      ...
      if (fromDatum.jsDate.getUTCFullYear() < (new Date().getUTCFullYear() - 1) ||
        ...
        (type === 'cgm' && getCGMTimeChangeType(timeChangeRecords[i]) === 'alarm')) {

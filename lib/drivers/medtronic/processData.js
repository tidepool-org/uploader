  ...
  for(var i=0; i < suspendResumeRecords.length; i++) {
    ...
    if(suspendEntry.type.value === RECORD_TYPES.PUMP_SUSPEND.value) {
      ...
      while(suspendResumeRecords[i].type.value === RECORD_TYPES.PUMP_SUSPEND.value) {
        ...
        if((suspendResumeRecords[i].head[1] & BITMASKS.RIGHT_FIVE) === REASONS.SUSPEND_ALARM.value) {
          ...
          var name = common.getName(REASONS, reason);
          reasons.push(name);
          debug('Suspend reason:',name);
        }
      ...
      if(resumeEntry && resumeEntry.type.value === RECORD_TYPES.PUMP_RESUME.value) {
        ...
        if(headCheck === REASONS.RESUME_AUTO_NO_RESPONSE.value) {
          suspendResume.reason.resumed = 'automatic';
          reasons.push(REASONS.RESUME_AUTO_NO_RESPONSE.name);
        ...
        if(headCheck === REASONS.RESUME_AUTO_USER_SUSPEND.value) {
          suspendResume.reason.resumed = 'automatic';
          reasons.push(REASONS.RESUME_AUTO_USER_SUSPEND.name);
          debug('LGS resumed automatically after user suspend at', resumeEntry.jsDate);
        }

        if(headCheck === REASONS.RESUME_USER.value) {
          reasons.push(REASONS.RESUME_USER.name);
          debug('Resumed by user at', resumeEntry.jsDate);
        ...
        suspendResume.with_payload({reasons : reasons});

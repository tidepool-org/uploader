...
function buildSettings(records) {
  ...
  var getWizardSettings = function(record) {
    ...
    oldSettings.bgTarget = getBGTargets(struct.extractBytes(record.body, index - 1, SIZES.BG_TARGET + 1), oldSettings.units.bg);
    ...
    index += SIZES.INSULIN_SENSITIVITY;
    newSettings.bgTarget = getBGTargets(struct.extractBytes(record.body, index - 1, SIZES.BG_TARGET + 1), newSettings.units.bg);

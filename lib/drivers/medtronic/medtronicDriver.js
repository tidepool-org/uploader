  ...
  return {
      ...
      async.series({
        ...
        current_cbg_page : function(callback){
          getOneRecord(readCurrentCBGPage(), false, true, function(err, result) {
            ...
            var currentPage = positionToPage(lastCBGPosition, result.currentCBGPosition, 'CBG history');
            var glucosePage = positionToPage(lastGlucosePosition, result.currentGlucosePosition, 'glucose');
            var isigPage = positionToPage(lastIsigPosition, result.currentIsigPosition, 'isig');

            callback(null, { currentPage : currentPage, glucosePage: Math.min(glucosePage, MAX_CBG_PAGES), isigPage: Math.min(isigPage, MAX_CBG_PAGES) });
          });
        },
    ...
    fetchData: function (progress, data, cb) {
      ...
      var readHistoryPages = function(historyType, numberOfPages, currentPage, pagesCb) {
          ...
          if (start - end === 0) {
            // we want to read at least one page
            end += 1;
          }
                          ...
                          getRecords(readPage(historyType,subType, partsPerPage), partsPerPage, function (err, result) {
                            ...
                            if(err) {
                              ...
                              if(result) {
                                ...
                                pages[count] = result;
                                count++;
                                progress(progressOffset + ((count/numberOfPages) * 20), cfg.isFirstUpload);

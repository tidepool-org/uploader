...
module.exports = function (config) {
  ...
  var rawPacket = [];
  var mismatch = false; //TODO: remove this
  ...
  var readCurrentCBGPage = function () {
    ...
    return {
      ...
      parser: function (packet) {
        ...
        var currentPage = struct.extractBEInt(medtronicMessage,0);
        var glucosePage = Math.min(struct.extractByte(medtronicMessage,5), MAX_CBG_PAGES);
        var isigPage = Math.min(struct.extractByte(medtronicMessage,7), MAX_CBG_PAGES);
      ...
      function (callback) {
        ...
        hidDevice.receive(function(raw) {
          if(finished) {
            ...
            console.log('Found packet we were still waiting for');
            rawPacket = raw;
            ...
            if ( packet.length === 0 ) {
              console.log('buffer emptied after flushing');
              return callback(true);
            }
            console.log('Flushing', common.bytes2hex(packet));
  ...
  return {
    ...
    getConfigInfo: function (progress, data, cb) {
      ...
      async.series({
        ...
        battery_status : function(callback){
          getOneRecord(readBatteryStatus(), false, function(err, result) {
            if(err) {
              return cb(err,null);
            }
            callback(null, result);
      ...
      var readHistoryPages = function(historyType, numberOfPages, currentPage, pagesCb) {
          ...
          async.whilst(
              ...
              function (callback) {
                    ...
                    if(result) {
                      ...
                      if(result.medtronicRecordType === undefined) {
                        ...
                        if(result.medtronicRecordType !== ASCII_CONTROL.ACK) {
                          console.log('Expected ACK, but got something else..');
                          messageBuffer.reset();
                            ...
                            if(result) {
                              ...
                              if(!result.page.every(function(e) { return e === 0; })) {
                                ...
                                if(calculated !== checksum) {
                                  ...
                                  mismatch = true; // TODO: remove this
                                  result.valid = false;
                                } else {
                                  result.valid = true;
                                }
                ...
                async.retry({times: RETRIES, interval: TIME_TO_WAIT}, readHistory, function(err, result) {
                  if(err) {
                    return callback(err,null);
                  }
                  callback(null, result);
      ...
      var retryInvalidPages = function(type, pages, start, retrycb) {
        async.timesSeries( pages.length, function(n, next){
          if(pages[n].valid || pages[n].nak) {
            ...
            console.log('Re-reading at position',start+n);
            readHistoryPages(type, 1, start+n, function(err, results) {
              console.log('Page', n, 're-read as', results[0]);
              next(err,results[0]);
            });
          }
        }, function(err, results) {
      ...
      async.series({
        ...
        readCBGHistory : function(callback) {
          ...
          readHistoryPages(MESSAGES.READ_CBG_HISTORY, data.cbg.glucosePage, data.cbg.currentPage, function(err, results) {
            retryInvalidPages(MESSAGES.READ_CBG_HISTORY, results, data.cbg.currentPage, function(err, pages) {
        ...
        readISIGHistory : function(callback) {
          ...
          readHistoryPages(MESSAGES.READ_ISIG_HISTORY, data.cbg.isigPage, data.cbg.currentPage, function(err, results) {
            retryInvalidPages(MESSAGES.READ_ISIG_HISTORY, results, data.cbg.currentPage, function(err, pages) {
    ...
    cleanup: function (progress, data, cb) {
      debug('in cleanup');
      console.log('Checksum mismatched:', mismatch); //TODO: remove this

      //TODO: remove this before we go to prod
      function beep() {

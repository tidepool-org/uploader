  ...
  var readCurrentPage = function () {
    ...
    return {
      ...
      parser: function (packet) {
        ...
        var currentPosition = struct.extractBEInt(medtronicMessage,0);
        localStorage.setItem('lastPosition', currentPosition); //TODO: switch to backend

        var currentPage = positionToPage(lastPosition, currentPosition, 'pump history');

        return { currentPage : Math.min(currentPage, MAX_HISTORY_PAGES) + 1 };
      }
    };
  };

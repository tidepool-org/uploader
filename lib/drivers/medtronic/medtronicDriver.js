...
module.exports = function (config) {
  ...
  var readCurrentCBGPage = function () {
    ...
    return {
      ...
      parser: function (packet) {
        ...
        var lastCBGPosition = localStorage.getItem('lastCBGPosition'); //TODO: switch to backend
        var lastGlucosePosition = localStorage.getItem('lastGlucosePosition');
        var lastIsigPosition = localStorage.getItem('lastIsigPosition');

        var currentCBGPosition = struct.extractBEInt(medtronicMessage,0);
        var currentGlucosePosition = struct.extractByte(medtronicMessage,5);
        var currentIsigPosition = struct.extractByte(medtronicMessage,7);

        localStorage.setItem('lastCBGPosition', currentCBGPosition);
        localStorage.setItem('lastGlucosePosition', currentGlucosePosition);
        localStorage.setItem('lastIsigPosition', currentIsigPosition);

        var currentPage = positionToPage(lastCBGPosition, currentCBGPosition, 'CBG history');
        var glucosePage = positionToPage(lastGlucosePosition, currentGlucosePosition, 'glucose');
        var isigPage = positionToPage(lastIsigPosition, currentIsigPosition, 'isig');

        return { currentPage : currentPage, glucosePage: Math.min(glucosePage, MAX_CBG_PAGES) + 1, isigPage: Math.min(isigPage, MAX_CBG_PAGES) + 1 };
      }
    };
  };
  ...
  return {
      ...
      async.series({
        ...
        current_page : function(callback){
          getOneRecord(readCurrentPage(), false, true, function(err, result) {
            ...
            api.getMostRecentUploadRecord(cfg.groupId, cfg.deviceInfo.deviceId, function(apiErr, lastUpload) {
              if (apiErr) {
                return cb(apiErr, null);
              }

              var lastPosition = null;
              if (lastUpload && lastUpload.client) {
                lastPosition = lastUpload.client.private.delta.lastPosition;
              }

              var currentPosition = result.currentPage;

              _.assign(cfg.delta, {lastPosition :  currentPosition});
    ...
    uploadData: function (progress, data, cb) {
      ...
      var sessionInfo = {
        delta: {
          lastPosition: cfg.delta.lastPosition
        },

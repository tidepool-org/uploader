...
module.exports = function (config) {
  ...
  var SUPPORTED_DEVICES = ['523', '723', '551', '751'];
  ...
  var buildMedtronicPacket = function (type, command, parameter, partsPerPage) {
    ...
    if(parameter != null ) {
      ...
      ctr += struct.pack(bytes, ctr, '7bi', 0, 0, 0, partsPerPage, 0x10, 0x10, 0x00, payload.length+secondPacketLength);
    } else {
      // for current basal profiles we are not sending parameters, but we're requesting
      // three records instead of the usual one
      if(partsPerPage === 3) {
        ctr += struct.pack(bytes, ctr, '7bi', 0, 0, 0xC0, 0x00, 0x04, 0x10, 0x00, payload.length);
      } else {
  ...
  var readSettings = function () {
    ...
    return {
      ...
      parser: function (packet) {
        ...
        var medtronicMessage = packet.slice(MEDTRONIC_PACKET_START);
        console.log('Settings:', common.bytes2hex(medtronicMessage));
        // TODO: what other settings here are useful?
  ...
  var readCurrentPage = function () {
    ...
    return {
      ...
      parser: function (packet) {
        ...
        var medtronicMessage = packet.slice(MEDTRONIC_PACKET_START);
        console.log('Current page:', common.bytes2hex(medtronicMessage));
        // pump only stores 36 history pages
        var MAX_HISTORY_PAGES = 36;
        var currentPage = Math.min(medtronicMessage[3], MAX_HISTORY_PAGES) + 1;
  ...
  var readCurrentCBGPage = function () {
    ...
    return {
      ...
      parser: function (packet) {
        ...
        var medtronicMessage = packet.slice(MEDTRONIC_PACKET_START);
        console.log('Current CBG page:', common.bytes2hex(medtronicMessage));
        // pump stores a maximum of 32 glucose and ISIG history pages
        var MAX_CBG_PAGES = 32;
        var currentPage = Math.min(struct.extractBEInt(medtronicMessage,0), MAX_CBG_PAGES) + 1;
        var glucosePage = Math.min(struct.extractByte(medtronicMessage,5), MAX_CBG_PAGES) + 1;
        var isigPage = Math.min(struct.extractByte(medtronicMessage,7), MAX_CBG_PAGES) + 1;

        return { currentPage : currentPage, glucosePage: glucosePage, isigPage: isigPage };
      }
    };
  };
  ...
  var getOneRecord = function (cmd, waitForENQ, callback) {
    ...
    var waitTimer = setTimeout(function () {
      bcnCommandResponse(cmd, waitForENQ, 1, function (err, record) {
        if (err) {
  ...
  var getThreeRecords = function (cmd, callback) {
    ...
    var waitTimer = setTimeout(function () {
      bcnCommandResponse(cmd, false, 3, function (err, record) {
        if (err) {
          return callback(err, null);
        } else {
          console.log('Record:', record);
    ...
    hidDevice.send(packet.command1, function () {
      hidDevice.send(packet.command2.command, function () {
        ...
        async.whilst(
            ...
            function (callback) {
                getMessage(20000, false, 0, function(err, result) {
                  if (err) {
                    return callback(err, null);
                  }
                  if (result) {
                    ...
                    if(result.resultsInReply &&
                      ...
                      if(decoded.message.length === 2 && decoded.message[0] === ASCII_CONTROL.NAK) {
                        ...
                        console.log('Part', count, 'of page (length: ', decoded.message.length ,'):', common.bytes2hex(decoded.message));
                        ...
                        if( (_sum_lsb(payload) !== checksum) || (decoded.message.length === 0)) {
                          debug('Invalid checksum or null data');
                          return callback(new Error(COMM_ERROR_TEXT));
                        };
                      }
                    }
            ...
            function (err, done) {
              ...
              if(!nak && (count < howMany)) {
                console.log('Only received', count, 'of', howMany, 'pages');
  ...
  var bcnCommandResponse = function (commandpacket, waitForENQ, howManyPackets, callback) {
    ...
    hidDevice.send(commandpacket.command, function () {
      ...
      async.timesSeries(howManyPackets, function(n, next){
        ...
        getMessage(20000, waitForENQ, n, function(err, result) {
          if (err) {
            return callback(err, null);
          }
          next(null, result);
  ...
  var getMessage = function (timeout, waitForENQ, replyNr, cb) {
    var done = false;
    ...
    async.doWhilst(
      function (callback) {
        hidDevice.receive(function(raw) {
          ...
          console.log('Last packet size:', message.lastPacketSize);
          ...
          if (message.messageLength >= MEDTRONIC_PACKET_START) {
            ...
            if(fetchingHistory) {
              ...
              if(message.lastPacketSize < 60) {
                console.log('last packet size < 60');
                clearTimeout(abortTimer);
                return callback(true);
              }
            }
  ...
  return {
    ...
    getConfigInfo: function (progress, data, cb) {
      ...
      async.series({
        ...
        open_connection : function(callback){
          var openConn = function(conncb) {
            getOneRecord(openConnection(), false, function(err, result) {
              if(err) {
                return conncb(err, null);
              }
              conncb(null, 'open');
            });
          };

          async.retry(RETRIES, openConn, function(err, result) {
            if(err) {
              return cb(err,null);
            }
          ...
          var getModel = function(modelcb) {
            ...
            getOneRecord(readModel(), false, function(err, result) {
              if (result.modelNumber.replace(/\0/g, '') === '' ) {
                return modelcb(new Error('Have you entered the correct serial number and is there enough battery power left?'),null);
              }
              if (_.indexOf(SUPPORTED_DEVICES,result.modelNumber) > -1 ) {
                  modelcb(err, result.modelNumber);
                } else {
        ...
        bg_targets : function(callback){
          getOneRecord(readBGTargets(), false, function(err, result) {
            if(err) {
              return cb(err,null);
            }
            callback(null, result);
        ...
        carb_ratios : function(callback){
          getOneRecord(readCarbRatios(), false, function(err, result) {
            if(err) {
              return cb(err,null);
            }
            callback(null, result);
        ...
        insulin_sensitivities : function(callback){
          getOneRecord(readInsulinSensitivities(), false, function(err, result) {
            if(err) {
              return cb(err,null);
            }
            callback(null, result);
        ...
        settings : function(callback){
          getOneRecord(readSettings(), false, function(err, result) {
            if(err) {
              return cb(err,null);
            }
            callback(null, result);
        ...
        current_page : function(callback){
          getOneRecord(readCurrentPage(), false, function(err, result) {
            if(err) {
              return cb(err,null);
            }
            callback(null, result);
        ...
        current_cbg_page : function(callback){
          getOneRecord(readCurrentCBGPage(), false, function(err, result) {
            if(err) {
              return cb(err,null);
            }
            callback(null, result);
        ...
        current_time : function(callback){
          getOneRecord(readRTC(), false, function(err, result) {
            if(err) {
              return cb(err,null);
            }
            callback(null, result);
    ...
    fetchData: function (progress, data, cb) {
      ...
      var readHistoryPages = function(historyType, numberOfPages, pagesCb) {
          ...
          async.whilst(
              ...
              function (callback) {
                var readHistory = function(historycb) {
                  ...
                  getOneRecord(sendCommand(historyType), true, function (err, result) {
                    if(err) {
                      if(err.name === 'TIMEOUT') {
                        // TODO: figure out if a retry here would cause meter to hang
                        return cb(err, null);
                      }
                    ...
                    if(result) {
                      ...
                      if(result.medtronicRecordType === undefined) {
                        ...
                        if(historyType === MESSAGES.READ_HISTORY) {
                          ...
                          subType = [0x04,0x00,0x00,0x00,count];
                        }
                        ...
                        getRecords(readPage(historyType,subType, partsPerPage), partsPerPage, function (err, result) {
                          ...
                          if(err) {
                            ...
                            if(result) {
                              ...
                              debug('Page', count, common.bytes2hex(result.page,true));
                              ...
                              if(checksumFlag) {
                                ...
                                count++;
                                progress(progressOffset + ((count/numberOfPages) * 20));
      ...
      async.series({
        readPumpHistory : function(callback){
          console.log('Reading pump history..');
          readHistoryPages(MESSAGES.READ_HISTORY, data.currentPage, function(err, results) {
            _.assign(data, { pages : results });
            progressOffset += 20;
            return callback(err);
          });
        },
        readCBGHistory : function(callback) {
          console.log('Reading CGM history..');
          readHistoryPages(MESSAGES.READ_CBG_HISTORY, data.cbg.glucosePage, function(err, results) {
            _.assign(data, { cbg_pages : results });
            progressOffset += 20;
            return callback(err);
          });
        },
        readISIGHistory : function(callback) {
          console.log('Reading ISIG history..');
          readHistoryPages(MESSAGES.READ_ISIG_HISTORY, data.cbg.isigPage, function(err, results) {
            _.assign(data, { isig_pages : results });
            return callback(err);
          });
        }
      },
    ...
    processData: function (progress, data, cb) {
      ...
      data.settings.deviceId = 'MedT-' + data.settings.modelNumber + '-' + serial;
      ...
      proc.processPages(data, function (err, records) {
        ...
        proc.processCBGPages(data, function (cgmErr, cgmRecords) {
          ...
          if(cgmDateRecords.length === 0) {
            console.log('No CGM time change records found on pump');
          } else {
            var cgmMostRecent = sundial.applyTimezone(cgmDateRecords[cgmDateRecords.length-1].jsDate, cfg.timezone).toISOString();
            console.log('CGM time changes:');
            var cgmResult = proc.buildTimeChangeRecords(cgmRecords, cgmMostRecent, 'cgm');

...
var struct = require('@tidepool/uploader-common/struct')();
var proc = require('./processData');
var common = require('../../commonFunctions');
var medtronicSimulator = require('./medtronicSimulator');
var crcCalculator = require('@tidepool/uploader-common/crc');
...
module.exports = function (config) {
  ...
  var buildMedtronicPacket = function (type, command, parameter, partsPerPage) {
    ...
    if(command != null) {
      ...
      if(parameter != null) {
        ...
        var padding = _.fill(Array.from({length: 22-parameter.length}), 0);
  ...
  var buildPaddingPacket = function (command, parameter) {
    var length = 43;
    var padding = _.fill(Array.from({length: length}), 0);

    var prevPacketPadding = _.fill(Array.from({length: 22-parameter.length}), 0);
  ...
  return {
    ...
    getConfigInfo: function (progress, data, cb) {
      ...
      async.series({
        ...
        eot : function(callback){
          debug('Connecting to meter..');
            ...
            getOneRecord(buildPacket([ASCII_CONTROL.EOT],1), true, false, function(err, _result) {
              if(err) {
                return cb(err,null);
              }
              callback(null, 'eot');
        ...
        open_connection : function(callback){
          ...
          getOneRecord(openConnection(), false, false, function(err, _result) {
            if(err) {
              return cb(err, null);
            }
            callback(null, 'open');
        ...
        model : function(callback){
          ...
          getOneRecord(readModel(), false, true, function(err, result) {
            // eslint-disable-next-line no-control-regex -- stripping NUL padding from device strings
      ...
      function(err, results){
          ...
          if(!err){
              ...
              common.checkDeviceTime(cfg, function(err, _serverTime) {
                return cb(err, data);
              });
          } else {
    ...
    cleanup: function (progress, data, cb) {
      ...
      if(isBrowser) {
        if(!data.disconnect){
          getOneRecord(buildPacket([ASCII_CONTROL.EOT],1), true, false, function(_err, _result) {

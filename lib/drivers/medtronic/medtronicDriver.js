...
module.exports = function (config) {
  ...
  var OPEN_CONNECTION_TIMEOUT = 30000;
  var SEND_MESSAGE_TIMEOUT = 5000;
  ...
  var COMMANDS = {
    ...
    OPEN_CONNECTION : [0x10,  // command
                       ...
                       OPEN_CONNECTION_TIMEOUT], // timeout = 30 seconds
    SEND_MESSAGE : [0x12, // command
                    0x51, // retries = 5 (5 << 4 + 1 = 0x51 )
                    SEND_MESSAGE_TIMEOUT] // timeout = 5 seconds
  };
    ...
    hidDevice.send(packet.command1, function () {
      hidDevice.send(packet.command2.command, function () {
        ...
        async.whilst(
            ...
            function (callback) {
                getMessage(SEND_MESSAGE_TIMEOUT, false, 0, function(err, result) {
                  if (err) {
                    return callback(err, null);
                  }
                  if (result) {
  ...
  var bcnCommandResponse = function (commandpacket, waitForENQ, howManyPackets, callback) {
    ...
    hidDevice.send(commandpacket.command, function () {
      ...
      async.timesSeries(howManyPackets, function(n, next){
        ...
        getMessage(OPEN_CONNECTION_TIMEOUT, waitForENQ, n, function(err, result) {
          if (err) {
            return callback(err, null);
          }
          next(null, result);

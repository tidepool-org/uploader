...
module.exports = function (config) {
  ...
  var writeRTC = function (serverTime) {

    var parameters = [
      7,
      _.toInteger(sundial.formatInTimezone(serverTime, cfg.timezone, 'H')),
      _.toInteger(sundial.formatInTimezone(serverTime, cfg.timezone, 'm')),
      _.toInteger(sundial.formatInTimezone(serverTime, cfg.timezone, 's')),
      sundial.formatInTimezone(serverTime, cfg.timezone, 'YYYY') >> 8, //high
      sundial.formatInTimezone(serverTime, cfg.timezone, 'YYYY') & 0xFF, // low
      _.toInteger(sundial.formatInTimezone(serverTime, cfg.timezone, 'M')),
      _.toInteger(sundial.formatInTimezone(serverTime, cfg.timezone, 'D')),
    ];

    return {
      command1: buildMedtronicPacket(COMMANDS.SEND_MESSAGE,MESSAGES.WRITE_TIME, parameters),
      command2: buildPaddingPacket(MESSAGES.WRITE_TIME, parameters),
      type: MESSAGES.WRITE_TIME,
      parser: function (packet) {
  ...
  var setDateTime = function(serverTime, cb) {
    getRecords(writeRTC(serverTime), null, function (err, result) {
      cb(err, result);
    });
  };
    ...
    getConfigInfo: function (progress, data, cb) {
      ...
      function(err, results){
          ...
          if(!err){
              ...
              _.assign(cfg.deviceInfo, {
                deviceTime : data.settings.currentDeviceTime,

...
module.exports = function (config) {
  ...
  var writeRTC = function (serverTime) {
    ...
    return {
      command: buildMedtronicPacket(COMMANDS.SEND_MESSAGE,MESSAGES.WRITE_TIME, parameters),
      parser: function (packet) {

        var medtronicMessage = packet.slice(MEDTRONIC_PACKET_START);
        console.log("Medtronic message:", medtronicMessage);
  ...
  var setDateTime = function(serverTime, cb) {
    getOneRecord(writeRTC(serverTime), false, true, function(err, result) {
      cb(err, result);
    });
  };

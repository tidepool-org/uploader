  ...
  * eventsOfTypeReverse(...eventTypes) {
  ...
  buildSettingsRecords(events) {
    ...
    currentSettings
      ...
      .with_basal({
        temporary: {
          type: this.currentPumpSettings.tempBasalType ===
      ...
      .with_manufacturers(this.currentPumpSettings.deviceManufacturers)
    ...
    for (const event of this.eventsOfTypeReverse(...oldEventTypes, ...newEventTypes)) {
      switch (event.eventType) {
        ...
        case NGPHistoryEvent.EVENT_TYPE.DUAL_BOLUS_OPTION_CHANGE:
          ...
          break;
        case NGPHistoryEvent.EVENT_TYPE.TEMP_BASAL_PROGRAMMED:
          if (prevTempBasal && event.type !== prevTempBasal.type) {
            // record previous temp basal type if it has changed
            settingsChange.with_time(prevTempBasal.timestamp.toDate().toISOString());
            this.addTimeFields(settingsChange, prevTempBasal.timestamp);
      ...
      switch (event.eventType) {
        ...
        case NGPHistoryEvent.EVENT_TYPE.BOLUS_WIZARD_SETTINGS_CHANGE:
          ...
          break;
        case NGPHistoryEvent.EVENT_TYPE.TEMP_BASAL_PROGRAMMED:
          settingsChange.basal.temporary.type = event.type ===
            NGPUtil.NGPConstants.TEMP_BASAL_TYPE.PERCENTAGE ?
            'percent' : 'Units/hour';
          prevTempBasal = event;
          break;

...
class MealWizardEstimateEvent extends NGPHistoryEvent {
  ...
  get finalEstimate() {
    // The user can't modify the amount do be delivered in a MealWizard
...
class NGPHistoryParser {
  ...
  * eventsOfTypeReverse(...eventTypes) {
  ...
  findWizardForBolus(bolus, programmedAmount) {
    ...
    for (let i = matchingBolusIndex; i >= 0; i--) {
      ...
      if ((event.eventType === NGPHistoryEvent.EVENT_TYPE.BOLUS_WIZARD_ESTIMATE ||
        ...
        bolus.bolusFromWizard() && event.finalEstimate === programmedAmount) {
        return event;
      }
    }
  ...
  buildTimeChangeRecords() {
    ...
    this.cfg.tzoUtil = new TZOUtil(this.cfg.timezone, lastEvent.timestamp.toDate(),
      timeChangeEvents);
  ...
  buildWizardRecord(wizardEvent, bolusRecord) {
    ...
    const wizard = this.cfg.builder.makeWizard()
      ...
      .with_bgTarget(wizardEvent.eventSource === NGPUtil.NGPConstants.BOLUS_SOURCE.BOLUS_WIZARD ? {

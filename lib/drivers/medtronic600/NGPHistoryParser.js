...
class NGPHistoryParser {
  ...
  * eventsOfTypeReverse(...eventTypes) {
  ...
  // as an identifier.
  findMatchingBolusProgrammedEvent(bolus) {
    const matchingBolusIndex = _.findIndex(this.events, event => event === bolus);

    return _.findLast(
      this.events, event => event.eventType === bolus.eventType - 199 &&
  ...
  buildBGRecords(events) {
    ...
    for (const event of [...this.eventsOfType(NGPHistoryEvent.EVENT_TYPE.BG_READING),
      ...
      if (event instanceof ClosedLoopBloodGlucoseReadingEvent) {
        debug(`*** SMBG: ${event.timestamp.toDate().toISOString()}, ${event.bgValue}, Origin: ${event.bgReadingOrigin}, Context: ${event.bgReadingContext}`);
        ...
        debug(`*** SMBG: ${event.timestamp.toDate().toISOString()}, ${event.bgValue}, Linked: ${event.bgLinked}, Calibration: ${event.calibrationFlag}`);
        reading = this.cfg.builder.makeSMBG()
          .with_units('mg/dL')
          .with_subType(event.bgLinked ? 'linked' : 'manual')
          .with_value(event.bgValue);
  ...
  buildTempBasalRecords(events) {
    for (const event of this.eventsOfType(NGPHistoryEvent.EVENT_TYPE.TEMP_BASAL_COMPLETE)) {
      ...
      if (!_.isUndefined(startTempBasal) && !_.isUndefined(suppressedBasal)) {
        ...
        if (event.canceled) {
          basal
            .with_duration(event.timestamp.toDate().valueOf()
              - startTempBasal.timestamp.toDate().valueOf());
        }
  ...
  buildNormalBolusRecords(events) {
    for (const event of this.eventsOfType(NGPHistoryEvent.EVENT_TYPE.NORMAL_BOLUS_DELIVERED)) {
      ...
      // take a few minutes.
      const programmedBolus = this.findMatchingBolusProgrammedEvent(event);
      const beginTimestamp = _.isUndefined(programmedBolus) ? event.timestamp :
        programmedBolus.timestamp;
  ...
  buildWizardRecord(wizardEvent, bolusRecord) {
    ...
    if (wizardEvent.eventType === NGPHistoryEvent.EVENT_TYPE.BOLUS_WIZARD_ESTIMATE) {

...
class Medtronic600Simulator {
        ...
        if (event.time > this.currBasal.basalEndTime) {
          ...
          if (this.currBasal.suppressed.type !== event.type ||
            ...
            const restoredBasal = this.config.builder.makeScheduledBasal()
              ...
              .set('index', newDeviceTimestamp.rtc)
              .set('jsDate', newDeviceTimestamp.toDate());
            this.config.tzoUtil.fillInUTCInfo(restoredBasal, newDeviceTimestamp.toDate());
            ...
            event
              ...
              .set('index', newDeviceTimestamp.rtc)
              .set('jsDate', newDeviceTimestamp.toDate());
            this.config.tzoUtil.fillInUTCInfo(event, newDeviceTimestamp.toDate());
          }
  ...
  applySuspendedBasals() {
    ...
    for (const entry of orderedBasals.entries()) {
      ...
      if (suppressedBasal != null) {
        console.log('*** SUPRESSED BASAL');
        console.log(suppressedBasal);
        console.log('*** ACTIVE SUSPENDED BASAL');
        console.log(activeSuspendedBasal);
        if (Date.parse(suppressedBasal.basalEndTime) > Date.parse(activeSuspendedBasal.time)) {
          if (Date.parse(activeSuspendedBasal.time) >= Date.parse(suppressedBasal.time)) {
            suppressedBasal.duration = Date.parse(activeSuspendedBasal.time) -
              Date.parse(suppressedBasal.time);
            console.log('*** NEW DURATION');
            console.log(suppressedBasal.duration);
          } else {
            console.log('*** SYNTHESIZE MORE THINGS');
            // The suspend event is the first basal event in the history.
            // Because we don't know what it's suspending, match it to the beginning of the
            // first suspended event.
            activeSuspendedBasal.time = suppressedBasal.time;
            activeSuspendedBasal.deviceTime = suppressedBasal.deviceTime;
            activeSuspendedBasal.duration -=
              (Date.parse(suppressedBasal.time).valueOf() -
               Date.parse(activeSuspendedBasal.time).valueOf());
            this.removeEvent(suppressedBasal);
          }
        }

        // It's possible for suppressed basal duration to get down to 0 here (which is fine).
        ...
        if (Date.parse(activeSuspendedBasal.basalEndTime) >
          ...
          activeSuspendedBasal.deviceTime = sundial.formatDeviceTime(
            sundial.parseFromFormat(activeSuspendedBasal.deviceTime).getTime() +

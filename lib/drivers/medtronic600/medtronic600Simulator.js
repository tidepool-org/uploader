...
class Medtronic600Simulator {
  ...
  static removeLogIndices(modifiedBasal) {
    if (modifiedBasal.payload) {
      _.unset(modifiedBasal.payload.logIndices);
      if (_.isEmpty(modifiedBasal.payload)) {
        _.unset(modifiedBasal.payload);
      }
    }
  }
  ...
  applySuspendedBasals() {
    ...
    for (const basal of orderedBasals) {
      if (basal.deliveryType === 'suspend') {
        ...
        if (activeBasal) {
          ...
          if (activeSuspend.basalEndTime < activeBasal.basalEndTime) {
            ...
            restoredBasal.deviceTime =
              sundial.formatDeviceTime(sundial.parseFromFormat(activeSuspend.deviceTime).getTime() +
              activeSuspend.duration);
            Medtronic600Simulator.removeLogIndices(restoredBasal);
        ...
        // (ie, first event is a suspend), modify it
        ...
        if (activeSuspend) {
          if (activeSuspend.time < activeBasal.time) {
            ...
            Medtronic600Simulator.removeLogIndices(restoredSuspend);

            if (restoredSuspend.duration < 0) {
              throw new Error('Should never get a duration of less than zero');
            } else if (restoredSuspend.duration > 0) {

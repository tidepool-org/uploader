...
class Medtronic600Simulator {
  ...
  basal(event) {
    ...
    if (this.currBasal != null) {
      ...
      if (this.currBasal.scheduleName === 'Auto-Basal' && event.scheduleName === 'Auto-Basal') {
        const autoBasalDifference = Date.parse(event.time).valueOf() -
          Date.parse(this.currBasal.time).valueOf();
        if (autoBasalDifference > 600000) {
          this.currBasal
            .set('duration', 300000)
            .set('basalEndTime', new Date(Date.parse(this.currBasal.time).valueOf() + 300000).toISOString());
          const currBasalIndex = this.currBasal.index;
          this.events.push(this.currBasal.done());

          const insertedBasalDate = new Date(Date.parse(this.currBasal.time).valueOf() + 300000);
          const insertedBasal = this.config.builder.makeScheduledBasal()
            .with_time(insertedBasalDate.toISOString())
            .with_deviceTime(sundial.formatDeviceTime(
            ...
            .with_clockDriftOffset(this.currBasal.clockDriftOffset)
            .with_timezoneOffset(this.currBasal.timezoneOffset)
            .set('index', currBasalIndex + 300000)
            .set('jsDate', insertedBasalDate)
      ...
      if (this.currBasal.deliveryType === 'temp') {
        ...
        if (event.time > this.currBasal.basalEndTime) {
          ...
          const newDeviceTimestamp = new NGPUtil.NGPTimestamp(
            currentNgpTimestamp.rtcFromDate(sundial.parseFromFormat(
              sundial.parseFromFormat(this.currBasal.deviceTime).getTime() +

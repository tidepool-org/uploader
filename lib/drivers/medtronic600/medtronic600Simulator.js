    ...
    if (this.currBasal != null) {
      ...
      if (this.currBasal.scheduleName === 'Auto-Basal') {
        ...
        if (autoBasalDifference > 360000) {
          ...
          const insertedBasalTime = sundial.parseFromFormat(
            Date.parse(this.currBasal.time).valueOf() + 300000);

          const newDeviceTimestamp = new NGPUtil.NGPTimestamp(
            currentNgpTimestamp.rtcFromDate(sundial.parseFromFormat(
              sundial.parseFromFormat(this.currBasal.deviceTime).valueOf() +
              300000)), currentNgpTimestamp.offset);

          const insertedBasal = this.config.builder.makeScheduledBasal()
            .with_time(insertedBasalTime.toISOString())

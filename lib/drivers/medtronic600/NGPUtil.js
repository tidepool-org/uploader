...
export class NGPLinkCipher {
  ...
  static encrypt(key, iv, clear) {
    const cipher = new aesjs.ModeOfOperation.cfb(key, iv);
    const encrypted = cipher.encrypt(clear);
    // const cipher = crypto.createCipheriv('aes-128-cfb', key, iv);
    // let encrypted = cipher.update(clear, 'binary', 'hex');
    // encrypted += cipher.final('hex');
    // const encryptedHex = aesjs.utils.hex.fromBytes(encrypted);
    return Buffer.from(encrypted, 'binary');
  }

  static decrypt(key, iv, encrypted) {
    const cipher = new aesjs.ModeOfOperation.cfb(key, iv);
    const clear = cipher.decrypt(encrypted);
    ...
    // const decryptedHex = aesjs.utils.hex.fromBytes(clear);
    return Buffer.from(clear, 'binary');
  }
}

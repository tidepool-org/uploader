...
class BCNLMessage {
  ...
  async send(hidDevice, readTimeout = BCNLMessage.READ_TIMEOUT_MS, sendDelay = 0) {
...
class NGPRequest extends NGPMessage {
  ...
  async send(hidDevice, readTimeout = NGPMessage.READ_TIMEOUT_MS, sendDelay = 0,
    get80response = true) {
    await this.sendMessage(hidDevice);
    await Timer.wait(sendDelay); // TODO - build wait into sendMessage?
...
class TransmitPacketRequest extends NGPRequest {
  ...
  // Override send(), because we need to be able to discard rogue HIGH_SPEED_MODE_COMMAND responses
  send(hidDevice, readTimeout = NGPMessage.READ_TIMEOUT_MS, sendDelay = 0,
    get80response = true) {
    // Can't use async/await and `super` here, because https://github.com/babel/babel/issues/3930
    // So for now, we need to wrap this in a Promise ourselves
    return new Promise((resolve) => {
      ...
      super.send(hidDevice, readTimeout, sendDelay, get80response)
        .then((responseMessage) => {
          ...
          // TODO - need while? Can there be more than one unsolicited HIGH_SPEED_MODE_COMMAND?
          if (responseMessage && responseMessage.comDCommand ===
            TransmitPacketRequest.COM_D_COMMAND.HIGH_SPEED_MODE_COMMAND) {
            debug('*** SKIPPING UNSOLICITED HIGH_SPEED_MODE RESPONSE', responseMessage);
            this.readMessage(hidDevice, readTimeout)
              .then((receiveMessageResponse) => {
                resolve(new ResponseClass(receiveMessageResponse, this.pumpSession));
              });
          } else {
...
class ReadHistoryCommand extends TransmitPacketRequest {
  ...
  async processSegment(packetNumber, payload, hidDevice) {
    ...
    if (payload != null &&
      ...
      try {
        await new AckMultipacketCommand(this.pumpSession,
...
class MM600SeriesDriver extends BCNLDriver {
  ...
  async getBolusWizardSettings() {
    ...
    response =
      await new BolusWizardSensitivityFactorsCommand(this.pumpSession).send(this.hidDevice);
    ...
    fetchData(progress, data, cb) {
      ...
      driver.getCnlInfo()
        ...
        .then(() => {
          ...
          // const threeMonthsAgo = new Date(new Date().valueOf() - 181e6); // Three weeks

    ...
    while (fetchMoreData) {
      try {
          ...
          case TransmitPacketRequest.COM_D_COMMAND.MULTIPACKET_SEGMENT_TRANSMISSION:
            {
              ...
              if (this.multipacketSession.payloadComplete()) {
                ...
                try {
                  ...
                  debug('Didn\'t get a HIGH_SPEED_MODE_COMMAND');
                  debug(err);
                }
              } else {
                fetchMoreData = true;
        ...
        if (err instanceof TimeoutError) {
          // TODO: Check that we're in an a Multipacket Session before doing anything
          debug(`Got timeout waiting for message. On retry ${this.ngpRetries}`);
          if (this.multipacketSession.retransmitNeeded()) {
          ...
          if (this.ngpRetries >= NGPRequest.MAX_RETRIES) {
            debug('Exceeded retries for NGPMessage');
            this.ngpRetries = 0;
            fetchMoreData = false;
          }
        }

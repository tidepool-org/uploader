...
class BasicNgpParametersResponse extends TransmitPacketResponse {
  get isBolusWizardEnabled() {
    return this.comDPayload[0x34];
  }

  get isExtendedBolusEnabled() {
    return this.comDPayload[0x3];
  }

  get maxBolusAmount() {
    return this.comDPayload.readUInt32BE(0x11);
  }

  get durationOfInsulinAction() {
    return this.comDPayload.readUInt16BE(0x21);
  }
}
...
class MM600SeriesDriver extends BCNLDriver {
  ...
  async getBolusWizardSettings() {
    ...
    settings.bolus = {
      calculator: response.isBolusWizardEnabled,
      combination: response.isExtendedBolusEnabled,
      amountMaximum: {
        value: response.maxBolusAmount,
        units: 'Units',
      },
    };
    settings.basal = {
      ...
      rateMaximum: {
        value: response.maxBasalAmount,
        units: 'Units/hour',
      },
    };
    settings.insulin = {
      duration: (response.durationOfInsulinAction / 60.0).toFixedNumber(1),
      units: 'hours',
    };

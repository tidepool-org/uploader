...
class MinimedPumpSession {
  ...
  async getHMAC() {
    ...
    const digest = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(`${this.bcnlSerialNumber}${paddingKey}`));
    const bytes = (new Uint8Array(digest)).reverse();
    return bytes;
...
class OpenConnectionRequest extends MinimedRequest {
  constructor(pumpSession, asyncHMAC) {
    if (typeof asyncHMAC === 'undefined') {
      throw new Error('Cannot be called directly');
    }
    super(
      MinimedMessage.COMMAND_TYPE.OPEN_CONNECTION, pumpSession, asyncHMAC,
      OpenConnectionResponse,
    );
  }

  static async build(pumpSession) {
    const asyncHMAC = await pumpSession.getHMAC();
    return new OpenConnectionRequest(pumpSession, asyncHMAC);
  }
}
...
class CloseConnectionRequest extends MinimedRequest {
  constructor(pumpSession, asyncHMAC) {
    ...
    super(
      MinimedMessage.COMMAND_TYPE.CLOSE_CONNECTION, pumpSession, asyncHMAC,
  ...
  static async build(pumpSession) {
    const asyncHMAC = await pumpSession.getHMAC();
    return new CloseConnectionRequest(pumpSession, asyncHMAC);
  }
}
...
class MM600SeriesDriver extends BCNLDriver {
  ...
  async openConnection() {
    const req = await OpenConnectionRequest.build(this.pumpSession);
    await req.send(this.hidDevice);
  }

  async closeConnection() {
    const req = await CloseConnectionRequest.build(this.pumpSession);
    await req.send(this.hidDevice);
  }

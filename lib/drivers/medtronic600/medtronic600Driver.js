...
class AdHocPairingCommand extends TransmitPacketRequest {
  ...
  async send(hidDevice, readTimeout = NGPRequest.READ_TIMEOUT_MS) {
    ...
    while (receivedEndPairingCommand !== true) {
      ...
      // then something has gone wrong with the key exchange sequence, and we're
      // not decypting the messages properly.
  ...
  return {
    ...
    getConfigInfo(progress, data, cb) {
      ...
      (async () => {
        ...
        // be 1. If it's not, then we have a decyption error, and something is wrong with the
        // pump link (either paired, or ad hoc)
        if (pumpTime.sequenceNumber !== 1) {
          throw new Error('Could not decrypt message from the pump. Either your pump or meter are not correctly linked');
        }

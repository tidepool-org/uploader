...
class RequestLinkKeyResponse extends MinimedResponse {
  ...
  linkKey(cnlModelAndSerial) {
    return NGPUtil.NGPLinkKeyCipher.unpackLinkKey(this.packedLinkKey, cnlModelAndSerial);
  }
}
...
class MultipacketSession {
  ...
  addSegment(segment) {
    ...
    if (this.packetsToFetch === this.segmentsFilled) {
      // TODO: Should this throw, or just debug?
...
class TransmitPacketRequest extends NGPRequest {
  ...
  async send(hidDevice, readTimeout = NGPRequest.READ_TIMEOUT_MS, get80response = true) {
    ...
    if (get80response) {
      ...
      // If we get a FORCE_TIME_CHANGE_REQUEST or HIGH_SPEED_COMMAND, peel it off and try again.
      if (response.comDCommand === TransmitPacketRequest.COM_D_COMMAND.FORCE_TIME_CHANGE_REQUEST ||
        response.comDCommand === TransmitPacketRequest.COM_D_COMMAND.HIGH_SPEED_MODE
      ) {
        debug(`Ignoring response 0x${_.toUpper((response.comDCommand + 0x10000).toString(16).substr(-4))}`);
        response = await this.read80Message(hidDevice, readTimeout);
      }
    }
...
class DeviceCharacteristicsCommand extends TransmitPacketRequest {
  // FIXME: Can I get rid of this after AdHoc works?
    ...
    while (receivedEndPairingCommand !== true) {
      ...
      switch (response.comDCommand) {
        ...
        case TransmitPacketRequest.COM_D_COMMAND.SET_LINK_KEY:
        {
          ...
          debug(`Temp link key: ${tempLinkKey.toString('hex')}`);
          const tempPackedLinkKey = NGPUtil.NGPLinkKeyCipher
            .packLinkKey(tempLinkKey, this.pumpSession.bcnlSerialNumber, 55);
          debug(`Temp packed link key: ${tempPackedLinkKey.toString('hex')}`);
          await Timer.wait(TransmitPacketRequest.SEND_DELAY_MS);
...
class MM600SeriesDriver extends BCNLDriver {
  ...
  setPumpMACFromSerial(serialNumber) {
    ...
    // TODO: should we use packedLinkMAC and packedPumpMAC?

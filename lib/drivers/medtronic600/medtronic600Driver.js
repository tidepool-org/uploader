...
class BCNLMessage {
  ...
  async readMessage(hidDevice, readTimeout = BCNLMessage.READ_TIMEOUT_MS) {
    ...
    do {
      ...
      const rawData = await hidDevice.receiveAsync();
      // FIXME: put timeouts back
...
class NGPRequest extends NGPMessage {
  ...
  static get READ_TIMEOUT_MS() {
    // TODO: this should be 256, but breaks things.
    return 4000; // Timeout for a NGPMessage is different than for Bayer
  }
        ...
        case TransmitPacketRequest.COM_D_COMMAND.INITIATE_MULTIPACKET_TRANSFER:
          {
            ...
            await new AckCommand(this.pumpSession, response).send(hidDevice);
            fetchMoreData = true;
            break;
          }
        case TransmitPacketRequest.COM_D_COMMAND.MULTIPACKET_SEGMENT_TRANSMISSION:
          {
            ...
            if (this.multipacketSession.payloadComplete()) {
              ...
              fetchMoreData = false;
              // Wait 256ms to try and get to the "right" state.
              // TODO: Should be NGPRequest.READ_TIMEOUT_MS
              await Timer.wait(256);
...
class AckCommand extends TransmitPacketRequest {
  constructor(pumpSession, message) {
    const params = Buffer.alloc(2);
    // 0x00 means 'no error'. TODO: add Error Consts?
    params.writeUInt16BE(message.comDCommand, 0x00);
    ...
    fetchData(progress, data, cb) {
      ...
      driver.getCnlInfo()
        ...
        .then(() => {
          ...
          const threeMonthsAgo = new Date(new Date().valueOf() - 3629e6);

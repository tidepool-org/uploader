    ...
    do {
      try {
        ...
        const rawData = await Promise.race(
          [
            // hidDevice.receiveAsyncWithTimeout(readTimeout),
      ...
      if (packet.length > 0) {
        ...
        message = Buffer.concat([message, packet.slice(BCNLMessage.HEADER_SIZE)],
          message.length + size);
      }
  ...
  async read80Message(hidDevice, readTimeout) {
    ...
    while (fetchMoreData) {
      try {
        ...
        // } else if (this.ngpRetries === NGPRequest.MAX_RETRIES) {

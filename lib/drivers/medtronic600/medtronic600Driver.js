    ...
    do {
      try {
        ...
        const rawData = await Promise.race([
          hidDevice.receiveAsync(),
          Timer.timeout(readTimeout),
        ]);
        ...
        if (this.ngpRetries > NGPRequest.MAX_RETRIES) {
          ...
          if (this.multipacketSession && this.multipacketSession.retransmitNeeded()) {
            ...
            if (nextMissingSegment !== undefined) {
              ...
              await Timer.wait(TransmitPacketRequest.MULTIPACKET_ACK_DELAY_MS);
              await new MultipacketResendPacketsCommand(this.pumpSession,
...
class ReadHistoryCommand extends TransmitPacketRequest {
  ...
  async send(hidDevice, readTimeout = 1000) {
...
class MM600SeriesDriver extends BCNLDriver {
  ...
  async enterRemoteCommandMode() {
    ...
    this.pumpSession = new MinimedPumpSession(infoResponse.getModelAndSerial());
  ...
  async getPumpTime() {
    return await new PumpTimeCommand(this.pumpSession).send(this.hidDevice);
  }

  async getPumpStatus() {
    return await new PumpStatusCommand(this.pumpSession).send(this.hidDevice);
  }
  ...
  async getDeviceCharacteristics() {
    return await new DeviceCharacteristicsCommand(this.pumpSession).send(this.hidDevice);
  }

  async getDeviceString() {
    return await new DeviceStringCommand(this.pumpSession).send(this.hidDevice);
  }
...
module.exports = (config) => {
  ...
  return {
    ...
    getConfigInfo(progress, data, cb) {
      ...
      // Wrap in async, because this function is not async, and we're dealing with other async functions
      (async () => {
        // The enter/exit/enter paradigm is what CareLink does, according to packet captures.
        try {
          await driver.enterRemoteCommandMode();
        } catch (error) {
          debug('Timeout whilst enabling remote commands. Ignoring');
        }
        ...
        settings.activeSchedule = NGPUtil.NGPConstants.BASAL_PATTERN_NAME[status.activeBasalPattern - 1];
        ...
        // We pass a null timezone, because settings.currentDeviceTime is already set to the correct TZ above
        common.checkDeviceTime(settings.currentDeviceTime, null, (err) => {
          progress(100);
          cb(err, data);
        })
      })().catch((error) => {
    ...
    fetchData(progress, data, cb) {
      ...
      // const threeMonthsAgo = new Date(new Date().valueOf() - 1210e6);
      let lastPosition = threeMonthsAgo;
      ...
      if (_.has(cfg, 'lastUpload.client.private.delta')) {
        ...
        // Always get at least a day back from today, in case we need to ammend any
      ...
      (async () => {
        ...
        const history = await driver.readHistory(
            ...
            NGPUtil.NGPTimestamp.maxRTC, progress, 0, 90);
    ...
    cleanup(progress, data, cb) {
      ...
      if (isBrowser) {
        ...
        (async () => {
          ...
          debug('Error which cleaning up: ', error);
          cb(error, null);
        });
      } else {
        progress(100);

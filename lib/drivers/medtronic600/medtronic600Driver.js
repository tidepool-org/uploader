...
class BCNLMessage {
  ...
  async sendMessage(hidDevice) {
    ...
    while (pos < message.length) {
      ...
      await hidDevice.send(bytes.buffer.slice(), () => Promise.resolve());
      pos += sendLength;
    }
  }
  ...
  async send(hidDevice, readTimeout = BCNLMessage.READ_TIMEOUT_MS, sendDelay = 0) {
    ...
    await BCNLMessage.wait(sendDelay);
...
class NGPRequest extends NGPMessage {
  ...
  async send(hidDevice, readTimeout = NGPMessage.READ_TIMEOUT_MS, sendDelay = 0,
    get80response = true) {
    await this.sendMessage(hidDevice);
    await BCNLMessage.wait(sendDelay); // TODO - build wait into sendMessage?

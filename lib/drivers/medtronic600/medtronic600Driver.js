...
class BCNLMessage {
  ...
  async readMessage(hidDevice, readTimeout = BCNLMessage.READ_TIMEOUT_MS) {
    ...
    do {
      ...
      /*
      const rawData = hidDevice.readTimeout(readTimeout);
      if (rawData.length === 0) {
        throw new TimeoutError('Timer timeout.');
      }
      */
      ...
      try {
        ...
        if (err instanceof TimeoutError) {
          ...
          */
          ...
          /* } else if (this.multipacketSession && this.multipacketSession.payloadComplete()) {
            debug('*** Multisession Complete');
            fetchMoreData = false;
            // TODO: do we need to rebuild response.payload? Or add the Checksum?
            response.decryptedPayload = this.multipacketSession.sessionPayload;
            ...
            await new AckCommand(this.pumpSession,
              TransmitPacketRequest.COM_D_COMMAND.MULTIPACKET_SEGMENT_TRANSMISSION)
              .send(hidDevice); */
          } else if (this.ngpRetries === 2 && this.multipacketSession && this.multipacketSession.retransmitNeeded()) {
            debug('*** Multisession missing segments');
            // TODO: `tuple` thing could be tidier.
            const tuple = this.multipacketSession.missingSegments[0];
            if (tuple !== undefined) {
...
class ReadHistoryInfoCommand extends TransmitPacketRequest {
  constructor(pumpSession, historyDataType, fromRtc, toRtc) {
    ...
    params[0x01] = 0x03; // Hard coded
    params.writeUInt32BE(0x00, 0x02);
    params.writeUInt32BE(0x00, 0x06);
...
class ReadHistoryCommand extends TransmitPacketRequest {
  constructor(pumpSession, historyDataType, fromRtc, toRtc, expectedSize, progressCb = () => {}) {
    ...
    // params.writeUInt16BE(0x00, 0x0A); // Hard coded
    params[0x01] = 0x03; // 0x03 = Full history. 0x04 = partial history.
    params.writeUInt32BE(0x00, 0x02); // If full history, fromRtc should be 0
    params.writeUInt32BE(0x00, 0x06); // If full history, toRtc should be 0
    params.writeUInt16BE(0x00, 0x0A); // Hard coded
      ...
      switch (response.comDCommand) {
        case TransmitPacketRequest.COM_D_COMMAND.END_HISTORY_TRANSMISSION:
        {
          ...
          if (this.bytesFetched < this.expectedSize) {
            ...
            debug('*** READ OFF EXTRA MESSAGE');
            // Discard remaining messages until we get an End High Speed Mode Command
            let gotEndHighSpeedMode = false;
            do {
              const discardableResponse = await this.readMessage(hidDevice, readTimeout);
              const discardableMessage = new TransmitPacketResponse(discardableResponse, this.pumpSession);
    ...
    fetchData(progress, data, cb) {
      ...
      driver.getCnlInfo()
        ...
        .then(() => {
          ...
          const oneMonthAgo = new Date(new Date().valueOf() - 2628e6);
          return driver.readHistory(
            settings.currentNgpTimestamp.rtcFromDate(twoMonthsAgo),
            // settings.currentNgpTimestamp.rtcFromDate(oneMonthAgo), progress, 30, 90);

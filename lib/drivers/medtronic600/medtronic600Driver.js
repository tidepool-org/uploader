...
class BCNLMessage {
  ...
  // TODO - should this be in BCNLCommand?
  ...
  async readMessage(hidDevice, readTimeout = BCNLMessage.READ_TIMEOUT_MS) {
    const abortTimer = new Timer(readTimeout).start();
    let message = Buffer.alloc(0);
    let size = 0;

    do {
      // eslint-disable-next-line no-await-in-loop, requests to devices are sequential
      const rawData = await new Promise(resolve => hidDevice.receive(resolve));
      // const rawData = hidDevice.receiveTimeout(readTimeout);
...
class MultipacketSession {
  ...
  // TODO - just take Message as an argument?
  addSegment(segment) {
  ...
  retransmitNeeded() {
    return this.segments[this.lastPacketNumber] !== undefined && !this.payloadComplete();
  }
}
...
class TransmitPacketRequest extends NGPRequest {
  ...
  async send(hidDevice, readTimeout = NGPRequest.READ_TIMEOUT_MS, get80response = true) {
    ...
    while (fetchMoreData && get80response) {
      debug('READING MORE');
      const responseMessage = await this.readMessage(hidDevice, readTimeout);
      ...
      switch (response.comDCommand) {
        case TransmitPacketRequest.COM_D_COMMAND.HIGH_SPEED_MODE_COMMAND:
          {
            debug('*** SKIPPING UNSOLICITED HIGH_SPEED_MODE RESPONSE', response);
            fetchMoreData = true;
            break;
          }
        case TransmitPacketRequest.COM_D_COMMAND.INITIATE_MULTIPACKET_TRANSFER:
          {
            debug('*** INITIATE_MULTIPACKET_TRANSFER');
        ...
        case TransmitPacketRequest.COM_D_COMMAND.MULTIPACKET_SEGMENT_TRANSMISSION:
          {
            ...
            if (this.multipacketSession.payloadComplete()) {
              ...
              fetchMoreData = false;
              debug(this.multipacketSession.sessionPayload.toString('hex'));
              ...
              // Send ACK
              await new AckCommand(this.pumpSession, response.comDCommand).send(hidDevice);
              debug(`READ HIGH_SPEED_MODE`);
              // We need to read another HIGH_SPEED_MODE_COMMAND, otherwise the CNL gets frozen.
              // Why is this needed, Medtronic?
              await this.readMessage(hidDevice, 4000);
            } else if (this.multipacketSession.retransmitNeeded()) {
              ...
              if (tuple !== undefined) {
                ...
                // TODO - should never get here, because `retransmitNeeded`
                fetchMoreData = false;
              }
            } else {
              debug('*** Multisession needs more segments');
              fetchMoreData = true;
            }
            break;
          }
...
class AckCommand extends TransmitPacketRequest {
  // TODO - change ackForType to whole response, and use response.comDCommand
  constructor(pumpSession, ackForType) {
    const params = Buffer.alloc(2);
    params.writeUInt16BE(ackForType, 0x00); // 0x00 means 'no error'. TODO - add Error Consts?
...
class ReadHistoryCommand extends TransmitPacketRequest {
  ...
  async processSegment(packetNumber, payload, hidDevice) {
    ...
    if (payload != null &&
      ...
      if (segmentPayload.length !== this.sessionSize) {
        throw new InvalidMessageError('Total segment size mismatch');
      }
      ...
      if (segmentPayload.readUInt16BE(0x00) === 0x030E) {
        ...
        throw new InvalidMessageError('Unknown history response message type');
      }
      ...
      try {
        ...
        debug('Error sending AckCommand');
        throw err;
      }
    }
  }
  ...
  send(hidDevice, readTimeout = NGPRequest.READ_TIMEOUT_MS, get80response = false) {
    return new Promise((resolve, reject) => {
      super.send(hidDevice, readTimeout, get80response)
        .then(() => {
          async.whilst(
            () => this.fetchMoreData(),
            (callback) => {
              this.readMessage(hidDevice, readTimeout)
                .then((receiveMessageResponse) => {
                  const response = new MultipacketSegmentResponse(receiveMessageResponse, this.pumpSession);
                    ...
                    case TransmitPacketRequest.COM_D_COMMAND.INITIATE_MULTIPACKET_TRANSFER:
                      {
                        ...
                        // Acknowledge that we're ready to start receiving data.
                        new AckCommand(this.pumpSession, response.comDCommand).send(hidDevice)
    ...
    fetchData(progress, data, cb) {
      ...
      driver.getCnlInfo()
        ...
        .then(() => {
          ...
          // 3 days
          const threeMonthsAgo = new Date(new Date().valueOf() - 2592e5);

...
module.exports = function (config) {
  ...
  var DMSetDateTime = DM.concat('DMT');
   // hex code '\x44\x4d\x3f'
   // hex code '\x44\x4d\x40'
  var DMUpload = DM.concat('DMP'); // hex code '\x44\x4d\x50';
   // hex code '\x44\x4d\x53\x55\x3f'
   // hex code '\x44\x4d\x53\x54\x3f'
  ...
  return {
    ...
    getConfigInfo: function (progress, data, callback) {
      ...
      async.retry({ times: RETRIES, interval: TIME_TO_WAIT }, sendReceive, function(_err, _result) {

        common.checkDeviceTime(cfg, function(err, serverTime) {
          if (err == 'updateTime') {
            cfg.deviceInfo.annotations = 'wrong-device-time';
            debug('Time set to');
            setDateTime(serverTime, progress, function(err2, _result) {
              if (err2) {

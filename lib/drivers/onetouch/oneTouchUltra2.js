...
module.exports = function (config) {
  ...
  cfg.deviceTags = ['bgm'];
  cfg.deviceManufacturers = ['LifeScan'];
  cfg.deviceModel = 'OneTouch Ultra 2';
  ...
  return {
      ...
      var sendReceive = function(cb) {
        otu2CommandResponse(DMDateTime, progress, function(err, result){
          if (err) {
            ...
            data.deviceTime = sundial.formatDeviceTime(buildDateTime(toMatch[2],  toMatch[3]));
            debug('Device date/time:', data.deviceTime);
    ...
    processData: function (progress, data, cb) {
      // processing data in previous step so that we can retry easier

...
class USBScsiDevice {
  ...
  openDevice(callback) {
    ...
    if (!this.device) {
      return callback(new Error(`Failed to open connection to ${DEVICE_MODEL_NAME}`));
    }
    this.device.open();
...
class BlockDevice {
  ...
  openDevice(callback) {
    ...
    drivelist.list((err, drives) => {
      ...
      if (this.devicePath) {
        ...
        // TODO: On Windows we need to open this device with the following flags:
        // FILE_FLAG_WRITE_THROUGH | FILE_FLAG_NO_BUFFERING
        // Otherwise writing to the device on Windows produces a permission error.
        // For this we need to wait for Node v8 and check that it is using libuv with this PR:
        // https://github.com/libuv/libuv/pull/1567
        // which will set the above flags if we fs.open using the following flags:
        // UV_FS_O_DIRECT | UV_FS_O_DSYNC | UV_FS_O_SYNC
        ...
        callback(new Error(`Could not find device "${DEVICE_MODEL_NAME}".`));
      }
    });
  }
  ...
  checkDevice(callback) {
    const readBuffer = Buffer.alloc(3 * BLOCKDEVICE_BLOCKSIZE);
  ...
  retrieveData(lbaNumber, requestData, callback) {
    ...
    fs.write(this.fileHandle, requestData, 0, BLOCKDEVICE_BLOCKSIZE, seekOffset,
      (err) => {
        if (err) {
          debug(`retrieveData: Error writing to device '${this.devicePath}':`, err);
          return;
        }
        // debug(`retrieveData: wrote ${numWritten} bytes:`, requestData);
        const readBuffer = Buffer.alloc(BLOCKDEVICE_BLOCKSIZE);
        fs.read(this.fileHandle, readBuffer, 0, BLOCKDEVICE_BLOCKSIZE, seekOffset,
          (err2) => {
            if (err2) {
              debug(`retrieveData: Error reading from device '${this.devicePath}':`, err2);
              return;
            }
            // debug(`retrieveData: read ${numRead} bytes:`, readBuffer);
            callback(readBuffer);
          });
      });
  }
...
class OneTouchVerio {
  ...
  openDevice(callback) {
    return this.communication.openDevice(callback);
  }

...
// eslint-disable-next-line
import direct_io from '@ronomon/direct-io';
...
class BlockDevice {
    ...
    drivelist.list((err, drives) => {
      ...
      drives.forEach((drive) => {
        if (drive.description && drive.description.includes('LifeScan') && !drive.system) {
          if ((process.platform === 'win32') && drive.mountpoints) {
      ...
      if (this.devicePath) {
        ...
        fs.open(this.devicePath, 'rs+', (err2, fd) => {
          if (err2) {
            ...
            direct_io.setFSCTL_LOCK_VOLUME(fd, 1, (err3) => {
              if (err3) {
  ...
  checkDevice(callback) {
    const readBuffer = direct_io.getAlignedBuffer(3 * BLOCKDEVICE_BLOCKSIZE, 4096);
  ...
  retrieveData(lbaNumber, requestData, callback) {
    ...
    fs.write(
      ...
      (err) => {
        ...
        const readBuffer = direct_io.getAlignedBuffer(BLOCKDEVICE_BLOCKSIZE, 4096);
        fs.read(
...
class OneTouchVerio {
  ...
  retrieveQueryData(queryType, callback) {
    const linkRequestData = direct_io.getAlignedBuffer(USB_BULK_BLOCKSIZE, 4096);

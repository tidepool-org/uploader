...
import directIO from '@tidepool/direct-io';
      ...
      try {
        ...
        if (this.devicePath) {
          ...
          fs.open(this.devicePath, 'rs+', (err2, fd) => {
            if (err2) {
              if (err2.message.toLowerCase().includes('operation not permitted') && os.platform() === 'darwin') {
                // eslint-disable-next-line no-param-reassign
                err2.code = 'E_VERIO_ACCESS';
              }
              callback(err2);
        ...
        if (err.message.toLowerCase().includes('operation not permitted') && os.platform() === 'darwin') {
          err.code = 'E_VERIO_ACCESS';
        }
        callback(err);
      }
    })();
      ...
      fs.write(
        ...
        (err) => {
          if (err) {
            ...
            let readBuffer = Buffer.alloc(USB_BULK_BLOCKSIZE);
            if (!env.browser) {
              readBuffer = directIO.getAlignedBuffer(USB_BULK_BLOCKSIZE, 4096);
            }
            fs.read(
...
class ElectronMessaging {
  constructor() {
    ipcRenderer.on('native-reply', (event, reply) => {
      console.log('Reply from Zig:', reply);
    });
  }

  static sendToNative(msg) {
    ipcRenderer.send('native-message', {
      command: msg
    });
  }

  openDevice(deviceInfo, callback) {
    ElectronMessaging.sendToNative('getAppVersion');
    callback(null);
  }
}
...
class OneTouchVerio {
  constructor(cfg) {
    ...
    if (process.platform === 'linux') {
      this.communication = new ElectronMessaging(); // new USBScsiDevice();
    } else if (env.browser) {
  ...
  retrieveQueryData(queryType, callback) {
    ...
    if (!env.browser) {
      directIO.getAlignedBuffer(USB_BULK_BLOCKSIZE, 4096);
    }

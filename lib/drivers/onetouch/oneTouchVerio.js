...
class BlockDevice {
  ...
  static async elevateScriptCatalina(path) {
    const escapeSh = (value) => `'${String(value).replace(/'/g, "'\\''")}'`;
      ...
      try {
        ...
        if (this.devicePath) {
          ...
          if (os.platform() === 'darwin' && semver.compare(os.release(), '19.0.0') >= 0) {
            // >= macOS Catalina
            const result = await BlockDevice.elevateScriptCatalina(this.devicePath);
            console.log('Result:', result); // TODO: handle this
          }

    ...
    */
    ...
    getConfigInfo(progress, data, cb) {
      ...
      driver.retrieveQueryData(querySerial, (err, serialNumber) => {
        ...
        driver.retrieveQueryData(queryModel, (err2, deviceModel) => {
          ...
          cfg.deviceModel = deviceModel;
          progress(100);

          driver.retrieveRTCData((rtcTime) => {
            data.deviceTime = sundial.formatDeviceTime(rtcTime);
            common.checkDeviceTime(data.deviceTime, cfg, (timeErr) => {
              if (timeErr) {
                driver.closeDevice(() => {
                  cb(timeErr, data);
                });
    ...
    processData(progress, data, cb) {
      ...
      data.deviceInfo.deviceId =
        `${cfg.deviceModel.replace(/\s+/g, '')}-${data.deviceInfo.serialNumber}`;

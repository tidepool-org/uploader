...
var annotate = require('@tidepool/uploader-common/eventAnnotations');
...
module.exports = function (config) {
  ...
  return {
      ...
      cfg.deviceComms.connect(data.deviceInfo, oneTouchPacketHandler, function(err) {
        ...
        getDeviceInfo({}, function(commsErr, obj) {
          if (commsErr) {
            ...
            getSerialNumber(obj, function (_err, _result) {
              progress(100);
              data.connect = true;
              _.assign(data, obj);
              cb(null, data);
    ...
    getConfigInfo: function (progress, data, cb) {
      getRecordCount({}, function(err, obj) {
        ...
        getUnits(obj, function(err, result) {
          ...
          data.id = 'OneTouch'+ data.deviceModel + '-' + data.serialNumber;
          getRTC(function(err, result) {
            ...
            common.checkDeviceTime(cfg, function(err, serverTime) {
              if (err === 'updateTime') {
                ...
                setRTC(newDateTime / 1000, function(err2, _result2) {
                  progress(100);
    ...
    uploadData: function (progress, data, cb) {
      ...
      cfg.api.upload.toPlatform(data.post_records, sessionInfo, progress, cfg.groupId, function (err, _result) {
        if (err) {
          debug(err);
          progress(100);
          return cb(err, null);

...
const debug = isBrowser ? require('bows')('OneTouchVerioBLE') : console.log;
...
export class VerioBLE extends EventTarget {
  ...
  async scan() {
    ...
    if (typeof navigator !== 'undefined') {
      ...
      self.emit('error', new Error('navigator not available.'));
    }
  }
  ...
  async getNextRecord() {
    self.abortTimer = setTimeout(() => {
      ...
      if (self.readUntil && self.readUntil > 0) {
        ...
        self.emit('error', new Error('Timeout error'));
      }
    }, 2000);
  ...
  async handleNotifications(event) {
    ...
    if (value.byteLength === 1) {
      ...
      if ((value.getUint8(0) & ACK) === ACK) {
        ...
        self.emit('error', new Error(`Invalid packet (${value.getUint8(0)})`));
      }
    } else {
        ...
        if (result.getUint8(0) === 0x06) {
          ...
          if (result.byteLength === 5) {
            ...
            if (val > 100000) {
              ...
              self.emit('dateTime', meterTimestamp);
            } else {
          ...
          if (result.byteLength === 3) {
            self.numberOfRecords = result.getUint16(1, true) + 1;
          ...
          debug(`Invalid packet (${value.getUint8(0)})`);
          if (self.readUntil && self.readUntil > 0) {
            self.readUntil -= 1;
...
export default (config) => {
  ...
  return {
    ...
    fetchData(progress, data, cb) {
      ...
      cfg.deviceComms.ble.on('error', (error) => cb(error, null));
      ...
      cfg.deviceComms.ble.once('dateTime', async (result) => {
        clearTimeout(self.abortTimer);
        cfg.deviceInfo.deviceTime = sundial.formatDeviceTime(result);

...
module.exports = (config) => {
  ...
  const buildBGRecords = (data) => {
    _.forEach(data.records, (record, index) => {
      ...
      let value = units ? raw.reading : raw.reading / 10.0;
      ...
      let annotation = null;
      if ((units === 1 && value > 600) || (units === 0 && value > 33.3)) {
        value = 601;
        annotation = {
      ...
      } else if ((units === 1 && value < 20) || (units === 0 && value < 1.1)) {
        value = 19;
        annotation = {
          code: 'bg/out-of-range',
          value: 'low',
        ...
        common.checkDeviceTime(
          ...
          (timeErr) => {
            ...
            if (timeErr) {
              ...
              /* There's a mismatch between the spec and the meter in terms of updating the
                 device time, so until we get an updated spec we can't update the meter time

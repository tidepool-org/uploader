...
module.exports = (config) => {
  ...
  const verifyChecksum = (record) => {
    ...
    const check = data[1];
    let sum = 0;
    const n = record.slice(0, record.length - 2);
    ...
    listenTimer = setInterval(() => {
      if (serialDevice.hasAvailablePacket()) {
        ...
        if (bytes[0] === ASCII_CONTROL.EOT
            ...
            || _.endsWith(raw, ASCII_CONTROL.ACK)) {
          clearTimeout(abortTimer);
          clearInterval(listenTimer);
          callback(null, raw);
        }
  ...
  const getNumberOfRecords = async () => {
    ...
    const dataPacket = await bcnCommandResponse(buildCmdWithParser(COMMANDS.NROFRECORDS, 2));
  ...
  const getOneRecord = async (index) => {
    const recNumStr = index.toString().padStart(3, '0');
    const recNum = [];
    ...
    await bcnCommandResponse(buildCmd(COMMANDS.READ, 2));
    await bcnCommandResponse(buildCmd(COMMANDS.DATA, 2));
    const result = await bcnCommandResponse(buildCmdWithParser(recNum, 4));
  ...
  const getHeader = async () => {
    ...
    let datatxt = await bcnCommandResponse(cmd);

    if (datatxt[0] === ASCII_CONTROL.ENQ) {
      datatxt = await bcnCommandResponse(cmd);
    }
  ...
  const enterRemoteCommandMode = async () => {
    let response;
    try {
      [response] = await bcnCommandResponse(buildCmd(0x92, 1));

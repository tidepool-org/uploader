...
module.exports = (config) => {
  ...
  const enterRemoteCommandMode = async () => {
    ...
    try {
      ...
      [response] = await bcnCommandResponse(buildCmd(ASCII_CONTROL.EOT, 1));

      if (response !== ASCII_CONTROL.ENQ) {
        throw new Error(`Expected ENQ, got ${response.toString(16)}`);
      }

      [response] = await bcnCommandResponse(buildCmd(ASCII_CONTROL.ACK, 1));
  ...
  const getDeviceInfo = (cb) => {
    ...
    (async () => {
      ...
      const dataPacket = await bcnCommandResponse(buildCmdWithParser([0x00, 0x01], 2));

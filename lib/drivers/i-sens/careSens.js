  ...
  buildPacket(command, payload = []) {
    if (this.protocol === 1) {
      ...
      if (command === COMMAND.READ_SERIAL) {
        const cmd = PROTOCOL_ONE.READ_SERIAL;
    ...
    listenTimer = setInterval(() => {
      if (this.serialDevice.hasAvailablePacket()) {
        ...
        if (this.protocol === 1) {
          ...
          console.log(`expectedLength: ${expectedLength}, received: ${raw.length}`)
          if (raw.length >= expectedLength) {
            ...
            debugger;
            try {
              // return callback(null, CareSens.extractPacketIntoMessages(raw));
            } catch (err) {

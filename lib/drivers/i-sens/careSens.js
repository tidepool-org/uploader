...
const PROTOCOL_ONE = {
  ...
  READ_GLUCOSE_CVP: { address: 0xe200, dataLength: 0x0a }, // TODO: check data length parameter
};
...
class CareSens {
  ...
  listenForPacket(timeout, command, callback) {
    ...
    if (this.protocol === 1) {
      ...
      if (command === COMMAND.READ_SERIAL) {
        ...
        expectedLength = (PROTOCOL_ONE.READ_GLUCOSE_CVP.dataLength - 2) * 3;
      }
    }
  ...
  async getRecords(nrOfRecords) {
    ...
    if (this.protocol === 1) {
      ...
      // TODO: check if all results are received; also HI/LO

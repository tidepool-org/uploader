...
class CareSens {
  ...
  async commandResponse(cmd, payload) {
    ...
    // retry after timeout
    if (message.command === ERROR.TIMEOUT.value) {
      retries += 1;

      if (retries < RETRY) {
        debug('Got timeout, retrying..');
        return this.commandResponse(cmd, payload);
      }
    }
  ...
  async setDateTime(dateTime) {
    ...
    const newDateTime = sundial.buildTimestamp(fields);

...
class CareSens {
  ...
  static buildPacket(command, payload = []) {
    ...
    const packetlen = datalen + 6; // adds header, size and STX
    ...
    struct.storeByte(ASCII_CONTROL.ETX, bytes, ctr); // to calculate CRC, overwritten belwo
    const crc = crcCalculator.calcCRC_A(bytes, packetlen - 2);
  ...
  async commandResponse(cmd, payload) {
    ...
    const bytesWritten =
      await this.hidDevice.sendPromisified(CareSens.buildPacket(cmd, payload));

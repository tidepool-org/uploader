...
const PROTOCOL_ONE = {
  READ_SERIAL: { address: 0x1034, dataLength: 0x0a },
  READ_GLUCOSE_CVP: { address: 0xe200, dataLength: 0xff }, // TODO: check data length parameter
};
...
class CareSens {
  constructor(cfg) {
    this.cfg = cfg;
    this.protocol = null;
    if (this.cfg.deviceInfo.driverId === 'ReliOn') {
  ...
  buildPacket(command, payload = []) {
    if (this.protocol === 1) {
      ...
      if (command === COMMAND.READ_SERIAL) {
        cmd = PROTOCOL_ONE.READ_SERIAL;
      } else if (command === COMMAND.GLUCOSE_RESULT) {
        cmd = PROTOCOL_ONE.READ_GLUCOSE_CVP;
      } else {
      ...
      struct.copyBytes(bytes, 1, [
        0x10 | ((cmd.address & 0xF000) >> 12),
        0x20 | ((cmd.address & 0x0F00) >> 8),
        0x10 | ((cmd.address & 0x00F0) >> 4),
        0x20 | (cmd.address & 0x000F),
        0x10 | ((cmd.dataLength & 0xF0) >> 4),
        0x20 | (cmd.dataLength & 0x0F),
      ], 6);
      /* eslint-enable no-bitwise */

      console.log(`Sending ${common.bytes2hex(bytes)}`);
  ...
  listenForPacket(timeout, command, callback) {
    ...
    if (this.protocol === 1) {
      if (command === COMMAND.READ_SERIAL) {
        expectedLength = (PROTOCOL_ONE.READ_SERIAL.dataLength - 2) * 3;
      } else if (command === COMMAND.GLUCOSE_RESULT) {
        expectedLength = PROTOCOL_ONE.READ_GLUCOSE_CVP.dataLength * 3;
      }
    }
    ...
    listenTimer = setInterval(() => {
      if (this.serialDevice.hasAvailablePacket()) {
        ...
        if (this.protocol === 1) {
          ...
          // TODO: determine when done for reading glucose values
  ...
  async getSerialNumber() {
    ...
    if (this.protocol === 1) {
      ...
      const serialNumber = this.model +
                     fields.massProductionCount.toString(16).toUpperCase().padStart(2, '0') +
  ...
  async getRecords(nrOfRecords) {
    ...
    if (this.protocol === 1) {
      const result = await this.commandResponse(COMMAND.GLUCOSE_RESULT);
      let ctr = 0;

      while (ctr <= result.length - 8) {
        if (result[ctr] === 0xFF && result[ctr + 1] === 0xFF) {
          return records;
        }

        // TODO: parse glucose record
      }
    }
...
module.exports = (config) => {
  ...
  if (cfg.deviceInfo.driverId === 'ReliOn') {

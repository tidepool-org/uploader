...
class CareSens {
  ...
  static verifyChecksum(bytes, expected) {
    ...
    const calculated = crcCalculator.ccitt_crc16(bytes);
    // TODO: maybe put crcCalculator.calcCRC_A(bytes, bytes.length) back
  ...
  static extractPacketIntoMessages(bytes) {
    ...
    if (response.command !== COMMAND.GLUCOSE_RESULT) { // FIXME
    ...
    do {
      ...
      if (result.length > 0) {
        ...
        if (foundSTX) {
          ...
          if (byte === ASCII_CONTROL.ETX && raw.length >= (packetSize + HEADER_SIZE)) {
            foundETX = true;
          }
        }
      }
  ...
  async getSerialNumber() {
    ...
    return String.fromCharCode.apply(null, _.dropRight(result.data));
  }
  ...
  async getRecords(nrOfRecords) {
    // TODO: read more than 27 records

    // eslint-disable-next-line no-bitwise
    const count = (nrOfRecords % 27) & 0xFF;
    const index = 1;
    let ctr = 0;
    const records = [];
    ...
    struct.pack(bytes, 0, 'Sb', index, count);
  ...
  async setDateTime(dateTime) {
    ...
    if (!_.isEqual(dateTime, fields)) {
      debug('Set date/time:', dateTime);
      debug('Received date/time:', fields);

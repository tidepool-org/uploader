...
const NGPUtil = require('../medtronic600/NGPUtil');
const NGPHistoryParser = require('../medtronic600/NGPHistoryParser');
const Medtronic600Simulator = require('../medtronic600/medtronic600Simulator');
...
class AckMultipacketCommand extends TransmitPacketRequest {
  ...
  constructor(pumpSession, segmentCommand, startPacket = 0, packetCount = 0) {
    let params = null;

    if (segmentCommand === AckMultipacketCommand.SEGMENT_COMMAND.REPEAT_SEGMENT_PACKETS) {
      params = Buffer.alloc(6);
      params.writeUInt16BE(startPacket, 0x02);
      params.writeUInt16BE(packetCount, 0x04);
    } else {
      params = Buffer.alloc(2);
    }
    params.writeUInt16BE(segmentCommand, 0x00);

    debug('*** Ack params', params.toString('hex'));
...
class ReadHistoryCommand extends TransmitPacketRequest {
  ...
  get missingSegmentKeys() {
    debug('*** MISSING SEGMENT KEYS', this.segments);
    // return this.segments.filter(value => value === undefined);
  ...
  initSession(payload) {
    ...
    for (let i = 0; i < this.packetsToFetch; i++) {
  ...
  retransmitNeeded() {
    debug('*** RETRANSMIT NEEDED?', this.segments[this.lastPacketNumber], this.segmentCount,
      this.packetsToFetch, this.segmentComplete());
    return this.segments[this.lastPacketNumber] !== undefined && !this.segmentComplete();
  }

  processSegment(packetNumber, payload, hidDevice, segmentTimedOut = false) {
    // eslint-disable-next-line no-unused-vars
    return new Promise((resolve, reject) => {
      if (segmentTimedOut ||
        (this.segments[this.lastPacketNumber] !== undefined && !this.segmentComplete())) {
        debug('*** NEED SOME PACKETS RETRANSMITTED! ***', this.missingSegmentKeys);

        const key = this.missingSegmentKeys[0];
        debug('*** REREQUEST SEGMENT', key);
        new AckMultipacketCommand(this.pumpSession,
            AckMultipacketCommand.SEGMENT_COMMAND.REPEAT_SEGMENT_PACKETS, key + 1, 1)
          .send(hidDevice, 15000, 0, true)
          .then((response) => {
            debug('*** GOT MESSAGE', response);
            debug('*** GOT MESSAGE, DECRYPT', response.decryptedPayload);
            resolve(response);
          })
          .catch((err) => {
            debug('*** WHAT HAPPENED?', err);
          });
      } else {
      ...
      super.send(hidDevice, readTimeout, sendDelay, get80response)
        .then(() => {
          async.whilst(
            () => this.fetchMoreData(),
            (callback) => {
              debug('*** DOING THE LOOP');
              this.readMessage(hidDevice, readTimeout)
                .then((receiveMessageResponse) => {
                  const response = new MultiPacketSegment(receiveMessageResponse, this.pumpSession);
                  debug('*** MESSAGE RESPONSE', response.decryptedPayload.toString('hex'));
                ...
                .catch((err) => {
                  ...
                  if (err instanceof TimeoutError) {
                    ...
                    this.processSegment(0, null, hidDevice, true)
                      .then((response) => {
                        debug('*** FINISH PROCESS SEGMENT');
    ...
    getConfigInfo(progress, data, cb) {
      ...
      driver.getCnlInfo()
        ...
        .then(() => driver.readHistory(
          settings.currentNgpTimestamp.rtcFromDate(new Date('2016-07-01T00:00:00')),
          // settings.currentNgpTimestamp.rtcFromDate(new Date('2017-02-01T00:00:00')),
          NGPUtil.NGPTimestamp.maxRTC))
        .then((history) => {

...
const NGPUtil = require('../medtronic/NGPUtil');
const NGPHistoryParser = require('../medtronic/NGPHistoryParser');
const medtronicSimulator = require('../medtronic/medtronicSimulator');
...
class AckMultipacketCommand extends TransmitPacketRequest {
  ...
  constructor(pumpSession, segmentCommand) {
    ...
    params.writeUInt16BE(segmentCommand);
...
class ReadHistoryCommand extends TransmitPacketRequest {
  constructor(pumpSession, historyDataType, fromRtc, toRtc, expectedSize) {
    const params = Buffer.alloc(12);
    params[0x00] = historyDataType;
    params[0x01] = 0x04; // Hard coded in CLPro
    params.writeUInt32BE(fromRtc, 0x02);
  ...
  processSegment(packetNumber, payload, hidDevice) {
    // eslint-disable-next-line no-unused-vars
    return new Promise((resolve, reject) => {
      debug('*** GOT A MULTIPACKET SEGMENT', packetNumber, ', count:', this.segmentCount);
      ...
      if (packetNumber === this.lastPacketNumber &&
        ...
        debug('*** NEED SOME PACKETS RETRANSMITTED, YO! ***', this.missingSegmentKeys);

        new AckMultipacketCommand(this.pumpSession,
            AckMultipacketCommand.SEGMENT_COMMAND.REPEAT_SEGMENT_PACKETS)
          .send(hidDevice)
          .then(() => resolve());
...
class MM600SeriesDriver extends BCNLDriver {
  ...
  // eslint-disable-next-line no-unused-vars, TODO - do we need this timezone?
  readHistory(fromRtc, toRtc, timezone) {
    ...
    return new Promise((resolve, reject) => {
      ...
      new ReadHistoryInfoCommand(this.pumpSession,
        ...
        .then((response) => {
          // FIXME - set expectedSize to 0. The next ReadHistoryCommand command should fail.
...
module.exports = (config) => {
  ...
  return {
    ...
    getConfigInfo(progress, data, cb) {
      ...
      driver.getCnlInfo()
        ...
        .then(() => driver.readHistory(
          settings.currentNgpTimestamp.rtcFromDate(new Date('2017-01-20T00:00:00')),
          NGPUtil.NGPTimestamp.maxRTC, cfg.timezone))
        .then((history) => {
    ...
    processData(progress, data, cb) {
      ...
      let events = [];
      const cgmProcessor = new NGPHistoryParser(cfg, data.cbg_pages);
      // const cgmProcessor = new NGPHistoryParser(data.pages.concat(data.cbg_pages));
      // TODO - try/catch here.
      cgmProcessor.processData();
      const cgmRecords = cgmProcessor.buildCGMRecords();
      debug(cgmRecords);
      _.assign(events, cgmRecords);
      events = _.sortBy(events, d => d.time);

      const simulator = medtronicSimulator.make({

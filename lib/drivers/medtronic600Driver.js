...
class NGPTimestamp extends Date {
  static fromRtcAndOffset(rtc, offset) {
    return new Date(946684800000 + (rtc * 1000) + (offset * 1000));
  }

  static fromBuffer(buffer) {
    const rtc = buffer.readUInt32BE(0x00);
    const offset = buffer.readUInt32BE(0x04) - 0x100000000;
    return NGPTimestamp.fromRtcAndOffset(rtc, offset);
  }
}
  ...
  return {
    ...
    getConfigInfo(progress, data, cb) {
      ...
      driver.getCnlInfo()
        ...
        .then((pumpTime) => {
          settings.pumpTime = sundial.applyTimezone(pumpTime.time, cfg.timezone);
        })
    ...
    uploadData(progress, data, cb) {
      ...
      const postRecords = [{
        ...
        timezoneOffset: 660,
        conversionOffset: 0,
        deviceTime: sundial.formatDeviceTime(data.settings.pumpTime),
        deviceId: `${data.settings.pumpModel}:${data.settings.pumpSerial}`,
        type: 'pumpSettings',

...
class TelDRequest extends TelDMessage {
  constructor(commandType, pumpSession, payload) {
    ...
    // TODO - do we want to do this everywhere??
    this.pumpSession = pumpSession;
  }
  ...
  static buildPayload(commandType, pumpSession, payload) {
    ...
    const payloadBuffer = Buffer.alloc(
      TelDRequest.ENVELOPE_SIZE + payloadLength + TelDRequest.CHECKSUM_SIZE);

    payloadBuffer.writeUInt8(commandType, 0);
    payloadBuffer.writeUInt8(TelDRequest.ENVELOPE_SIZE + payloadLength, 1);
    if (payloadLength > 0) {
...
class ReadInfoResponse extends MinimedResponse {
  constructor(payload) {
    ...
    const buffer = Buffer.from(payload.slice(0x21, 0x21 + 19));
    // Link MAC and Pump MAC are packed as 64-bit integers, but JavaScript doesn't support those,
    // so we'll store them as hex strings.
    this.linkMAC = buffer.slice(0, 8).toString('hex');
    this.pumpMAC = buffer.slice(8, 16).toString('hex');
    this.linkCounter = buffer.readUInt16LE(16); // Yes, LE. Way to be consistent, Medtronic
    // eslint-disable-next-line no-bitwise
    this.encryptionMode = buffer.readUInt8(18) & 1;
  }
...
class RequestLinkKeyResponse extends MinimedResponse {
  constructor(payload) {
    ...
    this.packedLinkKey = Buffer.from(payload.slice(0x21, 0x21 + 55));
  }
...
class JoinNetworkResponse extends TelDResponse {
  constructor(payload) {
    ...
    if (payload.length > 46 && payload[0x33] === 0x82 && payload[0x44] === 0x42) {
      this.radioChannel = payload[0x4c];
    } else {
      this.radioChannel = 0;
    }
  }
...
class TransmitPacketResponse extends TelDResponse {
  constructor(payload, pumpSession) {
    // Decrypt response and overwrite encrypted payload portion
    const payloadLength = MinimedRequest.ENVELOPE_SIZE + payload[0x22] + TelDRequest.CHECKSUM_SIZE;
    if (payloadLength >= 57) {
      const encryptedPayloadSize = payload[0x38];
      const encryptedPayload = Buffer.from(payload.slice(0x39, 0x39 + encryptedPayloadSize));
      const decryptedPayload =
        TransmitPacketResponse.decrypt(pumpSession.key, pumpSession.iv, encryptedPayload);

      const payloadBuffer = Buffer.concat(
        [
          payload.slice(0, 0x39),
          decryptedPayload,
          payload.slice(payloadLength - 2, payloadLength),
        ], payloadLength);
      super(payloadBuffer);
    } else {
      // TODO - throw before here?
      super(payload);
    }
  }
...
class ReadBasalPatternResponse extends TransmitPacketResponse {
  constructor(payload, pumpSession) {
    ...
    // Use this.payload so that we read the decrypted data.
    this.schedule = [];
    const numItems = this.payload[0x3D];

    for (let i = 0; i < numItems; i++) {
      this.schedule.push({
        start: this.payload[0x42 + (i * 5)] * 30 * sundial.MIN_TO_MSEC,
        rate: (this.payload.readUInt32BE(0x3E + (i * 5)) / 10000),
      });
    }
  }
}

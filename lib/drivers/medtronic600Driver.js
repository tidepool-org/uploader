...
class MultiPacketTransferSession extends TransmitPacketRequest {
  ...
  initSession(payload) {
    debug('*** INIT SESSION:', payload.toString('hex'));
    this.totalSize = payload.readUInt32BE(0x03);
  ...
  // this.addSegment(response.packetNumber, response.segmentPayload);
  addSegment(packetNumber, payload) {
    if (packetNumber !== this.packetsFetched) {
      debug('*** PACKET FETCH MISMATCH', packetNumber, ', ', this.packetsFetched);
    }

    debug('GOT A MULTIPACKET SEGMENT', packetNumber, ', fetchNo: ', this.packetsFetched);
    this.packetsFetched += 1;
        ...
        .then((message) => {
          ...
          async.whilst(
            ...
            () => ((typeof response !== 'undefined' && response.comDCommand ===
              ...
              this.packetsFetched < this.packetsToFetch),
            (callback) => {
              this.readMessage(hidDevice, readTimeout)
                .then((receiveMessageResponse) => {
            ...
            () => {
              ...
              let multiByteMessage = Buffer.concat(this.segments,
                ((this.packetSize * this.lastPacketNumber) + this.lastPacketSize));
              if (multiByteMessage.length !== this.totalSize) {

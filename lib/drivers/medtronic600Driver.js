        ...
        if (segmentPayload.readUInt16BE(0x00) === 0x030E) {
          ...
          if (historyCompressed) {
            ...
            if (blockPayload.length !== historySizeUncompressed) {
              debug(`Unexpected uncompressed message size. Expected ${historySizeUncompressed}, got ${blockPayload.length}`);
...
module.exports = (config) => {
  ...
  return {
    ...
    processData(progress, data, cb) {
      ...
      const historyParser = new NGPHistoryParser(cfg, data.pages.concat(data.cbg_pages));
      ...
      data.postRecords = timeChanges.postRecords;

...
class MultiPacketTransferSession extends TransmitPacketRequest {
  ...
  addSegment(packetNumber, payload) {
    ...
    debug('No OF SEGMENTS', this.segmentCount);
  ...
  fetchMoreData() {
    return this.segments.length === 0 || this.packetsFetched < this.packetsToFetch;
  }
      ...
      super.send(hidDevice, readTimeout, sendDelay, get80response)
        .then(() => {
          async.whilst(
              ...
              this.readMessage(hidDevice, readTimeout)
                .then((receiveMessageResponse) => {
                  ...
                  switch (response.comDCommand) {
                    // We can get HIGH_SPEED_MODE_COMMAND responses at the beginning of a
                    // multipacket session - no idea why. We just ignore them and readMessage again.
                    ...
                    case TransmitPacketRequest.COM_D_COMMAND.INITIATE_MULTIPACKET_TRANSFER:
                      {
                        ...
                        new AckMultipacketCommand(this.pumpSession).send(hidDevice)
            ...
            (err) => {
              debug('*** END OF WHILST', err);
              if (err) {
                reject(err);
              }

              let sessionPayload = this.sessionPayload;
              if (sessionPayload.length !== this.totalSize) {
                debug('*** TOTAL MESSAGE SIZE MISMATCH');
                reject(InvalidMessageError('Total session size mismatch'));
              }

              // TODO - all of this should go into different classes...
              // Decompress the message
              if (sessionPayload.readUInt16BE(0x00) === 0x030E) {
                ...
                const dataType = sessionPayload[0x02]; // Returns a HISTORY_DATA_TYPE
                const historySizeCompressed = sessionPayload.readUInt32BE(0x03);
                const historySizeUncompressed = sessionPayload.readUInt32BE(0x07);
                const historyCompressed = sessionPayload[0x0B];

                // Check that we have the correct number of bytes in this message
                if (sessionPayload.length - HEADER_SIZE !== historySizeCompressed) {
                  debug('*** Unexpected message size', historySizeCompressed, sessionPayload.length);
                  debug('*** Payload', sessionPayload);
                  reject(InvalidMessageError('Unexpected message size'));
                }

                if (historyCompressed) {
                  sessionPayload = lzo.decompress(sessionPayload.slice(HEADER_SIZE));
                  if (sessionPayload.length !== historySizeUncompressed) {
                    debug('*** Unexpected uncompressed message size');
                    reject(InvalidMessageError('Unexpected uncompressed message size'));
                  }
                }
              }

              const ResponseClass = this.finalResponseType;
              const response = new ResponseClass(sessionPayload, this.pumpSession);
              resolve(response);
            },
          );
        })
    ...
    getConfigInfo(progress, data, cb) {
      ...
      driver.getCnlInfo()
        ...
        .then(() => driver.readHistory(
          settings.currentDeviceTime.rtcFromDate(new Date('2017-01-23T00:00:00')),

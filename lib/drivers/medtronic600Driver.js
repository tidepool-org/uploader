  ...
  readMessage(hidDevice, readTimeout = BCNLMessage.READ_TIMEOUT_MS) {
    return new Promise((resolve, reject) => {
      ...
      async.doWhilst(
        ...
        readMoreBytes => readMoreBytes, // Show we doWhilst again?
        (err) => {
...
class NGPResponse extends MinimedResponse {
  constructor(payload, pumpSession) {
    ...
    if (this.payload[MinimedMessage.ENVELOPE_SIZE] !==
      NGPMessage.COMMAND_TYPE.PACKET_RESPONSE_HEADER) {
...
class PumpTimeResponse extends TransmitPacketResponse {
  get time() {
    ...
    return NGPTimestamp.fromBuffer(this.decryptedPayload.slice(0x04, 0x0C));
  }
}
...
class BolusWizardBGTargetsResponse extends TransmitPacketResponse {
  get targets() {
    ...
    for (let i = 0; i < numItems; i++) {
      ...
      // this.decryptedPayload.readUInt16BE(0x08 + (i * 9)); // in mmol/L
      const low = this.decryptedPayload.readUInt16BE(0x0A + (i * 9)); // in mg/dL
      // this.decryptedPayload.readUInt16BE(0x0C + (i * 9)); // in mmol/L
...
class BolusWizardSensitivityFactorsResponse extends TransmitPacketResponse {
  get factors() {
    ...
    for (let i = 0; i < numItems; i++) {
      ...
      // this.decryptedPayload.readUInt16BE(0x08 + (i * 5)); // in mmol/L
...
class DeviceStringResponse extends TransmitPacketResponse {
  ...
  get model() {
    ...
    return iconv.decode(modelUtf16, 'utf16-be').replace(/\0/g, '');
  }
}
...
class ReadHistoryInfoResponse extends TransmitPacketResponse {
  ...
  get dataStart() {
    return NGPTimestamp.fromBuffer(this.decryptedPayload.slice(0x08, 0x10));
  }

  get dataEnd() {
    return NGPTimestamp.fromBuffer(this.decryptedPayload.slice(0x10, 0x18));
  }
}
...
class ReadHistoryInfoCommand extends TransmitPacketRequest {
  constructor(pumpSession, historyDataType, fromRtc, toRtc) {
    ...
    params[0x01] = 0x04; // Hard coded in CLPro
    params.writeUInt32BE(fromRtc, 0x02);
    params.writeUInt32BE(toRtc, 0x06);
    params.writeUInt16BE(0x00, 0x0A); // Hard coded in CLPro
...
class MultiPacketTransferSession extends TransmitPacketRequest {
  constructor(pumpSession, comDCommand, parameters, finalResponseType = TransmitPacketResponse) {
    // Request a regular TransmitPacketResponse to process this Multipacket Segment, and return
    // the actual responseType in send()
    super(pumpSession, comDCommand, parameters, TransmitPacketResponse);
  ...
  send(hidDevice, readTimeout = BCNLMessage.READ_TIMEOUT_MS, sendDelay = 0, get80response = true) {
    ...
    return new Promise((resolve, reject) => {
      let response = null;
      let multiBytesTotal = 0;
      let multiBytesPerPacket = 0;
      let multiBytesLastPacket = 0;
      let multiPacketsToFetch = 0;
      let multiPacketsFetched = 0;
      let multiByteMessage = Buffer.alloc(0);

      super.send(hidDevice, readTimeout, sendDelay, get80response)
        .then((message) => {
          response = message;
          async.whilst(
            // We can also get HIGH_SPEED_MODE_COMMAND responses at the beginning of a
            // multipacket session - no idea why...
            () => ((typeof response !== 'undefined' && response.comDCommand ===
                TransmitPacketRequest.COM_D_COMMAND.HIGH_SPEED_MODE_COMMAND) ||
              multiPacketsFetched < multiPacketsToFetch),
            (callback) => {
              this.readMessage(hidDevice, readTimeout)
                .then((receiveMessageResponse) => {
                  response = new MultiPacketSegment(receiveMessageResponse, this.pumpSession);
                  debug('*** PAYLOAD', response.decryptedPayload.toString('hex'));
                })
                .then(() => {
                  let promise = Promise.resolve();
                  if (response.comDCommand ===
                    TransmitPacketRequest.COM_D_COMMAND.INITIATE_MULTIPACKET_TRANSFER) {
                    multiBytesTotal = response.decryptedPayload.readUInt32BE(0x03);
                    multiBytesPerPacket = response.decryptedPayload.readUInt16BE(0x07);
                    multiBytesLastPacket = response.decryptedPayload.readUInt16BE(0x09);
                    multiPacketsToFetch = response.decryptedPayload.readUInt16BE(0x0B);
                    const params = Buffer.alloc(2);
                    params[0x00] = 0xFF;
                    params[0x01] = 0x00;
                    promise = new AckMultipacketCommand(this.pumpSession).send(hidDevice);
                  } else if (response.comDCommand ===
                    TransmitPacketRequest.COM_D_COMMAND.MULTIPACKET_SEGMENT_TRANSMISSION) {
                    if (response.packetNumber !== multiPacketsFetched) {
                      debug('*** PACKET FETCH MISMATCH', response.packetNumber, ', ', multiPacketsFetched);
                    }

                    debug('GOT A MULTIPACKET SEGMENT', response.packetNumber, ', fetchNo: ', multiPacketsFetched);
                    multiPacketsFetched += 1;
                    multiByteMessage = Buffer.concat([multiByteMessage, response.segmentPayload],
                      (multiByteMessage.length + response.segmentPayload.length));

                    if (multiPacketsFetched === multiPacketsToFetch) {
                      if (response.segmentPayload.length !== multiBytesLastPacket) {
                        debug('*** LAST PACKET SIZE MISMATCH');
                      }
                      if (multiByteMessage.length !== multiBytesTotal) {
                        debug('*** TOTAL MESSAGE SIZE MISMATCH');
                      }
                    } else if (response.segmentPayload.length !== multiBytesPerPacket) {
                      debug('*** MULTI PACKET SIZE MISMATCH');
                    }
                  }
                  return promise;
                })
                .then(() => {
                  // Have a final .then() here so that we can use a single callback() in this block.
                  callback(null, response);
                });
            },
            () => {
              debug('*** END OF WHILST');
              const ResponseClass = this.finalResponseType;
              // TODO - all of this should go into different classes...
              // Decompress the message
              if (multiByteMessage.readUInt16BE(0x00) === 0x030E) {
                ...
                const dataType = multiByteMessage[0x02]; // Returns a HISTORY_DATA_TYPE
                const historySizeCompressed = multiByteMessage.readUInt32BE(0x03);
                const historySizeUncompressed = multiByteMessage.readUInt32BE(0x07);
                const historyCompressed = multiByteMessage[0x0B];

                // Check that we have the correct number of bytes in this message
                if (multiByteMessage.length - HEADER_SIZE !== historySizeCompressed) {
                  debug('*** Unexpected message size', historySizeCompressed, multiByteMessage.length);
                  debug('*** Payload', multiByteMessage);
                  throw new InvalidMessageError('Unexpected message size');
                }

                if (historyCompressed) {
                  multiByteMessage = lzo.decompress(multiByteMessage.slice(HEADER_SIZE));
                  if (multiByteMessage.length !== historySizeUncompressed) {
                    debug('*** Unexpected uncompressed message size');
                    throw new InvalidMessageError('Unexpected uncompressed message size');
                  }
                }
              }

              response = new ResponseClass(multiByteMessage, this.pumpSession);
              resolve(response);
            },
          );
        })
...
class ReadHistoryResponse {
  constructor(payload) {
    this.multiPacketPayload = payload;
  }
}

class ReadHistoryCommand extends MultiPacketTransferSession {
  constructor(pumpSession, historyDataType, fromRtc, toRtc) {
    const params = Buffer.alloc(12);
    params[0x00] = historyDataType;
    params[0x01] = 0x04; // Hard coded in CLPro
    ...
    params.writeUInt32BE(toRtc, 0x06);
    params.writeUInt16BE(0x00, 0x0A); // Hard coded in CLPro
    super(pumpSession, TransmitPacketRequest.COM_D_COMMAND.READ_HISTORY_REQUEST,
      params, ReadHistoryResponse);
  }
}
...
class MM600SeriesDriver extends BCNLDriver {
  ...
  readHistory(fromRtc, toRtc, timezone) {
    // eslint-disable-next-line no-unused-vars
    return new Promise((resolve, reject) => {
      const history = {};
      new ReadHistoryInfoCommand(this.pumpSession,
          MM600SeriesDriver.HISTORY_DATA_TYPE.PUMP_DATA, fromRtc, toRtc).send(this.hidDevice)
        .then((response) => {
          debug('*** PUMP HISTORY INFO:', response, ', from:', response.dataStart.toDate(timezone), ', to:', response.dataEnd.toDate(timezone));
        })
        .then(() => new ReadHistoryInfoCommand(this.pumpSession,
          MM600SeriesDriver.HISTORY_DATA_TYPE.SENSOR_DATA, fromRtc, toRtc).send(this.hidDevice))
        .then((response) => {
          debug('*** SENSOR HISTORY INFO:', response, ', from:', response.dataStart.toDate(timezone), ', to:', response.dataEnd.toDate(timezone));
        })
        .then(() => new ReadHistoryCommand(this.pumpSession,
          MM600SeriesDriver.HISTORY_DATA_TYPE.SENSOR_DATA, fromRtc, toRtc).send(this.hidDevice))
        .then((response) => {
          debug('*** SENSOR HISTORY:', response.multiPacketPayload.toString('hex'));
        })
...
module.exports = (config) => {
  ...
  return {
    ...
    getConfigInfo(progress, data, cb) {
      ...
      driver.getCnlInfo()
        ...
        .then((pumpTime) => {
          ...
          settings.currentDeviceTime = pumpTime.time;
        })
        ...
        .then(() => driver.readHistory(
          settings.currentDeviceTime.rtcFromDate(new Date('2017-01-20T00:00:00')),
          NGPTimestamp.maxRTC, cfg.timezone))
        .catch((err) => {
          error = err;
        })
        // Finally
    ...
    processData(progress, data, cb) {
      ...
      // TODO - add more time change records
      const currentDeviceTime = data.settings.currentDeviceTime.toDate(cfg.timezone).toISOString();
      cfg.tzoUtil = new TZOUtil(cfg.timezone, currentDeviceTime, []);

      data.postRecords = [];
      const postSettings = cfg.builder.makePumpSettings();
      postSettings.with_units(data.settings.units)
        .with_carbRatio(data.settings.carbRatio)
        .with_insulinSensitivity(data.settings.insulinSensitivity)
        .with_bgTarget(data.settings.bgTarget)
        .with_basalSchedules(data.settings.basalSchedules)
        .with_activeSchedule(data.settings.activeSchedule)
        // current settings do not have an index, so can't use TZOUtil.
        .with_time(currentDeviceTime)
        .with_deviceTime(sundial.formatDeviceTime(currentDeviceTime))
        .with_timezoneOffset(sundial.getOffsetFromZone(currentDeviceTime, cfg.timezone))
        .with_conversionOffset(0);
      data.postRecords.push(postSettings.done());

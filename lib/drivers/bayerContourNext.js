...
module.exports = function (config) {
  ...
  var bcnCommandResponse = function (commandpacket, callback) {
    ...
    hidDevice.send(commandpacket.packet, function () {
        ...
        receivePacket(5000, commandpacket.parser, function(err, packet) {
            ...
            //debug('rawtext del receivePacket', parseHeader(rawtext));
  ...
  var getAllRecords = function (cb) {
    var cmd = buildReadCmd();

    // send the first ack to the device
    hidDevice.send(cmd.packet, function () {
        hidDevice.receive(function(rawdata){
            debug('recibiendo del primer ack', rawdata);
  ...
  var getDeviceInfo = function (obj, cb) {
      ...
      bcnCommandResponse(cmd, function (err, headerText) {
          if (err) {
              ...
              _.assign(obj, parseHeader(headerText));
              cb(null, obj);
          }
      });
  };

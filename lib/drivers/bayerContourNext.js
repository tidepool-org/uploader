...
module.exports = function (config) {
  ...
  var parseHeader = function (s){
    var header = s.split('\n').filter(function(e){ return e !== ''}).shift();
  ...
  var getOneRecord = function (record, cb) {
    ...
    var r = record.match(^\dR\|(\d+).*Glucose\|(\d+)\|.*(\d{12})(\d{2})$);
    var robj = {};
  ...
  var prepBGData = function (progress, data) {
    ...
    data.id = 'BayerContour'+ data.model + data.serialNumber;
  ...
  return {
    ...
    fetchData: function (progress, data, cb) {
      ...
      function getOneRecordWithProgress(recnum, cb) {
        ...
        setTimeout(function() {
          getOneRecord(data.rawrecords.shift(), cb);
        }, 20);
      }

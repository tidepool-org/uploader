...
module.exports = function (config) {
  ...
  var extractPacketIntoMessage = function (bytes) {
    ...
    switch(packetTail['FRAME_TYPE']) {
      case ASCII_CONTROL.ETX: // Last ASTM Message
      case ASCII_CONTROL.ETB: // End of valid ASTM Message
        astmMessageBuffer.setValid();
        break;
    }
  ...
  */
   ...
   * TODO - return a listOfRecords, rather than a string with the whole frame.
   */
  function decodeMessage (message) {
    ...
    var response = struct.unpack(buf, 0, 'bb'+frameLength+'Z2Z2Z', ['STX', 'sequenceNumber', 'frame', 'checksum', 'CRLF']);
  ...
  */
  ...
  var getOneRecord = function (data, callback) {
    // TODO -- add error retries and sequence checks here
    ...
    bcnCommandResponse(cmd, function (err, record) {
      if (err) {
        ...
        var record_type = struct.extractByte(record.frame, 0);
        ...
        switch(record_type) {
          case 'R':
            ...
            break;
          case 'H':
            data.header = record.frame;
            var header = parseHeader(record.frame);
            if(header){
              _.assign(data, header);
              callback(null, data);
            } else {
              debug('Invalid header data');
              callback(new Error('Invalid header data'), null);
            }
            break;
          default:
            callback(null, null);

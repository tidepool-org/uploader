...
class GlucoRx {
  ...
  async getRecords(nrOfRecords) {
    ...
    for (let startIndex = 1; startIndex <= nrOfRecords; startIndex++) {


      // requests to devices are sequential
      // eslint-disable-next-line no-await-in-loop
      const result = await this.commandResponse(COMMAND.READ_DATA_TIME, startIndex);
      console.log(GlucoRx.parseDateTime(result));
    }
  ...
  async ping() {
    const bytesWritten = await this.hidDevice.sendPromisified(this.buildPacket(COMMAND.CONNECTION));
    debug('Sent', bytesWritten, 'bytes.');

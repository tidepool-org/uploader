...
class GlucoRx {
  ...
  buildPacket(command, payload = [0, 0, 0, 0]) {
    ...
    if (this.hidDevice) {
      ...
      struct.storeByte(packetlen - 1, bytes, ctr);
      ctr += 1;
    }

    ctr += struct.pack(bytes, ctr, 'bb4Bb', CONTROL.START, command, payload, CONTROL.STOP);

    let crc = 0;
    for (let i = 1; i < packetlen; i++) {
      crc += bytes[i];
    }
    console.log("CRC:", crc, ':',  crc & 0xFF);

    struct.storeByte(crc & 0xFF, bytes, ctr);
  ...
  static packetHandler(buffer) {
    ...
    const packet = GlucoRx.extractPacket(buffer.bytes());
        ...
        if (this.protocol === 1) {
          ...
          if (foundSTX) {
            ...
            if (raw.length >= HEADER_SIZE) {
              const fields = GlucoRx.extractHeader(raw);
  ...
  async commandResponse(cmd, payload) {
    ...
    do {
      ...
      if (result.length > 0) {
        if (this.cfg.deviceInfo.driverId === 'GlucocardShineHID') {
          // glucocard shine express & connex have custom HID chip which
          // sends report ID at the beginning, so we have to strip that out
          result.shift();
        }
  ...
  async getModel() {
    const result = await this.commandResponse(0x22); //COMMAND.READ_MODEL);
...
module.exports = (config) => {
  ...
  return {
    ...
    getConfigInfo(progress, data, cb) {
      ...
      (async () => {
        cfg.deviceInfo.model = await driver.getModel();

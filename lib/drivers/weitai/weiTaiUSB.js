...
class WeitaiUSB {
  ...
  static parseSettingPacket(packet, name) {
    const command = WeitaiUSB.getCommand(packet);

    if (command.length < 12) {
      // command 长度校验不通过
      // command length check failed
      return false;
    }
    ...
    let inComeRes = [];
    if (name) {
      inComeRes = WeitaiUSB.parseSettingAndNamePayload(payload);
    } else {
      inComeRes = WeitaiUSB.parseSettingPayload(payload);
    }
    return inComeRes;
  }

  static parseSettingAndNamePayload(payload) {
    //每22字节为一条数据
  ...
  static parseSettingPayload(payload) {
    //每22字节为一条数据
    // every 22 bytes is a piece of data
    ...
    if (payload.length % 22) {
      //数据异常，非22的倍数
      // payload data length should be a multiple of 22
      // return cb({ code: "E_READ_FILE" }, null);
    }
  ...
  static parsePayload(payload, cfg, cb) {
    ...
    if (payload.length % 28) {
      //数据异常，非28的倍数
      // payload length should be a multiple of 28
    ...
    for (let i = 0; i < payload.length; i += 28) {
      ...
      // if (eventPort == 4 && eventType == 5 && eventUrgency == 1) {
      //   inComeRes.alarm.push({
      //     index: eventIndex,
      //     type: ' suspendFor15',
      //     deviceTime: timeText,
      ...
      // debug(text1, text2, text3, text4);x
      debug(text4);
    }
  ...
  async getConfig(data) {
    ...
    for (let i = 0; i < this.setTypes.length; i++) {
      settings = await this.getSetting();
    }
      ...
      await this.usbDevice
        ...
        .then((res) => {
          ...
          if (count == 1) {
            ...
            //  console.log("transincoming length", incoming.length);
            if (commandBodyLength == 0) {
              //读取完毕
              //  console.log("getRes end");
              done = true;
            }
          }
        })
  ...
  async getSetting() {
    ...
    //   commandBody.writeUInt8(0x04, 0); //Port
    //   commandBody.writeUInt8(0x04, 1); //Parameter
    //   commandBody.writeUInt8(0x02, 2); //Operation
    // }
    // await this.usbDevice.iface.clearHalt();
    ...
    while (!done) {
      await this.usbDevice
        ...
        .then((res) => {
          const incoming = res;
          // let transincoming = Buffer.from(incoming.buffer);
          // debug("Received", incoming.toString("hex"));
          incomingA = Buffer.concat(
            [incomingA, incoming],
            incomingA.length + incoming.length
          );
          if (counter == 1) {
            const command = WeitaiUSB.getCommand(incomingA);
            const commandBody = new Buffer(unpackFrame(command));
            // debug("commandBody", commandBody.toString("hex"));

            commandBodyLength = commandBody.readInt32LE(4);
            counter++;
          } else {
            commandBodyLength = commandBodyLength - incoming.length;
            // debug("transincoming length", incoming.length);
            if (commandBodyLength == 0) {
              //读取完毕
              // debug("getSettingRes end",incomingA);
              done = true;
              let inComeRes = {};
              if (this.current > 8) {
                inComeRes = WeitaiUSB.parseSettingPacket(incomingA, 'name');
              } else {
                inComeRes = WeitaiUSB.parseSettingPacket(incomingA);
              }
              ...
              this.settingUpload[this.setTypes[this.current] + '_name'] =
                inComeRes.name;
              // debug(
              //   `${this.setTypes[this.current]}//设置读取结束`,
              //   this.settingUpload
              // );
        ...
        .catch((err) => {
          ...
          console.log('getSettingRes error:', err);
        });
    }
  ...
  return {
    ...
    getConfigInfo(progress, data, cb) {
      ...
      (async () => {
        ...
        cfg.deviceInfo.deviceId = 'MTM-I-00000'; // TODO: serial number
        cb(null, data);
      })().catch((error) => {
      ...
      for (let c = 0; c < bgList.length; c++) {
        if (c == 0) {
          bgTarget.push({
            start: 0,
            low: bgList[0] / 10,
            high: bgHighList[0] / 10,
          });
        } else {
          if (bgList[c] != senList[c - 1] && c < 48) {
            bgTarget.push({
              start: c * 30 * 60 * 1000,
              low: bgList[c] / 10,
    ...
    buildSettings(settings) {
      ...
      var postsettings = cfg.builder
        .makePumpSettings()
        .with_activeSchedule(settingRes.activeName || 'default')
        ...
        .with_manufacturers(['Weitai'])
        .with_serialNumber(cfg.deviceInfo.serialNumber)
        .with_deviceTime(sundial.formatDeviceTime(new Date()))
        .with_time(
    ...
    fetchData(progress, data, cb) {
      ...
      const inComeRes = WeitaiUSB.parsePacket(incomingA, cfg, cb); //inComeRes:{BloodGlucoses:[],BasalRates:[],BolusRates:[]}
    ...
    uploadData(progress, data, cb) {
      ...
      const sessionInfo = {
        ...
        deviceId: cfg.deviceInfo.deviceId || 'MTM-C2',

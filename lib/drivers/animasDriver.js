    ...
    processData: function (progress, data, cb) {
      ...
      for(var d in data.primeRewindRecords) {
        ...
        if(primedatum.primeFlags === 'cannula') {
          // Animas uses 1-minute resolution, so we add 30 seconds to cannula
          // prime so that it's not deduped later if tubing prime was during
          // same minute
          primedatum.jsDate.setSeconds(primedatum.jsDate.getSeconds() + 30);
          primedatum.deviceTime = sundial.formatDeviceTime(primedatum.jsDate);
        }

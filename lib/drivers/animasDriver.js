...
module.exports = function (config) {
  ...
  var setupConnection = function(destinationAddress) {
    ...
    var payloadlength = struct.pack(payload,0,'iib',primaryAddress,destinationAddress,connectionAddress);
    return {
      packet: buildPacket(
  ...
  var handshake = function(iter) {
    return {
      packet: buildPacket(
        ADDRESS_CONNECT, CMDS.HANDSHAKE.value, 10, [primaryAddress,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF,0x02,iter]
      ),
      parser: function (packet) {
        var data = {
  ...
  var sendAck = function (obj,cb) {
    var cmd = {
      packet: buildPacket(
        connectionAddress, CMDS.ACK.value | (counters.received << 5), 0, []
      ),
      parser: function (packet) {
        var data = {
  ...
  var resetConnection = function(obj, cb) {
    ...
    var cmd = {
      packet: buildPacket(
        connectionAddress, CMDS.DISCONNECT.value, 0, []
      ),
      parser: function (packet) {
  ...
  var getBolusType = function(byte) {
    ...
    if(type.status === BOLUS_TYPE_STATUS.CANCELLED.name) {
      // both triggeredBy and cancelledBy look to be valid only when bolus was cancelled
      var triggeredBy = (byte >> 2) & 0x01;
      switch (triggeredBy) {
        case 0:
            ...
            if(connectErr) {
              ...
              getNumberOfRecords(RECORD_TYPES.BOLUS.value, function (err, result) {
                if(err) {
                  ...
                  getRecords(requestBolus(), numRecords, function(err, result){
                    if(err) {
            ...
            if(connectErr) {
              ...
              getNumberOfRecords(RECORD_TYPES.BASAL.value, function (err, result) {
                if(err) {
                  ...
                  getRecords(requestBasal(),result.numRecords, function(err, result){
                    progress(20);
                    callback(null,result);
                  });
                }
            ...
            if(connectErr) {
              ...
              getNumberOfRecords(RECORD_TYPES.PRIME_REWIND.value, function (err, result) {
                if(err) {
                  ...
                  getRecords(requestPrimeRewind(),result.numRecords, function(err, result){
                    progress(30);
                    callback(null,result);
                  });
                }
            ...
            if(connectErr) {
              ...
              getNumberOfRecords(RECORD_TYPES.SUSPEND_RESUME.value, function (err, result) {
                if(err) {
                  ...
                  getRecords(requestSuspendResume(),result.numRecords, function(err, result){
                    progress(40);
                    callback(null,result);
                  });
                }

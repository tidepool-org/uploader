...
module.exports = function (config) {
  ...
  function buildCBGRecords(data, postrecords) {
    data.cbgRecords.forEach(function (event) {
      // TODO: handle cbg_types and HI/LO
      if(!_.contains(event.cbg_types,'time_changed') &&
          event.glucose >= 39 &&   // Dexcom range is 40-400 mg/dL
          event.glucose <= 401 ){  // 39 and 401 should be HI/LO
        var cbg = cfg.builder.makeCBG()
           .with_value(event.glucose)
         ...
         cfg.tzoUtil.fillInUTCInfo(cbg, event.jsDate);
         cbg = cbg.done();
         postrecords.push(cbg);
      }
    });
    return postrecords;
  }

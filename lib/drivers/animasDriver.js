...
module.exports = function (config) {
  ...
  function buildSettingsRecord(data, postrecords) {
    data.settings.basalPrograms.forEach(function (basalProgram) {
      //TODO: build pump settings records
    });
    return postrecords;
  }
  ...
  return {
    ...
    getConfigInfo: function (progress, data, cb) {
      ...
      readBasalPrograms(function (err, results) {
        if(err) {
          return cb(err,null);
        }
        else{
          data.settings = {};
          data.settings.basalPrograms = results;

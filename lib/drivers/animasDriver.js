...
module.exports = function (config) {
  ...
  var getBasalRecords = function(numRecords,recordSize,cb) {

    var cmd = readDataPages(RECORD_TYPES.BASAL.value,0,numRecords);
  ...
  var getBolusRecords = function(numRecords,recordSize,cb) {
    ...
    var cmd = readDataPages(RECORD_TYPES.BOLUS.value,0,numRecords);
  ...
  return {
    ...
    fetchData: function (progress, data, cb) {
      ...
      async.series({
        ...
        basalRecords: function(callback){
          // If you don't disconnect and reconnect, the pump will try to send you all 500 (empty) records
            ...
            if(connectErr) {
              ...
              getNumberOfRecords(RECORD_TYPES.BASAL.value, function (err, result) {
                if(err) {
                  ...
                  getBasalRecords(result.numRecords,result.recordSize, function(err, result){
                    progress(20);
                    callback(null,result);
                  });
                }
    ...
    processData: function (progress, data, cb) {
      ...
      data.postrecords = [];
      var settings = []; //TODO: fill in settings from pump
      ...
      for(var i in data.basalRecords) {
        if(i>0) {
          basaldatum = data.basalRecords[i];

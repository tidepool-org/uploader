  ...
  var requestCBG = function() {
    return{
      ...
      parser : function (payload) {
        ...
        var rawTimestamp = BASE_TIME + struct.extractInt(payload,6) * sundial.SEC_TO_MSEC;
        var dt = new Date(rawTimestamp); // rawTimeStamp is "UTC" unix time

        return {
          index: struct.extractShort(payload,2),
          glucose: bg,
          cbg_types: getFlagNames(CBG_TYPE_FLAGS, bgBytes[1]),
          jsDate: dt,
          deviceTime: sundial.formatDeviceTime(dt)
        };
      }
        ...
        if(connectErr) {
          ...
          animasCommandResponseAck(cmd, function (err, result) {
            if (err) {
              ...
              var BASE_TIME = Date.UTC(2008, 0, 1, 0, 0, 0).valueOf();
              timeAdjust.accTimestamp = BASE_TIME + struct.extractInt(result.payload,8) * sundial.SEC_TO_MSEC;
              timeAdjust.accDt = new Date(timeAdjust.accTimestamp); // accTimeStamp is "UTC" unix time
              cb(null, timeAdjust);
            }
          });
        }
    ...
    data.cbgRecords.forEach(function (event) {
      ...
      if(event.glucose >= 39 && event.glucose <= 401 ) {
        ...
        if(record) {
         ...
         debug('CGM adjustment:', event.cbg_types.toString(),event.jsDate,event.index);
       }
    });
    return postrecords;
  }

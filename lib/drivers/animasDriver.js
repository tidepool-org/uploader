...
module.exports = function (config) {
  ...
  var incrementSentCounter = function() {
    counters.sent +=1;
    if(counters.sent == 7) // only a 3-bit counter
      counters.sent = 0;
  };

  var incrementReceivedCounter = function() {
    counters.received += 1;
    if(counters.received == 7) // only a 3-bit counter
      counters.received = 0;
  };
  ...
  var readDataPages = function (rectype, offset, numRecords) {
    ...
    return {
      ...
      parser: function (packet) {
        incrementReceivedCounter(); //only with Information packets
        var data = {
          insulinSensitivity : struct.extractShort(packet.payload,10),
          bgTarget: struct.extractShort(packet.payload,8)
        };
        return data;
      }
    };
  ...
  var animasCommandResponse = function (commandpacket, callback) {
    serialDevice.writeSerial(commandpacket.packet, function () {
      if(commandpacket.command == getCounters()) //it's an Information packet
    ...
    getConfigInfo: function (progress, data, cb) {
      ...
      readMiscSettings(function (err, result) {
        if(err) {
          ...
          progress(100);
          data.settings = result.parsed_payload;

...
module.exports = function (config) {
  ...
  var ALARM_TYPES = {
    ...
    ALARM_CARTRIDGE_REMOVED: { value: 163, name: 'Cartridge Removed'},
    ALARM_EMPTY_CARTRIDGE: { value: 144, name: 'Empty Cartridge'},
    ALARM_BOLUS_CANCELLED: { value: 175, name: 'Bolus delivery canceled by user button'}
  };
    ...
    for(var b in data.wizardRecords) {
      ...
      if (bolusdatum.syncCounter !==  wizarddatum.sync_counter) {
        ...
        if ((wizarddatum.carb_amount > 0) && (bg!== null)) {
          ...
          // so we split in half as a best estimate
          carb /= 2;
          correction /= 2;
        }
  ...
  var buildAlarmRecords = function (data, postrecords) {
    var alarm = null;

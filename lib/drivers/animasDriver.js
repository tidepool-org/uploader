...
module.exports = function (config) {
  ...
  var decodeDate = function(encoded) {
    ...
    var date = new Date(year,month,encoded.day,encoded.hour,encoded.minute);
    return date;
  };

  var getBolusRecords = function(numRecords,recordSize,cb) {
    var cmd = readDataPages(RECORD_TYPES.BOLUS.value,0,2);
    animasCommandResponseAck(cmd, function (err, result) {
      if (err) {
        cb(err, null);
      } else {
        ...
        var data = {
          deviceTime: decodeDate(struct.unpack(result,4,'bbbb',['monthYear','day','hour','minute'])),
          deliveredAmount: struct.extractInt(result,8), // U x 10,000
          requiredAmount: struct.extractShort(result,12), // U x 1,000
          duration: struct.extractShort(result,14), // N x 0.1Hr
          bolusType: getBolusType(struct.extractByte(result,16)),
          counter_8bit: struct.extractByte(result,17)
        };
  ...
  return {
    ...
    fetchData: function (progress, data, cb) {
      ...
      getNumberOfRecords(RECORD_TYPES.BOLUS.value, function (err, result) {
        if(err) {
          ...
          getBolusRecords(result.numRecords,result.recordSize, function(err, result){
            cb(null, data);
          });
        }
      });
    ...
    processData: function (progress, data, cb) {
      ...
      bolus = cfg.builder.makeNormalBolus()
        .with_normal(data.volume_units)
        .with_deviceTime(data.deviceTime);

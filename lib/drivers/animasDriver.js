...
module.exports = function (config) {
  ...
  var getBolusRecords = function(numRecords,recordSize,data,cb) {
            ...
            if(result.payload) {
              ...
              if(struct.extractInt(payload,4) === 0) {// empty date
                ...
                var datum = {
                  ...
                  deliveredAmount: struct.extractInt(payload,8), // U x 10,000
                  requiredAmount: struct.extractShort(payload,12), // U x 1,000
  ...
  return {
    ...
    fetchData: function (progress, data, cb) {
      ...
      getNumberOfRecords(RECORD_TYPES.BOLUS.value, function (err, result) {
        if(err) {
          ...
          getBolusRecords(result.numRecords,result.recordSize, data, function(err, result){
            cb(null, data);
            progress(100);
          });
        }
    ...
    processData: function (progress, data, cb) {
      ...
      for(var b in data.bolusRecords) {
        var datum = data.bolusRecords[b];
        bolus = cfg.builder.makeNormalBolus()
          .with_normal(datum.deliveredAmount)
          .with_deviceTime(datum.deviceTime);
        cfg.tzoUtil.fillInUTCInfo(bolus, datum.jsDate);
        bolus = bolus.done();

...
module.exports = function (config) {
  ...
  function buildBolusRecords(data, postrecords) {
    ...
    for(var b in data.bolusRecords) {
      ...
      console.log("BOLUS:", bolusdatum);

      if(bolusdatum.bolusType.name === BOLUS_TYPE_NAME.COMBO.name) {
        bolus = cfg.builder.makeSquareBolus()
          .with_duration(0) // figure out actual duration
          .with_extended(bolusdatum.deliveredAmount);
        console.log("EXTENDED: ", bolus);

        if(bolusdatum.bolusType.status === BOLUS_TYPE_STATUS.CANCELLED.name) {
          // extended bolus was cancelled
          bolus = bolus.with_expectedExtended(bolusdatum.requiredAmount)

...
module.exports = function (config) {
  ...
  var requestWizard = function() {
    return{
      ...
      parser : function (payload) {
        //TODO: configuration: units, iob enabled etc.
        return {
          index : struct.extractShort(payload,2),
  ...
  function buildWizardRecords(data, postrecords) {
    ...
    for(var b in data.wizardRecords) {
      ...
      if (bolusdatum.syncCounter !==  wizarddatum.sync_counter) {
        console.log('Wizard bolus mismatch!', bolusdatum.syncCounter, wizarddatum.sync_counter);
      }
      else {
        wizard = cfg.builder.makeWizard()
          .with_bgInput(wizarddatum.bg)
          .with_carbInput(wizarddatum.carb_amount)
          ...
          .with_recommended({
            ...
            net: 0
            /*
            carb: bolusdatum.requiredAmount/2,
            correction: bolusdatum.requiredAmount/2,
            net: bolusdatum.requiredAmount
            */
          })
          .with_bolus(bolusdatum)
          .with_units('mg/dL') //TODO: get correct unit from config byte
          ...
          .set('syncCounter', wizarddatum.sync_counter);

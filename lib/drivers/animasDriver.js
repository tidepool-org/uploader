...
module.exports = function (config) {
  ...
  var setupConnection = function(destinationAddress) {
    ...
    primaryAddress = [(Math.floor(Math.random() * 255) + 1),0x00,0x00,0x00]; // use random primary address
    connectionAddress = (Math.floor(Math.random() * 255) + 1) | 0x01; // use random primary address with LSB as 1
  ...
  var getConnection = function(obj, cb) {
    ...
    var connectInterval = setInterval(function() {
      ...
      if(i == 8) {
        ...
        cb(new Error('Could not connect to device.'),null);
      }

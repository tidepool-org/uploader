...
module.exports = function (config) {
  ...
  function buildBolusRecords(data, postrecords) {
    ...
    for(var b in data.bolusRecords) {
      ...
      if(bolusdatum.bolusType.status === 'cancelled') {
        bolus.with_expectedNormal(bolusdatum.requiredAmount)
        .with_payload({
          // both these values look to be valid only when bolus was cancelled

...
module.exports = function (config) {
  ...
  var RECORD_TYPES = {
    ...
    CGM_SETTINGS: {value: 42, name: 'CGM SETTINGS'},
  ...
  var requestCBG = function() {
    return{
      ...
      parser : function (payload) {
        ...
        if(flagNames.length === 0 || _.contains(flagNames,'bg_calibration')) {
        ...
        if(connectErr) {
          ...
          animasCommandResponseAck(cmd, function (err, result) {
            if (err) {
              ...
              var timeAdjust = {};
              timeAdjust.dt = decodeDate(struct.unpack(result.payload,4,'bbbb',['monthYear','day','hour','minute']));
              var BASE_TIME = Date.UTC(2008, 0, 1, 0, 0, 0).valueOf();
              timeAdjust.accTimestamp = struct.extractInt(result.payload,8);
              cb(null, timeAdjust);
            }
          });
        }
  ...
  var readCGMSettings = function(cb) {
    if (modelNumber === 'IR1295') {
      debug('Reading CGM settings');
      resetConnection(true, function(connectErr, obj) {
        if(connectErr) {
          return cb(connectErr, null);
        }else{
          var cmd = readDataPages(RECORD_TYPES.CGM_SETTINGS.value,0,1);
          animasCommandResponseAck(cmd, function (err, result) {
            if (err) {
              cb(err, null);
            } else {
              var cgmSettings = {};
              cgmSettings.highAlertEnabled = struct.extractByte(result.payload,22) ? true : false;
              cgmSettings.highAlertLevel = struct.extractShort(result.payload,10);
              cgmSettings.highAlertSnooze = struct.extractShort(result.payload,16) * 60 * 1000;

              cgmSettings.lowAlertEnabled = struct.extractByte(result.payload,23) ? true : false;
              cgmSettings.lowAlertLevel = struct.extractShort(result.payload,12);
              cgmSettings.lowAlertSnooze = struct.extractShort(result.payload,18) * 60 * 1000;

              cgmSettings.outOfRangeAlertEnabled = struct.extractByte(result.payload,26) ? true : false;
              cgmSettings.OutOfRangeAlertSnooze = struct.extractShort(result.payload,20) * 60 * 1000;
              //TODO: find out if this is also the initial threshold time used

              cgmSettings.riseRateEnabled = struct.extractByte(result.payload,24) ? true : false;
              cgmSettings.riseRateValue = struct.extractByte(result.payload,14);
              cgmSettings.fallRateEnabled = struct.extractByte(result.payload,25) ? true : false;
              cgmSettings.fallRateValue = struct.extractByte(result.payload,15);

              cgmSettings.transmitterId = struct.extractString(result.payload,27,5);
  ...
  function buildCBGRecords(data, postrecords) {
    ...
    data.cbgRecords.forEach(function (event) {
      ...
      if(event.glucose >= 39 && event.glucose <= 401 ) {
        ...
        if(event.cbg_types.length === 0 || _.contains(event.cbg_types,'bg_calibration')) {
        ...
        if(record) {
         ...
         if(_.contains(event.cbg_types,'time_changed')) {
  ...
  function buildCGMSettingsRecord(data, postrecords) {
    var rec = data.settings.cgmSettings;

    // Animas doesn't return current date/time, so we use the last event for
    // cgm settings timestamp
    var lastEventTime = data.suspendResumeRecords[0].suspendJsDate;

    var postsettings = cfg.builder.makeCGMSettings()
      .with_units('mg/dL')
      .with_transmitterId(rec.transmitterId)
      .with_lowAlerts({
        enabled: rec.lowAlertEnabled,
        level: rec.lowAlertLevel,
        snooze: rec.lowAlertSnooze
      })
      .with_highAlerts({
        enabled: rec.highAlertEnabled,
        level: rec.highAlertLevel,
        snooze: rec.highAlertSnooze
      })
      ...
      .with_outOfRangeAlerts({
        enabled: rec.outOfRangeAlertEnabled,
        snooze: rec.OutOfRangeAlertSnooze
      })
      .with_deviceTime(sundial.formatDeviceTime(lastEventTime))
      .with_time(sundial.applyTimezone(lastEventTime, cfg.timezone).toISOString())
      .with_timezoneOffset(sundial.getOffsetFromZone(lastEventTime, cfg.timezone))
  ...
  return {
    ...
    getConfigInfo: function (progress, data, cb) {
      ...
      function(err, results){
          if(err) {
            ...
            data.settings.cgmSettings = results[7];
            progress(100);
    ...
    processData: function (progress, data, cb) {
      ...
      postrecords = buildCGMSettingsRecord(data, postrecords);

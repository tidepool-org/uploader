...
module.exports = function (config) {
  ...
  return {
    ...
    fetchData: function (progress, data, cb) {
      ...
      var allRecordsPresent = function(records) {
        for(var i = 1; i < records.length; i++)
        {
          if(records[i].index == records[i-1].index) {
            console.log('Dropping duplicate record in array');
            records = records.splice(i, 1);
          }
          else if(records[i].index - records[i-1].index != 1)
          {
            return false;
          }
        }
                      ...
                      if(connectErr) {
                        ...
                        getRecords(request, numRecords, function(errRetry, resultRetry){
                          if(errRetry) {
                            ...
                            progress(percentage);
                            if (allRecordsPresent(resultRetry)) {
                              callback(null,resultRetry);
                            }
                            else{
    ...
    processData: function (progress, data, cb) {
      ...
      for(var d in data.primeRewindRecords) {
        ...
        if(primedatum.primeFlags !== 'cannula') {
          ...
          // as a reservoir change (delivery is not possible on pump without priming).

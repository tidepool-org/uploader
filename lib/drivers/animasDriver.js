...
module.exports = function (config) {
  ...
  var decodeDate = function(payload,index) {
    ...
    var startYear = null;
    if (modelNumber === PING) {   // OneTouch Ping starts in year 2007 as 0
      startYear = 2007;
    } else if (modelNumber === VIBE) {
    ...
    for(var b in data.wizardRecords) {
      ...
      if (bolusdatum.syncCounter !==  wizarddatum.sync_counter) {
        ...
        if(wizarddatum.configuration.units === 'mmol/L') {
          // it is not clear why Animas divides by 1.8 on mmol/L pumps,
          // but we have reverse this to get the correct mg/dL value
          wizarddatum.bg_delta = (wizarddatum.bg_delta * 1.8).toFixedNumber(2);
          wizarddatum.target_bg = (wizarddatum.target_bg * 1.8).toFixedNumber(2);
          wizarddatum.isf = (wizarddatum.isf * 1.8).toFixedNumber(2);
        }
  ...
  function buildSettingsRecord(data, postrecords) {
    ...
    var postsettings = cfg.builder.makePumpSettings()
      ...
      .with_units({ carb: 'grams', bg: 'mg/dL' }) // BG targets from pump are always in mg/dL

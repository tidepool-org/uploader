...
module.exports = function (config) {
  ...
  var RECORD_TYPES = {
    ...
    BASAL_PROGRAM_NAMES: {value: 18, name: 'BASAL PROGRAM NAMES', numRecords: 4, recordSize: 16},
  ...
  var requestBasalProgramNames = function() {
    return{
      recordType: RECORD_TYPES.BASAL_PROGRAM_NAMES,
      parser : function (payload) {
        return {name: struct.extractString(payload,9,2)};
      }
    };
  };
  ...
  var animasCommandResponseAck = function (cmd, cb) {
    ...
    var retry = function (cb, results) {
      if(modelNumber === null) {
        //TODO: use shouldRetry instead
        return cb(new Error('Timeout error'),null);
      }
      else {
  ...
  var discoverDevice = function(cb) {
    ...
    var handshakeInterval = setInterval(function() {
      ...
      serialDevice.writeSerial(cmd.packet, function () {
        // once we've sent the command, start listening for a response
        listenForPacket(TIME_TO_TIMEOUT, cmd, function(err, result) {
          if(result && !found) {
            found = true;
            clearInterval(handshakeInterval);
            cb(null, result);
          ...
          animasCommandResponseAck(cmd, function (err, result) {
            if (err) {
              ...
              if(valid === 1) {
                ...
                debug('Read basal program',count,':',basalPrograms[basalPrograms.length-1]);
    ...
    animasCommandResponseAck(cmd, function (err, result) {
      if (err) {
        ...
        if((activeProgram + check) !== 0xFF) {
          cb(new Error('Invalid basal program'), null);
        }
      ...
      if(connectErr) {
        ...
        animasCommandResponseAck(cmd, function (err, result) {
          if (err) {
            ...
            var data = {
              insulinSensitivity : struct.extractShort(result.payload,6),
              bgTarget: struct.extractShort(result.payload,4)
            };
            cb(null, data);
          }
        });
  ...
  var getRecords = function(request,numRecords,recordSize,percentage,progress,cb) {
    ...
    var cmd = null;
    if(modelNumber === 'IR1295' && request.recordType.value !== RECORD_TYPES.BASAL_PROGRAM_NAMES.value) {
  ...
  function buildSettingsRecord(data, postrecords) {
    //TODO: see if we should use requestbasalProgramNames
    ...
    console.log(carbRatio, insulinSensitivity, bgTarget);
  ...
  return {
    ...
    getConfigInfo: function (progress, data, cb) {
      ...
      function(err, results){
          if(err) {
            ...
            data.settings = {};
            data.settings.basalPrograms = results[0];
            data.settings.activeProgram = results[1];
            _.extend(data.settings, results[2]);
            _.extend(data.settings, results[3]);
            data.settings.advancedSettings = results[4];
            progress(100);
    ...
    fetchData: function (progress, data, cb) {
      ...
      async.series({
        ...
        basalProgramNames: function(callback){
          readAllRecords(requestBasalProgramNames(), 70, function(err, data) {
            return callback(err,data);
          });
        },

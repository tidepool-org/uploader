...
class TrueMetrix {
  ...
  async commandResponse(cmd) {
    ...
    do {
      ...
      result = this.hidDevice.receiveTimeout(READ_TIMEOUT);
  ...
  async getDeviceId() {
    ...
    const model = TrueMetrix.filterByType(results, TYPES.MODEL);
...
module.exports = (config) => {
  ...
  return {
    ...
    getConfigInfo(progress, data, cb) {
      ...
      (async () => {
        driver.hidDevice.receiveTimeout(READ_TIMEOUT); // flush any data in the buffer
        progress(20);
    ...
    processData(progress, data, cb) {
      ...
      _.forEach(data.glucose, (nibbles, index) => {
        ...
        const value = (threeBits << 8) + parseInt(nibbles.slice(8, 10), 16);
        ...
        if (controlSolution === 0) {
          ...
          // this.constructor.addOutOfRangeAnnotation(
          // recordBuilder, GLUCOSE_LO, GLUCOSE_HI, 1, 'bg');
      ...
      console.log('POST records:', data.postRecords);
      progress(100);
      cb(null, data);
    },
    ...
    uploadData(progress, data, cb) {
      ...
      // TODO: check if data.postRecords.length empty
      ...
      cfg.api.upload.toPlatform(
        ...
        'jellyfish', // TODO: change to dataservices
      );
    },

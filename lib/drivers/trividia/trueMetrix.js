...
class TrueMetrix {
  constructor(cfg) {
    this.cfg = cfg;
    const hidDevice = this.cfg.deviceComms;
  }

  extractPacketIntoMessage(bytes) {
    const str = String.fromCharCode.apply(null, bytes);
    const results = str.match(/\(([^)]*)\)(\w{2})/);
    console.log('Parsed:', results);

    if (results != null) {
      if (this.verifyChecksum(results[1], results[2])) {
        return results[1];
      }
    }
    return null;
  ...
  static getOneRecord(cmd, data, callback) {
    let retry = 0;
    const robj = {};
    let error = false;
    ...
    async.doWhilst(
      (whilstCb) => {
        this.commandResponse(this.buildPacket(cmd, cmd.length), (err, record) => {
          if (err) {
  ...
  commandResponse(commandpacket, callback) {
    this.hidDevice.send(commandpacket, () => {
      this.getASTMMessage(5000, 3, (err, result) => {
        ...
        const message = this.decodeMessage(result);
      ...
      (callback) => {
        this.hidDevice.receive((raw) => {
          try {
            ...
            message = this.extractPacketIntoMessage(packet);
...
module.exports = (config) => {
  ...
  return {
    ...
    getConfigInfo: function (progress, data, cb) {
      ...
      driver.getOneRecord(COMMANDS.WAKEUP, data, (err, result) => {

        driver.getOneRecord(COMMANDS.IDENTIFY, data, (err, result) => {
...
/*
    ...
    disconnect(progress, data, cb) {
      ...
      hidDevice.send(driver.buildPacket(0x04, 1), (err, result) => {
        progress(100);
        cb(null, data);
      });
    },

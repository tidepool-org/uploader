...
const SEND_TIMEOUT = 500; // messages sent are small, so timeout can be shorter
...
class TrueMetrix {
  ...
  static verifyChecksum(frame, expected, type) {
    ...
    if (type === CHECKSUM_TYPE.DOWNLOAD) {
      checkStr = _.padStart(_.toUpper(checksum.toString(16)), 4, '0');
    } else {
  ...
  async commandResponse(cmd) {
    ...
    try {
      ...
      await Promise.race([
        this.hidDevice.sendPromisified(TrueMetrix.buildPacket(cmd, cmd.length)),
        new Promise((resolve, reject) => setTimeout(
          reject,
          SEND_TIMEOUT,
          new Error('Timed out while sending a message.'),
        )),
      ]);
      debug('Sent.');
    } catch (err) {
...
module.exports = (config) => {
  ...
  return {
    ...
    connect(progress, data, cb) {
      debug('in connect!');
      (async () => {
        await cfg.deviceComms.connectPromisified(data.deviceInfo, _.noop());
    ...
    getConfigInfo(progress, data, cb) {
      ...
      (async () => {
        ...
        common.checkDeviceTime(
          sundial.formatDeviceTime(cfg.deviceInfo.meterTime), cfg,
          (timeErr) => {
            if (timeErr) {

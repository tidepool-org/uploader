...
class TrueMetrix {
  ...
  async getDeviceID() {
    ...
    let serial;
    const results = [];
    results.push(await this.getRecord(COMMANDS.IDENTIFY));
    results.push(await this.getRecord(COMMANDS.GET_SERIAL));

    // TODO: look through array for i message and z message

    return `trueMetrix-${model}-${serial}`;
  }
  ...
  return {
    ...
    getConfigInfo(progress, data, cb) {
      ...
      (async () => {
        ...
        cfg.deviceInfo.deviceId = await driver.getDeviceID();
        [cfg.deviceInfo.firmwareVersion] = await driver.getRecord(COMMANDS.GET_FIRMWARE_VERSION);

        // TODO: parse time and check against server time
        const [meterTime] = await driver.getRecord(COMMANDS.GET_METER_TIME);
        console.log('Meter time:', meterTime);

        console.log('DeviceInfo:', cfg.deviceInfo);

...
class TrueMetrix {
  ...
  async commandResponse(cmd) {
    ...
    const bytesWritten = await this.hidDevice.sendPromisified(TrueMetrix.buildPacket(cmd, cmd.length));
  ...
  static filterByType(results, type) {
    const filtered = _.filter(results, (result, index, self) => result != null
  ...
  return {
    ...
    getConfigInfo(progress, data, cb) {
      ...
      (async () => {
        ...
        progress(40);
        [cfg.deviceInfo.firmwareVersion] = await driver.commandResponse(COMMANDS.GET_FIRMWARE_VERSION);
        progress(60);

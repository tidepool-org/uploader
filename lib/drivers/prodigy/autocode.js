...
class Autocode {
  ...
  async commandResponse(cmd) {
    ...
    while (!complete) {
      ...
      if (length === 1) {
        //this is a packet with the total message length
        messageLength = result[1];
      }
      ...
      for (let i = 1; i <= length && i < result.length; i++) {
        ...
        if (this.cfg.deviceInfo.v6 && buffer.at(0) != 0x01 && length == i) {
          // v6 meter serial number packet doesn't have STX, ETX or END,
          // so we use the length field to determine size of packet
          complete = true;
        }

        if (cmd === READ_SERIAL && messageLength === buffer.length) {
          complete = true;
        }
      }
    }
  ...
  async getSerialNumber() {
    ...
    let endIndex = nullIndex !== -1 ? nullIndex : result.indexOf(END);
    let beginIndex = 1;

    if (endIndex == -1 && this.cfg.deviceInfo.v6) {
      // V6 meter serial number packet is just serial number, no END byte or null bytes
      beginIndex = 0;
      endIndex = result.length - 1;
    }

    return String.fromCharCode(...result.slice(beginIndex, endIndex));
  }
...
module.exports = (config) => {
  ...
  function buildBGRecords(data) {
    _.forEach(data.records, (record) => {
      ...
      if (record.units === 'mmol/L') {
        if (record.value === 'Lo.') {
          record.value = 1.0;
        } else if (record.value === 'Hi.') {
          record.value = 33.4;
        } else {
  ...
  return {
    ...
    connect(progress, data, cb) {
      hidDevice.connect(cfg.deviceInfo, Autocode.getRecords, async (err) => {
        ...
        if (cfg.deviceInfo.hidDevice.collections[0].featureReports.length > 0) {
          const buf = new ArrayBuffer(9);
          const bytes = new Uint8Array(buf);
          cfg.deviceInfo.v6 = false;
          ...
          debug('No feature reports');
          cfg.deviceInfo.v6 = true;
        }

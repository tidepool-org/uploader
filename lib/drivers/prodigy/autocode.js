...
class Autocode {
  ...
  buildPacket(command, data = []) {
    ...
    let ctr = struct.pack(bytes, 0, 'bb', size, command);
  ...
  async sendCommand(bytes) {
    ...
    //   //await Autocode.wait(10); // give the meter some time to read the report
    // }
    debug('Sent command.');
  }
  ...
  async commandResponse(cmd, payload) {
    ...
    while (ctr < result.length) {
      if (result[ctr] !== 0x00) {
        const packetLength = result[ctr];
        message.concat(result.slice(ctr + 1, ctr + packetLength));
      }
  ...
  async getRecords() {
    const result = await this.commandResponse(COMMAND.READ_RECORDS);
...
module.exports = (config) => {
  ...
  return {
    ...
    connect(progress, data, cb) {
      hidDevice.connect(cfg.deviceInfo, Autocode.getRecords, (err) => {
        if (err) {
          return cb(err);
        }

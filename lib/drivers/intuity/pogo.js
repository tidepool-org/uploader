...
class POGO {
  ...
  static buildPacket(command, payload = '') {
    ...
    const checksum = crc.calcCRC_I(cmdpayload, cmdpayload.length);
    debugger;

    var ctr = struct.pack(bytes, 0, 'b1z', ASCII_CONTROL.STX, command);
    ctr += struct.copyBytes(bytes, ctr, payload, payload.length);
    ctr += struct.pack(bytes, ctr, '4zb', checksum.toString(16).toUpperCase(), ASCII_CONTROL.ETX);
  ...
  static extractPacketIntoMessages(bytes) {
    const checksum = parseInt(String.fromCharCode.apply(null, bytes.slice(-4)), 16);
  ...
  async getSerialNumber() {
    ...
    const response = struct.unpack(result, 0, 'b13Z.b5Z.b5Z.b5z', [
      'code',
      'serialNumber',
      'prefixFW',
      'mainSWVersion',

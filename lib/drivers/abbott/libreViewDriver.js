...
module.exports = (config) => {
  ...
  return {
    ...
    fetchData: (progress, payload, cb) => {
      ...
      Object.keys(TYPES_TO_READ).forEach((columnKey, index) => {
        const column = TYPES_TO_READ[columnKey];
        if (column) {
          const header = parsed.data[0][column];
          Object.keys(GLUCOSE_HI).forEach((unit) => {
            if (header.endsWith(unit)) {
              typesUnits[index] = unit;
            }
          });
        }
      });
      ...
      for (const recordType of [RECORD_TYPE_HISTORIC, RECORD_TYPE_SCAN, RECORD_TYPE_STRIP]) {
        const units = typesUnits[recordType];
        if (GLUCOSE_HI[units] === undefined) {
          const error = `Unexpected units for ${recordType}: ${units}`;
          debug('Error:', error);
          return cb(new Error(error));
        }
      }

      const ketoneUnits = typesUnits[RECORD_TYPE_KETONE];
      if (ketoneUnits !== 'mmol/L') {
        const error = `Unexpected units for ${RECORD_TYPE_KETONE}: ${ketoneUnits}`;
        debug('Error:', error);
        return cb(new Error(error));
      }
      ...
      for (let i = parsed.data.length - 1; i >= 0; --i) {
        ...
        datum.jsDate = sundial.parseFromFormat(datum[2],
          LIBREVIEW_TS_FORMAT);
        ...
        datum.units = typesUnits[recordType];
        ...
        events.filter(elem => (elem[3] === RECORD_TYPE_STRIP || elem[3] === RECORD_TYPE_SCAN))
          .forEach((record) => {
            ...
            if (record[3] === RECORD_TYPE_STRIP) {
              addOutOfRangeAnnotation(smbg, STRIP_GLUCOSE_LO[record.units], GLUCOSE_HI[record.units], 1, 'bg');
            } else {

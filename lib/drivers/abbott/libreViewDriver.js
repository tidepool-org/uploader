        ...
        events.filter(elem => (elem[3] === RECORD_TYPE_STRIP || elem[3] === RECORD_TYPE_SCAN))
          .forEach((record) => {
            ...
            if (record[3] === RECORD_TYPE_STRIP) {
              addOutOfRangeAnnotation(smbg, STRIP_GLUCOSE_LO[config.units], GLUCOSE_HI[config.units], 1, 'bg');
            } else {
              addOutOfRangeAnnotation(smbg, GLUCOSE_LO[config.units], GLUCOSE_HI[config.units], 1, 'bg');
            }

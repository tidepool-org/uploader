  ...
  return {
          ...
          .forEach((record) => {
            ...
            if (record[3] === RECORD_TYPE_SCAN) {
              if (record.device.includes('Libre 3')) {
                // Libre 3 "Views" are tagged as "scans", so don't record these
                // See https://tidepool.atlassian.net/browse/UPLOAD-970 for details
                return;
              }
    ...
    uploadData: (progress, payload, cb) => {
      ...
      async.eachSeries(Object.keys(payload.devices), (key, done) => {
        ...
        payload.deviceModel = payload.deviceModel.concat(device.info.deviceModel, '; '); // for metrics

  ...
  return {
    ...
    processData: (progress, payload, cb) => {
      ...
      Object.keys(payload.devices).forEach((key) => {
        ...
        events.filter((elem) => {
          if (elem.device.includes('Libre 3')) {
            // Libre 3 "views" are tagged as "scans" and should be handled as cbg
            return (elem[3] === RECORD_TYPE_HISTORIC) || (elem[3] === RECORD_TYPE_SCAN);
          }
          return elem[3] === RECORD_TYPE_HISTORIC;
        })
          .forEach((record) => {
            ...
            // TODO: add "view" to payload for Libre 3
        ...
        events.filter(elem => (elem[3] === RECORD_TYPE_STRIP || elem[3] === RECORD_TYPE_SCAN))
          .forEach((record) => {
            ...
            if (record[3] === RECORD_TYPE_SCAN) {
              if (record.device.includes('Libre 3')) {
                // Libre 3 "Views" are tagged as "scans" so handle them as cbg instead
                return;
              }

...
module.exports = (config) => {
  const LIBREVIEW_TS_FORMAT = 'DD/MM/YYYY HH:mm';
  ...
  return {
    ...
    fetchData: (progress, payload, cb) => {
      ...
      debug('Carelink data', config.fileData.length);

      const endOfPreamble = config.fileData.indexOf('\n') + 1;
      // Setup the preamble to have everything up to the header line
      payload.preamble = csv.parse(config.fileData.substr(0, endOfPreamble), {});

      // Store the rest of the data
      const parsed = csv.parse(config.fileData.substr(endOfPreamble), {
        header: true,
        dynamicTyping: true,
      });
    ...
    uploadData: (progress, payload, cb) => {
      ...
      async.eachSeries(Object.keys(payload.devices), (key, done) => {
        ...
        const sessionInfo = {
          ...
          // blobId: payload.blobId, TODO: save csv
        };

...
module.exports = function (config) {
  ...
  var patterns = {
    ...
    logEntry: {
      pattern: '^[^0-9HL]*([0-9HL][0-9OI][0-9 ])  ' + dateTimePattern + ' [0-9A-Fa-f][0-9A-Fa-f] (0x[0-9A-Fa-f][0-9A-Fa-f])',
      handler: function(match, packet) {
        var entry = {};
        var reading = match[1];
  ...
  var extractPacket = function (bytes) {
    ...
    _.forEach(packet.lines, function(line, _index) {
      var found = false;
  ...
  var listenForPacket = function (timeout, parser, callback) {
    var abortTimer = setTimeout(function () {
      ...
      error.code = 'E_UNPLUG_CABLE_AND_RETRY';
      callback(error, null);
    }, timeout);

...
export default class FreeStyleLibreProtocol {
          ...
          if (buffer !== undefined && buffer.length > 0) {
            ...
            if (hidPacket.responseType === COMMAND.TEXT_RESPONSE) {
              ...
              if (atpPacket === null) {
                ...
                debug('readResponse: Received wrong sequence number', atpPacket.sequenceTx,
              ...
              // TODO: handle device ack packets
              // either throw error on unexpected sequence number
              // or actually cache sent packets and resend starting from requested number
  ...
  requestBinaryResponse(opCode, aapData, cb) {
    ...
    this.hidDevice.send(
      this.constructor.buildHidPacket(COMMAND.BINARY_REQUEST, atpFrameArray), () => {
        const flushData = this.buildAtpFrame(this.constructor.buildAapFrame(OP_CODE.FLUSH_BUFFERS));
        this.sequenceTx = (this.sequenceTx + 1) % 0x100;
        this.sendCommand(COMMAND.BINARY_REQUEST, COMMAND.BINARY_RESPONSE, flushData,
          (err, aapPackets) => {
            if (err) {

 ...
 * --- CRC:     32 bit CRC (a variation of CRC32-CCITT, using 8 XORs but only a size 16 lookup table)
 * - ATP_DATA
...
export class FreeStyleLibreProtocol {
  ...
  requestTextResponse(command, successCallback, errorCallback) {
    debug('requestTextResponse: Sending command: 0x' + COMMAND.TEXT_REQUEST.toString(16),
      ', data: ', command);
  ...
  requestBinaryResponse(opCode, aapData, cb) {
    ...
    debug('requestBinaryResponse: Sending command: 0x' + COMMAND.BINARY_REQUEST.toString(16),

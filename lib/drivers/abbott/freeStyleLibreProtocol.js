...
import {OP_CODE, COMMAND} from './freeStyleLibreConstants';
...
// to know then all AAP responses for a request were received, we can look for the AAP data length of the final packet
const FINAL_AAP_DATA_LENGTHS = {};
FINAL_AAP_DATA_LENGTHS[OP_CODE.GET_DATABASE] = 0; // contrary to the specs the db tables are concluded by just 0x31, not 0x81 0x31 0xcc
...
export class FreeStyleLibreProtocol {
  ...
  readResponse(responseType, timeout, cb) {
    ...
    const aapPackets = [];
    let receiveBuffer = new Buffer(0);
    async.doWhilst(
        callback => {
          resetReceiveTimeout();
            ...
            if (buffer !== undefined && buffer.length > 0) {
              ...
              if (hidPacket.responseType === COMMAND.TEXT_RESPONSE) {
                ...
                if (this.sequenceRx === atpPacket.sequenceTx) {
                  ...
                  // try to get missing package by sending an ACK containing the next expected sequence number to the device
                  this.sendAck();
                }
        ...
        () => {
          if (responseType === COMMAND.TEXT_RESPONSE) {
            ...
            //debug('readResponse: binary data buffer length: ' + receiveBuffer.length);
            ...
            // we can recognise the last AAP packet of a response from its data length or from the number of AAP packets
            const lastAapPacket = aapPackets[aapPackets.length - 1];
            const finalLength = FINAL_AAP_DATA_LENGTHS[lastAapPacket['opCode']];
            // if the finalLength value is negative it describes the index of the final AAP packets required
            if (finalLength < 0) {
  ...
  static validateTextChecksum(dataString, expectedChecksum) {
    ...
    if (calculatedChecksum !== expectedChecksum) {
      debug('validateTextChecksum: checksum: ' + calculatedChecksum + ' ?= ' + expectedChecksum);
    }
  ...
  static parseAapFrame(buffer) {
    ...
    for (let i = 0; i <= 2; i++) {
      if (i >= buffer.length) {
        //debug('parseAapFrame: still missing bytes...');
        return null;
      }
  ...
  static buildAapFrame(opCode, dataArray) {
    ...
    const packetFormat = aapDataLengthBytes.length + 'B' + 'b' + dataArray.length + 'B';
  ...
  buildAtpFrame(aapFrameArray) {
    ...
    if (atpCrc === undefined) {
      debug('buildAtpFrame: Error looking up CRC for "' + Buffer.from(aapFrameArray).toString('hex') + '"');
      return null;
    }
  ...
  sendCommand(command, responseType, data, cb) {
    debug('sendCommand: Sending command: 0x' + command.toString(16), ', data: ', Buffer.from(data).toString('hex'));
  ...
  requestBinaryResponse(opCode, aapData, cb) {
    ...
    this.hidDevice.send(this.constructor.buildHidPacket(COMMAND.BINARY_REQUEST, atpFrameArray), () => {

      let flushData = this.buildAtpFrame(this.constructor.buildAapFrame(OP_CODE.FLUSH_BUFFERS));
      this.sequenceTx = (this.sequenceTx + 1) % 0x100;
      this.sendCommand(COMMAND.BINARY_REQUEST, COMMAND.BINARY_RESPONSE, flushData, (err, aapPackets) => {
        if (err) {

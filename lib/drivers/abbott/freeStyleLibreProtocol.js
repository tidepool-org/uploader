...
export default class FreeStyleLibreProtocol {
  ...
  requestBinaryResponse(opCode, aapData, cb) {
    ...
    this.hidDevice.send(packetToSend, () => {
  ...
  getReaderTime(cb) {
    this.requestTextResponse('$rdt?', (data) => {
      ...
      /*
        ...
        arr[7] contains offset (in seconds) between factory time and user time
      */
      const dateTime = new Date(Date.UTC(...arr.slice(0, -2)) + (arr[7] * sundial.SEC_TO_MSEC));

      if (arr[6] === 0) {
        debug('Reader time not valid');
        cb(null);
      } else {
        cb({ readerTime: dateTime });
      }
    }, cb);
  }

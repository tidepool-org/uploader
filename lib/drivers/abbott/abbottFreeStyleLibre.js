...
import {
  ...
  CFG_TABLE_ID,
  DEVICE_MODEL_NAME,
  COMPRESSION } from './freeStyleLibreConstants';
...
export default function (config) {
  ...
  return {
    ...
    fetchData(progress, data, cb) {
      ...
      getterFunctions = getterFunctions.map(getterFunction =>
    ...
    processData(progress, data, cb) {
      ...
      if (data.deviceInfo.productId === FSLIBRE_PRO_PRODUCT_ID) {
        data.deviceInfo.deviceId = `${data.deviceInfo.driverId}Pro-${data.deviceInfo.serialNumber}`;
      } else {

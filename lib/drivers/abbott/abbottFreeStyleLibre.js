...
export default function (config) {
  ...
  function readResponse(commandType, timeout, cb) {
    let abortTimer = null;
  ...
  function buildPacket(commandType, data) {
  ...
  function sendCommand(command, responseData, data, cb) {
  ...
  function parseTextResponse(responseData) {
  ...
  function requestTextResponse(command, successCallback, errorCallback) {
    sendCommand(COMMAND.TEXT, COMMAND.TEXT, command + '\r\n', function (err, responseData) {
      ...
      const data = parseTextResponse(responseData);
  ...
  function getDBRecords(command, successCallback, errorCallback) {
  ...
  function getFirmwareVersion(cb) {
  ...
  function initCommunication(cb) {
    const initFunctions = [
      function (cb) {
        sendCommand(0x04, null, '', cb);
      },
      function (cb) {
        sendCommand(0x05, null, '', cb);
      },
      function (cb) {
        sendCommand(0x15, null, '', cb);
      },
      function (cb) {
        sendCommand(0x01, null, '', cb);
      },
    ];
  ...
  return {
    ...
    connect: function (progress, data, cb) {
      ...
      hidDevice.connect(data.deviceInfo, probe, function (err) {
        if (err) {
          return cb(err);
        }
    ...
    disconnect: function (progress, data, cb) {
      ...
      hidDevice.send(buildPacket([ASCII_CONTROL.EOT], 1), function () {
        progress(100);
        cb(null, data);
      });
    },
    ...
    cleanup: function (progress, data, cb) {
      debug('in cleanup');
      if (!data.disconnect) {
        hidDevice.disconnect(data, function () {
          progress(100);
          data.cleanup = true;

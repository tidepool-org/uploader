...
import {
  clone, assign, forEach, forEachRight, map,
} from 'lodash';
...
export default function (config) {
  ...
  return {
    ...
    fetchData(progress, data, cb) {
      ...
      let getterFunctions;
      if (cfg.deviceInfo.productId === FSLIBRE_PRO_PRODUCT_ID) {
        getterFunctions = [
      ...
      getterFunctions = map(getterFunctions, (getterFunction) => ((callback) => updateProgress(getterFunction, callback)));
    ...
    processData(progress, data, cb) {
      ...
      data.post_records = dataParser.processAapPackets(data.aapPackets, cfg.deviceInfo.dbRecordNumber);

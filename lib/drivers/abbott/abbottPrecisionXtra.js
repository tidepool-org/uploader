...
module.exports = function (config) {
  ...
  var pxErrorHandler = function(info) {
  ...
  var pxCommandResponse = function (commandpacket, callback) {
    serialDevice.writeSerial(commandpacket.packet, function (err) {
      ...
      listenForPacket(10000, commandpacket.parser, function(err, result) {
        if (err === 'TIMEOUT') {
          callback('TIMEOUT', null);
        } else {
          callback(err, result);
  ...
  return {
    ...
    fetchData: function (progress, data, cb) {
      ...
      getAllData({}, function (err, result) {
        ...
        common.checkDeviceTime(data.deviceTime, cfg.timezone, function(checkErr) {
          cb(checkErr, data);
        });
      });
    },

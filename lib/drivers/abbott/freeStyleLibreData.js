...
import {
  ...
  GLUCOSE_LO,
  CRC16_TABLE,
} from './freeStyleLibreConstants.js';
import FreeStyleLibreProtocol from './freeStyleLibreProtocol.js';
  ...
  handleCompressedDatabase(aapPacket) {
    ...
    while (compressedOffset < aapPacket.dataLength - CRC32_LENGTH) {
      ...
      if (blockType === COMPRESSION_TYPE.UNCOMPRESSED) {
        ...
        decompressedBuffer = decompressedBuffer.concat(Array.from({length: blockLength}, () => 0));
      } else {

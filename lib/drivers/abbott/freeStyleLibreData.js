...
import {OP_CODE, ERROR_DESCRIPTION, DB_RECORD_TYPE, CFG_TABLE_ID, RESULT_VALUE_TYPE} from './freeStyleLibreConstants';
...
export class FreeStyleLibreData {
  ...
  addHeaderFields(recordBuilder, headerFields) {
    recordBuilder.with_deviceTime(headerFields.displayTime)
  ...
  buildTimeChangeRecords() {
    for (let record of this.records.filter(elem => elem.headerFields.recordType === DB_RECORD_TYPE.TIME_CHANGE_RESULT )) {
      ...
      this.addHeaderFields(timeChange, record.headerFields);
      this.postRecords.push(timeChange.done());
    }
  }
  ...
  buildCBGRecords() {
    for (let record of this.records.filter(elem => elem.headerFields.recordType === DB_RECORD_TYPE.HISTORICAL_DATA )) {
      ...
      this.addHeaderFields(cbg, record.headerFields);
  ...
  buildMeasurementRecords() {
    for (let record of this.records.filter(elem =>
      ...
      if (recordBuilder) {
        this.addHeaderFields(recordBuilder, record.headerFields);
    ...
    if (headerFields.recordType === DB_RECORD_TYPE.TIME_CHANGE_RESULT) {
      ...
      // TODO: validate CRC16
        if (timeChangeFields.valid) {
          this.records.push({headerFields, timeChangeFields});
        }
      ...
      if (historyFields.dataQualityErrorFlags === 0) {
        this.records.push({headerFields, historyFields});
      }
      ...
      if (measurementFields.dataQualityErrorFlags === 0) {
        this.records.push({headerFields, measurementFields});
      }

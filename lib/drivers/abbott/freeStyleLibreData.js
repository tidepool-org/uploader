...
export default class FreeStyleLibreData {
  ...
  processAapPackets(aapPackets, dbRecordNumber) {
    ...
    if (this.records.length === 0) {
      debug('processAapPackets: no valid database records found in',
  ...
  buildTimeChangeRecords() {
    this.records.filter(elem => elem.headerFields.recordType === DB_RECORD_TYPE.TIME_CHANGE_RESULT)
      .forEach((record) => {
        const oldDateTime = this.getDateTime(record.timeChangeFields.oldReaderTime,
  ...
  handleDatabase(aapPacket) {
    ...
    const headerFields = struct.unpack(aapPacket.data, RECORD_HEADER_OFFSET, FORMAT.RECORD_HEADER,
    ...
    if (headerFields.recordType === DB_RECORD_TYPE.TIME_CHANGE_RESULT) {
      const timeChangeFields = struct.unpack(aapPacket.data, RECORD_OFFSET, FORMAT.TIME_CHANGE,
        ['oldReaderTime', 'oldUserTimeOffset', 'valid', 'unused', 'CRC16']);

      if (this.checkCrc16(aapPacket.data.slice(RECORD_HEADER_OFFSET,
        RECORD_OFFSET + (FORMAT_LENGTH.TIME_CHANGE - 2)), timeChangeFields.CRC16)) {
      ...
      const historyFields = struct.unpack(aapPacket.data, RECORD_OFFSET, FORMAT.HISTORICAL_DATA,
      ...
      if (this.checkCrc16(aapPacket.data.slice(RECORD_HEADER_OFFSET,
        RECORD_OFFSET + (FORMAT_LENGTH.HISTORICAL_DATA - 2)), historyFields.CRC16)) {
      ...
      struct.unpack(aapPacket.data, RECORD_OFFSET + RESULT_VALUE_OFFSET, 's', ['resultValue'],
        measurementFields);
      ...
      struct.unpack(aapPacket.data, RECORD_OFFSET + DATA_QUALITY_ERROR_FLAGS_OFFSET, 's',
        ['dataQualityErrorFlags'], measurementFields);

      const CRC_OFFSET = 12;
      struct.unpack(aapPacket.data, RECORD_OFFSET + CRC_OFFSET, 's', ['CRC16'], measurementFields);
      if (this.checkCrc16(aapPacket.data.slice(RECORD_HEADER_OFFSET, RECORD_OFFSET + CRC_OFFSET),
      ...
      const CRC_OFFSET = DB_WRAP_RECORDS[headerFields.recordType].CRC_OFFSET;

...
module.exports = function (config) {
  ...
  */
  ...
  var getRecords = function(packet, callback) {
      ...
      async.series({
        setSuspend : function(callback){
            getOneRecord(proc.sendCommand(proc.MESSAGES.SUSPEND), true, function (err, result) {
              ...
              if(result) {
                getRecords(proc.readPage(proc.MESSAGES.SUSPEND,[0x01,0x01]), function (err, result) {
                  if(err) {
                    return callback(err,null);
                  }
                  if(result) {
          ...
          async.whilst(
              ...
              function (callback) {
                  getOneRecord(proc.sendCommand(proc.MESSAGES.READ_HISTORY), true, function (err, result) {
                    ...
                    if(result) {
                      getRecords(proc.readPage(proc.MESSAGES.READ_HISTORY,[0x01,count]), function (err, result) {
                        if(err) {
                          return callback(err,null);
                        }
                        if(result) {

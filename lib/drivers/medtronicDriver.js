...
module.exports = function (config) {
  ...
  */
  ...
  var getRecords = function(packet1, packet2, callback) {

    hidDevice.send(packet1.command, function () {
      hidDevice.send(packet2.command, function () {
        getMessage(2000, false, function(err, result) {
          if (err) {
            return callback(err, null);
          }
          var decoded = decodeMessage(packet1.parser, result.bytes);
          console.log('Records:', decoded);
          messageBuffer.reset();
    ...
    fetchData: function (progress, data, cb) {
      ...
      async.series({
        readHistory : function(callback){
            getOneRecord(proc.readHistory(), true, function (err, result) {
              if(err) {
                return callback(err,null);
              }
              if(result) {
                getRecords(proc.readPage(0x80,[0x01,0x00]), proc.buildPaddingPacket(43), function (err, result) {
                  if(err) {
                    return callback(err,null);
                  }
                  if(result) {
                    // TODO: do something with result
                    return callback(null, 'readHistory');
                  } else {

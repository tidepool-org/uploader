...
module.exports = function (config) {
            ...
            function (callback) {
                getMessage(20000, false, function(err, result) {
                  ...
                  if(result.resultsInReply) {
                    // for the last history page, the pump returns some of the
                    // records in the first reply... sneaky!
                    page.set(result.resultsInReply, 0);
                    page.set(result.bytes, result.resultsInReply.length);
                  } else {
      ...
      function (callback) {
        hidDevice.receive(function(raw) {
          ...
          if (message.messageLength >= MEDTRONIC_PACKET_START) {
            ...
            if (message.bytes.slice(MEDTRONIC_PACKET_START)[0] === ASCII_CONTROL.ACK ) {
  ...
  return {
    ...
    cleanup: function (progress, data, cb) {
      ...
      if(isBrowser) {
        /* FIXME: move this to the start, so that we can re-enter remote command even if upload fails

...
module.exports = function (config) {
  ...
  var readInsulinSensitivities = function () {
    ...
    return {
      ...
      parser: function (packet) {
        var medtronicMessage = packet.slice(MEDTRONIC_PACKET_START);
        return {insulinSensitivities : proc.getInsulinSensitivities(medtronicMessage)};
      }
    };
  };
    ...
    return {
      ...
      parser: function (packet) {
        ...
        for(var j = 0; j < 47; j++ ) {
          ...
          if ((j > 0) && (schedule.offset === 0 && schedule.rate === 0 && schedule.q === 0)) {
            break;
          }
            ...
            function (callback) {
                getMessage(20000, false, function(err, result) {
                  ...
                  if (result) {
                    if(result.error) {
                      messageBuffer.reset();
                      return callback(new Error('Error byte flagged'));
                    }
      ...
      function (callback) {
        hidDevice.receive(function(raw) {
          ...
          if(struct.extractByte(header,0) == 0x51 && _.isEqual(struct.extractString(header,2,6), serial)) {
            ...
            if(struct.extractByte(packet,31) !== 0) {
            // if(Math.floor(Math.random() * 6) === 3) { //TODO: use this for testing
              clearTimeout(abortTimer);
              console.log('Error byte flagged');
              message.error = true;
  ...
  return {
    ...
    getConfigInfo: function (progress, data, cb) {
      ...
      function(err, results){
          ...
          if(!err){
              ...
              settings.insulinSensitivity = results.insulin_sensitivities.insulinSensitivities;
              settings.units = {
    ...
    fetchData: function (progress, data, cb) {
      ...
      async.series({
        readHistory : function(callback){
          ...
          async.whilst(
              function () { return count < data.currentPage; },
              function (callback) {
    ...
    uploadData: function (progress, data, cb) {
      ...
      var sessionInfo = {
        ...
        deviceManufacturers: ['CareLink'], //TODO: change to Medtronic once viz is fixed

  ...
  var readInsulinSensitivities = function () {
    ...
    return {
      ...
      parser: function (packet) {
        ...
        for(var j = 0; j < 8; j++ ) {
          var sensitivity = struct.unpack(medtronicMessage,(j*2)+1,'bb',['offset', 'val']);
          if (sensitivity.offset === 0 && sensitivity.val === 0) {
            break;
          }
          var startTime = (sensitivity.offset & proc.BITMASKS.RIGHT_SIX) * 30 * sundial.MIN_TO_MSEC;
          var amount = sensitivity.val + ((sensitivity.offset & proc.BITMASKS.LEFT_TWO) << 2);
          insulinSensitivities.push( { start: startTime, amount: amount } );
        };

        return {insulinSensitivities : insulinSensitivities};
      }
    };
  };
  ...
  var readCarbRatios = function () {
    ...
    return {
      ...
      parser: function (packet) {
        ...
        var carbRatios = [];
        for(var j = 0; j < 8; j++ ) {
          var carbRatio = struct.unpack(medtronicMessage,(j*3)+2,'bS',['offset', 'ratio']);
  ...
  var readBGTargets = function () {
    ...
    return {
      ...
      parser: function (packet) {
        ...
        var bgTargets = [];
        for(var j = 0; j < 8; j++ ) {
          var bgTarget = struct.unpack(medtronicMessage,(j*3)+1,'bbb',['offset', 'low','high']);

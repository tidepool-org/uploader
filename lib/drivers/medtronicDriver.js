  ...
  var readModel = function () {
    ...
    return {
      ...
      parser: function (packet) {
        ...
        var strokesPerUnit = 40.0;
        var larger = true;
        // TODO: figure out which model number need 10.0 for strokesPerUnit and larger = false
  ...
  var bcnCommandResponse = function (commandpacket, waitForENQ, callback) {
    hidDevice.send(commandpacket.command, function () {
      getMessage(20000, waitForENQ, function(err, result) {
        ...
        var decoded, decodeErr = null;
        try {
          decoded = decodeMessage(commandpacket.parser, result.bytes);
        } catch (err) {
          decodeErr = err;
        }
        messageBuffer.reset();
        callback(decodeErr, decoded);
      });
    });
  };
    ...
    getConfigInfo: function (progress, data, cb) {
      ...
      async.series({
        ...
        profile_std : function(callback){
          getOneRecord(readProfiles(MESSAGES.READ_PROFILE_STD, strokesPerUnit), false, function(err, result) {
            if(err) {
              return cb(err,null);
            }
        ...
        profile_a : function(callback){
          getOneRecord(readProfiles(MESSAGES.READ_PROFILE_A, strokesPerUnit), false, function(err, result) {
            if(err) {
              return cb(err,null);
            }
        ...
        profile_b : function(callback){
          getOneRecord(readProfiles(MESSAGES.READ_PROFILE_B, strokesPerUnit), false, function(err, result) {
            if(err) {
              return cb(err,null);
            }

                ...
                getMessage(20000, false, function(err, result) {
                  ...
                  if (result) {
                    ...
                    if(result.resultsInReply &&
                      ...
                      console.log('Part', count, 'of page:', common.bytes2hex(decoded.message));
                      ...
                      if(_sum_lsb(payload) !== checksum) {
                        return callback(new Error('Invalid checksum. Please try to upload again.'));
                      };
                    }
      ...
      proc.processPages(data, function (err, records) {
        ...
        proc.processCBGPages(data, function (cgmErr, cgmRecords) {
          ...
          if(cgmDateRecords.length === 0) {
            console.log('No CBG records found on pump');
          } else {
            ...
            // only pump time change events are uploaded
            cfg.cgmTzoUtil = cgmResult.tzoUtil;
          }
          proc.updateConfig(cfg); //update config with the two TZOUtils

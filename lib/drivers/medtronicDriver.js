            ...
            function (callback) {
                getMessage(20000, false, function(err, result) {
                  ...
                  if(result.resultsInReply) {
                    ...
                    page.set(result.resultsInReply.slice(4), 0);
                    page.set(result.bytes, 64);
                  } else {
  ...
  return {
    ...
    processData: function (progress, data, cb) {
      ...
      proc.processPages(data, function (err, records) {
        var mostRecent = sundial.applyTimezone(records[records.length-1].jsDate, cfg.timezone).toISOString();

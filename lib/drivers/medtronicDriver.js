  ...
  var readCarbRatios = function () {
    ...
    return {
      ...
      parser: function (packet) {
        ...
        var units = medtronicMessage[0];
        //TODO: handle both grams and exchanges { 1 : 'grams', 2: 'exchanges' }

        var carbRatios = [];
        for(var j = 0; j < 8; j++ ) {
          var carbRatio = struct.unpack(medtronicMessage,(j*3)+2,'bS',['offset', 'ratio']);
          if (carbRatio.offset === 0 && carbRatio.q === 0 && carbRatio.r === 0) {
            break;
          }

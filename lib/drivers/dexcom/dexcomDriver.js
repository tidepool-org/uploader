...
module.exports = function (config) {
  ...
  var PREDICTED_FLAGS =  {
    URGENT_LOW_SOON: { value: 0x4000, name: 'URGENT_LOW_SOON'}
  };
  ...
  var parse_egv_records = function (header, data) {
    ...
    var format;
    var recordformat;
    console.log('Header:', header);  // TODO: remove this
    if (header.revision === 4) {
      ...
      format = 'iis.........b.ss';
      recordformat = ['systemSeconds', 'displaySeconds', 'glucose', 'trendArrowAndNoiseMode', 'predictedGlucose', 'crc'];
    } else {
    ...
    for (var i = 0; i < header.nrecs; ++i) {
      ...
      if (header.revision >= 5){
        // on G5 touchscreen receivers, predicted glucose will be zero
        // TODO: use software number in firmware header instead
        if (rec.predictedGlucose != 0) {
          rec.predictedFlags &= 0xFC00;
          ...
          console.log('Predicted glucose record:', JSON.stringify(rec)); // TODO: remove this
        }
      }
  ...
  var downloadDataPages = function (recordType, progress, callback) {
    ...
    dexcomCommandResponse(cmd, function (err, pagerange) {
      ...
      if((range.lo === 0xFFFFFFFF) && (range.hi === 0xFFFFFFFF)) {
        ...
        if(recordType === RECORD_TYPES.EGV_DATA) {
    ...
    for (var i = 0; i < pagedata.length; ++i) {
      ...
      for (var j = 0; j < page.data.length; ++j) {
        var reading = _.pick(page.data[j],
                             ...
                             'glucose', 'trendArrow', 'trendText', 'transmitterTimeSeconds', 'noiseMode', 'annotation');
  ...
  var prepCBGData = function (data) {
    ...
    for (var i = 0; i < data.cbg_data.length; ++i) {
      ...
      var payload = { trend: datum.trendText, internalTime: datum.internalTime, transmitterTimeSeconds:datum.transmitterTimeSeconds, noiseMode:datum.noiseMode };
  ...
  return {
    ...
    /* TODO: remove if not being used; detect function was bypassed as El Capitan hotfix,
             and we may need to do this again if Mac USB driver does not solve all issues

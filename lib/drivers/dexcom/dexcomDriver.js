...
import mungeUserSettings from './userSettingsChanges.js';
...
module.exports = (config) => {
  ...
  const EXERCISE_SUBTYPE = {
    LIGHT: { value: 1, name: 'low' },
    MEDIUM: { value: 2, name: 'medium' },
    HEAVY: { value: 3, name: 'high' },
  };
  ...
  const prepCBGData = (data) => {
    ...
    for (let i = 0; i < sorted.length; ++i) {
      ...
      let cbg = cfg.builder.makeCBG()
        ...
        .set('payload', payload);
      if (datum.backfilled != null) {
        cbg.with_backfilled(datum.backfilled);
      }
      ...
      switch (datum.eventType) {
        ...
        case EVENT_TYPE.EXERCISE.value: {
          record = cfg.builder.makePhysicalActivity()
            .with_reportedIntensity(datum.eventSubTypeName)
            .with_duration({
  ...
  return {
    ...
    processData(progress, data, cb) {
      ...
      if (data.deviceModel === 'ScoutReceiver') {
        data.deviceModel = 'G5 touchscreen receiver';
      } else if (data.deviceModel === 'OrionReceiver') {
        data.deviceModel = 'G6 touchscreen receiver';
      }

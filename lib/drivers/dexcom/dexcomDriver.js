...
module.exports = function (config) {
  ...
  var prepCBGData = function (data) {
    var dataToPost = [];
    var reverseSorted = _.orderBy(
      data.cbg_data, ['systemSeconds', 'systemTimeSeconds'], ['desc', 'asc']);
    var base = null;

    for (var i = 0; i < reverseSorted.length; ++i) {
      var datum = reverseSorted[i];
      var annotation = null;
      if (datum.glucose < 40) {
        annotation = {
      ...
      if (datum.backfilled != null) {
        ...
        if (datum.backfilled === true) {
          if(base && (base.systemSeconds === datum.systemSeconds)) {
            // if they have the same system seconds, they were backfilled
            // at the same time, so we use the systemTimeSeconds to calculate
            // the device time

            var difference = datum.systemTimeSeconds - base.systemTimeSeconds;

            datum.jsDate = new Date(datum.displayTimeMsec + (difference * 1000));

            console.log(datum.displayDate, '+', difference, '=', datum.jsDate, datum.glucose);

...
module.exports = function (config) {
  ...
  const HEALTH_SUBTYPE = {
    ...
    HIGH_SYMPTOMS: { value: 3, name: 'High Symptoms' },
    LOW_SYMPTOMS: { value: 4, name: 'Low Symptoms' },
  ...
  const parse_event_records = (header, data) => {
    const all = [];
    let ctr = 0;
    ...
    for (let i = 0; i < header.nrecs; ++i) {
      ...
      // TODO: event time can be changed by the user
      rec.internalTime = sundial.formatDeviceTime(new Date(rec.systemTimeMsec).toISOString());
      rec.eventTime = sundial.formatDeviceTime(new Date(rec.eventTimeMsec).toISOString());
      rec.displayDate = new Date(rec.displayTimeMsec);
      rec.data = data.subarray(ctr, ctr + flen);
      ctr += flen;
      all.push(rec);
  ...
  var readDataPages = function (rectype, startPage, numPages) {
    ...
    if (rectype == RECORD_TYPES.EGV_DATA || rectype == RECORD_TYPES.BACKFILLED_EGV_DATA) {
      ...
      parser = makeHeaderParser(parse_event_records);
    }
    ...
    _.forEach(pagedata, (page) => {
      ...
      _.forEach(parsed_payload.data, (record) => {
        const event = _.pick(record,'displaySeconds', 'displayDate', 'internalTime',
                             'systemSeconds', 'eventValue', 'eventTime',
  ...
  var prepEventData = function(data) {
    ...
    _.forEach(data.event_data, (datum, index) => {
      ...
      cfg.tzoUtil.fillInUTCInfo(record, datum.displayDate);
      record = record.done();
      delete record.index;

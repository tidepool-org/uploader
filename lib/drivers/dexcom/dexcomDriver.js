...
/* eslint-disable no-bitwise, no-use-before-define, no-plusplus */
...
module.exports = (config) => {
  ...
  const fetchOneDataPage = (recordType, pagenum, callback) => {
    const cmd = readDataPages(recordType, pagenum, 1);
  ...
  const downloadBackfilledEGVPages = function (deviceModel, progress, callback) {
  ...
  const downloadMeterPages = function (progress, callback) {
    downloadDataPages(RECORD_TYPES.METER_DATA, progress, callback);
  };

  const downloadSettingPages = function (progress, callback) {
    downloadDataPages(RECORD_TYPES.USER_SETTING_DATA, progress, callback);
  };

  const downloadEventPages = function (progress, callback) {
    downloadDataPages(RECORD_TYPES.USER_EVENT_DATA, progress, callback);
  };

  const processEGVPages = function (pagedata) {
    const readings = [];
  ...
  const processMeterPages = function (pagedata) {
    const readings = [];
    for (let i = 0; i < pagedata.length; ++i) {
      const page = pagedata[i].parsed_payload;
      for (let j = 0; j < page.data.length; ++j) {
  ...
  const processSettingPages = function (pagedata) {
    ...
    console.log(_.sortBy(settingRecords, (d) => d.systemSeconds));
  ...
  const getDeviceId = function (data) {
  ...
  const prepCBGData = function (data) {
    const dataToPost = [];
  ...
  const prepMeterData = function (data) {
    const dataToPost = [];
  ...
  const prepSettingsData = function (data) {
    const dataToPost = [];
  ...
  const prepEventData = function (data) {
    const dataToPost = [];
  ...
  return {
      ...
      cfg.deviceComms.connect(data.deviceInfo, dexcomPacketHandler, (err) => {
        ...
        getFirmwareHeader(true, (firmwareErr, obj) => {
          if (firmwareErr) {
            ...
            getPartitionInfo(true, (partErr, obj) => {
              if (partErr) {
                cb(partErr, obj);
              } else {
                do_ping((err, result) => {
          ...
          if (displayErr) {
            ...
            getSystemTime(true, (systemErr, systemTimeObj) => {
              if (systemErr) {
                ...
                common.checkDeviceTime(cfg, (err, serverTime) => {
                  if (err === 'updateTime') {
                    ...
                    setDisplayTimeOffset(newDisplayOffset, (err, result) => {
                      progress(100);
    ...
    fetchData(progress, data, cb) {
      ...
      const makeProgress = function (progfunc, start, end) {
        return function (x) {
      ...
      downloadEGVPages(makeProgress(progress, 0, 25), (err, result) => {
        ...
        if (err == null) {
          downloadBackfilledEGVPages(data.deviceModel, makeProgress(progress, 25, 50), (err, result) => {
            data.egv_data = data.egv_data.concat(result);
            if (err == null) {
              // second half of the progress bar segment
              downloadMeterPages(makeProgress(progress, 50, 75), (err, result) => {
                data.meter_data = result;
                if (err == null) {
                  downloadSettingPages(makeProgress(progress, 75, 90), (err, result) => {
                    data.user_setting_data = result;
                    if (err == null) {
                      downloadEventPages(makeProgress(progress, 90, 100), (err, result) => {
    ...
    processData(progress, data, cb) {
      ...
      const non_setting_data = data.cbg_data.concat(data.calibration_data);
      const sorted = _.sortBy(non_setting_data, (d) => d.systemSeconds);
      const mostRecent = sundial.applyTimezone(sorted[sorted.length - 1].displayDate, cfg.timezone).toISOString();
    ...
    uploadData(progress, data, cb) {
      ...
      ); // TODO: REMOVE THIS! (switching to platform now would cause dupes)
    },

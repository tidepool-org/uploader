...
module.exports = function (config) {
  ...
  var EGV_FLAGS = {
    COMPUTED_ON_DISPLAY: { value: 0x4000, name: 'COMPUTED_ON_DISPLAY'},
    COMPUTED_ON_TRANSMITTER: { value: 0x2000, name: 'COMPUTED_ON_TRANSMITTER'},
    BACKFILLED: { value: 0x1000, name: 'BACKFILLED'}
  };
    ...
    for (var i = 0; i < header.nrecs; ++i) {
      ...
      if (header.revision >= 5){
        ...
        if (softwareNumber !== 'SW10751') {
          ...
          // FIXME rec.predictedBackfilled = hasFlag(EGV_FLAGS.BACKFILLED, rec.predictedGlucose);
          // according to the spec, only 10 bits are the actual glucose value
          rec.predictedGlucose &= 0x3FF;
        }

        // FIXME rec.backfilled = hasFlag(EGV_FLAGS.BACKFILLED, rec.glucose);
      }
  ...
  var parse_setting_records = function (header, data) {
    ...
    switch (header.revision) {
      ...
      case 6:
        // it's a G6
        format = 'iinn6Zissssssssbbsb4Zbbbis';
        break;
      default:
    ...
    for (var i = 0; i < header.nrecs; ++i) {
      ...
      if (header.revision === 6)  {
        rec = struct.unpack(data, ctr, format, ['systemSeconds', 'displaySeconds',
          ...
          'sensorCode', 'reserved', 'currentGraphHeight', 'reserved', 'reserved', 'crc'
        ]);
      } else {
        rec = struct.unpack(data, ctr, format, ['systemSeconds', 'displaySeconds',
          'systemOffset', 'displayOffset', 'transmitterId', 'enableFlags',
    ...
    for (var i = 0; i < data.cbg_data.length; ++i) {
      ...
      if (datum.predictedGlucose) {
        _.assign(payload, {
          ...
          // FIXME predictedBackfilled : datum.predictedBackfilled,

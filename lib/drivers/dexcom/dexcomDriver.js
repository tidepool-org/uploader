          ...
          if (displayErr) {
            ...
            getSystemTime(true, function(systemErr, systemTimeObj) {
              if (systemErr) {
                ...
                cfg.deviceInfo.deviceTime = new Date(BASE_DATE_DEVICE + 1000 * (systemTimeObj.parsed_payload + displayTimeOffsetObj.parsed_payload));
                common.checkDeviceTime(cfg, function(err, serverTime) {
                  if (err === 'updateTime') {
                    var difference = Math.floor((sundial.applyTimezone(cfg.deviceInfo.deviceTime, cfg.timezone) - serverTime) / 1000);

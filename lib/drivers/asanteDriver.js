...
module.exports = function (config) {
  ...
  var getUTCTime = function (t) {
    ...
    var atime = _asanteBaseTime + t * cfg.timeutils.SEC_TO_MSEC;
    var time = cvtRTCTime(atime);
    return moment.tz(time, cfg.timezone).toISOString();
  ...
  var unpackDataRecord = function (rec) {
    switch (rec.rectype) {
      ...
      case PUMP_DATA_RECORDS.LOG_BASAL_CONFIG.value:
        ...
        rec.timezoneOffset = cfg.timeutils.computeTimezoneOffset(rec.deviceTime, rec.UTCTime);
  ...
  var asanteBuildBolusRecords = function (data) {
    ...
    for (var i = 0; i < data.bolusRecords.length; ++i) {
      ...
      b.timezoneOffset = cfg.timeutils.computeTimezoneOffset(b.deviceTime, b.UTCTime);
  ...
  var asanteBuildWizardRecords = function (data) {
    ...
    for (var i = 0; i < data.smartRecords.length; ++i) {
      ...
      wz.timezoneOffset = cfg.timeutils.computeTimezoneOffset(wz.deviceTime, wz.UTCTime);
  ...
  var asanteBuildBasalRecords = function (data) {
    ...
    for (var i = 0; i < data.basalRecords.length; ++i) {
      ...
      basal.timezoneOffset = cfg.timeutils.computeTimezoneOffset(basal.deviceTime, basal.UTCTime);
  ...
  return {
    ...
    detect: function (deviceInfo, cb) {
      ...
      cfg.deviceComms.connect(asantePacketHandler, function() {
        ...
        listenForBeacon(function (err, result) {
          if (err) {
            ...
            // callback-hellish, sorry
            console.log('got beacon, looking for header');
            asanteGetHeader(0, function (err, result) {
              if (err) {
                console.log(err);
                cleanup(err, null);

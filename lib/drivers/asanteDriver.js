...
module.exports = function (config) {
  ...
  var cvtWizCalcs = function (asanteReading) {
      ...
      if (err) {
        ...
        if (result.descriptor == DESCRIPTORS.RESPONSE_RECORD.value) {
          ...
          if (retval.length >= 3000) { // 3000 is bigger than any log's capacity
            next = stopSending();
            // TODO: ask KJQ whether the cutting it short applies only to values
            // less than 3000, as the above comment would indicate
  ...
  var asanteBuildWizardRecords = function (data, records) {
    ...
    for (var i = 0; i < data.smartRecords.length; ++i) {
      ...
      wz.totalCalculated_units = cvtWizCalcs(wz.TotalInsulin);
      wz.recommended_carb_units = cvtWizCalcs(wz.NetCarbInsulin);
      wz.recommended_correction_units = cvtWizCalcs(wz.NetBGInsulin);
      wz.IOB_units = cvtWizCalcs(wz.IOB);

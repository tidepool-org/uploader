...
var pumpConstants = {
  69: require('../asante_data/asante_pump_version_69.js'),
  72: require('../asante_data/asante_pump_version_72.js')
};
var pumpVersion = 0;
...
module.exports = function (config) {
  ...
  function setupUsefulConstants(version) {
    console.log('pump version is', version);
    if (!pumpConstants[version]) {
      console.log('Unknown pump version!');
      return false;
    }
    pumpVersion = version;
    ...
    switch (rec.rectype) {
      ...
      case PK.DATA_RECORDS.LOG_BASAL_CONFIG.value:
        ...
        switch (rec.EventType) {
          case PK.EVENT_TYPES['0_ProfileEvent'].value:
            ...
            break;
          case PK.EVENT_TYPES['1_TempBasal'].value:
            ...
            break;
          case PK.EVENT_TYPES['2_PumpStopped'].value:
      ...
      case PK.DATA_RECORDS.LOG_TIME_EDITS.value:
        struct.unpack(rec.data, 0,
          ...
          rec);
        break;
      case PK.DATA_RECORDS.LOG_TIME_MANAGER_DATA.value:
        struct.unpack(rec.data, 0,
          PK.DATA_RECORDS.LOG_TIME_MANAGER_DATA.struct,
          PK.DATA_RECORDS.LOG_TIME_MANAGER_DATA.fields,
          rec);
        ...
        break;
      case PK.DATA_RECORDS.LOG_USER_SETTINGS.value:
        struct.unpack(rec.data, 0,
          PK.DATA_RECORDS.LOG_USER_SETTINGS.struct,
          PK.DATA_RECORDS.LOG_USER_SETTINGS.fields,
          rec);
        break;
    }
  };
  ...
  var asanteDownloadRecords = function (recordtype, callback) {
    ...
    var retval = [];
    console.log('requesting recordtypes ' + getDataRecordName(recordtype));
    function iterate(err, result) {
      cfg.deviceComms.emitLog(true);
      if (err) {
        if (err == 'TIMEOUT' && nak_count < 10) {
          console.log('NAK #', ++nak_count);
        ...
        console.log('RESULT NOT VALID (2)');
        callback(result, null);
      }
    }
  ...
  var asanteFetch = function (progress, callback) {
    ...
    async.series([
                   getRecords(PK.DATA_RECORDS.LOG_TIME_MANAGER_DATA.value, 10),
                   ...
                   getRecords(PK.DATA_RECORDS.LOG_USER_SETTINGS.value, 95)
                 ],
                 function (err, result) {
  ...
  return {
    ...
    fetchData: function (progress, data, cb) {
      ...
      progress(0);
      asanteSetBaudRate(function(e, r) {
        if (e) {
          return cb(e, r);
        }
    ...
    uploadData: function (progress, data, cb) {
      ...
      cfg.api.upload.toPlatform(data.upload_records, sessionInfo, progress, cfg.groupId, function (err, result) {
        if (err) {
          console.log(err);
          console.log(result);
          progress(100);

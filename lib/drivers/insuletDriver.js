...
module.exports = function (config) {
  ...
  var buildAlarmRecords = function (data, records) {
    ...
    function makeSuspended(anAlarm) {
      ...
      cfg.tzoUtil.fillInUTCInfo(suspend, anAlarm.jsDate || alarm.alarm.jsDate);
      if (anAlarm.detail) {
  ...
  return {
    ...
    uploadData: function (progress, data, cb) {
      ...
      for (var i = 0; i < postrecords.length; ++i) {
        ...
        if (i === 0) {
          first_time = rec.time;
          debug('First UTC timestamp:', first_time);
          ...
          if (rec.time >= first_time) {
            ...
            debug(util.format('Removing event of type %s at %s', rec.type, rec.deviceTime));
          }
        }
      }

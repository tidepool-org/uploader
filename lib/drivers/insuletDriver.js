...
module.exports = function (config) {
  ...
  var buildBolusRecords = function (data, records) {
    ...
    function netRecommendation(details) {
      var total = details.carb_bolus_units_suggested;
      var iob = details.meal_units_iob + details.corr_units_iob;
      // only consider IOB if there's a bgInput
      if (details.current_bg !== 65535) {
        if (iob < details.corr_units_suggested) {
          total += details.corr_units_suggested - iob;
        }
        else {
          total -= details.corr_units_iob;
        }
      }
    ...
    for (var b = 0; b < bolusrecs.length; ++b) {
      ...
      if (wizRecords[wiz_idx]) {
        if (wizRecords[wiz_idx].isDual) {
          ...
          if (ext2 != null) {
            postbolus.with_extended(common.fixFloatingPoint(
            ...
            postbolus.with_extended(data.log_records[ext].detail.volume_units)
          ...
          console.log('This should be a zero-volume bolus:', bolus.deviceTime);
          console.log(bolus.detail);
        }
      } else {
        // here be quick boluses (entered w/o wizard)
        // OmniPod doesn't provide an interface for delivering square- or dual-wave boluses
        // outside of the wizard, so we don't have to worry about those
        postbolus = cfg.builder.makeNormalBolus()
          .with_normal(bolus.detail.volume_units)
          .with_time(bolus.timestamp)
          .with_deviceTime(bolus.deviceTime)
        ...
        if (wizRecords[wiz_idx]) {
          ...
          var postwiz = cfg.builder.makeWizard()
            .with_recommended({
              ...
              net: netRecommendation(wiz.detail)
            })
            .with_bgInput(bg)
            .with_carbInput(carb)
  ...
  var buildBGRecords = function (data, records) {
    ...
    for (var i = 0; i < bgrecs.length; ++i) {
      ...
      if (bg.detail.error_code !== 0 || bg.detail.bg_reading === 65535) {
        ...
        // JEB: what do we expect to do with these for first version in the uploader?
        console.log('meter error');
        console.log(bg.deviceTime);
  ...
  return {
    ...
    uploadData: function (progress, data, cb) {
      ...
      for (var i = 0; i < postrecords.length; ++i) {
        ...
        console.log(datum.type, datum.deviceTime);
        switch (datum.type) {
          case 'basal':
          ...
          case 'deviceMeta':
            if (datum.subType === 'status') {
              if (datum.status === 'suspended') {
                ...
                console.log('Unknown deviceMeta status!', datum.status);
              }
            }
            break;
          case 'settings':
          ...
          case 'wizard':
            ...
            break;
          default:
            console.log('Unhandled type!', datum.type);
        }
      }

...
var lookupMaker = require('../timezoneOffsetLookup');
...
module.exports = function (config) {
  ...
  var buildAlarmRecords = function (data, records) {
    ...
    function makeSuspended(anAlarm) {
      var suspend = cfg.builder.makeDeviceMetaSuspend()
        ...
        .set('dateObj', anAlarm.dt || alarm.alarm.dt)
    ...
    for (var a = 0; a < alarmrecs.length; ++a) {
      ...
      postalarm = cfg.builder.makeDeviceMetaAlarm()
        ...
        .set('dateObj', alarm.dt || alarm.alarm.dt);
      ...
      if (wizRecords[wiz_idx]) {
        if (wizRecords[wiz_idx].isDual) {
          ...
          postbolus = cfg.builder.makeDualBolus()
            ...
            .set('dateObj', data.log_records[imm].dt)
  ...
  var buildTimeChangeRecords = function (data, records, settings) {
    ...
    var offsetLookup = lookupMaker(cfg.timezone, settings.time, postrecords);
    cfg.builder.setOffsetLookup(offsetLookup.fn);
    cfg.offsetLookup = offsetLookup.fn;

    return records.concat(offsetLookup.records);
  };
  ...
  var buildSettingsRecord = function (data, records) {
    ...
    var settingsUTCTime = sundial.applyTimezone(download.dt, cfg.timezone).toISOString();
  ...
  return {
    ...
    uploadData: function (progress, data, cb) {
      ...
      var filteredrecords = [], first_time;
      for (var i = 0; i < postrecords.length; ++i) {
        var rec = postrecords[i];
        if (rec.dateObj) {
          var utc = cfg.offsetLookup(rec.dateObj, rec.index || null);
          rec.time = utc.time;
          rec.timezoneOffset = utc.timezoneOffset;
        }
        if (i === 0) {
          first_time = rec.time;
          console.log('First UTC timestamp:', first_time);
          ...
          if (rec.time >= first_time) {
            ...
            debug('Removing event at', rec.deviceTime);
            debug(rec);
          }
        }
      }
      ...
      _.each(filteredrecords, function(rec) {
        debug(rec.time, rec.timezoneOffset);
      });

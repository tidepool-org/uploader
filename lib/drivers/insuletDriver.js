...
module.exports = function (config) {
    ...
    for (var b = 0; b < bolusrecs.length; ++b) {
      ...
      if (postbolus) {
        ...
        if (wizRecords[wiz_idx]) {
          ...
          // log_index -1 means wiz was not found
          // (probably because PDM ran out of storage space)
          if (!_.isEmpty(wiz) && wiz.log_index !== -1) {
  ...
  var buildSettingsRecord = function (data, records) {
    ...
    for (var l = 0; l < starts.length; ++l) {
      var start = starts[l];
      // use the previous target or threshold if there isn't a new one for this start time
      var thisTarget = bgSettingsByStart[start].target ||
        bgSettingsByStart[starts[l - 1]].target;
      var thisThreshold = bgSettingsByStart[start].threshold ||
        bgSettingsByStart[starts[l - 1]].threshold;
  ...
  return {
    ...
    uploadData: function (progress, data, cb) {
      ...
      postrecords = _.sortBy(postrecords, function(d) {
        ...
        else if (d.payload && d.payload.logIndices) {
          return d.payload.logIndices[0];
        }
        else {
          return undefined;
        }

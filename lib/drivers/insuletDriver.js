...
module.exports = function (config) {
  ...
  var buildBolusRecords = function (data, records) {
    ...
    // it appears that the Insulet suggested calc does not add values < 0.05 units
    // into the total recommendation
    function minSuggestion(suggestion) {
      return suggestion > 0.05 ? suggestion : 0.0;
    }

    function netRecommendation(details) {
      var total = minSuggestion(details.carb_bolus_units_suggested);
      ...
      if (details.current_bg !== 65535) {
        if (iob < details.corr_units_suggested) {
          total += minSuggestion(details.corr_units_suggested) - iob;
        }
        else {
          total -= details.corr_units_iob;
        }
      }
      return common.fixFloatingPoint(total, 2);
    }

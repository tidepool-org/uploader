  ...
  return function (config) {
    ...
    return {
      ...
      processData: function (progress, payload, cb) {
        ...
        function getUnits() {
          var units = null;
          _.each(payload.colLabels.fields, function(label) {
            if (label.search('Sensor Calibration BG') !== -1) {
              if (label === 'Sensor Calibration BG (mmol/L)') {
                units = 'mmol/L';
              }
              else if (label === 'Sensor Calibration BG (mg/dL)') {
                units = 'mg/dL';
              }
              else {
                throw new Error(util.format('Unknown CalBGForPH units; column label is[%s]', label));
              }
            }
          });
          if (units != null) {
            ...
            throw new Error('CalBGForPH column label ("Sensor Calibration") not found!');
          }
        }
        ...
        for (var k = 0; k < payload.theData.length; ++k) {
          ...
          if (device == null) {
            ...
            device.processors = initializeProcessors(cfg.timezone, {model: key, units: getUnits(), colNames: colNames});

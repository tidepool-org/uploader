    ...
    return {
      ...
      fetchData: function (progress, payload, cb) {
        ...
        for (var i = parsed.data.length - 1; i >= 0; --i) {
          ...
          if (typesToRead[datum['Raw-Type']]) {
            pumpUploadId = datum['Raw-Upload ID'];
            pumpSeqNum = datum['Raw-Seq Num'];
          }
          // if this row is a ChangeTimeGH, decorate with current pumpUploadId and pumpSeqNum
          if (datum['Raw-Type'] === 'ChangeTimeGH') {
        ...
        payload.pumpData = _.filter(rows, function(datum) {
          return typesToRead[datum['Raw-Type']] || datum['Raw-Type'] === 'ChangeTimeGH';
        });
        payload.cgmData = _.filter(rows, function(datum) {
          return cgmTypesToRead[datum['Raw-Type']];
        });
        ...
        for (var k = 0; k < payload.pumpData.length; ++k) {
          var key = payload.pumpData[k]['Raw-Device Type'];
          ...
          if (uploads[payload.pumpData[k]['Raw-Upload ID']] != null) {
            device.data.push(payload.theData[k]);
          }
        }
        if (_.includes(_.pluck(payload.cgmData, 'Raw-Type'), 'GlucoseSensorData')) {
          for (var j = 0; j < payload.cgmData.length; ++j) {
            var cgmKey = payload.cgmData[j]['Raw-Device Type'] + ' : CGM';
            ...
            if (uploads[payload.cgmData[j]['Raw-Upload ID']] != null) {
        ...
        Object.keys(payload.devices).forEach(function(key) {
          ...
          var bisection = _.partition(device.data, function(datum) {
            return !_.includes(['ChangeTime', 'ChangeTimeGH'], datum['Raw-Type']);
          });
          ...
          for (var n = 0; n < dateTimeSettingsChangeRows.length; ++n) {
            ...
            var newTime = change['Raw-Values'].NEW_TIME;
          ...
          debug(key, device.timeChanges.length);

    ...
    return {
      ...
      fetchData: function (progress, payload, cb) {
        ...
        if (_.isEmpty(uploads)) {
          var err = new Error('CareLink data contains data from an unsupported pump model.');
          err.code = 'E_CARELINK_UNSUPPORTED';
          throw err;
        }
        ...
        Object.keys(payload.devices).forEach(function(key) {
          ...
          if( device.data.length === 0 ) {
            // Remove devices that have no data. This can happen if we have
            // multiple devices registered in CareLink, but no data for this time
            // period (eg we got a new pump).
            delete payload.devices[key];
          } else {
      ...
      processData: function (progress, payload, cb) {
        ...
        function getUnits(colName) {
          ...
          if (units != null) {
            ...
            throw new Error(util.format('fingerstickGlucose column label[%s] units not recognized!', colName));
          }
        }
      ...
      uploadData: function (progress, payload, cb) {
        ...
        async.eachSeries(Object.keys(payload.devices), function(key, done) {
          ...
          var sessionInfo = {
            payload: {skippedUploads: payload.skippedUploads},
          ...
          if (cfg.deviceInfo.hasMultiple()) {
            sessionInfo.payload =
              _.assign({}, sessionInfo.payload,
                cfg.deviceInfo.getPayload()
              );
          }

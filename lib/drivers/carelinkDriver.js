  ...
  return function (config) {
    ...
    return {
      ...
      fetchData: function (progress, payload, cb) {
        ...
        var mostRecentDatum = dataToParse[dataToParse.length - 1];
        var mostRecent = sundial.applyTimezone(mostRecentDatum.jsDate, cfg.timezone).toISOString();
        debug('Most recent timestamp in data:', mostRecentDatum.deviceTime);

        var bisection = _.partition(dataToParse, function(datum) {

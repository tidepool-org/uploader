    ...
    return {
      ...
      fetchData: function (progress, payload, cb) {
        ...
        for (var i = 0; i < parsed.data.length; ++i) {
          ...
          datum.index = datum['Raw-Upload ID'] + '-' + datum['Raw-Seq Num'];
        ...
        var dataToParse = _.filter(rows, function(datum) {
          return typesToRead[datum['Raw-Type']];
        });
        var uploadGroups = _.groupBy(dataToParse, function(d) { return d['Raw-Upload ID']});
        var uploads = Object.keys(uploadGroups).sort();
        _.each(uploadGroups, function(group) {
          group = _.sortBy(group, function(d) { return d['Raw-Seq Num']; });
          group.reverse();
        });
        var sortedUploads = [];
        _.each(uploadGroups, function(group) {
          sortedUploads = sortedUploads.concat(group);
        });
        var i = 0;
        var indexedRows = _.map(
          sortedUploads,
          function(row) {
            row.index = i;
            ++i;
        ...
        var bisection = _.partition(indexedRows, function(datum) {
          return datum['Raw-Type'] !== 'ChangeTime';
        });
        payload.theData = bisection[0];
        ...
        var tzoUtil = new TZOUtil(cfg.timezone, new Date().toISOString(), changes);
        payload.timeChanges = _.groupBy(_.map(
            tzoUtil.records,
            function(rec) { return _.omit(rec, 'index'); }
          ),
          'model'
        );
        _.each(payload.theData, function(datum) {
          tzoUtil.fillInUTCInfo(datum, datum.jsDate);
        });
      ...
      uploadData: function (progress, payload, cb) {
        ...
        async.map(Object.keys(payload.devices), function(key, done) {
          ...
          if (payload.timeChanges[key]) {
            deviceRecords = deviceRecords.concat(
              _.map(
                payload.timeChanges[key],

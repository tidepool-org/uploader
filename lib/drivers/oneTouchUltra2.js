...
module.exports = function (config) {
  ...
  var otu2ErrorHandler = function(info) {
    ...
    if (info.connectionId && info.error) {
      if (info.error == 'timeout') {
        console.log('OTU2ERROR: ', info.error);
      }
    }
  };
  ...
  var prepBGData = function (progress, data) {
    ...
    if (!toMatch || toMatch.length !== 5){
        console.log('Incorrect device header');
        return null;
    }
    ...
    if(calcchks !== parseInt(checksum, 16)){
         console.log('error...bad checksum in header', calcchks, checksum);
         return null;
     }
    ...
    _.forEach(splites.slice(1), function(l){ // <-- jump the header
        ...
        if(ms){
            ...
            if(createchk !== parseInt(chk, 16)){
              console.log('error...bad checksum in data lines', createchk, chk);
              return null;
            } else {
          ...
          console.log('ERROR: no match in dpattern');
        }
    });

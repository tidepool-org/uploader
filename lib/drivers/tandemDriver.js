...
module.exports = function (config) {
  ...
  var buildCGMRecords = function(data, records) {
    var cgmRecords = filterLogEntries([PUMP_LOG_RECORDS.LID_CGM_DATA], data.log_records);
    ...
    for (var b = 0; b < cgmRecords.length; ++b) {
      ...
      var record = null;
      var annotation = null;
      console.log('Status',event.status,'value', event.value);
      ...
      if(event.status === CGM_STATUS.CGM_NORMAL.value ||
         event.status === CGM_STATUS.CGM_LO.value ||
         event.status === CGM_STATUS.CGM_HI.value) {
        record = cfg.builder.makeCBG();
      }
      else if (event.status === CGM_STATUS.CGM_CALIBRATION.value ||

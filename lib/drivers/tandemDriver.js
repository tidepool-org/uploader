...
module.exports = function (config) {
  ...
  var PUMP_LOG_RECORDS = {
    ...
    LID_CGM_CALIBRATION: {
      ...
      format: 'iiihh',
      fields: ['current_time','timestamp','cal_timestamp','cal_reading', 'value']
    }
  };
  ...
  var CGM_STATUS = {
    ...
    CGM_CALIBRATION: { value: 3, name: 'Calibration Value'},
    CGM_HI_CALIBRATION: { value: 4, name: 'HI calibration reading'},
    CGM_LO_CALIBRATION: { value: 5, name: 'LO calibration reading'}
  };
  ...
  var multiLogRequester = function (start, end, progress, callback) {
    ...
    var retryTimeout = function() {
      debug('Retrying from record ',receive_seq,' onwards.',end-receive_seq,' record(s) left..');
      tandemCommand(COMMANDS.LOG_ENTRY_SEQ_MULTI_REQ, [receive_seq,end-receive_seq], function (err) {
        if(err) {
          callback(err,null);
        }
      });
    ...
    tandemCommand(COMMANDS.LOG_ENTRY_SEQ_MULTI_REQ, [send_seq,end-send_seq], function (err) {
      if(err) {
        callback(err,null);
      }
      else {
  ...
  var tandemDownloadRecords = function (progress, data, callback) {
    ...
    if(data.firmware_version >= COMMANDS.LOG_ENTRY_SEQ_MULTI_REQ.version) {
      ...
      multiLogRequester(start_seq, end_seq+1, progress, iterate);
    }
    else{
  ...
  var buildCGMRecords = function(data, records) {
    var cgmRecords = filterLogEntries([PUMP_LOG_RECORDS.LID_CGM_DATA,PUMP_LOG_RECORDS.LID_CGM_CALIBRATION], data.log_records);
    for (var b = 0; b < cgmRecords.length; ++b) {
      var event = cgmRecords[b];
      var value = event.value;
      var record = null;
      var annotation = null;
      console.log('Name', event.name, 'Status',event.status,'value', event.value);

      if(event.header_id === PUMP_LOG_RECORDS.LID_CGM_CALIBRATION.value
        && event.value <=401 
        && event.value >=39) { // don't include magic numbers
        ...
        record.with_value(event.value)
              ...
              .with_payload({
                ...
                calibration_reading: event.cal_reading,
        ...
        if (event.status === CGM_STATUS.CGM_CALIBRATION.value ||
          ...
          //TODO: in response to calibration
        }
        ...
        if (record !== null) {
          ...
          throw Error ('Unknown CGM record');
        }
      }

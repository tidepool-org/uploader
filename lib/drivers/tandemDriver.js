...
module.exports = function (config) {
  ...
  var tandemCommand = function (command, args, callback) {
    ...
    if (format) {
      ...
      struct.pack(payload, 0, format, ...args); //TODO: is spreading operator ES2016?
    }
                ...
                if (receive_seq != pkt.payload['header_log_seq_no']) {
                  ...
                  tandemCommand(COMMANDS.LOG_ENTRY_SEQ_MULTI_REQ, [receive_seq,end-receive_seq], function (err) {
                    ...
                    retryRecoverTimer = setTimeout(function() {
                      if(recovering) {
                        debug('Retrying to recover..',receive_seq);
                        tandemCommand(COMMANDS.LOG_ENTRY_MULTI_SEQ_REQ, [receive_seq,end-receive_seq], function (err) {
                          if(err) {
                            callback(err,null);
                          }
                        });
  ...
  var tandemDownloadRecords = function (progress, data, callback) {
    ...
    function iterate(err, result) {
      if (err) {
        ...
        if (result.payload.header_log_seq_no === end_seq) {
          ...
          //TODO: check firmware version is >= 4744
    ...
    // TODO: check firmware version is >= 47144, else use tandemLogRequester(start_seq, end_seq, progress, iterate);

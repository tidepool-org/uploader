...
module.exports = function (config) {
  ...
  var oneTouchCommandResponse = function (commandpacket, callback) {
    ...
    var ackparser = function(packet) {
      if (!isValidAcknowledgePacket(packet)) {
        // debug('expected ACK failed to validate!');
        // debug(packet);
        return false;
      }
      return true;
  ...
  return {
    ...
    data: function (progress, data, cb) {
      debug('in connect!');

      console.info('data en el mini',data);

      console.info('data en el data.deviceInfo',data.deviceInfo);

      console.info('data en el data.id',data.id);

      data.deviceInfo.deviceId = data.id;

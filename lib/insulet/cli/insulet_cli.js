...
var carelinkDriver = require('../insuletDriver.js');
...
function loadFile(filePath, tz) {
  fs.readFile(filePath, 'utf8', function(error, data) {
    ...
    var drivers = {'Insulet': carelinkDriver(pwdSimulator, api)};
    var cfg = { 'Insulet': { filename: filePath, fileData: data, timezone: tz, version : pkg.name+' '+pkg.version } };
...
if(program.file && program.username && program.password) {
  if (fs.existsSync(program.file)){
    ...
    console.log('csv file at [%s] not found', program.file);
    return;
  }
}else{
  program.help();

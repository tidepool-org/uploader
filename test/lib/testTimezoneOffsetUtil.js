...
describe('TimezoneOffsetUtil.js', () => {
  ...
  test(
    'throws an error if a named timezone not provided as first param',
    () => {
  ...
  test(
    'throws an error if a valid timestamp is not provided as second param',
    () => {
  ...
  test(
    'throws an error if `changes` not empty and not all events are `timeChange`',
    () => {
  ...
  test(
    'defaults to across-the-board timezone application if no `changes` provided as third param',
    () => {
  ...
  test(
    'identifies the type of timezone offset production used as `utc-bootstrapping` or `across-the-board-timezone`',
    () => {
...
describe('TimezoneOffsetUtil in practice', () => {
  ...
  test(
    'applies a timezone across-the-board when no `changes` provided',
    () => {
  ...
  test(
    'applies a timezone across-the-board (including offset changes b/c of DST) when no `changes` provided',
    () => {
      var data = _.map(_.range(0,100), function(d) { return {value: d, type: 'foo'}; });
      var startDate = sundial.parseFromFormat('2015-02-01T00:00:00').getTime();
      var dates = _.map(_.range(100), (days) => new Date(startDate + days * oneDay));
  ...
  test(
    'applies the offsets inferred from `changes`, resulting in no gaps or overlaps',
    done => {
      //this.timeout(5000);
      setTimeout(function() {
